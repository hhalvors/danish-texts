...
%
%  PAGE-JOINT AND PARAGRAPH REPAIR (2026 pass, after the same fault was found
%        and traced in texts/nielsen/speculative-methode):
...
%          * 17 bare-hyphen joints -> `\-%` (pp. 25, 49, 50, 51, 68, 70, 75, 77,
%            78, 118, 120, 125, 140, 141, 142, 145, 148)
%          * 11 FALSE paragraph breaks removed (pp. 2--12), each confirmed on the
%            page image to open flush at the body margin
%          * 16 MISSING paragraph breaks added (pp. 21, 45, 56, 59, 69, 80, 90,
%            93, 130, 132, 134, 152, 153, 154, 163, 172), each confirmed on the
%            image to open with the printed indent (~70 px at 250 dpi against a
%            continuation's 0 px)
%        `paragraphs.py` in this directory performs the audit and should be run
%        before any future publication. It decides by two independent tests —
%        the first line's left edge against the page's own margin, and whether
%        the previous page ends mid-word or mid-sentence — and reports rather
%        than acts on anything they disagree about. The built PDF now contains
%        no "word- word" artefact at all.
%
%  TWO DEFECTS OF A DIFFERENT KIND, found by that pass and NOW REPAIRED. Both
%        came from paragraphs.py's UNCERTAIN list — pages where the image said
%        "new paragraph" and the text said "continues mid-sentence". Neither was
%        a splice artefact; both were settled at 600 dpi.
%
%        (1) printed p. VII: THE PAGE MARKER SAT ONE CLAUSE TOO EARLY, and has
%            been moved. The words "tænker lidt nøiere efter." are at the FOOT OF
%            PRINTED p. VI — its last two lines read "Hvor mislig min Opgave
%            imidlertid viser sig, maa vist Enhver indrømme, / der tænker lidt
%            nøiere efter." — and p. VII opens, indented, with "Skal her virkelig
%            strides om, ...". The contradiction the audit reported WAS the
%            misplaced marker. p. VII now also takes its paragraph break.
%
%        (2) printed pp. 7/8: the printing has A COMMA AND A PARAGRAPH BREAK AT
%            THE SAME JOINT, which is odd enough to set out. p. 7 ends
%            "...opkommer i Christenheden," — a comma, not a full stop, verified
%            at 600 dpi and ~1.7x, where the mark plainly carries a descending
%            tail. The line is short but the page is NOT short: p. 7's text ends
%            at y4370 of 4908 and p. 8's at y4337. And printed p. 8 then opens an
%            INDENTED new paragraph. Both features are now reproduced — the comma
%            stands and the paragraph break is set. Previously the two pages ran
%            together as one paragraph, reproducing neither.
%            Sense-wise the comma reads as a colon-like pause introducing the
%            elaboration that follows ("Den Christne bliver naturkyndig; den
%            Naturkyndige er en Christen"), which may be why it was set.
%            paragraphs.py will GO ON REPORTING p. 8 AS UNCERTAIN for as long as
%            the book exists, because a comma before an indented paragraph is
%            exactly the contradiction it is built to notice. That flag is
%            expected and correct; do not "resolve" it by changing the text.
%
%  ACCURACY: full collation, 24 September 2026, of the Forord (III--VIII),
%  all 174 printed pages and the Indhold, against the scan (sha256
%  4c2821350fce...7efc4f): every page read at the image word against word by
%  one of 18 collators (about 10 pp. each, 2 waves), letterspacing and
%  antiqua checked at 450--800 dpi; every non-emphasis change and every
%  doubtful item put to a BLIND second reader (4 readers) before it was
%  applied, and five pages re-collated after one agent's renders may have
%  been cut short (0 divergences). Where the readers disagreed the file was
%  left as it stood (see "Held" below).
%    Transcriber's errors corrected: 28 -- 8 punctuation marks (p. V
%    comma and stop inside the closing quote; p. 1 comma for
%    semicolon; p. 11 quotation marks the print does not have, round the
%    letterspaced Confessio definition; p. 101 comma for stop; p. 144
%    semicolon for comma; p. III comma inside the quote), 10 wrong words or
%    letters, given as printed (Forord `laae', file `ligger'; p. 1
%    `eenstydig', file `eensbetydig'; pp. 9, 10 `orthodoxe', file `orthodore' x3; p. 31 `løde'; p. 63 an added
%    `i'; p. 125 `korser'; p. 127 `famlede'; p. 136 `ak!'), 9 modernised or
%    normalised spellings undone (beroe, gierne, giennem, Iilfærdighed,
%    liig x2, intrædende, conseqvent, nøiagtig), and 1 word division
%    (p. 160 `udenfor liggende').
%    Printer's errors the file had silently corrected, restored as printed
%    with a comment: 16 (p. 12 `træder,'; p. 27 `spindsfindig'; p. 34
%    `orthodxe'; p. 72 `Selvfornegielse'; p. 73 `Selfornegtelse'; i for j in
%    `obiective' p. 90, `erkiendende' p. 98, `Erkiendelse' p. 144,
%    `subiective' p. 153; p. 107 `jor', `ten'; p. 111 `ererkjendes' (the
%    doubled syllable across a line end, formerly resolved); p. 114 `ttl';
%    p. 129 `Inledninger'; p. 130 `ander'; p. 172 `nnder'). Already as
%    printed, comment added: p. 47 `Evangelitroen', p. 73 `let' (turned l),
%    p. 147 `Inspirationtheorie'.
%    Emphasis: 28 items -- letterspacing added (Forord, p. 4), removed
%    (p. IV `personlig', p. 173) or its extent corrected (pp. 6, 11, 44, 46,
%    50, 75, 96, 124/125, 143, 147); antiqua: `De Wette' x5 (p. 57) and
%    `grammatikalsk' (p. 35) are Fraktur, \textit removed; `Ergo' (p. 152)
%    is antiqua, \textit added.
%    Markers: 7 moved to the true turn (pp. IV, 11, 55, 57, 61, 109, 110;
%    six of them to split-word turns written `viden\-%'). Comments that
%    were wrong have been corrected: p. 125 (only `fordi' is spaced), p. 139
%    (the German couplet is Fraktur, not antiqua), p. 152 (`Ergo').
%    Mechanical: at all 17 split-word page turns listed in the repair note
%    above, the `\-%' joint was followed by `\opage{N} ' WITH a space, so
%    the PDF had no "word- word" but did set "word word" (e.g. "Betragt
%    ningen"). The space is now closed at every turn.
%  Held, not changed (readers disagreed or the evidence is an absent feature):
%    p. III whether `at' before `stride'/`strides' is spaced; p. 13 `Tro;'
%    (a comma?); p. 44 `Forudsætning'; p. 45 `ufeilbar' and antiqua `etc.';
%    p. 64 `Conseqvens;' (a stop?); p. 126 the year 1813 (possibly 1843);
%    p. 137 `i' (a damaged sort, read k by one reader); p. 167 `sit' (read
%    `sti' by one reader). Two page-turn splits are written `Over%' (p. 82)
%    and `Paaskeberegnin%' (p. 94) without `\-'; the text is right.
%  ocrdiff (embedded ABBYY layer, per batch): 1308 candidates, 6 real,
%    1 unresolved (wave 1: 593 / 3; wave 2: 715 / 3).
%  Rate: 44 transcriber's errors (28 + 16 silently corrected misprints) over
%    180 pp. = ~0.24/page, more than half of them word-level; emphasis wrong
%    at ~0.16 runs/page besides.
%  Post-repair re-sample (seed `nielsen/evangelietroen-theologien|post':
%    pp. 12, 50, 73, 74, 90, 150, a fresh reader): 0 errors, 0 paragraph,
%    0 marker; 1 emphasis extent (p. 50 note, `Tro' and `Forargelse' spaced,
%    `og' solid), corrected.
% ============================================================
...
laae saa nær: Gud i Himlene, jeg takker Dig, at Du ikke har fordret af et
...
Disse Ord af en christelig Psycholog, der kjender „\emph{Sygdommen til Døden},“
...
Adskillelsen imellem det, at \emph{stride for Troen}, og det, at \emph{strides om Troen}.
...
personligt. Hvormeget jeg end i disse Forelæsninger har be\-%
% --- p. IV ---
stræbt mig for fra
...
Grunde i en eneste afgjørende personlig Grund; mellem det, at strides
...
tænker lidt nøiere efter.

% MARKER MOVED (2026). It stood before "tænker lidt nøiere efter.", which put
% those words at the head of p. VII. They are at the FOOT OF PRINTED p. VI:
% verified at 600 dpi, where p. VI's last two lines read
%   "Hvor mislig min Opgave imidlertid viser sig, maa vist Enhver indrømme,
%    der tænker lidt nøiere efter."
% and p. VII opens, INDENTED, with "Skal her virkelig strides om, ...".
% Found by paragraphs.py, which flagged the page UNCERTAIN because the image
% said "new paragraph" while the text said "continues mid-sentence" — the
% contradiction was the misplaced marker itself.
% --- p. VII ---
Skal her virkelig strides om, hvo det er, der har
...
Skulde det omsider komme saavidt, at man ikke engang fandt det Umagen værd \emph{at
...
Umagen værd \emph{at stride for Troen}: et Tegn paa, at Troen ikke mere fandtes paa
Jorden?
...
\bigskip
\noindent \emph{Kjøbenhavn}, d.\ 17 Februar 1850.
\hfill R.\ Nielsen.
...
% --- p. 1 ---
\opage{1} Det er Forholdet mellem Evangelietroen og Theologien, der gjøres til
...
gjøre „Theologien“ eenstydig med „Videnskaben“. Spørgsmaalet om
...
% --- p. 2 ---
\opage{2} Parter tilrette og tilveiebringe en Forsoning ved lidt efter lidt at gjøre
...
% --- p. 3 ---
\opage{3} thi, idet den paatager sig at føre Beviset, begynder den netop med at
...
% --- p. 4 ---
\opage{4} er ueensartet med enhver Videnskab, af hvad Navn nævnes kan, eller ikke.
...
Underet er i sit Væsen \emph{over} og derfor i sin Tilsyneladelse \emph{imod} Naturens
...
om, at der i
% --- p. 5 ---
\opage{5} Naturens Rige intet Spring er, i Lovenes System ingen Plads for de
...
% --- p. 6 ---
\opage{6} altsaa faaer Lov til at gjøre med den, hvad den vil, erklære den for
...
% --- p. 7 ---
\opage{7} og strides igjen. Ifølge den herskende Synsmaade, ængstes man meget for
...
disse Ophævelser egentlig føre til, da det Hele synes at beroe paa en
Misforstaaelse.
...
% AS PRINTED, AND THE COMBINATION IS ODD ENOUGH TO RECORD. Printed p. 7 ends
% "...opkommer i Christenheden," — a COMMA, not a full stop: verified at 600 dpi
% and ~1.7x, where the mark plainly carries a descending tail. The line is short
% and sits at the page's normal foot (p. 7's text ends at y4370 of 4908, p. 8's
% at y4337, so p. 7 is not a short page), and PRINTED p. 8 THEN OPENS AN
% INDENTED NEW PARAGRAPH, also verified at 600 dpi.
% So the printing has a comma AND a paragraph break at the same joint. Both are
% reproduced: the comma stands, and the paragraph break is now here. Earlier the
% two pages were run together as one paragraph, which reproduced neither.
% Sense-wise the comma reads as a colon-like pause introducing the elaboration
% that follows, which may be why the compositor set it.
% --- p. 8 ---
\opage{8} Den Christne bliver naturkyndig; den Naturkyndige er en Christen. Begge
...
% --- p. 9 ---
\opage{9} denne Videnskab er Theologien: er den christelige Theologie da nu ogsaa
...
Den gamle orthodoxe Theologie, der i alle Punkter vilde være conform med
...
% --- p. 10 ---
\opage{10} Form og Indhold, en Modsigelse, for hvilken den orthodoxe Theologie
omsider bukkede under.

Den orthodoxe Theologie fremstiller enhver Læresætning thetisk,
...
% --- p. 11 ---
\opage{11}Videnskab; først i det 19de Aarhundrede sees det klart, at Theologien er
...
Kirken er det Troens Samfund, i hvilket \emph{Evangeliet rettelig
...
% --- p. 12 ---
\opage{12} Prædiken da ikke med raisonnerende Kritik, men med Bøn og Paakaldelse, og
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «Saa træder, „Hermeneutiken“ til»
...
% --- p. 13 ---
\opage{13} Anderledes i Theologien; thi Theologien er jo Videnskab. Som objectiv
...
% --- p. 14 ---
\opage{14} Den positive Aabenbaring --- i Jødedom og Christendom --- er en historisk
...
% --- p. 15 ---
\opage{15} Gud har aabenbaret sig for Abraham; det er skeet, at Gud fordum mange Gange og
...
blot prøve
% --- p. 16 ---
\opage{16} paa at høre et theologisk Cursus over den kritiske Indledning, og han vilde da
...
% --- p. 17 ---
\opage{17} den opstandne Frelser, som Maria Magdalena, der hun mødte ham i Urtegaarden,
...
% --- p. 18 ---
\opage{18} Indledning“ skal have mindste Indflydelse paa den religiøse Overbeviisning;
...
% --- p. 19 ---
\opage{19} levende, Menigheden iboende Aand, og ikke paa en større eller mindre Sum af
...
gierne indrømme --- for den lærde Videnskabs egen Skyld, men ogsaa vigtige, ja
...
% --- p. 20 ---
\opage{20} bliver krænket af den kritiske Indledning. Beviset herfor er, at Kritikens
...
% --- p. 21 ---
\opage{21} Nu reiser sig en Mængde Vanskeligheder, en Mængde Betænkeligheder; nu strides
...
% --- p. 22 ---
\opage{22} Mere. Men ved denne foregivne Usandsynlighed ere to Ting at bemærke:
...
% --- p. 23 ---
\opage{23} Afgjørelse er i Troen, ikke i Theologien. Dette vil ogsaa fremgaae af en
...
% --- p. 24 ---
\opage{24} endelige Sætninger. Disse Led og Sætninger kunne da vel ikke Ord til andet være
...
% --- p. 25 ---
\opage{25}des kan og Ingen døbes paa en christelig Bekjendelse i Almindelighed, eller i
...
% --- p. 26 ---
\opage{26} At ville paa een Gang, i eet og samme Nu, forholde sig troende
...
% --- p. 27 ---
\opage{27} Naar da nu den kritiske Indledning tager Bekjendelsen for sig,
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «af en meget spindsfindig og sophistisk Natur»
alligevel af en meget spindsfindig og sophistisk Natur. Af Frygt for,
at her muligviis kunde have indsneget sig
% --- p. 28 ---
\opage{28} Noget i det Historiske (i \textit{fides historica}), som ikke var
...
% --- p. 29 ---
\opage{29} tvivlende Sjæl nu begyndte at undskylde sig og sige: „Herre, der vare
...
han være
% --- p. 30 ---
\opage{30} værdig eller uværdig. Det skal man troe fast; thi de kjære Christne
...
% --- p. 31 ---
\opage{31} Der har reist sig Klammerie om dine Ord i Nadveren: Nogle ville, at
...
om din Lidelse og Opstandelse, og de dog beholdt Ordene, som de løde,
...
% --- p. 32 ---
\opage{32} Skrift. Inspirationen er Skriftens Qvalitet. Denne Qvalitet bliver
...
% --- p. 33 ---
\opage{33} (thi det væsentlige Indhold maa jo i Grunden være det samme i
...
% --- p. 34 ---
\opage{34} udvortes Gjenstand; for den Religiøse er den en hellig Gjenstand: for
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «den orthodxe Protestant skal nok»
den orthodxe Protestant skal nok vedblive at forsvare Bibelens
...
% --- p. 35 ---
\opage{35} ved Inspirationen bevirkede Eenhed af det Guddommelige og
...
grammatikalsk Eenhed. Saaledes opkom Striden om
...
% --- p. 36 ---
\opage{36} Grunden vare uvæsentlige for Troen, forstod den kritiserende
...
% --- p. 37 ---
\opage{37} ikke-religiøst til Skriften; Theologen kan aldrig bringe det dertil,
...
% --- p. 38 ---
\opage{38} Bibelen, Kirkens hellige Skrift, er paa mange Maader bleven misforstaaet,
...
% --- p. 39 ---
\opage{39} I Forhold til Sværmeren og Fritænkeren bevarer Bibelen dog altsaa endnu sin
...
% --- p. 40 ---
\opage{40} Misgreb --- maa bestemmes som det egentlige principielle Misforhold, som det
...
% --- p. 41 ---
\opage{41} eget Liv. Maatte hine hellige Mænd, endskjøndt de vare Guds Udvalgte, dog
...
% --- p. 42 ---
\opage{42} men det følger slet ikke af det her opstillede Princip. Vi ivre ligesaa lidt
...
% --- p. 43 ---
\opage{43} Opgaver ere af reent videnskabelig Art, sees deraf, at de kunne behandles af
...
% --- p. 44 ---
\opage{44} bliver det tillige et lærd videnskabeligt Spørgsmaal, nemlig om Bøgernes Ægthed
...
I. Lad os nu for det Første vælge \emph{Troesforholdet.} Den religiøse Faktor
...
% --- p. 45 ---
\opage{45} Den religiøse Faktor er derimod ikke varierende, men constant. For den
...
% --- p. 46 ---
\opage{46} enkelte uvæsentlige Punkter.\footnote{„Juristerne sige, at en
...
II. Vælge vi nu det \emph{videnskabelige Forhold,} og forsøge fra et reent
...
% --- p. 47 ---
\opage{47} historiske Tro, og viser tydelig nok, hvorledes den historiske Faktor
...
% `Evangelitroen': as printed, e omitted (collation 2026-09-24, two readers).
...
% --- p. 48 ---
\opage{48} ind i det Sandselige, det Mirakel, hvorved Moses lader Kilden fremspringe i
...
% --- p. 49 ---
\opage{49}skjellige Undergjerninger have gjort paa de forskjellige Øienvidner. Hvor
...
% --- p. 50 ---
\opage{50}sindige Forklaring dog altid yder ham en vis subjectiv Lindring, bevarer han
...
Skriften kjender kun de subjective: \emph{Tro} og \emph{Forargelse}, men Vanvidet
...
% --- p. 51 ---
\opage{51}kel, saa maa det jo vel ogsaa forholde sig med det, at skulle læse, opfatte og
...
% --- p. 52 ---
\opage{52} Skal Bibelen, Kirkens hellige Skrift, ikke blive borte i Theologiens
...
% --- p. 53 ---
\opage{53} kan blive tilbørlig forstaaet og rettelig anvendt: Bibelen maa fortolkes.
...
% --- p. 54 ---
\opage{54} vedkomme ham dog i Grunden slet ikke; Bibelen er nu eengang den, den er,
...
theologiske Bedrag, at man ved fortsat viden\-%
% --- p. 55 ---
\opage{55}skabelig Anvendelse af videnskabelige Regler skulde trænge dybere og
...
% --- p. 56 ---
\opage{56} Denne Uenighed er ikke tilfældig eller blot forbigaaende, men dybt
...
eensartede ved at uddrage religiøse Slutninger af viden\-%
% --- p. 57 ---
\opage{57}skabelige Præmisser, eller omvendt: videnskabelige Slutninger af
...
Exempel, vil jeg her minde om Theologen De Wette.

Den for nylig afdøde Theolog, De Wette, har visselig udmærkede
...
behandle Skriften letsindig. Tvertimod! De Wette var vistnok en
...
Eller er det ikke en Misforstaaelse, naar De Wette vil frakjende
...
Eller er det ikke en Misforstaaelse, naar De Wette tillader sig
at kritisere Propheten Ezechiel, som man kritiserer en middelmaadig
Forfatter i den moderne Litteratur? naar han t.
% --- p. 58 ---
\opage{58} Ex. dadler denne Prophets Mangel paa Dybde og Aandsrigdom og paa store
...
% --- p. 59 ---
\opage{59} Ere Propheterne inspirerede: da maa jo den Forskjel, der med Hensyn til
...
eller
% --- p. 60 ---
\opage{60} Rationalist, eller Pantheist, eller hvilkesomhelst --- ister, der ellers
...
kan opgive det Religiøse uden at op\-%
% --- p. 61 ---
\opage{61}give sin Charakteer, saa fulgte det af sig selv, at hiin videnskabelige
...
fastsætte
% --- p. 62 ---
\opage{62} det theologiske Moment saaledes, at de almindelige Udlægningsprinciper
...
% --- p. 63 ---
\opage{63} Behandlingen af Miraklerne, hvor Fortolkeren nu paa saa mangfoldige
...
især saadanne, som udtrykkelig ere bestemte til at ydmyge den naturlige
...
% --- p. 64 ---
\opage{64} at forholde sig religiøs-ethisk til den inspirerede Skrift. Det er denne
...
% --- p. 65 ---
\opage{65} Lægmanden, vil denne Dannelse og Lærdom sætte ham istand til at fortolke
...
% --- p. 66 ---
\opage{66} At fortolke Bibelen, Kirkens hellige Skrift, er med andre Ord at udfinde,
...
Modsætning,
% --- p. 67 ---
\opage{67} der allerede tidlig gav sig et videnskabeligt Udtryk i Striden mellem den
...
For\-%
% --- p. 68 ---
\opage{68}legenhed. Vil man derimod disputere med ham om Stedets Forhold til det hele
...
% --- p. 69 ---
\opage{69} I disse Christi Ord er her ikke blot Eenhed af Aand og Bogstav, men her er
...
Old\-%
% --- p. 70 ---
\opage{70}tidens Moralister og vise Mænd have fremsat lignende moralske Forskrifter i
...
% --- p. 71 ---
\opage{71} til at tage dig selv til Rette? --- Aldrig er Selvkjærligheden farligere, end
...
% --- p. 72 ---
\opage{72} Kind, men, til Beviis paa at han ikke agtede det, endog ligesom haane
Forhaanelsen, og vende den venstre Kind til.

En saadan Trods og Foragt, hvori Selvkjærligheden hemmelig triumpherer, en
% AS PRINTED (collation 2026-09-24, second reader agrees): «den christelige Selvfornegielse, at Mennesket»
saadan Trods og Foragt er dog saa himmelvidt forskjellig fra den christelige
Selvfornegielse, at Mennesket i al Skrøbelighed hellere maatte gjengjælde det
...
% --- p. 73 ---
\opage{73} Inderlighedens Kraft maa det Menneske ikke besidde, der skal opfylde dette
% AS PRINTED (collation 2026-09-24, second reader agrees): «der øver Selfornegtelse saaledes»
Bud! thi Enhver, der øver Selfornegtelse saaledes, at Verden forstaaer, at
...
% `let': the l is a turned sort, printed like `jet' (collation 2026-09-24, two readers).
...
% --- p. 74 ---
\opage{74} existentielle Tænkning adskiller sig netop derved fra den videnskabelige, at
...
% --- p. 75 ---
\opage{75}delse, ere da heller ikke forestillede, men virkelige; men det virkelige
...
% --- p. 76 ---
\opage{76} Underviisning, Formaning og Veiledning, og splitter Menneskene ad, at Enhver
...
Men\-%
% --- p. 77 ---
\opage{77}nesket det rette Valg i Afgjørelsens Stund; den Aand, der bestemmer
...
% --- p. 78 ---
\opage{78}dets Kraft, men skuffer sig selv ved en indbildt Forstaaelse og bliver at
...
% --- p. 79 ---
\opage{79} Antagelser man end foretrækker, maa den herskende Theologie gaae tilgrunde. I
...
% --- p. 80 ---
\opage{80} At Theologerne, jeg mener de ægte-videnskabelige, især de
...
% --- p. 81 ---
\opage{81} Som bekjendt, ere Tydsklands Theologer for nærværende Tid ivrigt
...
satte den i
% --- p. 82 ---
\opage{82} Skole hos Hegel, og derpaa siden efter lod den optræde i sit navnkundige
...
% --- p. 83 ---
\opage{83}tro, og beviser saa deraf, at alle Mirakelhistorierne ere Myther, i det
...
vise, at det Skete
% --- p. 84 ---
\opage{84} ikke saaledes er guddommeligt, altsaa bortforklare det absolute Indhold
...
fortsatte giennem Aarhundreder, at takke for den Indsigt, at Alt i Verden
...
Aar%
% --- p. 85 ---
\opage{85}sager og Virkninger, kunne de bibelske Fortællinger, i hvilke dette Væv
...
% --- p. 86 ---
\opage{86} Christendommen, saa ligger Religionen paa sit Yderste, og forventer kun
...
% --- p. 87 ---
\opage{87} det evige Liv. Langtfra! Philosophien kan jo ikke leve uden Idee: naar
...
% --- p. 88 ---
\opage{88} % The Greek is set in antiqua italic against the surrounding Fraktur.
...
% --- p. 89 ---
\opage{89} og altsaa heller ikke opløfte sig over denne endelige Verden. Ideen er i
...
% --- p. 90 ---
\opage{90} Man har meent, at Den, der antog en Aabenbaring uden først at prøve dens
% AS PRINTED (collation 2026-09-24, second reader agrees): «obiective Sandhed i objectiv Viden»
obiective Sandhed i objectiv Viden, forsømte en Sandhedspligt, hvorfor
...
% --- p. 91 ---
\opage{91} disputere med hinanden; men i Virkeligheden kan den Ene ikke ved noget
...
Naar da nu et Men%
% --- p. 92 ---
\opage{92}neske træder op og forkynder sig som Guds eenbaarne Søn: hvilke Kriser
...
% --- p. 93 ---
\opage{93} Det Forhold, hvori Christus ved sin første Optræden sætter sig til sine
...
% --- p. 94 ---
\opage{94}gen“; saa har den lærde Videnskab glemt Troen, og bevæger sig i lutter
Endelighed.
...
% --- p. 95 ---
\opage{95} ingen Apostel kunde, saa at sige, holde Dagbog over Jesu Gjerninger (jfr.\
Joh.\ 20, 30---31. 21, 25).
...
% --- p. 96 ---
\opage{96} heller ikke et \textit{jacta alea esto}, men det høieste Løsen er i
...
ogsaa --- \emph{ueensartet} med \emph{Theologien}.
...
% --- p. 97 ---
\opage{97} Endskjøndt Theologerne naturligviis ikke kunne gaae ind paa, hvad de i
...
% --- p. 98 ---
\opage{98} I Ideen og Grundanskuelsen mener nu Theologen at forsone Christendommen med
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «maa det erkiendende Subject gaae i sig selv»
maa det erkiendende Subject gaae i sig selv, stille sig i et afgjørende
...
% --- p. 99 ---
\opage{99} heraf, at de hverken kunne forholde sig ligefrem til hinanden eller
...
% --- p. 100 ---
\opage{100} Viden, men meget mere viser sig som en tvetydig Sammensmeltning af
...
% --- p. 101 ---
\opage{101} af Jomfru Maria. Den halvspeculative Christologie kan nu vistnok fremsætte
...
% --- p. 102 ---
\opage{102} Forsoning af Tro og Viden begynder uvilkaarlig med den Grundfeil, at stille
...
% --- p. 103 ---
\opage{103} Brevene derimod, der fremsætte Læren i større Almindelighed, synes at komme
...
% --- p. 104 ---
\opage{104} Videnskaben ikke magte, og hvad Videnskaben ikke kan magte, vil den heller
...
% --- p. 105 ---
\opage{105} Heraf indsees da tillige, hvad man skal dømme om de christologiske Steder,
...
paa de stedfindende Forargelser, med al den Iilfærdighed, som Øieblikkets
...
Skikkelse, ikke holdt det for et Rov at være Gud liig, men forringede sig
selv og tog en Tjeners Skikkelse paa, og blev Mennesker liig; og da han var
...
% --- p. 106 ---
\opage{106} man da ikke Misforholdet! Hvad Troens Apostel skriver til Troende til
...
% --- p. 107 ---
\opage{107} i al Almindelighed; men denne almindelige Indrømmelse fører os dog
vel endnu ikke ind i speculative Betragtninger?

Som Inspirationsprincipet er et overvidenskabeligt Existentialprincip, der
% AS PRINTED (collation 2026-09-24, second reader agrees): «kun gjælder jor den Troende i Troen»
kun gjælder jor den Troende i Troen, saaledes er ogsaa
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «var i ten bedste Forstaaelse med den»
om den menneskelige Forstand nu var i ten bedste Forstaaelse med den
...
enten maa forkaste
% --- p. 108 ---
\opage{108} denne Fristelse som en absurd Phantasie, eller ogsaa tage Fornuften fangen
...
Betragt\-%
% --- p. 109 ---
\opage{109}ningen, og tilhører det æsthetisk-videnskabelige Ideegallerie, hvor Enhver,
...
„\emph{aaben\-%
% --- p. 110 ---
\opage{110}bare Hjerternes Tanker},“
...
% --- p. 111 ---
\opage{111} at give Troen, hvad Troens er, og Videnskaben, hvad Videnskabens er.
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «seire og vinde, naar det i Sandhed ererkjendes og i Gjerning bevises,»
seire og vinde, naar det i Sandhed ererkjendes og i Gjerning bevises,
% The line ends „i Sandhed er=“ and the next begins „erkjendes“: the print doubles
% the syllable, and it now stands as printed (collation 2026-09-24; formerly silently resolved).
...
% --- p. 112 ---
\opage{112} Den Række af Indsigelser, der i disse Foredrag fremføres, saavel imod
...
% --- p. 113 ---
\opage{113} Typus for Kirkelæren i Almindelighed, men Kirkens Historie viser tilstrækkelig,
...
% --- p. 114 ---
\opage{114} kan Lærestanden heller ikke undvære Lærebegrebet og Dogmatiken. Vistnok kunne
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «lade sig afbenytte ttl et ligefremt Angreb»
Vending, at de med største Eftertryk lade sig afbenytte ttl et ligefremt Angreb
paa Theologien selv.
...
% --- p. 115 ---
\opage{115} Dogmatiken gjør jo Fordring paa at behandles systematisk; jeg vil denne Gang
...
vilde da netop
% --- p. 116 ---
\opage{116} have viist sig deri, at han forvanskede Læren, fordi han raisonnerede over
...
% --- p. 117 ---
\opage{117} Væsen som Faderen; uagtet Athanasius dog bestræbte sig for at gjøre ham det
...
idio\-%
% --- p. 118 ---
\opage{118}matum} og \textit{ubiquitas corporis Christi} givne til Priis for opløsende
Kritik.
...
% --- p. 119 ---
\opage{119} Vei. Skulle de nu desuagtet indskjærpes som rene objective, almindelige
...
% --- p. 120 ---
\opage{120}nemmelig takke sine Theologer for; thi det var da især dem, der ved deres
...
% --- p. 121 ---
\opage{121} Statskirken sandelig ogsaa saaledes objectiv, at den i Forhold til den religiøse
...
Nei, den
% --- p. 122 ---
\opage{122} Dogmatik, hvortil Kirken trænger, er ingen anden end den Dogmatik, Kirken selv
...
Tages Ideen bort,
% --- p. 123 ---
\opage{123} bliver Dogmatiken aandløs; men Kirken kan dog vel ikke være tjent med at have
...
% --- p. 124 ---
\opage{124} skal erkjendes objectivt, fordi det selv er objectivt, viser sig at være en
...
% Only `fordi' (p. 124) is letterspaced; `de ere objective;' is solid (collation 2026-09-24,
% two readers; an earlier note here said otherwise).
% --- p. 125 ---
\opage{125}jective; aleneste Aabenbaringens Object vil, baade før Troen og i Troen og
...
Det er mig ikke ubekjendt, at den theologiske Bevidsthed korser sig for
...
% --- p. 126 ---
\opage{126} --- maaskee endog til Aaret 1853\footnote{Dette Aarstal er vel egentlig angivet
...
% [collation 2026-09-24: two readers now see faint crossbar traces on the third digit and
% lean to a weakly inked 4 (i.e. 1843 as printed); the 1813 reading is held,
% unresolved.]
...
Egne, ikke stirret
% --- p. 127 ---
\opage{127} paa disse Sphinxer: hvo veed? maaskee skulde min Bevidsthed da endnu have
...
Noget, da maatte det nok være, at jeg famlede forlænge, og ikke var resolut nok
...
deels gribe
% --- p. 128 ---
\opage{128} Leiligheden til at vinde --- om end kun paa anden Haand --- en lille Seier over
...
% --- p. 129 ---
\opage{129} Det er bekjendt, hvorledes de græske Philosopher spurgte, om Dyden var
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «kritiske Inledninger, og har vel»
Paaskeberegninger og kritiske Inledninger, og har vel desaarsag kun liden
...
% --- p. 130 ---
\opage{130} Theologien deler sig selv i den theoretiske og den praktiske. Den
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «et heelt ander Svar»
et heelt ander Svar, end den theoretiske.
...
% --- p. 131 ---
\opage{131} om man vilde regne baade Dimisprædikenen, den catechetiske Prøve og
...
% --- p. 132 ---
\opage{132} I Statskirkens egentlige Myndighedsperiode, da man ivrede for objectiv
...
% --- p. 133 ---
\opage{133} hele kirkelig-borgerlige Fremtid: hvorledes, siger jeg, var dette muligt,
...
% --- p. 134 ---
\opage{134} Vel var det paa en Maade, at den gamle Spørger døde, dengang han døde!
...
% --- p. 135 ---
\opage{135} da ogsaa snart mærke paa sig selv, om han overhovedet er oplagt til at være
...
% --- p. 136 ---
\opage{136} som om de mange Forklaringer joge Klarheden bort, thi Theorierne modsige jo
...
bliver nogen rigtig Theolog, ak! saa spildte han sin Tid og Flid, saa
...
% --- p. 137 ---
\opage{137} ikke er større, end hvad en nogenledes begavet Student kan overkomme, naar
...
% `i' (dannet i Almindelighed): a damaged sort; the collator read `k', a second reader an i
% with a stray blob, a third a k-like sort with a dot. Held as `i' (collation 2026-09-24).
...
selv var bleven
% --- p. 138 ---
\opage{138} en rigtig Theolog, en Theolog i det Større, igjen arbeidede paa at danne
...
at han
% --- p. 139 ---
\opage{139} ligefrem maa bortkaste sin Theorie, for at finde sin Praxis: thi Digterens
Ord:

% The two German lines are set in Fraktur, like the body (collation 2026-09-24, two readers;
% an earlier note here called them antiqua). Set as a plain quotation.
\begin{quote}
...
bedømme det reli\-%
% --- p. 140 ---
\opage{140}giøse Livs forskjelligartede Phænomener og finde sig tilrette i de
praktiske Forhold.“
...
% --- p. 141 ---
\opage{141}tiske Skole ikke var istand til at afgive en nok saa retfærdig Maalestok,
...
% --- p. 142 ---
\opage{142}kant prædiker i Reglen, om ikke just efter en bestemt Dogmatik, saa dog
...
Spidsen,
% --- p. 143 ---
\opage{143} og hævde Christendommens christelige Charakteer, grebe Præsterne til den
...
at \emph{Christendommen er høiere end Videnskaben, og Evangelietroen
ueensartet med Theologien.}
...
% --- p. 144 ---
\opage{144} % The italic argument printed at the head of the lecture on p. 144 agrees
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «imod en religiøs Erkiendelse af Christendommen»
Christendommen, ei heller imod en religiøs Erkiendelse af Christendommen. Vil
...
% --- p. 145 ---
\opage{145}hold til Christendommen. --- Den saaledes forandrede Theologie vil da igjen
...
% --- p. 146 ---
\opage{146} % „imod“ and „for“ are letterspaced in the print (Sperrsatz); verified at
...
% --- p. 147 ---
\opage{147} igjennem, at den kan naae det sidste og reneste Resultat. Men det kommer nu
...
Sandsynlighedsbeviser \emph{for} og \emph{imod}, saa bliver Videnskaben upartisk og
...
% `Inspirationtheorie' (no linking s): as printed (collation 2026-09-24; transcriber and collator agree).
En videnskabelig Inspirationtheorie er en Modsigelse. En\-%
% --- p. 148 ---
\opage{148}hver, der forsøger at udvikle en saadan Theorie, bliver ved Conseqvensernes Magt
...
% --- p. 149 ---
\opage{149} Bølgebrydningen ved det stormende Forbjerg. I en saadan Bølgebrydning er det
...
hvert Dogme staaer
% --- p. 150 ---
\opage{150} „et Kors for Tanken“, at den videnskabelige Reflexion forholder sig skeptisk til
Troen.
...
Videnskab;
% --- p. 151 ---
\opage{151} den er en Skole for den christelige Opdragelse, den existentielle Erkjendelse,
...
% --- p. 152 ---
\opage{152} Saaledes bevæger den theoretiske Theologie sig imellem lutter hypothetiske
...
% „Ergo“ is set in ANTIQUA in both syllogisms (two readers) and read as letterspaced
% by the transcriber and the collator (a third reader measured it solid) (collation 2026-09-24).
\emph{\textit{Ergo}}: har den christelige Kirke ogsaa sin sande fuldgyldige Realitet.“
...
\emph{\textit{Ergo}}: maa Kirkens Begreb ogsaa lade sig fastsætte og fastholde med
...
% --- p. 153 ---
\opage{153} Kirken er da nu efter sit Begreb at bestemme som den faktisk nærværende
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «og Menighedens subiective Tro. For at oplyse»
Væsen og Menighedens subiective Tro. For at oplyse dette ved et Exempel henviser
...
% --- p. 154 ---
\opage{154} Hvad følger saa heraf? Ingenlunde, at det Objective nu skulde formildes i
...
% --- p. 155 ---
\opage{155} % PRINTER'S DEFECT: the quotation opened on p. 154 with „et tveegget Sværd is
...
% --- p. 156 ---
\opage{156} med sit Enkelte, men hver Enkelt har i sit Enkelte tillige det Totale. Hver
...
% --- p. 157 ---
\opage{157} Menighedens Subjectivitet bygger paa Kirken som paa sin faste Klippe, sit
...
forstaaes i al
% --- p. 158 ---
\opage{158} Eenfoldighed, thi Troen er det Eenfoldige. Hvormegen dialektisk Kunst den
...
% --- p. 159 ---
\opage{159} % The italic argument printed at the head of the lecture on p. 159 agrees
...
% --- p. 160 ---
\opage{160} udenfor liggende Formaal; i første Henseende afgiver den en Høiskole for
...
% --- p. 161 ---
\opage{161} svække Tilhørernes Interesse, de foranledige et skjult Misnøie paa begge
...
% --- p. 162 ---
\opage{162} Qvalificationsprøve kunde fordre, og fordre det med ufravigelig Strenghed at
...
intrædende Tilhører møder her overalt de gamle velbekjendte
...
% --- p. 163 ---
\opage{163} Paa Læren om Kirken følger den religiøs-ethiske Fortolkning af den hellige
...
% --- p. 164 ---
\opage{164} Fordannelse. Denne Forskjel paatrænger sig endnu fra en anden Side.
...
% --- p. 165 ---
\opage{165} Mangler han denne Sands, det vil sige: er den religiøs-ethiske Interesse ikke
...
% --- p. 166 ---
\opage{166} kan være Tanke om nogen udvortes Forandring af det Bestaaende, maa jeg dog
...
% --- p. 167 ---
\opage{167} udsat for at forvexle den praktiske Theorie med den virkelige Praxis, og
...
% `sit': a collator read `sti' (transposed sorts); a second reader sees `sit' with a
% broken t. Held as `sit' (collation 2026-09-24).
...
% --- p. 168 ---
\opage{168} bevæget til at berede den objective Theologie et udvortes objectivt Nederlag;
...
% --- p. 169 ---
\opage{169} de ere endog i en vis Forstand --- jeg siger det uden Fortrydelse --- de ere
...
de
% --- p. 170 ---
\opage{170} Studerendes Side: saa haaber jeg da paa disse Vilkaar at kunne passe mine
...
paa, at Aanden var
% --- p. 171 ---
\opage{171} vegen bort, og den hele Underviisning sunken ned i en tom Mechanisme?
...
% --- p. 172 ---
\opage{172} Theologiens unge Dyrkere kunne da endnu, som før, med Lyst og Iver studere
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «kun nnder alle disse Studier tillige»
kun nnder alle disse Studier tillige ville erindre, hvad der ligger i denne
...
Det bliver altsaa, kort og godt, ved den Sætning: Troen er et Paradox.
...
% --- p. 174 ---
\opage{174} bortforklare „Paradoxet“ og conservere Troen; men heller ikke denne
...
Jødedom, baade Apostlenes og Abrahams Tro bliver nøiagtig og conseqvent ---
