...
#   Bodleian religion-1869.pdf : PDF = printed + 14   <-- note: NOT the same
#
# The Bodleian PDF has NO usable OCR text layer (only the Google front matter),
# so collation there is necessarily visual.
set -e
D="$(cd "$(dirname "$0")" && pwd)"
B="$HOME/bibliotek/Nielsen, Rasmus/religion-1869.pdf"
[ -f "$B" ] || { echo "Bodleian scan not found at: $B" >&2; exit 1; }
mkdir -p "$D/.render"
OFF=14
A=$(( $1 + OFF ))
