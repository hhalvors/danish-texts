...
%
%  BATCH SEAMS: the file was built by splicing thirteen page-range fragments
%        into a skeleton that separated them with blank lines. A blank line is a
%        paragraph break, so at every seam where the printed text simply runs on
%        it had to go — at p. 96/97 it was breaking a word in half. Which seams
%        run on was settled by MEASUREMENT, not by reading: for each seam page,
%        the left edge of its first full-measure line was compared with the
%        median left edge of the lines below it, on the page image.
%        ELEVEN seams continue the previous paragraph (indent within ±10 px):
%        pp. 13, 25, 49, 61, 73, 85, 97, 109, 121, 133, 157 — blank line removed.
%        TWO begin a new indented paragraph: p. 37 (+223 px) and p. 145 (+230 px)
%        — blank line kept.
%        NB the folio at the head of the page must be excluded from that
%        measurement, and on many leaves a sliver of the facing leaf's black edge
%        survives the crop and joins the folio's row band, making the folio look
%        like a full-measure line and putting its own left edge — about 1300 px
%        in — forward as the paragraph indent. Ignoring the right-hand 8 % of the
%        sheet fixes it. Without that, every seam reads as a new paragraph.
%
