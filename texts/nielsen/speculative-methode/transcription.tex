...
%        THE KB COLOUR PDF (kb.dk/e-mat/dod/110208016515-color.pdf) HAS NOW
%        BEEN CHECKED AND LACKS THE LEAF TOO. Both digitisations are of the
%        same copy — KB shelfmark DA 1.-2.S 2 8° — so the leaf is presumably
%        missing from that copy rather than skipped by the scanner. No other
%        copy has been found (Google Books, nb.no). The preface is therefore
%        transcribed as the fragment it is, and marked as such at its site.
%        Whether the leaf survives in the physical volume, and whether KB holds
%        a second copy, is on the standing list of library tasks in the repo
%        root (KB-VISIT-TODO.md).
%
...
%
%  PARAGRAPHING ACROSS THE LEAF — and how it was got wrong twice.
%        The file was built by splicing thirteen page-range fragments into a
%        skeleton that separated them with blank lines. A blank line is a
%        paragraph break, so wherever the printed text simply runs on across a
%        leaf, that blank invents a paragraph the book does not have — and at
%        the p. 96/97 seam it was splitting a word in half.
%        The first pass audited only the thirteen BATCH SEAMS and removed eleven
%        blanks (pp. 13, 25, 49, 61, 73, 85, 97, 109, 121, 133, 157), keeping the
%        two that open a genuinely indented paragraph (pp. 37, 145).
%        THAT WAS NOT ENOUGH: the same fault sits at page joints INSIDE a batch,
%        which the seam audit never looked at. It was caught by eye in the built
%        PDF, at p. 147, after the edition had been called complete. A full audit
%        of all 162 markers then found eight more false breaks (pp. 64, 68, 90,
%        94, 98, 102, 147, 161) and five pages that DO open an indented paragraph
%        and had been run on (pp. 33, 41, 51, 57, 115). All thirteen are fixed.
%        `paragraphs.py` now performs that audit and should be run before any
%        future publication of this file. It decides by two independent tests —
%        the left edge of the page's first full-measure line against the margin
%        below it, and whether the previous page ends mid-word or mid-sentence —
%        and reports, rather than acts on, anything the two disagree about.
%        THE IMAGE TEST ALONE IS NOT SAFE. Two traps: the folio, joined to a
%        residual sliver of the facing leaf's black edge, poses as a full-measure
%        line whose own left edge reads as a 1300 px indent; and a centred § head
%        measures as a huge indent. Even trimmed and guarded it still gives four
%        false positives here (pp. 8, 12, 14, 151), all rejected by the text test
%        and all confirmed flush on the image.
%
%  PAGE-JOINT WORD BREAKS — and the second, quieter half of the same fault.
%        Fixing the hyphen is not enough. A word broken across a printed page
%        boundary is joined with `\-%` (below), but the joint also has to survive
%        whatever stands between the two halves in the file, and at 59 page
%        markers that is a `\setcounter{footnote}{0}` line. Its trailing newline
%        is an interword space. Where the page merely continues a sentence that
%        space is harmless — the previous line's newline already supplied one and
%        TeX collapses the pair — but where the joint breaks a WORD there is no
%        other space, and "gjennem Til\-%" + a bare \setcounter line + "egnelsen"
%        typesets as "Til egnelsen". Six words in this book were split that way.
%        They survived the first repair of the hyphens precisely because the
%        hyphens had by then been fixed correctly: the defect looked mended in
%        the source and was still there in the PDF.
%        Every \setcounter{footnote}{0} in the body therefore carries a trailing
%        `%`. It is bookkeeping and must contribute nothing to the typeset line.
%        The general lesson, which cost three passes to learn: VERIFY A JOINT IN
%        THE BUILT PDF, not in the source.
%
...
%        ſ -> s (8730), œ -> æ (552), ﬀ ﬁ ﬂ ﬅ -> ff fi fl st, < -> ch (53,
...
%        without an accent.
%
%  QUOTATION MARKS: *** A QUOTATION IN THIS BOOK USES ONE SORT AT BOTH ENDS. ***
%        It does NOT use the Danish low-high pair „…“ that opens on the baseline
%        and closes at cap height. The editor, reading the paper, reports that
%        the opening and closing marks are meant to sit at the SAME HEIGHT — and
%        which height that is varies:
%          * the BODY, throughout, uses the RAISED mark U+201D ” at both ends.
%            Found independently by six transcribers working from the image at
%            2--6x, over the front matter and the whole body.
%          * SOME NOTES use the LOW mark „ at both ends instead. Printed p. 50's
%            Guerike note is the clear case: both of its quotations open low and
%            close low. Measured against the line's own baseline and x-height,
%            its three marks sit in one vertical band, while the closer in
%            p. 63's note sits distinctly higher.
%        Both are reproduced as printed; p. 50 accordingly reads „…„ twice.
%
%        NOT YET RE-EXAMINED under this principle: the notes on pp. 48, 70, 79,
%        84 and 106, transcribed „…“ by the batches that read them, before the
%        same-height principle was known. If it holds everywhere, their closers
%        are low rather than high. Whoever settles that should MEASURE rather
%        than look — the two heights differ by well under a millimetre on the
%        page, and a 1-bit scan flatters whichever reading you expect.
%
%  QUOTE BALANCE: there is no single signed balance to take, because a „ used as
%        a closer has no matching “ and would otherwise read as an unclosed
%        quotation. check.py therefore runs two tests — a signed „ vs “ balance
%        and a parity test on ” — each compared against a hand-kept table of
%        sites. That table currently carries p. 49 (a quotation opened and never
%        closed, as printed), p. 50 (the two low-at-both-ends quotations above),
%        p. 80 (an outer quotation opened raised and never closed) and p. 151
%        (the same). Both tests are consistent with it.
%
%  STATUS: TRANSCRIPTION COMPLETE — the front matter and printed pp. 1--162,
%        the whole of the book that survives in this scan, every page
%        image-verified. §§ 1--23 all present and continuous across both Deele;
%        no gaps, no duplicate page markers; braces balanced; all twelve
%        Rettelser applied and flagged % ERRATA at their sites (pp. 4 twice, 7,
%        10, 12, 15, 41, 42, 48, 71, 89, 132). 100 footnotes on 66 printed
%        pages, each of those pages carrying its \setcounter{footnote}{0}.
%        Compiles 0 errors / 0 missing characters / 0 undefined control
%        sequences / 0 hyperref token warnings / 0 duplicate PDF destinations.
%        QUOTE COUNTS CLOSE against the five logged printer's defects at
%        pp. 49, 50 (twice), 80 and 151.
%        Six places where the Indhold and the printed § head disagree are
%        recorded with BOTH readings and the printed head set: §§ 3, 7, 10, 13,
%        15, 21. They are not normalised; the disagreement is a datum.
%        STILL OPEN: (1) the missing preface leaf above; (2) the two closing
%        marks in p. 50's Guerike note, raised or low — see RESUME-NOTES;
%        (3) three pointed Hebrew words (pp. 33, 43, 159) transliterated for
%        want of a Hebrew font, each flagged at its site.
...
% NB every \setcounter{footnote}{0} in the body is written with a trailing % .
% It is bookkeeping, not text, and must contribute nothing to the typeset line.
% Without the %, its newline becomes an interword space — harmless where the page
% simply continues a sentence (the space is already there from the previous
% line's newline, and TeX collapses the pair), but WRONG at a page joint that
% breaks a word: "gjennem Til\-%" then a bare \setcounter line then "egnelsen"
% typesets as "Til egnelsen". Six words in this book were split that way, and it
% survived the first repair of the bare-hyphen joints because the hyphen itself
% had been fixed correctly and the space came from somewhere else.
...
% --- p. 1 ---
\opage{1} % DISPLAY FORMS, read at 1.8--3x on PDF 10, 14 and 20:
...
naar
% --- p. 2 ---
\opage{2} Tænkningen skal lempes efter Erfaringen og Erfaringen omvendt corrigeres af
...
% --- p. 3 ---
\opage{3} jectiviteten paa den ene Side gjelder saa Lidet, at den paa den anden Side
gjelder Alt.
...
% --- p. 4 ---
\opage{4} liteten. Ere nemlig alle Objecter kun Forestillinger og Billeder, dannede af
...
betragte
% --- p. 5 ---
\opage{5} vi kun den ydre Verden, kunne vi ikke falde paa at nægte, at jo hvert Menneske
...
% --- p. 6 ---
\opage{6} Indflydelse; Alt opfattes ved indre og ydre Sandser, og man forsvarer Alt,
...
% --- p. 7 ---
\opage{7} at tænke Andet, end at opsøge og afsløre det Almindelige i det Særegne,
...
vare nedstødte
% --- p. 8 ---
\opage{8} i Abstractionens Afgrund, og dog tillige indesluttede alle Viisdommens Skatte
...
% --- p. 9 ---
\opage{9} sigelses Kraft udelukke derfor disse Momenter hinanden. Men grundfalsk er
...
% --- p. 10 ---
\opage{10} % The quoted words here are NOT letterspaced, unlike the parallel quotations on
% p. 7; checked at 3x.
...
% --- p. 11 ---
\opage{11} % Printed head: "§ 3." (no period after the section sign), as at § 1.
...
% --- p. 13 ---
\opage{13} % p. 13 opens FLUSH LEFT (no indent), i.e. it continues the paragraph running
...
% --- p. 14 ---
\opage{14} skeligt. Thi det er ikke blot en Andens Feiltagelse, man har at befrygte; men da
...
% --- p. 15 ---
\opage{15} heden bliver der dog stedse en Ulighed tilbage; der vilde jo ellers intet Nyt
...
% --- p. 16 ---
\opage{16} der paa dette Standpunkt ikke være Tale. Men spørger man nu, hvorledes man i den
...
% --- p. 17 ---
\opage{17} hvervet sig samme. Vi dadle ikke Sammenligningen af de enkelte Momenter i og for
...
% --- p. 18 ---
\opage{18} \parag{4}{Overgang til den speculative Empirie.}
...
% --- p. 20 ---
\opage{20} Den sande Historie fordrer altsaa vel, at man skal søge den objective Fornuft i
...
Nogen blot sige:%
% --- p. 21 ---
\opage{21} \emph{”jeg har Intet,”} saa vilde man ikke ret forstaae ham; sagde han derimod:
...
% --- p. 22 ---
\opage{22} ket skal foregaae i det ontologiske Beviis, have faaet en anden Form for Guds
...
% --- p. 23 ---
\opage{23} neskelige Bevidsthed, og han vilde da maaskee være kommen dertil ret at
...
almindelige Ka%
% --- p. 24 ---
\opage{24} tegorier: Frihed, Despotisme, Hierarchie eller andre lignende, som Indklædning
...
% ============================================================
% --- p. 25 ---
\opage{25} % p. 25 opens mid-paragraph: p. 24 ends "...og heller ikke" (complete word, no
...
% --- p. 26 ---
\opage{26} kommenhed, i det denne vel kan adskille den objective Fornufts
...
% --- p. 27 ---
\opage{27} af det Forhold, hvori den bestemte Gjenstand staaer til det
...
% --- p. 28 ---
\opage{28} saadan Maade, at de vel i enkelte Punkter berøre hinanden, men
...
indrømme gjerne, at
% --- p. 29 ---
\opage{29} alle Hverdagsforstandens Regler maae antages af den, der staaer paa
...
Indbildningskraft.
% --- p. 30 ---
\opage{30} Thi vel fordrer Systemet, at de nødvendige Begreber skulle fastholdes,
...
% --- p. 31 ---
\opage{31} den speculative Logik; det andet negerer Logiken og fører os ind paa
...
% --- p. 32 ---
\opage{32} see hen til de specielle Gjenstande, en slet og ret Negation af
...
Adskillelse.
% The printed page opens an INDENTED new paragraph (image-verified,
% ~225 px against this page's own body margin), so the blank line below
% is a real paragraph break, not batch-skeleton residue.

% --- p. 33 ---
\opage{33} Vi geraade da for det Første ikke oftere paa den Afvei, under Navn af
...
% --- p. 34 ---
\opage{34} Skriftsteder i Spidsen, som man fordriver Djævelen ved at slaae Kors
for sig.
...
Speculationen,
% --- p. 35 ---
\opage{35} denne Hverdagsforstanden og Kritiken. Thi det ligger i de kritiske
...
% --- p. 36 ---
\opage{36} Gud skabte Verden eller hvorledes han inspirerede Propheterne og
...
% --- p. 37 ---
\opage{37} Den Modsigelse, hvorfra den sande Methode nødvendigt maa gaae ud, viser sig
...
% --- p. 38 ---
\opage{38} imellem Jeg og Mig, for saaledes at benævne den formelle Videns Subject og
...
der maa
% --- p. 39 ---
\opage{39} have givet Verden sin Oprindelse; vi maae som en Følge deraf, hvis vi ville
...
% --- p. 40 ---
\opage{40} kerne udviste, da de forkjettrede Luther, af en anden Beskaffenhed end den,
...
% --- p. 41 ---
\opage{41} Det næste positive Grundlag fremkommer, naar man gaaer ud fra at negere
...
% --- p. 42 ---
\opage{42} % The printed head and the Indhold AGREE here: both read "Begriben og Anskuen."
...
% --- p. 43 ---
\opage{43} Sider er formaliter tilstede i den formelle Viden, men er realiter og absolut
...
% --- p. 44 ---
\opage{44} fuldkomment gjennemtrænge Tiugene med sin Viden; --- uden Abstraction, ingen
...
Videns Urtypus.
% --- p. 45 ---
\opage{45} Den menneskelige Selvbevidsthed, der er et Afbillede af den guddommelige,
...
% --- p. 47 ---
\opage{47} % NO \setcounter here: the block under the rule on this page is the tail of
...
da han
% *** QUOTATION MARKS IN THIS NOTE: LOW AT BOTH ENDS. ***
% Both Guerike quotations open with the low „ and CLOSE with the low „ —
% „Die heilsam … weiter etc.„ and „Gott also … cap. XIV.).„ — and are set so
% here. They were first transcribed as closing with a raised ”, on a 4x reading;
% the editor then read the paper and reported that this book means its opening
% and closing marks to sit at the SAME HEIGHT, which a measurement against each
% line's own baseline and x-height bears out: this note's marks sit in one
% vertical band, while the closer in p. 63's note sits distinctly higher.
% So the note is consistent Danish setting with the low sort, not a mixed pair,
% and there is no compositor's defect here to log.
% --- p. 50 ---
...
% --- p. 51 ---
\opage{51} % NO \setcounter HERE, AND NONE IS MISSING: p. 51 carries no note of its own.
...
% --- p. 52 ---
\opage{52} ment klar, deels fordi de reelle Momenter, den omfatter, ere aldeles universelle
...
% --- p. 53 ---
\opage{53} sætning for enhver historisk Viden. Man kan nemlig ikke begribe Menneskeslægtens
...
% --- p. 54 ---
\opage{54} naturlige og naturlige Rige, maa den dog ligesaavist lade en Modsætning mellem
...
% --- p. 61 ---
\opage{61} % ELISIONS. Throughout pp. 61--72 the notes set elisions as three, four or six
...
% footnote mark.
% --- p. 81 ---
\opage{81} Bogstaver som det eneste og tilstrækkelige Grundlag for den positive Lære,
...
% --- p. 85 ---
\opage{85} % JOINT WITH p. 84, verified on the image (PDF 93 foot / PDF 94 head, 3x):
...
Grunde,
% --- p. 86 ---
\opage{86} % No footnote block on this page either.
...
% --- p. 87 ---
\opage{87} % "%" below suppresses the newline: the printed page continues the previous
...
% --- p. 88 ---
\opage{88} % No footnote block on this page.
...
% --- p. 89 ---
\opage{89} % "%" below suppresses the newline: the printed page continues the previous
% page's sentence, and a bare newline here would double the interword space.
\setcounter{footnote}{0}%
...
% ERRATA (Rettelser, p. 89 l. 5 fra oven) --- the Greek entry:
...
% --- p. 91 ---
\opage{91} % NO \setcounter HERE ON PURPOSE: p. 91's footnote block carries only the tail
...
% --- p. 92 ---
\opage{92} % The trailing "%" on the next line is load-bearing: p. 91 ends mid-word
...
% --- p. 93 ---
\opage{93} % "%" below suppresses the newline: the printed page continues the previous
% page's sentence, and a bare newline here would double the interword space.
\setcounter{footnote}{0}%
...
% --- p. 95 ---
\opage{95} % "%" below suppresses the newline: the printed page continues the previous
% page's sentence, and a bare newline here would double the interword space.
\setcounter{footnote}{0}%
...
% --- p. 96 ---
\opage{96} % "%" below suppresses the newline: the printed page continues the previous
% page's sentence, and a bare newline here would double the interword space.
\setcounter{footnote}{0}%
...
subjective Ratio\-%
% BATCH SEAM, REPAIRED. p. 96 ends mid-word: "Ratio-" is completed by "nalisme"
% at the head of p. 97, and the discretionary hyphen \-% joins them without
% printing a hyphen. The blank line that the batch skeleton left here would have
% broken the paragraph in mid-word; it has been removed, as at every other seam
% where the printed text runs on. See the PAGE-JOINT and BATCH-SEAM notes in the
% header for how all thirteen seams were decided.
% --- p. 97 ---
...
% --- p. 98 ---
\opage{98} Ved \emph{Villien} er Mennesket istand til med sin hele Subjectivitets Kraft at
...
% a colon only.
% --- p. 100 ---
\opage{100} fremstilles paa følgende Maade: Enhver, der har christelig Tro, erkjender, at
...
% --- p. 103 ---
\opage{103} denne Theologie ikke kan ringeagte, langt mindre udelukke Philosophien, hvis
...
% --- p. 104 ---
\opage{104} sophen beholde, hvad der tilhører ham som saadan; thi saaledes give vi
...
% --- p. 105 ---
\opage{105} christelige Lære, og saa fremdeles; --- men fordi den berører alle Sandhedens
...
% --- p. 107 ---
\opage{107} Gyldighed, som Skriftens Lære; --- eller at antage Kirkens Former og
...
% --- p. 108 ---
\opage{108} Er han nemlig tilfredsstillet ved den Gudsbeskrivelse, Kirken har givet ham,
...
% --- p. 110 ---
\opage{110} Anvendelse af enkelte Skriftsteder, men i positiv Henseende aldeles Intet kunne
...
% --- p. 112 ---
\opage{112} blot er kjærlig og barmhjertig, men tillige, at han er vred paa dem, der have
...
% --- p. 115 ---
\opage{115} Man faaer ofte at høre, at den hellige Skrift ikke udgjør et philosophisk
...
% not text.
% --- p. 116 ---
\opage{116} gaaer ud fra den menneskelige Subjectivitet, at den tvertimod udleder al
...
% --- p. 117 ---
\opage{117} der ere eiendommelige for den hellige Historie, dem nemlig, i hvilke Gud er
...
% --- p. 118 ---
\opage{118} kunde ikke i Virkeligheden være det sande Udtryk for Ideen om Guds Søn; hiin
...
% --- p. 121 ---
\opage{121} % NO NOTE OF ITS OWN, so no \setcounter{footnote}{0} here. The whole footnote
...
% ============================================================================
% --- p. 122 ---
\opage{122} af Kriticismens egen Natur, at den adskiller de positive Momenter fra hverandre
...
% --- p. 123 ---
\opage{123} sætning til og indskrænke, begrændse hverandre, og saaledes vil nu
...
% --- p. 124 ---
\opage{124} usande. --- Erkjendelsen kræver altsaa, at Begivenheder, der ere overnaturlige,
...
% --- p. 125 ---
\opage{125} neskenaturen isolerede Gud: Christus bliver et almindeligt Menneske, en
...
% --- p. 126 ---
\opage{126} sidste Rest af den ældre Supranaturalisme. Verden er dernæst et Complex af
...
% --- p. 127 ---
\opage{127} fremgaae. Og spørger man, hvorfor det fortrinligere nu først fremtræder efter
...
% text is identical.
% --- p. 128 ---
\opage{128} \emph{Grundlag er derfor Religionen, dens absolute Maal Guds Aabenbaring.}
...
% --- p. 130 ---
\opage{130} herliger de naturlige Phænomener: saa at ikke blot Menneskene troe i et eller
...
% --- p. 133 ---
\opage{133} Mytherne, som nu atter har bevirket Erkjendelsen af disses Usandhed. Det er da
...
tager dog den sande
% --- p. 135 ---
\opage{135} Messias Sæde paa den panlogistiske Lyksaligheds Bjerg og lærer sit Folk:
...
begrundet.
% --- p. 136 ---
\opage{136} Hos Orientalerne, hvor den menneskelige Sjels sløve Øie ikke kan opdage nogen
...
vel den sande
% --- p. 138 ---
\opage{138} Historie Andet end Virkelighedens evige Nærværelse? Hvorfor skulde man da
...
en uop\-%
% --- p. 139 ---
\opage{139} hørlig Drift til Alteration og Tilintetgjørelse Tingens Marv, tilintetgjør den
...
Realitet,
% --- p. 140 ---
\opage{140} Midleren (\textit{mediator}) mellem Gud og Verden. Vi maae da nøiere prøve denne
...
% --- p. 141 ---
\opage{141} heder satte udenfor sig, saa at der ikke er nogen Idealitet tilstede i deres
...
Idealitetens
% --- p. 142 ---
\opage{142} og Realitetens Modsigelse, --- der gjør den efter Tilværelsen gribende Faders
...
% --- p. 145 ---
\opage{145} % JOINT p. 144/145 verified on the image: p. 144's note ("Hvor kraftigt og
% skarpsindigt J.~H. Fichte har bekjæmpet ...") does NOT run over. p. 145 carries
% 32 body lines and no small type at the foot at all --- the only thing below the
% last text line is the centred antiqua sheet signature "10", which is not
% transcribed. p. 145 therefore carries NO note of its own and gets no
% \setcounter. Its first line IS indented (x570 against the page's own 340 left
% edge), so a new paragraph begins here; p. 144 ended a sentence.
Panlogismen har en falsk Gud; den er derfor ulogisk (ἄλογος), dersom den ikke
% GREEK, read at 3x: ἄλογος, alpha with smooth breathing and acute. No fount
% variants in this word.
gaaer ud over sig selv; men den har en sand Logik og behøver derfor kun at tye
hen til Christus for at finde en Gud af sand logisk Natur. Den erklærer selv, at
Veien til Immanentsen kun gaaer igjennem de største Modsætninger, men dens egne
Modsætninger ere af en altfor let Natur; kommer den derfor til fuldkommen
Erkjendelse af den logiske Grundsætning, at der Intet kan fremkomme i Slutningen,
som ikke i Virkeligheden har ligget i Principet, nødes den ogsaa til at erkjende,
at den Modsætning, der viser sig i denne Verden imellem det Ideale og Reale, har
sin Forudsætning og Grund i en fra Evighed af. medieret Immanents af begge, med
% PRINTER'S DEFECT, p. 145: a POINT after "af" --- "i en fra Evighed af."
% --- where the sentence runs straight on into "medieret Immanents af begge".
% Transcribed as printed. Read at 2.5x: the mark sits on the baseline and is the
% same square dot as the other periods on the page, not a speck; both OCR
% witnesses see ink there (tesseract "af.", ABBYY "a f-").
andre Ord, i en guddommelig, uskabt, fra vor menneskelige Fornuft, som det
hedder, \emph{qvalitativt} forskjellig Fornuft. Men har man saaledes erkjendt
% \emph{qvalitativt}: letterspaced, confirmed at 2.5x (spacing.py misses it; the
% ABBYY layer preserves it as "q v a l i t a t i v t"). spacing.py's flags on
% this page ("til", 2.19; "det,", 1.35) are justification only --- checked on the
% image, the letters inside those words are tight. Rejected.
Guds absolute Frihed, vendes hele Forholdet imellem de positive Momenter og de
logiske Kategorier om; vi indsee da, at det er en positiv Gudskraft, der har
begrundet den hele reelle Tilværelse og netop derved sat de ontologiske Former
immanente i Tingene. Vi henvises da til det virkelige Livs hele Fylde, i hvilken
alle Skikkelser maae betragtes, saaledes som de møde os i Livet; og saaledes gjør
da Troens og Anskuelsens Moment atter sin Ret gjældende. Vi betragte da det
menneskelige Samfund, hvori vi befinde os, i hvilket den evige Personligheds Idee
skinner igjennem; vi see hen til dette Samfunds guddommelige Stifter, der
fremgik i Tidens Fylde, da den Lov, der var givet Jøderne, havde udviklet
Syndsbevidstheden; vi erkjende da, at Gud ad denne Aabenbarings Vei er fremtraadt
som den Treenige, og saa fremdeles. Det er fornuftstridigt at ville drage nogle
af disse Elementer i Tvivl, i den Tanke ved Hjælp af andre Elementer at komme til
en dialektisk Nødvendighed af det,
% --- p. 146 ---
\opage{146} \setcounter{footnote}{0}%
der er Gjenstand for Tvivl; thi nærer man virkelig Tvivl om et eller andet
Moment, maa man ogsaa nødvendigt tænke de øvrige positive Momenter, med hvilke
det staaer i Sammenhæng, paa en saadan Maade, at Tvivlen om hiint ligger skjult i
disses Begreb. Det nytter derfor kun lidet imod Deisten at appellere til Christi
Aabenbaring, da han har et eensidigt Begreb om Gud. Derfor skal heller ikke
Theologien bevise, at der følger af Guds Treenighed, at Christus med dialektisk
Nødvendighed har maattet aabenbare sig paa dette eller hiint Sted; men da han nu
eengang har aabenbaret sig, skal Theologien kunne begribe, hvorledes dette
positive Aabenbaringsphænomen hænger sammen med de øvrige positive Phænomener;
denne Aabenbaring er ligesom en reel Tanke, der skal kunne forsones med de øvrige
reelle Tanker. Den religieuse Begribens absolute Forudsætning er derfor den
absolute Tro, og det er saa langtfra, at Videnskaben krænkes ved denne Troens
Uforkrænkelighed, at det meget mere først er ved dennes Hjælp, at den dialektiske
Nødvendighed og Negationen kan gjennemtrænge alle Systemets Elementer. Som derfor
den abstracte Logik gaaer over i de positive Elementer, saaledes maae paa den
anden Side de positive Elementer nødvendigviis gaae over i en concret Metaphysik
og speculativ Empirie. Naar Gjenstandene sammenstilles paa udvortes Maade, er det
ude med Videnskaben, og det er derfor næsten blevet et almindeligt Ordsprog, at
enhver Philosophie bør begynde med at tvivle, skjøndt man hellere maatte sige:
”med at negere”; thi Tvivlen\footnote{At der ligger en Usandhed skjult under
% PRINTER'S DEFECT in the MARK of this note, p. 146: the note itself is signed
% "(1" --- an OPENING parenthesis before a raised 1 --- where every other note in
% the batch is signed "1)". Read at 2x. The BODY mark on the same page is the
% ordinary "1)" (after "Tvivlen", with a space before it), so only the note-foot
% mark is turned. Set as an ordinary \footnote; the printed form is recorded here.
denne Tvivl, have allerede Philosopherne anerkjendt. Saaledes J.~L. Heiberg: Al
sand Philosophie maa forene begge de af Hr. Martensen anførte Valgsprog:
Gudsfrygten maa den have i sit Hjerte, Tvivlen i sit Øie. Perseus Nr. \textit{1}.
P. \textit{41}.} staaer i Modsætning
% This note is FRAKTUR throughout and carries NO quotation marks of any kind ---
% neither the body's raised ” nor the Danish „ “ --- around either the Heiberg
% sentence or the Valgsprog. Read at 2x. Only the two figures are antiqua, hence
% \textit{}; "P." is the Fraktur P (= Pagina), following the precedent at p. 46.
% The note RUNS OVER onto p. 147, where its last two lines ("Valgsprog: ... P.
% 41.") stand under the rule above p. 147's own note. The whole of it is set here.
% No paragraph break anywhere on p. 146: every line begins at the same left edge.
% --- p. 147 ---
\opage{147} \setcounter{footnote}{0}%
% NOTE on the reset: the block under the rule on this page begins with two lines
% that carry no mark of their own --- they are the tail of p. 146's note and are
% inside that \footnote. The reset is for p. 147's OWN note, marked 1).
til Troen, men at Negationen og Abstractionen ikke krænke den, have vi alt forhen
godtgjort. Ligesom derfor det positive Objectivitetsmoment er det absolut
overveiende i Troen, saaledes vender ved Negationen Subjectivitetens absolute
Frihed tilbage, thi alle Momenter maae negeres og prøves paa en saadan Maade, at
der ikke bliver et eneste Phænomen urørt\footnote{Det er denne Prøvelse, Sibbern
kræver, naar han fordrer: ”en altomfattende Debat af Alt imod Alt.”}. Hvad der
% p. 147, note 1): the mark stands after "urørt" and BEFORE the period. The
% quotation inside it takes the body's RAISED ” at both ends --- read at 2x, the
% opening mark is at cap height, not on the baseline. So this note does NOT join
% the group (pp. 48, 70, 79, 84, 106) that uses the Danish low-high pair.
anføres som Fortjeneste ved Hegels Logik, den store Skarpsindighed nemlig, med
hvilken den ved en grundig Negation og Modsigelse skjærper den dialektiske Spore,
det Samme ville vi ogsaa finde i Dogmatiken, naar vi kun betræde den rette Vei.

De enkelte Momenters gjensidige Modsætning har langt større Dybde i den
christelige Dogmatik end i Panlogismen, og det er netop derved, at hiin fører til
en sand Identitet og Forsoning. Netop fordi Panlogismen for at komme til
Immanentsen, deelte Tilværelsen i to abstracte Dele, netop derfor kunde den ikke
komme til Identiteten; den christelige Dogmatik derimod sætter en absolut
Modsætning imellem den i Sandhed treenige, i sig fuldtfærdige Gud og den skabte
Verden; men ved denne Grundmodsigelse kan en sand Forsoning komme istand imellem
Evighedens Fred og Timelighedens Kampe, i det de Knuder, der indvikles her,
allerede der i Principet selv ere løste.

For at forene og forlige alle Ideerne, opoffrer Panlogismen de enkelte
Selvbevidstheder og bevæger sig netop derved i en evig Modsætning mellem
Realiteten og Idealiteten; den christelige Dogmatik derimod tillægger de enkelte
Selvbe\-%
% --- p. 148 ---
\opage{148} vidstheder en indre uendelig Immanents i sig, ifølge hvilken de vel døe
\textit{ad extra} (ὁ ἔξω ἄνθρωπος), men ikke destomindre \textit{ad intra} (ὁ ἔσω
ἄνθρωπος) bevare sig selv i al Evighed. Men i det den saaledes ligesom
% GREEK, p. 148, read at 3x. NORMALISED FOUNT VARIANTS per the header: the fount
% sets script theta (U+03D1) and cursive rho (U+03F1) in ἄνθρωπος; both are typed here as plain
% θ and ρ. Accents and breathings are as printed (ὁ rough, ἔξω and ἔσω smooth +
% acute, ἄνθρωπος smooth + acute). "ad extra" / "ad intra" are antiqua -> \textit{}.
% NO FOOTNOTE on this page: 32 body lines run to the foot, no rule, no small type.
atomistisk adskiller dem alle fra hverandre, giver Syndens Natur den dybeste
Udvikling, urokkeligt fastholder det ethiske Modsætningsforhold imellem Gud og
Mennesket, trænger den ogsaa ind til en sand Immanents af alle disse forskjellige
Momenter, i det den uddanner det dybeste Friheds=, Omvendelses= og
Kjærligheds=Begreb.
% The line is loosely justified and the print shows a space on each side of the
% last double-hyphen ("Kjærligheds = Begreb"), where "Friheds=," and
% "Omvendelses=" are tight; set closed, as the series requires. spacing.py's
% flags on this page ("Kjærligheds", 1.72; "selv", 1.39) are that same
% justification, not Sperrsatz --- checked at 1.5x.

I det Panlogismen vil overføre paa hele Menneskeslægten, hvad Kirken tillægger
% "I det" is set as two words, and the line IS indented (x922 against the page's
% own 697 left edge; the ink at x753 that the line-scan reports is a speck in the
% margin). Measured, not eyeballed, because the preceding line ends short and the
% Fraktur I/J glyph carries a long low tail.
sin Christus, uden at erindre paa den ene Side den indre, uendelige Fylde, der er
den menneskelige Selvbevidstheds Væsen, paa den anden det gjensidige Forhold
imellem Christus og den hellige Aand, sammenblander den i ynkelig Forvirring
Momenter med hinanden, der ere hinanden reelt modsatte. Den christelige Dogmatik
tillægger derimod sin Christus en fuldkommen og absolut Identitet af Gud og
Menneske, og det er saa langt fra, at Ideernes lige Ret og Alles Identitet
forstyrres ved denne, at meget mere Livets Uforkrænkelighed igjennem den Ene
bliver Alle tildeel, idet Legemets enkelte Lemmer hænge organisk sammen med
Hovedet.

Naar endelig Panlogismen uden at kjende det kosmiske Systems sande og virkelige
Mediation, bestandigt fører Ordet Mediation i Munden og antager, at den hele
dialektiske Udviklingsgang i Virkeligheden bestandigt har været den samme, som om
det Forhold, der i vor Tid finder Sted imellem Naturen og den subjective Aand,
var evigt, synes den paa en mærkelig Maade at modsige sig selv, naar den
desuagtet forjetter en Tid, da Aanden og Naturen skulle uendeligt gjennemtrænge
% "skulle", NOT the Fraktur OCR's "kunne" --- the two witnesses disagreed and the
% line was read at 1.4x: the word is s-k-u-l-l-e.
hinanden. Den christelige Dogmatik fremstiller derimod, uagtet den gaaer ud fra
en sand metaphysisk Iden\-%
% --- p. 149 ---
\opage{149} titet af den guddommelige Villie og Naturlovene, den objective Mediation paa en
saadan Maade, at Gud paa den ene Side har gjort Naturen til Grundlaget for den
subjective Aand og derfor ogsaa lader sine Ideer udvikle sig igjennem Naturens
objective Skikkelser, og paa den anden Side tillige paa særegen Maade forklarer
Naturen ved sin Nærværelse, da Synden havde formørket den menneskelige
Selvbevidsthed og givet det verdslige og profane Moment en isoleret Udvikling,
hvoraf da følger, at der undertiden fremtræder en skarp og virkelig Modsætning
% spacing.py reports a footnote block starting at y=1265 on this page and flags
% "skarp" and "Oprindelse" as [note]. Both are wrong: a row-by-row scan of the
% page finds 32 body lines of uniform height and NO small type or rule anywhere,
% so there is no note here and no \setcounter. "skarp" and "Oprindelse" both fall
% at the loose end of a justified line; checked at 1.5x and rejected.
imellem den almindelige guddommelige Styrelse og den overnaturlige Aabenbaring,
--- en Mediationslære, der indeholder en sand Fremstilling af Guds væsentlige
Accommodation efter de forskjellige Tiders og Menneskers særegne Charakteer.

Den menneskelige Fornuft skyer ikke disse virkelige Modsætninger og
Grundmodsigelser; thi den erkjender, at de alle have deres Oprindelse fra og
Tilværelse i den selv. Ligesom derfor det gjenfødte Menneskes Jeg bliver det
samme, uagtet et ganske nyt Liv skabes i det, saaledes er det ogsaa saa langtfra,
at de Modsætninger, der gjennemtrænge og ligesom opløse den menneskelige Fornuft,
ophæve dennes Identitet med sig selv, at den tvertimod overvinder Smertens
Modsigelse og opnaaer Forsoning, alteftersom den mere og mere nedsænker sig i og
tilegner sig disse Modsætninger. Heraf bliver det indlysende, at den skabte
Fornuft i Virkeligheden bestemmes og begribes ved den uskabte, og at Mediationen
af de overnaturlige og naturlige Momenter netop foregaaer igjennem den absolute
Modsætning; ja man kan med Sandhed sige, at enhver hellig Historie, det er,
enhver Modsætning mellem den pragmatiske Verdensudvikling og Miraklerne, netop
har den Opgave at føre til en Forsoning af hine modsatte Momenter.

% --- p. 150 ---
\opage{150} \parag{23}{Miraklet og den hellige Historie.}
% § 23 --- the last § of the book. The printed head AGREES verbatim with the
% Indhold ("Miraklet og den hellige Historie."), so there is nothing to record on
% both sides here. Read at 2x: "§ 23." centred in bold ANTIQUA, the argument below
% it centred, letterspaced Fraktur, not bold --- the standard form, so \parag.
% NO FOOTNOTE on this page.

Som det forholder sig med Aabenbaringen, saaledes forholder det sig ogsaa med den
hellige Historie; Aabenbaringens Sandhed afhænger af Miraklets Virkelighed, men
dette udvikles igjennem Tingenes kosmiske \emph{Mediation}. Undersøgelsen om
% \emph{Mediation}: letterspaced, read at 2x ("M e d i a t i o n"); "kosmiske"
% before it is not. The ABBYY layer preserves the spacing and agrees.
Miraklets Virkelighed gaaer nemlig ikke ud paa at udvikle, at Guds Villie har
Indflydelse paa Naturen; thi der er en virkelig Forbindelse mellem Guds
Virksomhed og Naturens Kræfter, og hvergang vi bevæge Haand eller Fod, give vi
selv et Exempel paa, hvorledes Tanken har Indflydelse paa Legemet; --- men dens
Maal er, som vi alt før have bemærket, at paavise en umiddelbar Ophævelse af den
Modsætning, der er imellem de guddommelige Raadslutninger og Naturens Love.

Alle Mirakler synes saaledes fornuftstridige, fordi Gud stedse virker overalt
lige middelbart og umiddelbart; men en saadan Fremstilling af det metaphysiske
Forhold er ligesaa utilfredsstillende, som det vilde være, dersom Nogen meente at
have forstaaet den menneskelige Friheds Begreb, fordi han havde iagttaget, at Gud
virker Alt i Mennesket, og at Mennesket tillige er selvvirkende. Alle saadanne
Grundforhold maae nødvendigt udfolde sig gjennem Timelighedens Modsætninger.
Saaledes bliver det da umuligt at fremstille en grundig Mediation af de
guddommelige og menneskelige Forhold uden at indrømme Miraklet en bestemt Plads.

Deismen, der fremfor Alt urgerer en naturlig Verdensorden, nødes dog selv til at
appellere til eet Mirakel, som Udgangspunkt for dens hele Pragmatisme, Skabelsen
nemlig. Det samme gjælder om Panlogismen; thi medens den har opdaget
Illusionsmomentet i de christelige
% --- p. 151 ---
\opage{151} \setcounter{footnote}{0}%
Traditioner, saa at disse ikke længere fremstille os Gud umiddelbart nærværende,
indrømmer den dog tillige, at den objective Aand ene og alene gjennem dette
Illusionsstandpunkt har kunnet naae det Oplysningsstandpunkt, der udmærker vor
Tid. Hvad der fremstilles middelbart i Philosophien, var altsaa umiddelbart
tilstede i hine Beretninger, saa at Adskillelsen imellem de almindelige
Begivenheder og Aabenbaringsphænomenerne atter her vender tilbage. Alle Tings
Udvikling har da det Maal at føre til en gjensidig Immanents af Natur og Aand paa
ethvert Punkt, forat det \emph{Middelbare} og \emph{Umiddelbare} overalt kunne
% \emph{Middelbare}, \emph{Umiddelbare}: letterspaced, read at 1.6x; the ABBYY
% layer preserves both. Nothing else on the line is.
falde sammen; men skal denne Harmonie blive virkelig, maae Elementerne først
adskille sig fra hinanden, saa at Naturen og Aanden paa den ene Side hver for sig
have sine særegne Punkter, hvor de virke isolerede, for paa den anden Side netop
paa Grund af denne Adskillelse paa andre Punkter at kunne gjennemtrænge hinanden
indbyrdes.\footnote{Et Exempel paa dette middelbare og umiddelbare Forhold
% p. 151, note 1): the mark stands after "indbyrdes." --- AFTER the period, at
% the end of the short line that closes the paragraph. Located by reading the
% whole body block; neither OCR witness recovers it.
frembyder det daglige Liv selv: Den ydre Natur omdannes nemlig mere og mere af
den menneskelige Aand, men paa \emph{middelbar} Maade. Derfor siger Strauß med
Rette: .\,.\,.\,.\ ”sie (die Menschheit) ist der Wunderthäter: sofern im Verlauf
der Menschengeschichte der Geist sich immer vollständiger der Natur, im Menschen
wie außer demselben, bemächtigt, diese ihm gegenüber zum machtlosen Material
seiner Thätigkeit heruntergesetzt wird. J.~L. Schlußabhandl. § \textit{149} Pag.
\textit{767}.

Men denne middelbare Virksomhed er knyttet til den umiddelbare og reelle
Forbindelse, der finder Sted mellem Sjel og Legeme; og vilde man nu kalde denne
mythisk, maatte man ogsaa kalde alle Beretninger om saadanne Arbeider Myther.}
% p. 151, note 1) --- four things recorded here, all read at 2x off the image:
%  (a) PRINTER'S DEFECT, QUOTATION MARK: the Strauß quotation OPENS with the
%      body's raised ” before "sie" and is NEVER CLOSED --- it runs to
%      "heruntergesetzt wird." and then straight into the citation with no second
%      mark. Transcribed as printed. This is the ONLY quotation-mark defect in
%      pp. 145--156 and it is what puts the range's ” count at an odd 17.
%      It must also go into check.py's QUOTE_DEFECTS table.
%  (b) The mark used is the RAISED ” --- this note does NOT use the Danish low-high pair
%      of the notes on pp. 48, 70, 79, 84 and 106 (low-high).
%  (c) \emph{middelbar} inside the note is letterspaced ("m i d d e l b a r");
%      "mere og mere" beside it is not.
%  (d) PRINTER'S TRANSPOSITION: the citation reads "J.~L. Schlußabhandl." with
%      the Fraktur J FIRST, where pp. 152 and 153 both print "Strauß L.~J. Pag."
%      (= Leben Jesu) with the L first. Read at 2x on all three pages and set as
%      printed on each. The German is Fraktur, so nothing in it is \textit{};
%      only the two figures are antiqua.
%  The print sets four spaced points before the quotation; rendered .\,.\,.\,.
%  The note has two paragraphs, the second indented in the printing.

Betragter man derfor Miraklerne isolerede, synes de fornuftstridige; men Grunden
hertil er netop den, at det Umiddelbare altid bestemmes af det Middelbare,
ligesom dette atter alene bestemmes af hiint. Betragtes de derimod i det almin\-%
% --- p. 152 ---
\opage{152} \setcounter{footnote}{0}%
delige Lys af Alt det, der gaaer forud for dem og følger efter dem, ere de af
logisk Natur og kunne som en Følge deraf ogsaa begribes. Miraklet kan derfor
kaldes en Anticipation af det Forhold, som ifølge den religieuse Aand
metaphysisk taget allerede finder Sted og eengang skal sees virkeliggjort imellem
den skabte Natur og Skaberen. Den Gud, der fremtræder i Miraklet, er derfor den
samme, der er tilstede i den religieuse Bevidsthed. Miraklet er altsaa
Religionens egen Rod (vi tale nemlig om det Mirakel, der er det vigtigste i
enhver Religion); det indeholder det særegne Anskuelsesmoment, hvis indre Væsen
Religionens Aand mere og mere skal udfolde. Da Miraklet saaledes udgjør Aandens
eget Indhold, maa ogsaa den religieuse Aand selv vakle, saasnart der opstaaer
Tvivl om Miraklets Virkelighed, og opdages det, at Miraklet er usandt, maa som en
Følge deraf Aanden selv forsvinde og bøie sig under en mægtigere Aand, der
medieres gjennem en sandere Virkelighed. At den panlogistiske Aand ikke kan finde
sig i den historiske Christus\footnote{Smlgn. Strauß L.~J. Pag. \textit{766}.}
% p. 152, note 1) --- the plain next number, mark after "Christus" with a space
% before it. The note is a single line, FRAKTUR throughout ("Smlgn.", "Strauß",
% "L. J.", "Pag."); only the figure is antiqua. "L. J." here has the L FIRST ---
% contrast p. 151's "J. L.", read at 2x on both pages.
har derfor sin naturlige Grund deri, at den ikke er medieret igjennem Christus;
--- betragter man dens trende Hovedindvendinger, vil dette let blive indlysende.
Den benægter nemlig for det Første Muligheden af en individuel Forening af Gud og
Mennesket, fordi den ikke kjender de enkelte Bevidstheders indre Fylde; men
Grunden til denne Uvidenhed have vi allerede ovenfor angivet. Den bemærker
dernæst, at det vilde stride imod Ideens Natur som saadan at berige og ophøie et
% "ophøie", NOT "opholde": read at 1.2x, and "indlysende." above it ends with a
% POINT, not the comma the Fraktur OCR gives. Neither OCR witness reads this
% stretch reliably, so lines 20--30 were read off the image as a block.
særegent Organ paa de øvrige Organers Bekostning; men heri tager den ikke
tilbørligt Hensyn til Mediationens reelle Moment (smlgn. § \textit{22}). Og naar
den endelig indvender, at Christus, som individuelt Phænomen, hverken er Mere
eller
% --- p. 153 ---
\opage{153} \setcounter{footnote}{0}%
Mindre, end det, der fremstiller sig for os i hans phænomenale
Tilværelse\footnote{Allein, wie Schmid gründlich nachgewiesen hat, ein
...
kommt. Strauß L.~J. Pag. \textit{740}}, bemærke vi hertil, at Tilværelsen ikke
% p. 153, note 1) --- mark after "Tilværelse", set tight against the word and
% before the comma. The note is German FRAKTUR throughout, so nothing in it takes
% \textit{} but the page figure; it carries NO quotation marks at all. Read at 2x.
% FOUR PRINTER'S DEFECTS in it, all transcribed as printed:
%   "algemeine"      for allgemeine   (single l)
%   "Individnalitet" for Individualität (turned u, and the Danish -itet ending)
%   "beschränckt"    for beschränkt   (intrusive c)
%   "Pag. 740"       with NO closing point, where pp. 151 and 152 both have one.
% "Zeit= und" carries the Fraktur double-hyphen, kept as =.
kan betragtes fra dens sandselige Side alene, --- thi deraf vilde man blot
udbringe den Modsigelse, at det Sandselige ikke er det Aandelige; forstaaes det
derimod saaledes, at der ikke ligger Andet i Phænomenet, end det, der kan udledes
ved en Negation af Phænomenet, bliver netop en aandelig Fortolkning nødvendig;
men hvilken Udstrækning denne skal have, det kan alene Aanden selv lære os. Der
bliver saaledes en inderlig Forbindelse imellem Christus og den hellige Aand:
hvad der ligger \textit{implicite} i Christus, fremsætter Aanden
\textit{explicite}. Aanden vilde saaledes ikke have fremstillet den evige
% "implicite" and "explicite" are antiqua ITALIC against the Fraktur -> \textit{};
% the emphasis is by fount, not by spacing. spacing.py's flag on "plicite." (1.51)
% is the antiqua fount being wider than the Fraktur around it, and its RUN on
% "Han skal" (line 22) is word spacing after a period --- both checked at 1.5x
% and rejected.
Personligheds Idee for os i Kirken, hvis ikke Guddommens uendelige Fylde (πᾶν τό
πλήρωμα τῆς θεότητος) havde boet i Christus. Derfor siger Christus intetsteds, at
% GREEK, p. 153, read at 3x. NORMALISED FOUNT VARIANTS: cursive rho (U+03F1) in πλήρωμα
% and script theta (U+03D1) in θεότητος are typed as plain ρ and θ. ACCENT AS PRINTED,
% and it departs from the grammar: the print sets τό with an ACUTE where the
% received text has the grave τὸ. Kept as printed and logged here.
den hellige Aand engang vil komme efter egen Tilskyndelse og lære, at han selv
kun var et forsvindende Individ; men han forjetter, at Aanden skal komme fra
Faderen i hans Navn og tilføier: ”Naar Sandhedens Aand kommer, da skal han
veilede Eder til Sandhed. Thi ikke skal han tale af sit Eget, men hvad han hører,
skal han udsige, og de tilkommende Ting skal han forkynde Eder. Han skal
forherlige mig; thi han skal modtage af Mit, og forkynde det til Eder. Alt hvad
Faderen har, er mit. Derfor sagde jeg, at han skal tage af Mit og
% --- p. 154 ---
\opage{154} forkynde det til Eder.” (Joh. \textit{16, 13---15}). Der ere derfor tvende
% The quotation opened on p. 153 and closes here with the body's raised ”; both
% marks read at 2x. "Joh." is Fraktur, the figures antiqua; the print sets a long
% dash, not a short rule, between 13 and 15.
% NO FOOTNOTE on this page.
Grundmirakler i den christelige Religion, \emph{Incarnationen} og
\emph{Inspirationen}.
% \emph{Incarnationen}, \emph{Inspirationen}: both letterspaced, read at 1.6x
% ("I n c a r n a= t i o n e n", the compound broken over the line with the
% Fraktur double-hyphen). spacing.py's RUN and the ABBYY layer both agree.

Da Disciplene i Begyndelsen samlede sig om Christus, allerede da var det i
Sandhed Aanden, der ubevidst drog dem, men kun derved, at den umiddelbart
fremskinnede af Frelserens egen Person; derfor opfordrer Philippus Nathanael, der
tvivler om, at Messias kan komme fra Nazareth, med de Ord til at følge sig: ”Kom
og see!” (Joh. \textit{1, 47}). Dog først da Apostlene efter i Samlivet med
Christus og ved hans mundtlige Lære at have modtaget den fornødne religieuse
Foruddannelse, havde seet hans Død, --- da han efter Opstandelsen forherliget
havde staaet for deres beskuende Blikke ligesom paa Grændsen af det himmelske og
jordiske Rige, i sit Væsen fremstillende for dem det Individuelle og det
Universelle i sand Eenhed, --- da han endelig, opløftet til Himmelen efter at
have givet dem Forjettelsen om den Trøster, der skulde komme, havde overladt dem
% spacing.py's flag on "Trøster" (1.31) is the loose comma-space of a justified
% line, not Sperrsatz --- checked at 1.5x and rejected.
for en Tid til deres egne Betragtninger: først da var det, der umiddelbart
aabenbarede sig, Aandens Udsendelse i Pintsefestens Mirakel, forberedet ved en
sand Mediation i deres Indre. Christi ydre Phænomen var negeret og Fortolkeren,
der alt laa skjult i deres Bevidsthed, fremtraadte pludseligt i Bevidsthedens
fulde Lys. Som derfor Christus før havde været aabenbaret i sin Herlighed i den
ydre Verden, saaledes var han nu først tilstede i fuldkommen Herlighed i
Bevidsthedens indre Rige. Dette Forhold imellem Christi umiddelbare Nærværelse og
den af ham udsendte Aands umiddelbare Virksomhed giver netop den apostoliske
Tidsalder dens særegne Charakteer. Betragter man derfor Christi Liv i dets
Heelhed, vil man let gjenfinde hiint gjensidige Forhold. Thi ligesom hans
offentlige Liv nærmest henhører til Historiens phænomenale Virkelighed, saa at
Anskuelsens Moment her er
% --- p. 155 ---
\opage{155} overveiende, men den sande Forstaaelse og Udvikling af dette først gives af
Aanden, saaledes aabenbarer hans Herlighed hos Faderen sig nærmest for den
granskende Aand, men Sandheden af denne Aabenbarings Tilegnelse er atter betinget
ved den troende Modtagelse af Læren om hans Liv her paa Jorden. Christi Indgang i
Verden og hans Udgang af den ere saaledes Aabenbaringsformer, der ifølge Sagens
Natur svæve imellem den phænomenale Virkelighed og den aandelige Idealitet:
Enhver, der ikke troer paa Aanden, mishandler Historien.
% NO FOOTNOTE on this page.

Heraf er det tillige indlysende, at ogsaa Verdens Forsoning maa henføres til
% spacing.py's flag on "indlysende" (1.45) is justification --- checked at 1.5x
% and rejected.
Christi hellige Historie i den rigtige Opfattelse af samme. Thi Forsoningen er
virkeliggjort derved, at Christus undergik Livets haardeste Modsætninger og
seierrig gik frem af Kampen. Ligesom det naturlige Menneske indtræder i denne
Verden paa en saadan Maade, at han har Mørket bagved sig, saa at han ikke veed,
hvorfra han kommer, og ligesom han maa gjennemgaae den haarde Prøvelse i Kampen
imod alle Livets og Dødens Modsigelser, uden at vide, hvor han gaaer hen,
saaledes gaaer omvendt Christus, der er Veien og Sandheden og Livet, frem af det
evige Lysets Rige, aabenbarer Guds Nærværelse i Smertens Verden, i det hans
guddommelige Aand stedse bliver sig selv liig, og vandrer gjennem Dødens Mørke
% "liig", not "lig" --- the two OCR witnesses disagreed and the word was read at
% 1.2x; the print doubles the i.
tilbage til Herlighedens Rige. (”Jeg udgik fra Faderen og kom til Verden; jeg
forlader Verden igjen og gaaer til Faderen.”). Han viser saaledes ikke blot de
enkelte Mennesker Veien, men aabenbarer ogsaa den hele Verdens sande Historie.

Men ligesom Inspirationens Mirakel er medieret igjennem den individuelle
Christus, saaledes forbereder ogsaa den apostoliske Tidsalder Kirkens følgende
Uddannelse.

Den fremstiller os nemlig paa den ene Side et fast og
% --- p. 156 ---
\opage{156} uforanderligt Christusbillede, saa at Skriften til Enhver, der længes efter at
see Christus, udtaler sit: ”Kom og see!”; men paa den anden Side fører den ud
over sig selv til en anden Form af Kirken. Alle Kirkens Institutioner sigte
nemlig dertil mere og mere at uddanne Frihedens Idee, for at den menneskelige
Bevidsthed, reflecteret i sig, gjennem Erkjendelsen af sin egen Natur kan søge,
finde og grundigt tilegne sig Gud. At vor Tid ikke kan opvise noget Mirakel, er
altsaa saa langt fra at begrunde nogen Mistanke imod Sanheden af den apostoliske
% PRINTER'S DEFECT, p. 156: "Sanheden" for "Sandheden" --- the d is not there.
% Read at 1.4x; both OCR witnesses see the same seven letters. Transcribed as
% printed. "Mistanke" on the same line is sound.
Tids Mirakler, at vi meget mere maae vende Sagen om og sige, at hiin Tids
Mirakler netop forklare os Grunden til, at der i vor Tid ingen Mirakler finde
Sted.

Den mangfoldige Fluctuation, der i Tidernes Løb finder Sted i Kirken imellem den
reelle Objectivitet og den abstracte Idealitet, i det de afvexlende afløse
hinanden og gaae over i hinanden, tjener til mere og mere sandt og inderligt at
forsone Begrebet og Anskuelsen med hinanden. Bliver derfor det ene af disse
% spacing.py's flag on "det" (1.50) falls on this line and is justification ---
% checked at 1.4x and rejected.
Momenter til en eller anden Tid overveiende, see vi stedse det andet dukke op og
moderere hiint.

Den dialektiske Udvikling foregaaer nemlig bestandigt saaledes, at paa den ene
Side Aanden uddannes igjennem en Negation af Livets Phænomener, paa den anden
Side disse udvikles og forklares ved hiin; som derfor Aanden er, saaledes er
ogsaa Phænomenernes Udtydning og Fortolkning. Derfor lyde Apostelens Ord: ”I
Elskelige, troer ikke enhver Aand, men prøver Aanderne, om de ere af Gud!”
bestandigt igjennem Kirken. Men den, der prøver Aanden, er netop Aanden selv, thi
om Aanden gjælder det: ”\textit{verum est index sui et falsi}”. Derfor vil den
% "verum est index sui et falsi" is ANTIQUA (upright roman, not italic in the
% print) against the Fraktur -> \textit{}, per the standing convention. The
% quotation marks around it are the Fraktur fount's raised ”, both read at 1.4x.
abstracte Friheds og Personligheds Aand stedse gaae seierrig frem af Prøven. ---
Det vil da heraf være klart, at Principet i den hellige Historie er Guds sande,
reelle Accommodation. Den sig fra Evighed
% JOINT p. 156/157 verified on the image, as the pp. 157--162 batch reports:
% p. 156 ends on the word "Evighed" with NO word broken across the leaf, and it
% carries NO note --- 32 body lines run to the foot, with no rule and no small
% type below them. So nothing runs over onto p. 157 and no \setcounter belongs
% on this page.
% BATCH SEAM, REPAIRED. p. 157's first line is NOT indented --- it continues this
% sentence ("Den sig fra Evighed / af sin absolute Frihed bevidste Gud"), so the
% blank line the skeleton left after the marker has been removed. The blank line
% BEFORE the pp. 145--156 fragment is correct and stays: p. 144 ends a sentence
% and p. 145 begins an indented new paragraph (+230 px against its own margin).
% --- p. 157 ---
\opage{157} % JOINT p. 156/157 verified on the image: p. 156 ends "Den sig fra Evighed" with
...
% --- p. 158 ---
\opage{158} % NO FOOTNOTE on this page. spacing.py reports "a footnote block appears to
...
% --- p. 159 ---
\opage{159} % NO FOOTNOTE on this page.
...
% --- p. 160 ---
\opage{160} % NO FOOTNOTE on this page.
...
% --- p. 161 ---
\opage{161} % NO FOOTNOTE on this page. The rule below is the Slutning divider, not a
...
% --- p. 162 ---
\opage{162} % *** THE LAST PAGE OF THE BOOK. *** Fourteen body lines and then nothing: no
