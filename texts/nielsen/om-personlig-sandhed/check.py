...
report progress, so put one in for every page. The Forord's single unfoliated
leaf uses the non-digit marker `% --- p. [Forord] ---`, which the arabic-only
regex below ignores by design — see pagemap.py and RESUME-NOTES.md.
"""
import re, sys, os
...
if pages:
    ...
    print(f"progress: {hi}/144 = {hi/144*100:.1f}%   next page to transcribe: {hi+1}")
else:
    print("pages: no arabic  % --- p. N ---  markers yet "
          "(the Forord's non-digit marker is ignored)")
    print("progress: 0/144 = 0.0%   next page to transcribe: 1")
