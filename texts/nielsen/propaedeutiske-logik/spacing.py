...
# Calibrated on batch 1 (pp. 1-17) of THIS book: at a common 1.16 the tool gave
# 21 RUNs, of which 14 confirmed on the image and 7 were false (all on loosely
# justified lines), and ~45 single-word flags, of which only about a third
# confirmed. RUNs are worth 1.16; singles need a higher bar to be worth reading.
SINGLE_THRESH = float(os.environ.get("SPACING_SINGLE_THRESH", "1.30"))
# It also has FALSE NEGATIVES -- it missed "særskilt" (p.12 l.1) and the whole
# p.6 display line -- so it narrows the search, it does not replace the image.
# Printed p.14 is the worst page in batch 1; treat its output as noise.
...
def report(printed):
    ...
    def dump(run):
        ...
        elif run and run[0][2] >= SINGLE_THRESH:
