% =====================================================================
%  R. Nielsen, Almindelig Videnskabslære i Grundtræk.
%  Kjøbenhavn: Gyldendalske Boghandels Forlag (F. Hegel & Søn), 1880.
%  Thieles Bogtrykkeri. 281 pp. + errata leaf.
%  DIPLOMATIC TRANSCRIPTION (Danish). Copy digitised by Google.
% =====================================================================
%
%  SCAN AND PAGE MAP. bibliotek `Nielsen, Rasmus/1880-videnskabslaere.pdf',
%  sha256 d04796c0a224bf7f895bee54aed144c6e5ebd2bb3c5a42690d2b4e6cf36e9e8f,
%  302 PDF pp., 1-bit JBIG2 at 600 ppi. PDF = printed + 14, uniform
%  (folio agrees on 270 of 281 pages; the rest carry none or misread).
%  Title PDF 9, Forord PDF 11-12, Indhold PDF 13-14 (not transcribed;
%  \tableofcontents), Rettelser leaf PDF 296. The PDF carries 222 modern
%  highlight/pen annotations (PDF 28-90) that are not in the book.
%
%  CONVENTIONS (established by the recon pass and the collation, 2026-09-24).
%  Letterspacing (Sperrsatz) is the book's only in-text emphasis -> \emph{}.
%  Three paragraph-opening run-ins are fat-face -> \textbf{} (pp. 99, 101,
%  104). Latin, French, German and botanical names are ROMAN in the print;
%  the transcription formerly set 48 of them in \textit{}, all removed. The
%  only italic in the book is the Greek (pp. 92, 197, 227), set here in the
%  ordinary Greek fount. Abbreviations are printed spaced (o. s. v., d. v. s.,
%  d. e., f. Ex., o. dsl.); 85 closed-up forms were respaced. Page turns are
%  \apage{N} at the first word of printed page N (before a word split across
%  the turn). \part/\chapter/\section are editorial; their words follow the
%  printed heads.
%
%  THE JBIG2 RULE. This scan rebuilds letters from a symbol dictionary, so a
%  single substituted letter is not evidence of the compositor. Where the
%  scan shows a non-word differing from a real word by one SUBSTITUTED letter,
%  the sense-reading is set and the printed form recorded in a `%' comment
%  (14 sites). A letter OMITTED or ADDED cannot come from substitution, so
%  those are printed as they stand, with an AS PRINTED comment (Endkjøndt,
%  Indentitet, Pilosophien, Forkjel, Stofvirsomhed, phanerogram, Dannnelse,
%  tilsidt).
%
%  ACCURACY: full collation, 24 September 2026, of all 281 printed pages
%  plus title, Forord and the Rettelser leaf, against the scan: every page
%  read at the image word against word by one of 28 collators (10 pp. each,
%  4 waves), letterspacing measured by gap at 600 dpi on every line; every
%  non-emphasis change and every doubtful item put to a BLIND second reader
%  (9 readers, 107 items) before it was applied. Where the two disagreed
%  the file was left as it stood (pp. 52 `Art', 64 `Axer,', 66 `en', 144
%  `d. v. s', 200 `i', 248 `af'), except p. 76, settled 2-1 by a third.
%    Transcriber's errors corrected: 156 -- about 43 dropped dashes (often
%    one of a pair), about 55 punctuation marks (dropped colons before
%    formulas, commas added or dropped, quotation marks dropped or on the
%    wrong side of a stop), 28 modernised spellings silently undone
%    (Æsthetik x14, viist, Henviisning, naturligviis, Reqvisition, Æqvator,
%    Synspunct, fælleds, Iisdække ...), about 15 of the book's word divisions
%    closed up (hvor af, her af, saa kaldte, vidt gaaende, forud gaaende, for
%    fra, lige frem ...), 4 lost capitals (Samme, Siden, Stand), and 26 wrong,
%    dropped, added or reordered words and signs -- among them p. 22 an added
%    `atter', p. 45 `Den Forsætning ... Forstanden' for `Den anden ...
%    Forsætningen', p. 41 word order, p. 74 dropped `I', p. 169 `Trias-' for
%    `Triasgruppen', p. 251 `Leo X's' for `Leo Xdes', p. 126 `+' for `±',
%    p. 129 dropped `= g', p. 54 capital X for x.
%    Printer's errors the file had silently corrected, restored as printed:
%    about 23 (p. 24 `Frø, vækker'; p. 73 `Laplacèske'; p. 92 the Greek
%    accentuation and no ano teleia; p. 108 head `Opløsning,'; p. 119 no
%    closing «; p. 126 `Paralellogram'; p. 155 «Affinivalens«; p. 189 stray
%    «; p. 196 `Lyélls'; p. 201 `de temps'; p. 232 `les detailles'; p. 263
%    `Retfærdige,»'; p. 267 `sikker'; the eight omitted/added letters above;
%    the Greek `ἀναγκη' twice). Errata sites pp. 107, 157, 160 carry comments.
%    Emphasis: 17 \emph{} before, 269 after -- some 250 letterspaced runs had
%    never been marked. Paragraphing: 17 breaks restored, removed or moved to
%    the right side of a marker. Markers: 117 of 281 moved to the true turn
%    (mostly one word early). Heads: 3 corrected (Mechanisk og chemisk
%    Physik; Den biologiske Afstamningshypothese; Aands-Videnskab). The
%    Rettelser block re-set as printed (its first entry reads § 11, not § 2).
%  Held, not changed: p. 108 the shorter first rule in two schemata; p. 160
%    a mark over `et'; p. 167 `ene,'; p. 187 `phanerogame' where sense wants
%    `kryptogame' (the author's, not the compositor's); p. 131 four-dot
%    ellipses; the glyph forms ϱ and a sloped Θ on p. 137.
%  ocrdiff (embedded Google layer, per batch): 437 candidates, 6 real.
%  Rate: about 179 transcriber's errors (156 + ~23 silently corrected
%    misprints) over 281 pp. = ~0.64/page, of which roughly a third
%    word-level; emphasis missing at ~0.9 runs/page besides.
%  Post-repair re-sample (seed `nielsen/videnskabslaere|post': pp. 31, 58,
%    101, 131, 132, 167, a fresh reader): 0 errors, 0 emphasis, 0
%    paragraph, 0 marker disagreements.
% =====================================================================
...
\author{R.\ Nielsen}
\date{Kjøbenhavn: Gyldendalske Boghandels Forlag (F.\ Hegel \& Søn). Thieles Bogtrykkeri. 1880}
...
\bigskip
\noindent \emph{Kjøbenhavn}, den 23de August.
\nopagebreak
\par\hfill R.\ Nielsen.
...
Vor første Viden kommer, saa at sige, af sig selv. Vi ere
...
\emph{forskjellige} Udviklingstrin og indsamle et rigt Materiale af
...
Jord, om Guder  \apage{2} og Mennesker, om Familieliv, Stat og sædelig Orden kan i
...
omsider saa vidt, at en skoleret Underviisning kan begynde. Underviisningen,
...
\emph{oprindelige Forudsætninger}.
...
den  \apage{3} Paagjældende kunne begynde en virkelig Efterforskning af existerende
...
% `vi synes': the scan shows `si synes'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
...
er i vor egen Hjerne? Naar slige Spørgsmaal paanøde sig  \apage{4} Bevidstheden,
...
Tankesætning, som Modsigelsen ikke kan angribe? Ja, her er  \apage{5} Lighedens,
...
handledes om Videns almene Grundlag, om den \apage{6} Gyldighed, der i det Hele kan
...
indlysende, saa er her jo Intet at \apage{7} undersøge; skal en Undersøgelse om
...
er \apage{8} den dog ikke ørkesløs; den er tvertimod den første og eneste Vei til
...
\apage{9} Og dog er Forklaringen ingenlunde ørkesløs; thi under Cirkelbevægelsen
...
% AS PRINTED: `Endkjøndt' (no s) (collation 2026-09-24, two readers).
\noindent Endkjøndt det ved sig selv er indlysende, at Viden maa have
...
og Bevidsthed ligne hinanden; Modsætningen er saa  \apage{10} fremtrædende, at det
...
Bevidstheden, da maa man jo ogsaa antage, \apage{11} at de ere den sandsende
...
Tingene, der sandses, maae være baade \emph{indenfor} og \emph{udenfor} vor Bevidsthed.
...
sandsende Bevidsthed er paa een Gang baade \emph{activ} og \emph{passiv}, activ, for saa
...
den sandsede Gjenstands Objectivitet. Antagelsen af et \emph{Udenfor} beroer paa
...
Vidnesbyrd om Gjenstandens Realitet. Viden om Gjenstandens  \apage{12} Realitet er en
reel Viden, en Viden af \emph{udvortes Erfaring}.
...
% `Synspunkt er det': the scan shows `et det'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
...
Egenskaber. At antage Ting, som falde udenfor alle Egenskaber, er \apage{13} at
...
Kjendsgjerninger, er at faae dem fuldstændig sandsede: \emph{dette er
Sandsningens Andeel i Viden}.

\apage{14} At man desuagtet ikke kan aflede al Viden af Sandsning og sandselige
...
det nu an at sige, at en Nerve selv \apage{15} bliver sig bevidst, idet Bevidstheden
...
Noget, vi ikke kunne gjennemskue; men kan \apage{16} det Samme siges om Bevidstheden?
...
er guult, at  \apage{17} det kan krystallisere i Octaedre og lade sig udvalse til
...
umiddelbart. Da han sandsede Egenskaben \emph{guult}, maatte han henføre den
...
Virkning, det \apage{18} Individuelle og det Almene, Love o.\ s.\ v. Men han kan ikke
...
Moment, et uundværligt Moment i al vor Viden om sandselige \apage{19} Ting; men naar
...
Bevidsthedens \emph{active} Side kjendelig og svarer til, hvad vi kalde
...
allerede ved den umiddelbare \apage{20} Sandsning af Gjenstanden selv har været
...
ligger i \emph{Anskuelsen}. Det forholder sig ganske vist med Anskuelsen som med
...
Ikke desto mindre er det Sandsningen af virkelige Gjenstande, der giver \apage{21} os
...
Uden Indbildning er det umuligt at sandse nogen \emph{Forandring} i det Udvortes.
...
deres \apage{22} Billeder, men at de forskjellige Gjenstandsbilleder sammenlignes med
...
Gjenstand for \emph{indvortes Erfaring}.
...
Naar der tales om Tingenes \emph{Indre}, er det jo indlysende, at dette Indre
ikke kan henføres til nogen udvortes Erfaring. Tage vi det umiddelbare
Ydre bort, saa er der for Sandsningen et nyt Ydre. Hvorfra faae vi
...
\emph{Muligheden}. Hvad der skal paavirke den udvortes Sands maa være virkelig
...
vi Sandseindtrykket ved $i$, \apage{23} Forestillingen ved $f$, da giver Rækken: $i$,
...
her maa forudsættes \apage{24} et Indre. Før og efter Forandringen er B det samme, og
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «Synet af et Frø, vækker Forestillingen om en Plante»
...
Indbildning \apage{25} om \emph{Aarsag}. At A er Aarsag, og B Virkning, kan ingenlunde
...
sige: paa exacte Bestemmelser af \emph{Indbildningens Andeel i Viden}.

At Indbildningskraften, selv naar den støtter sig til udvortes Erfaringer,
ikke formaaer at grundlægge \apage{26} nogen virkelig sand eller selvgyldig Viden,
ligger i Sagens Natur og har viist sig i Kulturens Historie. Et Folks
...
alt Forestillet, at \apage{27} de rene Tankebestemmelser, de tomme Almeenformer,
...
ingen Lighed mellem \apage{28} Ordet Træ og det virkelige Træ, Ordet Huus og det
...
videnskabelig Indsigt ikke nok, at den Vidende forefinder en  \apage{29} Tingenes
...
apriorisk; hvorimod den Viden, der forudsætter en \apage{30} Sandsning af existerende
...
Ulighed forud, d.\ v.\ s.\ \emph{a priori}. Er Talen derimod om saadanne Gjenstande
...
desaarsag komme bagefter Tingene, kalde vi \emph{aposterioriske, empiriske}.

Men naar Grændsen mellem \emph{Apriorisk} og \emph{Empirisk} skal opfattes bestemt og
nøiagtigt, indtræde Vanskeligheder. At lægge Eftertrykket paa det
Aprioriske er charakteristisk for \emph{Idealismen}, at gjøre det Empiriske til
Et og Alt er charakteristisk for \emph{Sensualismen}. Ved at underkaste
Idealistens »medfødte Ideer« en skarp Kritik kan Sensualisten let bilde
sig ind, at det Aprioriske kun er et Skin, at al vor Viden  \apage{31} lader sig
aflede af Sandseindtryk, Sandsning, Afspeilinger af det Sandselige,
...
vi \apage{32} finde dem hos Alle med nogen Bevidsthed; men at Børn og Vilde ingen
...
Lighed og Ulighed in abstracto har han intet Blik. Siig ham, at
...
forestillende, tænkende Bevidsthed, kan Erfaringen jo umulig selv \apage{33} være det
...
være den samme som før. Hvis Rummet var en Bestemmelse, der tilhørte  \apage{34}
...
Tilfældet, efterdi Huller i Rum selv ere Rum med indbildte Grændser, saa følger her af, at alle Ting i Rummet kunne forsvinde, uden at Rummet
...
viist sig i Erfaringen, som et Forudgaaende, Daværende og Efterfølgende,
...
være en uendelig \apage{35} fortsat Nutid; thi Fortiden er forsvunden og svarer til
...
abstracte Bestemmelser og blive kun \apage{36} forstaaelige derved, at den ene
...
Tingene. Forstandens almeengyldige  \apage{37} Reflexionsbestemmelser og Kategorier
...
Bevidstheden, \emph{indenfra}, og det, der kommer fra Tingene, \emph{udenfra}, har
...
Forholdsbestemmelser bevidst  \apage{38} som den rene Forstands aprioriske Former.
...
Formbestemmelser, som, een Gang bragte til Bevidsthed, vise sig \apage{39} at være
...
\apage{40} Sand Viden er Indsigt; fremadskridende Indsigt beroer paa en stadig
...
af sit eget Befindende, sin Attraa og Længsel i ethvert givet \apage{41} Øieblik,
...
blind Nødvendighed paatrænger sig udenfra, eller fremhæves vilkaarlig
indenfra. Først idet Videns Gjenstand opfattes og bestemmes af den
Vidende, d.\ v.\ s.\ objectiveres, bliver den et Videns Object. \emph{Intet
...
hvert Punkt er et Moment af det Subjective, og omvendt: \emph{Objectiveringen
selv er Eenheden af det Subjective og det Objective}, men \emph{en Eenhed af
modsatte Led, en Modsætningseenhed}.

Først naar vi opfatte Modsætningen af subjectiv og objectiv Viden
in concreto, er der Grund til at spørge om en kritisk Grændse.
...
Synskreds objectiverer, \apage{42} angaaer alene dens egen specifiske Væren, dens
...
kan hentes fra noget i sig Objectivt, men oprindelig ligger i
...
objective.
Her kunne flere Tilfælde vise sig.

Er Individualiteten svag, og har
...
have en objectiv Viden om sin høieste Bestemmelse. Er  \apage{43} Individualiteten
...
det vi kalde \apage{44} Sjæl egenlig er, kan den Enkelte faae at vide, om han
...
allertydeligst af saadanne Exempler, hvor det Objectiverede ikke afhænger
...
$x = \frac{bc}{a}$. Af subjective Forudsætninger fremgaae objective  \apage{45}
...
men ogsaa en reel Objectivering. Et Par Exempler kan oplyse dette. --- Dersom det menneskelige Selv er anlagt paa Udødelighed, da maa ethvert
...
mellem Realitet og Indbildning. Den anden derimod er en
Objectivitetssætning. Forsætningen, der støtter sig paa udvortes
Erfaring, kan gjøres indlysende for Alle, som have almindelig Sands \apage{46} og
...
objectiverer sig Verdensbygningen paa en ganske anden Maade end den,
...
indeholde Grundbetingelser, elementære \apage{47} Forudsætninger, der ere fælles
...
saa kan Du spørge, \apage{48} om det Du objectiverer ogsaa virkelig er det, Du
objectiverer. --- Men Objectiveringen indeslutter to uligeartede Momenter, det subjective
...
forskjellige; men Lampen i sig, \apage{49} det egenlige Object, staaer altid
...
\section{Erkjendelsesproblemet} \apage{50} 
\label{sec:6}
% pp. 50--59
...
Forestillingen, men bliver staaende for Bevidstheden \apage{51} som noget Udvortes.
...
beraabe sig paa Ligheden mellem Aarsagen og Virkningen er yderst  \apage{52} misligt.
...
altsaa: det er jo muligt, at der gives en Forstand, saa qvalitativt  \apage{53}
...
Legemets Bygning, men veed ikke, hvad Livet er, eller hvorfra det kommer; \apage{54} 
...
$x_0$, $x_1$, $x_2$ \ldots, saa ville disse Bestemmelser, naar de sandses i
...
erkjendes?  \apage{55} Ingenlunde; det er just et Beviis paa Vexelvirkning mellem
...
om, at Virkningen \apage{56} svarer til Aarsagen. Kun Abstractionen i Forening med
...
reelt Skin, og det Skin, hvortil ingen Realitet svarer, et falsk Skin.  \apage{57}
...
Er man allerede naaet til \apage{58} at opdage den fjerde Dimension, saa er det jo
...
\apage{59} Forstandsudvikling; men vi maae bestandig forudsætte, at Kategorier og
...
reent ud sagt, et Indbildningens Salto mortale, der underholder
...
\apage{60} \section{Fuldkommen og ufuldkommen Viden}
\label{sec:7}
% pp. 60--69
...
som en Cirkel med en uendelig Radius. Forskjellen imellem den \emph{fuldkomne} og
\emph{ufuldkomne} Viden beroer paa en væsenlig Forskjel mellem iboende \emph{Grændser}
og \emph{Skranker}.
...
oprindelig frembringer \apage{61} dem. Vi støde her paa en gjennemgaaende Forskjel
...
psychologiske Betingelser i Reqvisition, aldrig kan komme til Rette med
...
som ikke er \apage{62} objectiveret; og dog faaer den sandsende Bevidsthed et Substrat
...
Ideal in mente. Den Afviisning, at Antagelsen af en fuldkommen
...
skiftende Opmærksomhed fremkalder.  \apage{63} Nøiagtig talt, er der kun eet Element,
...
Evigheden. Hvor nær den Vidende, der er fordybet i sine \apage{64} Opgaver, kan bringe
Tidens Uro til evig Ro, derom minder hiint Ord af Archimedes: \emph{noli
...
Hurtighed ikke ganske forholder sig, som man sædvanlig antager. Lad en Alen \apage{65} 
...
nok skulle rette sig efter Lovene. Men Opfattelsen lider af  \apage{66} betænkelige
...
Andet, \apage{67} Noget almindeligere end Andet, at Noget saa ofte gaaer forud, at det
...
ogsaa er muligt. Ligesom ingen Tanke har Betydning for \apage{68} sig, men kun i
...
% AS PRINTED: `Indentitet' (collation 2026-09-24, two readers).
Indentitet, at Delene er det Heles og det Hele Delenes Bestaaen, er netop
...
Legeme er mere end tungt; det har en Mængde andre \apage{69} Egenskaber og er ved
...
Hele, en Mangfoldighedseenhed, bestaaende \apage{70} deri, at hvert enkelt Led
...
mere vil en Alt beherskende  \apage{71} Nødvendighed i Delenes Dannelse ud af
...
Nødvendigheden vil gaae i Eet med Friheden, og vi have da \emph{et teleologisk
System}.
...
charakteriserer \emph{Methoden}, er altsaa: \emph{Veien til Maalet}. Enhver videnskabelig
...
saa kan Bevægelsen i Sagen dog ikke siges at være  \apage{72} teleologisk; intet
...
Alt, hvad man behøver. Dersom A (Forudsætningen med Aarsag og Betingelser) \apage{73} 
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «Kant-Laplacèske»
System uden Teleologi, derpaa er den Kant-Laplacèske Theori om Solsystemets
...
Men, siger Kritiken, et saadant dobbelgenetisk System gives ikke, og kan ikke gives. --- Naar vi spørge: hvorfor? faae vi to forskjellige Svar. Der maa
...
at faae Forudsætningen \apage{74} begrundet; det er en subjectiv Cirkelbevægelse. I Virkeligheden opnaaer man kun at gjøre Forudsætning og Resultat lige
...
har ikke sjældent gjort sig skyldigt i teleologiske Misgreb. \apage{75} For den
...
Dogmatikerens naive Tillid til sin »\emph{Totalanskuelse}« sees tydelig af den
...
\emph{Det dogmatiske System er i det Hele overveiende synthetisk}. Ud fra et
...
(methodus mathematica). Med Anstrøg af Uomstødelighed optræder det
...
\apage{76} Den gamle Ptolemæiske Verdensbygning havde den Aristoteliske Dogmatisme
...
Analysen i Forgrunden. Kritiken er intet System, den er en Methode:
% `er væsenlig': the scan shows `et væsenlig'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
% `den kritiske Methode er væsenlig analytisk' spaced (two of three readers on `analytisk').
\emph{den
...
analytisk-synthetisk fra Først til Sidst, en Methode, som \apage{77} hverken vil være
dogmatisk eller kritisk, men dogmatisk-kritisk fra Begyndelsen til Enden,
det er \emph{det speculative System med den dialektiske Methode}.
...
gjælde ubetinget, saa \apage{78} kan ingen Ting forandre sig, Intet opstaae og Intet
...
identiske; d.\ v.\ s.\ \emph{den dialektiske Methode er immanent}.
...
at Noget er Andet; men den bliver lige \apage{79} saa lidt staaende ved den sidste
...
Sagen selv uvedkommende, Hensyn (diversi respectus) uden om
...
endelige, maae de endelige Hensyn (diversi respectus) gjælde.
...
\apage{80} Videnskaber give forskjellige Systemer og kræve forskjellige Methoder.
...
kalder Videnskabernes \emph{Encyklopædi}. Allerede i Oldtiden gjordes Forsøg paa
...
græske Tvedeling: Musik og Gymnastik, fremgik siden de saakaldte \emph{syv frie
Kunster}: »Grammatik, Dialektik, Rhetorik, Arithmetik, Geometri, Astronomi
og Musik«. \apage{81} Da Tænkningen stod over Erfaringen, optog Philosophien de
særskilte Videnskaber i sine Lærebygninger og endte med den stoiske
Tredeling: \emph{Logik}, \emph{Physik}, \emph{Ethik}. I Middelalderen havde man Compendier,
mere eller mindre fuldstændige Oversigter over det samtlige Indbegreb af
menneskelig Viden paa den Tid under Titler som: \emph{Summa}, \emph{Speculum},
\emph{Theatrum vitæ humanæ} o.\ dsl. Senere opgav man det systematiske Hensyn
...
\emph{Novum organon} (1620) og \emph{De dignitate et augmentis
...
Vægtfylde betydelig, naar den \apage{82} underkastes en kritisk Prøvelse. Af dybt
...
Erkjendelser: \emph{empiriske}, \emph{mathematiske}, \emph{philosophiske}. Enhver af disse
...
i de philosophiske deres Sammenknytningsformer in abstracto
...
»De tre Erkjendelsesmaader svare til tre forskjellige
...
System, og den rene empiriske Gehalt under det conjunctive System. \apage{83} 
...
sammenordner Dele i det Hele.« (Apelt: Die Theorie der Induction.)
...
ene Videnskab, og lutter Sammensætninger af anskuelige Led uden Love i \apage{84} den
...
Adskillelse gjælde for principiel, \apage{85} der dog kun grunder sig paa et:
\emph{a potiori fit denominatio}, hvorledes forholder det sig da med det
...
theoretiske Kundskab om Naturen, altsaa kun med de fundamentale Begreber, \apage{86} 
...
forefindes. De skulle deri fuldstændig \apage{87} opregnes og ordnes derved, at det
...
Rummet og Tiden. Naturvidenskaben falder i tre Dele: a) \emph{Mechanik} med
Hensyn til Materie og Bevægelse. b) \emph{Det Uorganiskes Physik} med Hensyn paa
Lys, Magnetisme, Elektricitet, de physiske Legemer og den chemiske Proces.
c) \emph{Det Organiskes Physik}, hvorunder da igjen hører Geologien, der behandler
...
animale Natur, ligesom Medicinen \apage{88} den syge. Ogsaa Aandsvidenskaben falder i
tre Afsnit: a) Aanden i sit Begreb, \emph{Anthropologi} og \emph{Psychologi} (Sproget --- Philologi). b) den praktiske Aand, der yttrer sig i Daad og Handling,
hvorunder \emph{Retslæren}, \emph{Statsvidenskaben}, \emph{Historien}. c) Aandens Fuldendelse i
Kunst, Religion og Videnskab, hvortil svarer \emph{Æsthetik}, \emph{Religionslære},
\emph{Philosophi}.

Her behøves ikke vidt gaaende Undersøgelser over den Maade, hvorpaa
...
Hegel synes jo i dette Stykke at være fuldkommen enige, og dog --- hvilken Forskjel! Hos Comte er det i Fornuftens Navn Mathematiken, som baade er en
...
absolut Viden, maa Positivismen \apage{89} ifølge sit Princip forkaste som absolut
...
% AS PRINTED: `Pilosophien' (collation 2026-09-24, two readers).
ere villige til at tjene, berige og fremhjælpe Pilosophien paa bedste
Maade; men udmunde i Philosophien som Floder i Havet, gjøre sig til blotte
Hjælpevidenskaber, »Haandlangere« under Philosophiens Formynderskab \apage{90} ville
...
Philosophien,  \apage{91} Middelalderen i Theologien, Nutiden finder det i
...
concreteste Bestemmelser, selv naar den gjør sit Yderste, maae \apage{92} betinges af
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «εἱς κοῖρανος ἐστω! det vilde»
πολυκοιρανίη εἱς κοῖρανος ἐστω! det vilde være et oplivende Syn. Samtlige
...
hine afgive Grundlaget for disse; men allerede den Omstændighed, \apage{93} at her
...
er Iagttagelsen af de virkelige Phænomener, de virkelige Stjerner, de  \apage{94}
...
Det almene Grundlag, \emph{fundamentum divisionis}, maa søges i Videns
...
Empiriske. Videnskaberne maae da med Hensyn hertil dele sig i \emph{aprioriske} og
\emph{empiriske}. Det Aprioriske antager igjen forskjellig Charakteer, eftersom
...
hertil svare: \emph{Logik}, \emph{Mathematik} og \emph{rationel Mechanik}. Ligeledes vil det
...
mod et bestemt Maal. Med Hensyn hertil inddele vi de empiriske \apage{95} Videnskaber
i \emph{realempiriske} --- mechanisk og chemisk Physik, Chemi og Geologi --- og
\emph{teleologisk-empiriske} --- Botanik og Zoologi, Anthropologi og Historie.
...
behandles i \emph{Aandslæren}. Dersom Friheden selv faldt udenfor Nødvendigheden,
...
have seet, forblive en Differens mellem den sandsende, objectiverende \apage{96} 
...
--- gjennem \emph{Æsthetik}, \emph{Ethik} og \emph{Religionsphilosophi} --- fuldbyrder sin egen
Opløsning i Idealet selv. Hvad endelig Forholdet mellem \emph{Fleerhed} og \emph{Eenhed}
angaaer, er der Grund til at dele Videnskaberne i \emph{Fagvidenskaber} og
\emph{Aandsvidenskab}, dog saaledes, at Inddelingsgrunden ikke fastsættes
...
% `videnskabelige': the scan shows `vidensbabelige'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
...
\apage{97} særlige Indhold, Aandsvidenskaben optager fra Fagvidenskaberne i sine
...
Tankeformer og \apage{99} udvikle dem i et System; man kan opløse den endelige
Tænkning i det Uendelige saaledes, at den tænkende Selveenhed skinner
igjennem i tænkende Ideeformer. Herved bestemmes Forholdet mellem \emph{formel}
eller \emph{subjectiv Logik}, \emph{immanent} eller \emph{objectiv Logik} og \emph{Grundideernes
Logik}.

\textbf{Den formelle Logik} deler sig i Læren om \emph{Begreb}, \emph{Dom} og \emph{Slutning}.

\emph{Begrebet} dannes paa menneskelig Viis, idet Tænkningen, som oprindelig, om
...
\emph{Dommen} dannes paa menneskelig Viis, og Begrebet finder sit logiske Udtryk
i Sproget: Ord forbindes til Sætninger, og i Sætninger udtales Dommen.
Dommen maa da formelig angive: \emph{Subjectet}, den Forestilling, der skal
bestemmes, \emph{Prædicatet}, den \apage{100} bestemmende Forestilling, og \emph{Copula}, det
...
forskjellige Domme; og Dommene inddeles efter \emph{Qvalitet}: i bekræftende,
benægtende og uendelige; efter \emph{Qvantitet}: i singulære, particulære og
generelle; efter \emph{Relation}: i kategoriske, hypothetiske og disjunctive;
efter \emph{Modalitet}: i assertoriske, problematiske og apodiktiske. Fremgaaer
Prædicatet ved formel Udvikling af Subjectet, saa er Dommen \emph{analytisk}; kan
Prædicatet kun ifølge en særegen, være sig apriorisk eller empirisk,
Anskuelse, føies til Subjectet, saa er Dommen \emph{synthetisk}.

\emph{Slutningen} dannes paa menneskelig Viis; thi Dommen maa begrundes: den
Tænkningsact, hvorved en Doms Gyldighed udledes af een eller flere andre
Domme, er \emph{en logisk Slutning}. Har Slutningen kun een Præmis kaldes den
\emph{umiddelbar}, har den flere Præmisser da \emph{middelbar}. De middelbare Slutninger
inddeles i: de \emph{kategoriske}, der betinges af, at det underordnede Begrebs
Kjendemærke (nota notæ) subsumeres under det overordnede; de
\emph{hypothetiske}, der beroe paa det Betingedes Afhængighed af sin Betingelse
(modus ponens og modus tollens); og de \emph{disjunctive}, som
grunde sig paa, at Leddene af et Hele gjensidig forholde sig saaledes, at,
naar eet udtrykkelig er sat, saa ere de øvrige udelukkede (modus
ponendo tollens), og at, naar de øvrige ere udelukkede, saa maa det
tilbageblevne sættes (modus tollendo ponens).  \apage{101} --- Den tænkende Subjectivitets Indskrænkning er saavel i Begrebsdannelse som i Dom og
Slutning kjendelig paa, at Tænken og Væren overalt falde ud fra hinanden.

\textbf{Den immanente Logik,} grundlagt af Hegel, abstraherer fra den tænkende
...
Systemet deler sig i Læren om \emph{Væren}, \emph{Væsen} og \emph{Begreb}.

\emph{Væren.} Den almene Væren er Eet med den almene Tænken; men Tankens og
Værens Eenhed er i sin abstractere Form indholdsløs: den rene \emph{Væren} er det
Samme som det rene \emph{Intet}. Og dog er Væren, det Positive, forskjellig fra
Intet, det Negative. At Væren baade er og ikke er Intet, vil sige, at Væren
uophørlig gaaer over i Intet, og Intet i Væren: denne Overgang er \emph{Vorden}. I
Vorden er Intet en Grændse for Væren, og Væren for Intet. Den ved sit Intet
bestemte Væren er et Værende, et \emph{Noget}, og det ved Væren bestemte Intet, et
\emph{Andet}. Noget er forskjelligt fra Andet, thi Andet er dets Grændse; og dog
...
\emph{Forandring}. I Forandringen er Noget Eet med sit Andet og for saa vidt
uforanderligt. Det er altsaa det Uforanderlige, der forandres. Al den Uro,
som sees \apage{102} i Væren og Tilværen, er forstaaelig deraf, at Grændser som: Intet
...
Uforanderlige, der reflecterer sig i det Foranderlige, er \emph{Væsen}; det
Foranderlige, Flygtige, der reflecterer det Uforanderlige, er \emph{Skin}:
% `Identiteten Forskjel': the scan shows `Idenditeten'; sense-reading
% (JBIG2 rule; collation 2026-09-24, two readers).
Vexelskin af Skin og Væsen er \emph{Reflexion}. I Reflexionen har Identiteten
...
\emph{Ting}; men Tingen kan kun reflecteres i sig ved at reflectere sin Existens i
andre existerende Ting: de til disse Reflexioner svarende Bestemmelser ere
\emph{Egenskaber}. Under den vexelsidige Reflecteren opløse Tingene sig i
...
sig i det Ubestandiges Vexlen; dette Bestaaende er \emph{Substansen}, og de i
Ubestandighed vexlende Fremtrædelsesmaader ere \emph{Accidenserne}. Forholdet
mellem Substansen og dens Accidenser er et Causalitetsforhold: Substansen
er Aarsag, den \apage{103} oprindelige Sag; Accidenserne ere Virkninger. Men Aarsagen
...
Vexelvirkning. I \emph{Vexelvirkningen} er Virkningen Aarsag og Aarsagen Virkning;
...
% AS PRINTED: `Forkjel' (collation 2026-09-24, two readers).
det \emph{Almene}, i sin Forkjel fra sig selv det \emph{Særskilte}, i sin totaliserede
Bestemthed ved sig selv det \emph{Enkelte}. Begrebets Selvudvikling er dets
Grunddeling og Mediation, d.\ v.\ s.\ \emph{Dommen} og \emph{Slutningen}. Formedelst
Grunddelingen er Totaliteten i sin første umiddelbare Eenhed det Enkelte,
sat som \emph{Subjectet}, og Totaliteten i den reflecterede Form, det Almene, sat
som \emph{Prædicatet}: Subjectet er, hvad Prædicatet er, det \emph{Enkelte er det
...
i indbyrdes Vexelbestemmelse som det \emph{Særskilte}. Grunddelingens Yderled,
Extremerne, have altsaa under Begrebets Selvudvikling afsat et Mellemled,
et \emph{Medium}, hvori de kunne medieres. Denne Mediation er \emph{Slutningen}.
--- Begrebets Selvmediation gjennem Dom og Slutning er dets \emph{Subjectivitet}, den
ved Mediationen  \apage{104} gjenvundne Umiddelbarhed er dets \emph{Objectivitet}. Som
...
i deres udvortes, indifferente Gjensidighed ere \emph{mechaniske}, virkende paa
hverandre ved Tryk og Stød. Den herved indtrædende Adskillelse mellem
Bevægelse og Hvile er en relativ Tilsyneladelse af den \emph{absolute Mechanik},
hvori Bevægelsen er Hvile, og Hvilen Bevægelse. --- Under de mechaniske
Objecters Indifferens skjuler sig en \emph{Differens}, der kommer tilsyne i den
chemiske Tiltrækning. Objecternes Differens medfører en Spænding, der
udlignes i \emph{det chemiske Produkt}. Men Produktet, hvori Differensen er
neutraliseret, maa som det \emph{Neutrale} igjen opløse sig i Differens, og
...
anticiperede Resultat er \emph{Formaal}, Forudsætningerne med deres
Mellembestemmelser ere \emph{Midler}, Bevægelsen er \emph{teleologisk}. Det teleologiske,
sig selv i sine Elementer begribende, Begreb er en Form, der giver sig
selv Indhold, et Indhold, der giver sig selv Form, er i \emph{absolut Eenhed} af
Form og Indhold, af \emph{Subjectivt} og \emph{Objectivt}, af Formaal og Midler, med eet
Ord: \emph{den absolute Idee}.

\textbf{Grundideernes Logik} lægger an paa en videre Udvikling og nærmere
Bestemmelse af den absolute \apage{105} Idees Indhold; de særlige Grundformer,
hvorunder Indholdet opfattes, ere \emph{Viden}, \emph{Magt} og \emph{Sandhed}.
...
sandset. Denne Venten betyder da ikke nogen Mangel eller \apage{106} væsenligt Savn.
...
er, kan der ingen Tænkning \apage{107} være. At sandse et Intet er umuligt, og naar
...
% `Mediationer naae': as printed; the Rettelser read `Mediationer aldrig naae' (collation 2026-09-24).
...
Tilværen \emph{udenfor}, det abstraherede Artsbegreb derimod \emph{indenfor} Bevidstheden.
...
der bestemmer Individerne; Sandsningen paastaaer, at \apage{108} Individet er det
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «\emph{γ) Slutningens Opløsning,}»
...
Slutningsschema egenlig betyder. Det betyder nemlig, at af Slutningens tre
...
Ideen maa indeholde alle Bestemmelser \apage{109} for Arterne, saa maa Dannelsen af
...
skjærende Modsigelser, at den blotte Tænkning, den  \apage{110} være nok saa dialektisk,
...
\apage{111} Tingen er, da er dette en Skuffelse, som den logiske Dom foranlediger. Hvad
...
objectiv Selvstændighed er jo netop Indtrykket af den Magt, \apage{112} hvormed de hver
...
Idealitet og Realitet uendelig mæglende Væsen \apage{113} er det i sin Viden absolut
...
Jeg-Former; dens Mangel paa Formudvikling er den væsenlige Grund \apage{114} til dens
...
Fra \apage{115} Kjendsgjerningernes, de endelige Realiteters Synspunkt, er det ganske
...
begrebsløs \apage{116} Facticitet vilde Tingene existere ved Siden af hverandre,
...
\apage{117} Objectivt, som ikke er objectiveret, lige saa lidt kan der være noget i
...
Mathematiken. Den begynder med det abstract Almene, \apage{118} for saa vidt den
...
Reflexionens Almindelighed, den kræver \emph{Operationen}. For Reflexionen er
...
tjene til Exempel. I Reflexionens Form fremstilles den saaledes: »Naar \apage{119} et
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «den samme Høide fra Begyndelsen? Enhver»
...
Eftertryk, den exacte Videnskab lægger paa \emph{Beviset}. Den kan bruge Billedet,
...
antager, at en Curve parallel med en Ellipse nødvendig maa være en \apage{120} Ellipse,
...
Udtryk i \emph{den mathematiske Function}. Uden nogensomhelst Indskrænkning i
...
Størrelser, $a$ er et Tal. \apage{121} Afhængighedsforholdet mellem $y$ og $x$ er
...
articulere Forholdet mellem \emph{Endeligt} og \emph{Uendeligt} paa en saadan Maade, at
...
$\dfrac{dy}{dx} = a$. Dette Resultat, saa simpelt det end er, er uendelig
...
det Umaalelige, det Uopnaaelige; men gaae \apage{122} vi ud fra Ligningen: $dy = a\,dx$,
...
Forholdet mellem \emph{Discretion} og \emph{Continuitet}.
...
Tilnærmelsen, \emph{den uendelige Tilnærmelse}. Adskilt fra Operationsbegrebet kan
...
naar det Uendelige altid er uendelig langt borte fra det  \apage{123} Endelige, saa
...
\apage{124} Den uendelige Discretion er altsaa Mellembestemmelsen mellem Continuiteten
...
% `Peripheriens': the scan shows `Perepheriens'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
Peripheriens Element og $\displaystyle\int_0^{2\pi} \frac{-x}{\sqrt{a^2 -
...
Men, mærkeligt nok, medens man paa realistisk, positivistisk Viis \apage{125} absolut
...
hvorpaa den rationelle Mechanik har sikkret sig Betingelserne \apage{126} for
...
og i modsat Fald liig med deres Differens: $R = P \pm Q$. Ere Retningerne
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «Kræfternes Paralellogram. Betegnes»
\emph{Kræfternes Paralellogram}. Betegnes den Vinkel, de to Kræfter $P$ og $Q$
...
Er Vinklen $(P.Q) = 0^\circ$, saa \apage{127} er $\cos = 1$, og $R = P + Q$; er Vinklen
...
Billede paa Eenhed af \emph{Realbegreb} og \emph{Lov}. Men den exacte Videnskab udvikler
...
Maxima \apage{128} og Minima ikke blot afhænge af varierende Led, men hvor disse Led
...
Mechanik. Vi see det tydelig i Vexelbestemmelsen af Kraft og Aarsag. Aarsagen med
...
Materien \apage{129} værende Inerti og det af begge Resulterende faae vi det exacte
...
igjen $3\,dv$ o.s.fremdl., saa at den i det $n^{\mathrm{te}}$ Øieblik er voxet op til
...
til den forestillede Masse. Er Massen $M$ større end Massen $m$, og $g = g$ Accelerationen, da har den bevægende Kraft i $Mg$ unegtelig en større
...
\apage{130} Acceleration uden kjendelig Bevægelse: $Mg = P$. Af Vægten $P$ sees det, at
...
kunne sige, at deres Kræfter ere forskjellige? Det vilde \apage{131} jo, for at bruge
...
Bevægelsesmængden $Mv$ en \apage{132} levende Kraft $Mv^2$. Da nu $mV^2$, ifølge
...
Inertien Betingelser for  \apage{133} Accelerationen; de forestilles hver for sig, og
...
dobbeltsidige \apage{134} Væsen. I Ophævelse af Udstykning maae, vel at mærke, alle
...
en forskjellig Aarsag. Reflectere vi paa Forskjellen, saa have \emph{Statiken} og
\emph{Dynamiken} hver sin Kreds af Opgaver, og falde for saa vidt ud fra
...
\apage{135} Reflexion af to til hinanden svarende Principer, de virtuelle Hastigheders
...
\section{Mechanisk og chemisk Physik}
\label{sec:14}
% pp. 135--151
...
\emph{mechanisk Physik}. Her er en ny Methode, fordi her er et nyt Grundlag, d.\ e.\
de ydre Erfaringers Grundlag. Hverdagserfaringer ere for  \apage{136} usammenhængende og
...
Faldloven --- $15\tfrac58$ Fod i 1ste Secund; den Kraft, der drager \apage{137} Maanen kan,
...
paa Jorden ligge os \apage{138} anderledes nær og paa en ganske anden Maade kunne
...
at der er tiltrækkende Kræfter, forskjellige fra Tyngden.  \apage{139} Physiken deler
sig da med Hensyn hertil i \emph{mechanisk og chemisk Physik}.
...
følgende Lysgran, \apage{140} saa at der altsaa ikke er Fare for Sammenstød. Derimod
...
ere de incommensurable, thi Tingen er antilogisk. Her af følger, at ikke
blot \emph{Hypothesen}, men ogsaa \emph{Analogien} maa \apage{141} ved Inductionens Anvendelse spille
...
efter samme Lov: \emph{Udfaldsvinklen} $=$ Indfaldsvinklen o.\ s.\ frmdl. Men Varme er
...
Legemet frembringe Varme, \apage{142} tillige bringe Legemets egne Smaadele i
...
Forkastelsen af Newtons  \apage{143} »Principia« deraf være en uafviselig Conseqvens.
...
saa \ldots\ --- Ja, hvad saa? Saa maa det være, fordi den er passiv og dvask og
...
man anvender modus ponens eller modus tollens. Den
...
(Slægten) voxer, jo større det Antal særlige Tilfælde (Arter) er, hvor om
det gjælder. Da en saadan Induction imidlertid kan have sine »Instanser«,
saa kunne de ved Inductionen fundne Love ogsaa have deres Undtagelser; \apage{144} 
...
\apage{145} Hvad dette betyder, kan sees paa Forholdet mellem Keplers og Newtons
...
om, \apage{146} at Beviserne, hvad det Physiske angaaer, hverken ere blot aprioriske ei
...
aflede og forklare de særskilte Phænomener. Hvad er \emph{Magnetisme}? Et Udtryk
...
Eensformigheden er \apage{147} i Strid med Leddelingens Princip, og Frastødningen
...
men ikke en Magnet, som kun har een Pol. Hvad er \emph{Elektricitet}? Analogien
...
variable, saa at  \apage{148} Legemer under forskjellige Betingelser kunne blive snart
magnetiske, snart diamagnetiske, snart igjen ikke-magnetiske, eller snart
positivt, snart negativt elektriske, snart igjen ikke-elektriske. Det er
...
Legeme \apage{149} B saaledes, at der ved Fordeling i B fremkommer negativ Elektricitet
...
og slette Ledere, \emph{Isolatorer}, der er saa betegnende for Leddelingen, vil
...
\apage{150} Udslag mod Vest, naar den selv befinder sig under Strømmen? Hvad er Grunden
...
mere fremtrædende, jo mere \apage{151} indviklede Concretionerne ere, medens den
empiriske Erkjendelse, for saa vidt den i Sandhed er videnskabelig, om dens
Interesse for Kjendsgjerninger end er nok saa stor, altid er henviist til
...
Voltastøtten er Vandet blevet adskilt. \apage{152} Ligesom Vandet, saaledes adskilles
...
derved udskilte Brintatom trækker endvidere \apage{153} Ilten fra den tredie Vanddeel
...
Den indre Forbindelse mellem Elektricitet og Chemi er tydelig viist i \emph{den
...
Forbindelser af anden Orden og danne \emph{Salte}, i hvilke den ene af
Forbindelserne, \emph{Syren}, er det elektronegative, den anden, \emph{Basen}, det
elektropositive Led; Salte \apage{154} forbindes igjen til Dobbelsalte o.\ s.\ frmdl.
...
dette Maal er Varmeudviklingen. Saaledes komme vi til den \emph{thermochemiske
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «»Valens«, «Affinivalens«,»
sig med fire Chloratomer. Saa  \apage{155} fremkommer \emph{Atomitetslæren}, Læren om »Valens«,
...
\emph{Chemien} som fundamental i Modsætning til \emph{Mineralogien}. Mineralogi og Chemi
betinge hinanden gjensidig, om vi end maae indrømme, at den sidste har en
% `physisk': the scan shows `phy-|cisk'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
...
Aarsagers Samvirken. Som det gaaer med Dannelserne, \apage{156} saaledes ogsaa med
...
Grundformer; men hvad det ved en Systemdannelse især \apage{157} kommer an paa, er
Axernes Antal, Stilling og Størrelse; hvor vidt der i en Gruppe haves f.\
% `Axler': as printed; the Rettelser read `Axer' (collation 2026-09-24).
...
krystallisere sammen, maa det vistnok forudsættes, at de \apage{158} have samme
...
væsenlig  \apage{159} Indsigt; ifølge det andet gaaer den fornemmelig ud paa
...
\emph{Begrebet}, \emph{Loven}, og \emph{Tingen} eller \emph{den concrete Legemlighed}.
...
Udgangspunkter, \apage{160} fra Realbegrebet, Loven og den concrete Legemlighed, kan
...
% `Vare:': as printed; the Rettelser read `Varer' (collation 2026-09-24).
...
egenlige Forskning, medens det Geologiske --- mindende om det Geogoniske --- nærmest falder ind under  \apage{161} Hypotheser og Theorier. Geognosiens Methode er
analytisk, Geologiens synthetisk.

Sammenfatte vi alle Dele af Naturlæren, der høre med ved Undersøgelsen af
Jordlegemets Skikkelse og Sammensætning, under Fællesbenævnelsen \emph{Geognosi},
...
Resultat; hvorimod Geologien, der gaaer ud fra \apage{162} Heelheden og seer paa den
...
endog antage, at Begivenheden har en saa stor Sandsynlighed \apage{163} for sig, at den
...
beriges, idet den \apage{164} gjennembrydes af Eruptivmasser --- Graniter, Porphyrer,
...
et Iisdække paa Vandet danner sig i Blæstveir.

Tager det geologiske Sprog undertiden et halvpoetisk Opsving, saa kommer
% `Geognosien': the scan shows `Geognosion'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
...
Hieroglypher, hvis Mening det \apage{165} gjælder at tyde. Hvad er da Meningen med disse
...
Sammenhæng at gjøre, men udelukkende dreier sig om \emph{naturlig Selvdannelse},
om \emph{Anlæg og Formaal, Realbegreb og Existens}. De tre storartede Hypotheser:
den \emph{kosmologiske}, om Planetsystemets Oprindelse, den \emph{geologiske}, om
Jordklodens Oprindelse, og den \emph{biologiske}, om de levende Organismers
...
Vorden og trinvise Dannelse, som det Nuværende i \apage{166} Tidernes Løb maa have
...
Rotation og Varme. Dampmassen bliver kugleformet, ophøiet i Æqvator;
...
at Solen, hvad der med Hensyn til »Perturbationerne« jo \apage{167} er særdeles vigtigt,
...
\emph{Philosophe, montre moi la main qui a jeté les Planètes sur la
...
Hyperbelbaner vilde i Urtiden have forladt Systemet, og vi altsaa beholde \apage{168} 
...
Masser, Atomer og Kræfter, at det er \emph{Naturen} \emph{selv}, \apage{169} Tilværelsens evige
...
Existenser ikke blot sine Muligheder, men, i væsenlig Forstand, sit \emph{Anlæg}.
...
\emph{geologiske Hypothese}. Sammenfatte vi de Formationer, Systemer og Grupper
--- Graavakkegruppen, Kulgruppen, Zechsteensgruppen, Triasgruppen, Jura-, Kridt-
...
gjennembrøde de slentede Lag. Hvorfor har den faaet \apage{170} et Hav? Fordi der i det
...
vil, som, hvad Indholdets Realitet \apage{171} angaaer, ikke kan gjøre Fordring paa
...
saaledes er det organiske Livs Anlæg væsenlig indeholdt i Jordklodens. \apage{172} Den
...
ligger da nær at fremhæve \apage{173} Ligheden imellem de uorganiske og organiske
...
Henviisning til Kulstoffets mærkværdige Evne til at forbinde sig med de
...
fuldstændig Consequens, idet de alt for ofte belæmres med dobbelsidige  \apage{174}
...
\apage{175} At Teleologi og Biologi ere sammenhørende Begreber, giver sig uvilkaarlig
tilkjende i det betegnende Slagord: \emph{Kamp for Livet}; det er en teleologisk
...
Kampen for Livet som uafviseligt Vilkaar for alt Levende. Selvopholdelsen \apage{176} 
...
blot, \emph{saa at}, men udtrykkelig, \emph{for at} det organiske Individ kan opholde
Livet.

Spørge vi om de materielle Aarsager og Betingelser, hvor af den ernærende
...
\apage{177} netop, hvorledes det gaaer til, at Virksomheden stadig vedligeholdes. Hvad
...
Vi begynde desaarsag med at gjøre en Adskillelse mellem \emph{Stofvirksomhed} og
\emph{Selvvirksomhed}, idet vi ved Stofvirksomhed forstaae enhver Kraftyttring,
...
man sig to Massedele $m_1$ og $m_2$ \apage{178} virkende paa hinanden, og lader den
...
$\gamma$) Systemets Overgang \apage{179} fra Bevægelse til Hvile, eller omvendt, har
...
hvorunder \apage{180} Modsætningen af Midtpunkt og Peripheri stadig afspeiler Eenheden.
...
Svar maa naturligviis forkastes af dem, der paa Forhaand ere hildede i den
Fixidee, at virksomme Realbegreber med væsenlige Anlæg ikke høre med i
Naturens Orden. Men selv om man indrømmer Gyldigheden af et \apage{181} almindeligt
...
den almene, og af den Grund intetsigende, Henviisning til Forholdet mellem
...
derimod Stofvirksomheden sat \apage{182} Livsmediet i Bevægelse, saa vil der have
...
Molekuler, saa fremkommer den styrende Eenhed derved, at $s$, $s$, $s$ \ldots\ ikke
...
Organismen af flere Celler, saa ville $S$, $S$, \ldots\ gaae ind under en høiere
...
\apage{183} Vil man endelig opgive alle Hypotheser, beraabe sig paa Videns
...
\section{Botanik og Zoologi} \apage{184} 
\label{sec:18}
% pp. 184--195
...
Forhold, afgiver  \apage{185} herpaa et træffende Exempel. Man classificerer Planter,
...
(Berberis) og en Vintergjæk (Galanthus) begge til sjette
Klasse, en Ask (Fraxinus) og en Salturt (Salicornia)
...
Saaledes hører, ifølge det Linneiske System, Agermaane (Agrimonia)
til ellevte, Mjødurt (Spiræa) til tolvte, Løvefod
(Alchemilla) til fjerde, og Bibernella (Poterium) til een
...
to store, omfattende Grupper, \emph{Kryptogamer} \apage{186} (de blomsterløse Planter) og
\emph{Phanerogamer} (Blomsterplanter) nærmere, saa viser sig ogsaa
% `f. Ex.': the scan shows `f. Fx.'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
Formudviklingens teleologiske Charakteer. Der danner sig f.\ Ex.\ Sporer og
Frø, ikke blot, \emph{saa at}, men udtrykkelig, \emph{for at} nye Exemplarer kunne
fremkomme. Sættes det kryptogame Sporegjemme (sporangium) imod det
phanerogame Frøgjemme (pericarpium), saa kunde man ved en
...
at en mellem Laaget og Huset værende Springring (elater) kaster
Laaget af; det saa kaldte Peristom har snart en enkelt, snart en dobbelt
...
naar dette Frøgjemme, som hos Lathyrus latifolius, bliver staaende
...
\emph{Cellen}; hvad Frøgjemmet angaaer, er det \emph{Bladet}. Kalde vi for Organismens
...
Moment, et høiere \apage{187} Element, et Element af anden Orden. Hvor høit Dannelsen af
...
Selvvirksomhed maa være underforstaaet, \apage{188} og da have vi det Teleologiske. At
...
% AS PRINTED: `Stofvirsomhed' (collation 2026-09-24, two readers).
Led og Punkt for Punkt er betinget af Stofvirsomhed.
...
% AS PRINTED: `phanerogram' (collation 2026-09-24, two readers).
phanerogram Plantes forskjellige Dele, lige fra Cotyledonen til Støvveien,
...
Sammenstykning, men udvikles \apage{189} indenfra af eet sig differentierende Anlæg: da
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «neppe tilstrækkelig undersøgt«. At de»
...
\apage{190} Have Planterne en Sjæl? Dette ældgamle Spørgsmaal er i den nyere Tid vendt
...
Leven, thi for de blotte Gradationer gives der \apage{191} ingen fast Grændse; det seer
...
(Animalia mollusca) uden virkelige Lemmer egner sig det spredte,
uregelmæssige Nervesystem; Bugknudesystemet for de eenssidede, leddede Dyr
(A.\ articulata) uden indre Skelet med mange eller ingen Lemmer;
Rygmarvssystemet for de eenssidede, uleddede Dyr (A.\ vertebrata),
...
man i det \apage{192} paagjældende Exemplar havde for sig. »Protisterne« have ikke blot
...
mellem Zoologi, Protistik og Botanik (Ernst Haeckel: Generelle
...
% AS PRINTED: `Dannnelse' (collation 2026-09-24, two readers).
Dyrene udmærke sig ved Dannnelse af kvælstofholdige Syrer (Urinsyre,
...
Physikeren nærmere skal bestemme, \apage{193} hvorledes det egenlig forholder sig med
...
Vækkelse i Gangliecellerne ved de \emph{centripetale} Traade, som \emph{Fornemmelse}, den
Forestilling derimod, som ved Vækkelse i de \emph{centrifugale} Traade frembringes
af Ganglieceller, som \emph{Villie}. Den dyriske Sjæls dunkleste og høieste
Function, der er vanskeligst at begribe, er \emph{Tankedannelsen}, der bestaaer i
...
\apage{194} Tanke er, thi jeg kan tænke; jeg veed, hvad Liv er, hvad det er at leve,
...
end kommer, \apage{195} saa følger jo heraf, at der i de andre organiske Functioner
...
\section{Den biologiske Afstamningshypothese}
\label{sec:19}
% pp. 195--205
...
mere besværligt at udfinde Skaberens Hensigt, navnlig med
\apage{196} Snyltedyrene --- men
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «Lyélls »Principles«»
overordenlige Skabelsesacter. Først med Lyélls »Principles« undergik den
hele Synsmaade en væsenlig Forandring. \emph{Naturen gjør ingen Spring}, og \emph{Alt i
...
plastisk Evne bevirke den ellers uforklarlige Urdannelse, \emph{Archegoni}.
Archegonien er, hvad  \apage{197} Protisterne vise, ikke oprindelig bunden til nogen
...
% AS PRINTED: `ἀναγκη', smooth breathing only, no accent (collation 2026-09-24, two readers).
Nødvendighed (ἀναγκη).«

I denne naturvidenskabelige Stil er Afstamningshypothesen paa en i Sandhed
storartet Maade bleven gjennemarbeidet og med et overordenligt  \apage{198}
Lærdomsapparat udstyret af E.\ Hæckel. \emph{Protisterne}, Monerer, Protoplaster,
Diatomeer o.\ s.\ v., danne det autogoniske Grundlag; fra dem nedstamme i den
ene Retning \emph{Planterne}, i den anden \emph{Dyrene}. Planternes Stamtræ forfølges
...
Plantedyrene (Coelenterata) og Stjernedyrene
(Echinodermata) gjennem Leddyr (Articulata) og Mollusker
...
% `palæontologiske': the scan shows `polæontologiske'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
charakteristisk, at Rækker af palæontologiske Dannelser \apage{199} opad maae afgive
...
% `Uhensigtsmæssighedslære': the scan shows `Uhænsigts-'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
...
Trin til de med Hvirvler Begavede. Søpungen (Ascidia) paa det
ottende og Lancetdyret (Amphioxus lanceolatus), den laveste
...
som vor Overbeviisning, at den stillede \apage{200} Opgave ikke er løst, da skeer det
...
objective Forskning har Opgavens hele \emph{videnskabelige} Indhold i sin Magt, da
...
Tiltrækning til Centrum og Bevægelse efter \apage{201} Tangenten. Vistnok er Sagen
ingenlunde hermed afgjort; thi Forholdene ere indviklede. Den af Delaunay i
% AS PRINTED (collation 2026-09-24, second reader agrees): «Connaissance de temps»
Connaissance de temps for 1865 fremstillede Formel for Maanens
...
kan bruges i physisk objectiv Forskning? Vi svare: den \apage{202} physisk objective
...
Stofbetingelserne og betragte den hele Dannelse som et Værk af  \apage{203} Selvvæsnets
...
% `gjennemløbe': the scan shows `gjennenløbe'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
...
Udtryk til Ære for physiske \apage{204} Kræfter og mechaniske Aarsager. Den
...
Tilnærmelse \apage{205} opnaae en relativ Løsning. Afstamningstheorien, langt fra at
...
\apage{206} Naar man begynder med Næse-Aben, Schimpansen, Gorillaen og derfra gaaer
...
Indogermanen, det kaukasiske Menneske, (Homo iranus); naar man
...
Hjernen: saa maa Alt, hvad der i det Foregaaende er \apage{207} sagt om Selvvirksomhed
...
Subject, hvorledes skulde man da forstaae saadanne Hverdagsudtryk som \emph{min},
\emph{din}, \emph{hans} Hjerne. For at kunne sige: \emph{min} Hjerne, maa \emph{Jeg} jo udtrykkelig
...
da nu egenlig er Jeg, der er \emph{Hjerne}, eller Hjernen, der er \emph{Jeg}. Det er et
...
\apage{208} Idet Psychologien erkjender de organisk-physiske Betingelsers Nødvendighed
...
Menneskets \apage{209} Sjæl er anlagt paa Selvbevidsthed: heri bestaaer den væsenlige
...
\emph{Jeg} og at være Dyr udelukke hinanden; Sætningen: \emph{Jeg er et Dyr}, er lige saa
modsigende som Sætningen: \emph{Cirklen er en Fiirkant}.
...
netop \apage{210} i Selvvirksomhedens Væsen, at det sjælelige Liv maa arbeide sig frem,
...
Naturbegavelse; de Fordele, som en forud gaaende Slægt ved sin
Udviklingsenergi efterlader i Arv til en  \apage{211} følgende, kan udøve en betydelig
...
Overleverede og at begynde for fra med noget Nyt og Oprindeligt. Kun dette
...
Mennesker er \emph{Samvittigheden}. Det gjælder her, som ved Bevidstheden, at man
...
særegen Art; uvilkaarlig yttrer den sig som en Følelse af  \apage{212} Individets
...
kan man jo \apage{213} vistnok sige, at det er Samvittigheden, der i høieste Instans maa
...
Menneskenaturen ikke tillader Mennesket \apage{214} at slaae sig til Ro i et Slags
...
vi i \emph{Personligheden}. Dyrenaturen bringer det kun til at sætte en besjælet
...
Høider; men  \apage{215} Videnskaben er ikke for Slavehandlere. De sandhedssøgende
...
Inderlighed i den \apage{217} Selvforstaaelsens, Selvbearbeidelsens og Perfectibilitetens
...
absolut Forstand Eet med Selvviden, svarer den \apage{218} objective: Væren af Andet er
...
som Magt. \apage{219} I de vexlende Phænomeners Kreds viser Magten sig fra utallige
...
Realbegreberne og \apage{220} det Antilogiske i Tingene punktlig maae svare til
...
% `udvortes': the scan shows `udvortcs'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
af udvortes, tilfældige Betingelser; \apage{221} man opløse Atomerne i Kræfter,
...
\apage{222} Disse Grundbestemmelser af Viden og Magt i oprindelig Selveenhed ere dog
...
i Andet ere paa een Gang \emph{Nødvendighedssætninger} og \emph{Frihedssætninger}. At de
...
gjælder med Hensyn til den objectiverende \apage{223} Magt. Viden af Andet er altsaa
...
ikke \apage{224} sige, at kun Aanden er absolut nødvendig fordi den er det Oprindelige,
...
$B_n$, der ifølge de uendelig mange forud gaaende Betingelser skulde være
...
\apage{225} Den absolute Nødvendighed kan lige saa lidt være uden Betingelser som den
...
\emph{At det absolute Nødvendighedsforhold} tillige er et \emph{Frihedsforhold}, følger
...
blev ganske \apage{226} tilfældig. I en saadan Tilfældighed er al Natur og Naturgrund
forsvunden.
...
Tilfældighed; en Verden maatte der frembringes, og saa \apage{227} blev det
...
% AS PRINTED: `ἀναγκη', smooth breathing only, no accent (collation 2026-09-24, two readers).
til Rang med »absolut Nødvendighed«, ἀναγκη. Der bliver altsaa kun tilbage
...
kan være. For saa vidt er da \apage{228} ogsaa Selvviden Naturviden, Selvbegriben
...
En Sætning som denne: \emph{Naturen har sin Oprindelse fra Aanden, fordi Aanden
...
\apage{229} Forskjel, ja en Modsætning, de ville holde skjult i den umiddelbare Eenhed,
...
Lommecompasset og de smaa Ænder, har en Magnet skjult i Ærmet. Ordet Natur
...
Muligheder maa i sin første umiddelbare Form være Grund for Aandens \emph{Natur},
...
Naturen er objectiv, fordi den har objectiveret \emph{sig selv}, thi Selvheden i
...
\apage{230} \section{Menneske-Aandens Selvudvikling: Historie}
\label{sec:22}
% pp. 230--243
...
Hjemmelsmænd, en historisk Tro (fides historica). Kritiken har
...
\apage{231} Sandsynligheden har mange Grader, og de historiske Sandsynlighedsbeviser
...
\apage{232} Det Videnskabelige i den historiske Viden vil da især være kjendeligt paa
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «les detailles sont l'âme de l'histoire»
Enkeltheder: les detailles sont l'âme de l'histoire. Den lærde
...
Hovedrollen. Seet fra Existenssiden er det virkelig Individerne, \apage{233} der handle;
...
Guldalderen, en sikker  \apage{234} Maalestok for den Aandens Høide, hvorpaa den staaer,
...
Dyriske kunne drives meget vidt, \apage{235} naar man blot vil tage til Takke med det
...
Denne, snart Hiin, men alle Skikkelser fra den første  \apage{236} Begyndelse ere
...
misbruges paa det Skammeligste, maa \apage{237} han dog frygte dens Magt, thi den kan
...
bære paa noget Væsenligt: Misbrug af den tvetydige Mængdevillie, \apage{238} som under
...
Selvstændighed, er Folketroen i Fare.  \apage{239} Derimod er Folketroen villig til at
...
naturligt opfatter Enkeltvillien som sin Villie, og trods al Egoisme maa \apage{240} 
...
og bearbeidet i Middelalderen, betegner \apage{241} det andet Stadium. Nutidens
...
væsenlig kommer an, er en sand Vurdering af den Arv, Fortiden  \apage{242} overlader
...
% `kritisk-berygtede': printed `kritisk-|berygtede' across a line end; compound hyphen assumed (collation 2026-09-24).
...
Menneske-Aanden; det er umiskjendeligt et stort Fremskridt, at \apage{243} det
...
føle disse Tryk, \apage{244} disse Savn og Lidelser? Dette Spørgsmaal maa, ifølge vort
...
velforstaaet \apage{245} Samstemning af Physiologi og Psychologi tilstrækkelig viser, at
...
Indbildningskraften optager det tilfældig Givne \apage{246} og omdanner det tilfældig;
...
Frembringelser svare til hinanden. Den  \apage{247} frugtbare, af Aanden bevægede Phantasi
...
Forhold til lokale Omgivelser og Folkestammernes Eiendommelighed, har \apage{248} afrundet
...
aabne de Bevidstheden en viid Synskreds, aabne den for et høiere Lys, og vise
...
overmenneskelige Skikkelser blive \apage{249} efterhaanden roligere, idet de derhos blive
...
at \apage{250} Phidias's Zeus maatte virke befriende, at Synet af Guders og Menneskers
...
dog  \apage{251} guddommelig, saa sandselig skjøn og dog saa ophøiet; man maa være Humanist
og Katholik, for tilfulde at fatte det Salige i denne Aabenbarelse. Vi kunne
neppe betegne en Leo Xdes Henrykkelse mere træffende end ved at minde om de
Ord, hvormed Goethe slutter sin Faust: »Das Unbeschreibliche, hier
...
søges i Æsthetiken; men det for Videnskabslæren afgjørende Spørgsmaal er
dette: i \emph{hvilken Betydning kan Æsthetiken egenlig siges at være en Videnskab}?
...
ikke kan der indvendes Noget imod, at Æsthetiken underkaster de forskjellige
...
gjør ikke blot i Almindelighed, men \apage{252} endog i de fineste Overgange, Enkeltheder
...
Tidsalderen. Men Æsthetiken kan ikke nøies med kunst- og litteraturhistoriske
...
til fri, \apage{253} selvstændig Frembringelse? Sculptur og Maleri synes at tale for den
...
Naturgjenstande som et Landskab eller en Blomst? Kunstneren skal --- hedder det --- holde sig strengt til den givne Natur, altsaa til Virkeligheden; men ---
...
deraf, at det jo  \apage{254} ingenlunde er en Fremstilling af den rene blanke
...
paa en almindelig Skjønhedssands. Vi have da paa den ene Side \apage{255} Kunstneren, der
...
fyldestgjort, og at andet Beviis ikke kunde forlanges. Æsthetikens
...
Æsthetikeren tillagde Kunstneren og hans Publicum Følelser, som de ikke vilde
...
der dadler Kunstværket; Æsthetikeren med sin Kritik skal da mægle imellem dem.
...
videnskabelig, \apage{256} som om man sammenkaldte en Generalforsamling, fremsatte sin
...
gjælder det bekjendte Ord: chacun son gout, og Discussionen mellem de
mange Uenige ender med det negative Resultat: de gustibus non
...
Æsthetiken skal hævdes som Videnskab, er at gaae ud fra Naturindtryk og
...
Form \apage{257} saa fuldkomment som muligt realiseret i sit Exemplar. En saadan
...
Betingelserne have tilladt det. I Æsthetiken blive Realbegreberne derimod ikke
...
Synspunct maa Australnegerens Realbegreb siges at være realiseret med samme
Nødvendighed i Australnegeren, som Europæerens Realbegreb i Europæeren; her er
ingen anden Maalestok for det Fuldkomne end Nødvendigheden. Naar Æsthetiken
...
skal gjælde for den eneste sande videnskabelige, saa er Æsthetiken ingen
...
Subjectivitet, der er i Høide med den hele naturbetingede \apage{258} Aandsudvikling i
...
\apage{259} At det villende Subject hverken kan være et enkelt Stofsubject ei heller en
...
uvilkaarlige Yttringer --- som naar et Foster i Modersliv bevæger sine Lemmer --- er der hverken Villie eller Frihed. I Valgfriheden og den tilsvarende
...
mulige; Villien har nemlig ikke blot Valg mellem \apage{260} A og B, men den har tillige
...
Grunde \apage{261} fra objectiv Side vurdere Villiesobjecterne, og fra subjectiv Side
...
% `Villiens': the scan shows `Villlens'; sense-reading (JBIG2 rule; collation 2026-09-24, two readers).
Phænomener i Frihedslivet, som ellers vilde være uforstaaelige: Villiens
Afhængighed \apage{262} og Uafhængighed saa vel af Indtryk fra Omgivelsen som af
...
Videnskaben nok vil tage ad notam, saa er det dog en egen Sag med den
negative Kritik. Det kan umulig være Kritikens Alvor, at den Oplyste, der
emanciperer sig fra Religionen, ogsaa skulde emancipere sig fra Ethiken,  \apage{263}
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «Retfærdige,» eller, hvad Positivismen»
...
en Personlighedens Idee, det sees af \emph{Forpligtelsen}. Lad Ideen for sig \apage{264} være
...
Ideeforsvarernes Side. Men, hvor  \apage{265} selvmodsigende et saadant Forsvar trods sin
...
vel som Uretten; hver strider mod  \apage{266} Uretten for hvad han kalder sin Ret: hvad
...
den store Mængde, der nærmest er henviist til at leve umiddelbart af sine
...
til det Heles Vel udføre dem saaledes, at de tjene baade de Enkeltes \apage{267} og
...
% AS PRINTED (collation 2026-09-24, second reader agrees): «der fremmer og sikker det Heles Vel»
hvorpaa Aandens Udvikling, der fremmer og sikker det Heles Vel, væsenlig
beroer.
...
Forstand erkjendes for \apage{268} Aandsstridigheder, da ville de Love, som styre disse
...
en fælleds Enhed, et høiere Maal for Aandens og Individernes Frihed, en Frihed,
...
der bestemmer Udviklingen af Individernes \apage{269} Aand og Aanden i Samfundet, maa
...
subjective Fordringer, hvor under Aandslivet stridende maa bevæge sig fremad.
...
\noindent \apage{270} At den absolute Aand umulig kan give sig concret Form eller endelig
...
\apage{271} Opstaaer da Spørgsmaal om Forholdet mellem det Blivende og det Forsvindende i
...
forgængelige Individers Aand, om den end beriget kan vandre fra \apage{272} Slægt til
...
% AS PRINTED: `tilsidt' (collation 2026-09-24, two readers).
med Tiden meer og meer, og begraves tilsidt i Glemsel.
...
naturligviis ikke være Tale om at afbryde de videnskabelige Forhandlinger paa
...
unddrage visse Ting fra  \apage{273} Videnskabens Undersøgelse, er et Beviis paa
...
Overnaturlige ligger, kan den vistnok ikke lige frem paavise; thi det Naturlige
...
mellem Jorden, Maanen og Solen, et Forhold, som \apage{274} Videnskaben maa kunne bedømme,
...
\apage{275} Videns Natur. At ville holde paa en Religion, der har antaget en videnskabelig
...
\apage{276} trænge sig frem i Forgrunden. Philologi, Historie og Philosophi sloge stedse
...
lede det bort fra virkelig Oplevelse og  \apage{277} virkelig Udvikling af Samfundets
...
Løsning. Enhver Bedømmelse af Løsningens relative Værd overlades naturligviis
...
Erkjendelseslære, sine egne af Videnskaben uafhængige Forudsætninger. Dog \apage{278} er
...
den eiendommelige Opgave at klare Brydningsforholdet mellem de  \apage{279} ueensartede
Tanker er Religionsphilosophien nærmest at bestemme som en \emph{Grændsevidenskab}.
...
og \apage{280} Videnskaben væsenlig vendt om, og Religionsphilosophien bliver med Hensyn
hertil at charakterisere som »en \emph{omvendt Videnskab}.«
...
Vilkaar Betydning som en paa  \apage{281} Religionen sig offrende, \emph{sig selv i Troeslære
ophævende Videnskab}«. (Religionsphilosophi S.\ 8-11).
\end{quote}
...
% Rettelser leaf (PDF 296), as printed: one entry per line, ditto marks `-' for `S.', `L.' and
% `fra oven', no italics (collation 2026-09-24; the first entry's `§ 11' had been read as `§ 2').
{\footnotesize
\begin{center}\textbf{Rettelser:}\end{center}
\noindent
S.\ \ 98 L.\ \ 5 fra oven §\ 1 --- læs: §\ 11.\\
- 107 - 9 - - Mediationer naae --- læs: Mediationer aldrig naae.\\
- 157 - 3 - - Axler --- læs: Axer.\\
- 160 - 11 - - Vare --- læs: Varer.\\
- 196 - 14 - - Lyélls --- læs: Lyells.\par}
