...
%  anywhere.  The heading of the unnumbered Trykfeil leaf is a SLAB-SERIF
%  (Egyptienne): heavy square unbracketed serifs, low stroke contrast,
%  stem/x-height 0.191 on a 47 px x-height at 600 dpi.  It is a third
%  display fount, and it is NOT sans-serif -- an earlier text of this
%  section said sans, which the image at 1200 dpi refutes.  \textbf{} is
%  used for it here for want of a slab in Libertinus.  The title page was
%  not re-examined in the final pass.
%    THREE FOUNTS carry the text: the ROMAN, the BOLD, and the cursive
%  Greek.  Roman and bold are told apart by STEM WIDTH RELATIVE TO
%  X-HEIGHT, never by size or prominence: the roman is 0.15 (a 6 px stem on
%  a 40 px x-height at 600 dpi) at every size it is cut in, and the bold is
%  0.27--0.30, twice that, at every size and at every threshold.  The
%  method, the sweep that validates it, and the head-by-head results are
%  in §3.
%    BOLD IS USED FOR DISPLAY HEADS at two sizes -- §3's L2 (one size up
%  from the body) and L3b/L5 and the run-in heads such as p. 97
%  "a) Opfattelsen af Vorden." (body size).  It is NOT used for the large
%  display heads of L0/L1, which are the ROMAN at a display size, and not
%  for L3a or L4.  Two earlier readings of this are corrected in §3.
%
%  BOLD AS A SECOND IN-PARAGRAPH EMPHASIS.  Letterspacing is the book's
%  ordinary in-paragraph emphatic device (below), but it is not the only
%  one.  Bold occurs inside paragraphs at thirteen places, and the final
%  pass adjudicated them together, over the whole book:
%
%    * the single-letter word `i' (= "in"), set from the bold fount, at
%      pp. 42, 89, 95 (twice), 100, 119, 147 (twice), 190, 258 and 272 --
%      ELEVEN occurrences;
%    * "er" and "er vordet" at p. 101 ll. 5--6, four bold sorts standing
%      inside an otherwise letterspaced passage;
%    * the figures "362." in the p. 76 note *), against a roman "259." on
%      the same line.
%
%  THE READING ADOPTED IS EMPHASIS, NOT A FOUL CASE, and all thirteen are
%  transcribed \textbf{}.  Four earlier batch reports had called the `i's a
%  wrong sort (a stray heavy sort in the compositor's i-box); that reading
%  is withdrawn here.  The grounds, in the order that matters:
%
%   1. CALIBRATION.  Lower-case `i' was found mechanically (a narrow
%      x-height body with its dot above it) on 29 pages carrying none of
%      the sites, and its stem measured at 900 dpi: 18,719 glyphs, median
%      9 px, 99th percentile 12 px, 99.9th 13 px.  NOT ONE ordinary Latin
%      `i' anywhere in that set exceeds 13 px.  (Every higher value the
%      sweep returned was checked by eye and is a capital S, G, Ø or A, or
%      a Greek letter, admitted by the detector and not an `i' at all.)
%      The eleven sites measure 16, 17, 18, 18, 18, 19, 19, 19, 19, 20 and
%      20 px -- 1.8x to 2.2x the median and above every ordinary `i' in the
%      calibration set.  This is a positive signal, not an impression.
%   2. COMPLETENESS.  The same detector was then run over ALL 288 body
%      pages at 600 dpi.  It returns the twenty-odd bold display heads, the
%      p. 101 bold words, and the eleven `i's, and nothing else: every other
%      page maximum is at the ordinary-roman ceiling, and the handful of
%      pages above it are Greek or a question mark, checked by eye.  So the
%      eleven are believed to be all of them.  (The sweep looks for heavy
%      `i'-shaped sorts only; the p. 76 figures were found by eye.)
%   3. THE FOUL-CASE HYPOTHESIS FAILS A COUNT.  A stray heavy sort in the
%      i-box would fall in random `i' positions.  Of the 32,139 letters `i'
%      in the body text, 2,825 -- 8.8 % -- are the standalone preposition
%      "i".  All eleven heavy ones are that word.  Under the foul-case
%      hypothesis that has probability 0.088^11, about 2 x 10^-12.
%   4. THE SENSE FITS.  Every one of the eleven stands where "in" carries
%      the argument -- immanence against transcendence: "virker i Stoffet",
%      "men i det Sandselige", "Vorden, i det Stofagtige", "en Væren i
%      Verden", "som værende i Tingene", "ind i Stoffet", "i denne
%      Realisationsproces", "idealiseres i sin reale Virkelighed".  At
%      p. 147 l. 4 the phrase is "dette som Dele i, ikke af", and the word
%      contrasted with the bold `i', namely "af", IS LETTERSPACED: the two
%      halves of one antithesis carry the book's two emphatic devices.
%   5. A ONE-LETTER WORD CANNOT BE LETTERSPACED -- there is no internal gap
%      to widen -- so for the word "i" bold is the compositor's only means.
%      That is exactly why the device appears on this word and no other.
%   6. p. 101 IS DECISIVE INDEPENDENTLY.  Four consecutive bold sorts, two
%      whole words, cannot be a foul case; and they stand inside
%      letterspaced matter, where further letterspacing would have been
%      invisible.  Bold is therefore in the compositor's hands as an
%      emphatic device in running matter.  So is it in the Register, which
%      sets the PRINCIPAL page reference of an entry in bold throughout
%      (p. 289 "Aristoteles . . . 94 ff." bold against "Aristippos . . .
%      193 ff." roman two lines above) -- which is also the best reading of
%      the p. 76 note figures.
%
%  What survives of the foul-case reading is nothing; it was adopted by the
%  early batches on the ground that "bold does not otherwise occur inside a
%  paragraph in this book", and that ground is false.  A reader who wants to
%  check any of this has the printed page and the line at each site, and a
%  % comment there carrying that site's own measurement.
%
...
%  ---------------------------------------------------------------
%  REWRITTEN IN THE FINAL PASS, over the whole book at once.  It corrects
%  the recon pass, two batch reports, and the earlier text of this section.
%  Every claim below is a measurement; the method is stated first because
%  this section has now been wrong twice for want of it.
%
%  HOW A HEAD WAS CLASSIFIED.  Bold and letterspacing are INDEPENDENT
%  properties, and a head may be either, both or neither.  Neither can be
%  read off size, prominence or the way a head looks at low magnification,
%  and letter gaps measure only the second of them.  Two measurements were
%  made for every head, at 600 dpi, against the body of the SAME page in
%  the same render at the same threshold:
%
%    WEIGHT -- stem width divided by x-height, then divided by the same
%    quantity for the body roman of that page.  Call it r:
%        the ROMAN, at whatever size      r = 0.92 -- 1.19   (12 sites)
%        the BOLD                         r = 1.32 -- 2.02   (19 sites)
%    Nothing in the book lies between 1.19 and 1.32.  In absolute terms at
%    600 dpi: the body roman is a 6 px stem on a 40 px x-height (0.150);
%    the large display heads are 11--13 px on 69--79 px (0.158--0.174); the
%    bold is 10--13 px on 37--46 px (0.26--0.30), i.e. TWICE the roman.
%    Checked under a threshold sweep from 35 % to 65 % of the ink depth:
%    the bold's ratio is stable throughout (0.256--0.295), while the large
%    display heads converge on the body's own value at 35 % (0.169--0.181
%    against a body 0.171) -- so the 0.17-against-0.15 residue at a 45 %
%    threshold is a thin-stroke artefact of thresholding, not weight.
%    Confirmed by eye at 900 dpi with the founts scaled to a common
%    x-height, where the difference is not subtle.
%
%    LETTERSPACING -- median inter-letter gap within a word: 4--8 px
%    unspaced, 13--16 px spaced at body size, 36--51 px spaced at display
%    size.  The distributions do not overlap.
%
%    THE TWO ERRORS THIS CORRECTS.  (a) The recon pass called the L3a heads
%    "bold centred" because they are prominent; four batches refuted that by
%    measuring letter gaps, which gave the right answer for the wrong
%    reason -- gaps show letterspacing, not weight.  (b) The pp. 281--288
%    batch then measured stem against x-height, correctly found that
%    pp. 1, 31, 216 and 284 are ONE fount at ONE size, and inferred that
%    that fount is the bold, because 0.18 is above the body's 0.14.  It had
%    no BODY-SIZE bold to calibrate against.  With one, the inference
%    fails: the book's bold is 0.27--0.30, and the display heads are the
%    ROMAN at a display size.
%
%  THE HIERARCHY, with the printed page of each head.  The level names are
%  unchanged, so that the % comments at the sites stay valid.
%
%    L0  DISPLAY ROMAN, large (x-height 69--79 px at 600 dpi against a body
%        40), centred, alone on its line.  NOT BOLD (r = 0.92--1.19).
%          letterspaced (gaps 36--51 px):
%              Indledning. (1)
%              Slutning. (284)
%              Register. (289)
%          unspaced (gaps 4--9 px):
%              Hovedepocherne i Philosophiens Historie. (16--17, two lines)
%              Charakteristik af Philosophiens historiske Hovedformer.
%                  (216, two lines)
%        Encoded {\Large ...} inside \begin{center}, with \emph{} where the
%        head is spaced and nothing where it is not.  NO \textbf{}: the
%        \textbf{} that stood at pp. 216, 284 and 289 has been removed.
%    L1  Den græske Philosophies Udvikling. (31)
%        THE SAME FOUNT AND SIZE AS L0 (x-height 71--72, r = 1.07--1.13),
%        and unspaced.  It keeps a level of its own only because it opens
%        the first main division of the book; typographically it belongs
%        with the unspaced L0 heads.  This section formerly called it
%        "large BOLD centred"; it is not bold, and its \textbf{} is gone.
%    L2  BOLD, ONE SIZE UP from the body (x-height 43--46 against a body
%        39--41 on the same page, r = 1.32--1.86), unspaced (gaps 4--7 px),
%        set to the FULL MEASURE with the last line centred.  (p. 137's
%        head fills its page's top and leaves too little body text on the
%        page for a clean baseline, so its r is not quoted here; its weight
%        is the same 12 px stem as the other three theses.)  The heads:
%          the four theses  I. (50)  II. (79)  III. (137)  IV. (170)
%          Første Hovedepoche: (219)   Anden Hovedepoche: *) (226)
%          Tredie Hovedepoche: (269)
%        The last line is measurably centred at pp. 50, 79 and 170; at
%        p. 137 it fills the measure, so the print cannot there distinguish
%        a centred block from a justified one, and the other three settle
%        it by analogy.  All seven are encoded
%        \begin{center}\textbf{\large ...}\end{center}; the \noindent that
%        pp. 137 and 170 carried, and the missing \large at pp. 50, 226 and
%        269, are gone.  The roman numeral is part of the bold and stays
%        inside the \textbf{}.
%    L3a LETTERSPACED ROMAN at body size (r = 0.9--1.0, gaps 15--16 px
%        against a body 5--7), set as a justified PARAGRAPH on the full
%        measure, first line indented by the ordinary paragraph indent and
%        last line flush LEFT.  Not bold and not centred:
%          1./2./3. under thesis I:    50, 54, 65
%          1./2./3. under thesis II:   80, 94, 127
%          1./2./3. under thesis III:  137, 145, 150
%        ONE EXCEPTION, printed and kept: the p. 80 head is CENTRED -- its
%        first line is not indented (x 552 against a body left margin of
%        544) and its short last line is centred (centre 1654 against a
%        body centre 1652).  It alone of the nine is wrapped
%        \begin{center}; the difference is the print's, not the edition's.
%    L3b BOLD at BODY size (r = 1.81--2.02), unspaced, CENTRED and short:
%          1. Den græske Atomlære. (186)
%          2. Den Aristippiske og Epikureiske Lystlære. (192)
%          3. Sokrates. (201)  -- ALSO LETTERSPACED (gaps 15 px), the only
%             head in the book that is both, and encoded \textbf{\emph{}}
...
%        L3a and L3b are two typographically distinct settings that this
%        section formerly listed together as one level, "L3"; three batch
%        reports flagged the discrepancy and it is discharged here.  The
%        outline sequence is not affected.
%    L4  ROMAN, one size up from the body (x-height 43--44 against 39--41),
%        centred, alone on its line.  NOT BOLD (r = 1.12--1.16) and NOT
%        LETTERSPACED (gaps 4--4.5 px against a body 6):
%          α. Den dogmatiske Idealisme. (240)
%          β. Den kritiske Idealisme. (250)
%        This section formerly called L4 letterspaced.  It is not.
%        α. and β. ARE Greek letters used as outline levels, confirmed on
%        the image at pp. 240 and 250, and they sit BELOW a./b. in the
%        hierarchy.  On the page they are set in the book's cursive Greek
%        fount; they are typed here as plain upright α and β OUTSIDE
%        \gk{}, because they are enumerators and not running Greek, and the
%        discrepancy is recorded at the site.
%    L5  BOLD at body size, centred -- typographically identical to L3b;
%        the distinction is one of outline, not of setting:
%          I.  Den kritiske Idealisme i snævrere Betydning. (252)
%          II. Den speculative Idealisme. (259)
%        The I./II. here is a DIFFERENT sequence from the I.--IV. at
%        pp. 50/79/137/170.  Neither is renumbered or harmonised.
%    RUN-IN HEADS, not previously listed in this section.  Enumerator in
%    ordinary roman, the words in BOLD at body size, unspaced, run into the
%    opening line of the paragraph and followed by an em-dash:
%          a) Opfattelsen af Vorden. (97)
%          b) Enhed og Dualisme. (117)
%          c) Det Enkelte og Erfaringen. (120)
%
%  THE ENUMERATOR IS NOT ALWAYS OF THE SAME FOUNT AS ITS HEAD, and the
%  difference is printed.  Median stroke width of the enumerator against
%  that of the head words, at 900 dpi, confirmed by eye in every case:
%        p. 186  "1."    13 : 11   BOLD
%        p. 192  "2."     6 : 11   roman
%        p. 201  "3."     8 : 11   roman
%        p. 231  "A."     8 : 12   roman
...
%        p. 252  "I."    12 : 11   BOLD
%        p. 259  "II."   14 : 11   BOLD
%        pp. 29, 30 "1." "2." "3."   8--9 : 11--12   roman
%        p. 180  "A." "B."           BOLD
%        pp. 50, 79, 137, 170  "I."--"IV."  15--18 : 12--13   BOLD
%        pp. 97, 117, 120  "a)" "b)" "c)"   roman
%  A roman enumerator is written OUTSIDE the \textbf{}, a bold one inside.
%  That p. 186's numeral is bold while p. 192's and p. 201's are not, in
%  one and the same sequence, is the compositor's inconsistency and is kept.
%
%  ENCODING CONVENTIONS SETTLED IN THE FINAL PASS AND APPLIED THROUGHOUT:
%    * bold -> \textbf{}; letterspaced -> \emph{}; both -> \textbf{\emph{}},
%      in that order (p. 201 is the only instance in the book).
%    * L0 and L1 take {\Large ...} and NO \textbf{}.  L2 takes
%      \textbf{\large ...}.  L3b, L5 and the run-in heads take \textbf{} at
%      body size.  L3a takes \emph{} at body size.  L4 takes neither.
%    * A head the print centres is wrapped \begin{center}; a head the print
%      sets as a paragraph is not.  Where the print really differs -- p. 80
%      against the other eight L3a heads -- the difference is KEPT and
%      noted at the site.  Consistency of encoding is the aim, not
%      flattening of the book.
%    * The space between an enumerator and its head measures 87--94 px at
%      900 dpi against an ordinary word space of 45--48 px on the same
%      line, i.e. two word spaces.  It is \enspace throughout, replacing
%      the earlier mixture of \quad, \hspace{1.5em} and a plain space.
%    * A \footnote{} inside a bold head must open \normalfont\normalsize,
%      or the whole note is set bold and large (p. 226).
%
%  DIVISIONAL RULES stand above almost every display head, but the pattern
%  is not uniform: the rule belonging to the heads at pp. 16, 80, 170 and
%  240 stands at the FOOT of the PRECEDING page (15, 79, 169, 239);
%  at pp. 31, 216, 284 and 289 it stands BELOW the head;
%  the p. 237 head has none at all; and two of them (at the foot of p. 215
%  and at p. 288) are DOUBLE hairlines where the rest are single.  Each is
%  transcribed where it is printed, as \divrule, with the measurement and
%  any difference of sort recorded in a % comment at the site.
%
...
%  chosen for it, is recorded in a % comment at the site.  In summary:
%  pp. 29--30 are one `center' with the two columns as a two-column
%  `tabular' of half-measure \makebox{} cells; p. 163 is a two-column
%  `tabular' whose crossbar is \rule{0.5\textwidth}{0.4pt} spanning both
%  columns, so that it terminates over each branch's centre as printed;
%  p. 180 is two `tabular's, labels flush right and matter flush left,
%  with the braces set as amsmath \left\lbrace ... \right. around an `array'
%  \text{} rows (hence amsmath in the preamble).  A third kind of rule,
%  the schemata's short thin VERTICAL CONNECTOR, is defined in the
%  preamble as \connrule; it occurs only on pp. 163 and 180.
%    TWO ORNAMENTS ARE NOT REPRODUCED, and this is deliberate: nothing is
%  invented to stand in for them, and the difference is recorded at each
%  site instead.  (1) The wide SWASH ORNAMENT on p. 29 -- an engraved wave
%  with two swelling lobes, tapering to hairlines at both ends and meeting
%  in a soft upward cusp under head 2 -- is set as an \overbrace of its
%  measured width, which has the right topology and the wrong sort.
%  (2) The SWELLED (spindle-shaped) rule that closes p. 180 -- hairline at
%  both ends, thickening to about three times that weight at the middle --
%  is set as the ordinary even \divrule of its measured length.  Whether a
%  second rule macro is wanted depends on whether the swelled sort turns
%  up again later in the book; it has not been seen elsewhere so far.
%    One correction to the capturing batches was made in the schema pass:
%  the pp. 29--30 schema is NOT set to a measure wider than the body.
%  Measured on the image, its widest line and the body text above it share
%  both edges (p. 29: x 112--1233 against x 112--1235; p. 30: x 240--1351
%  within a measure of x 240--1355).  No word was changed, only the measure.
%
...
\usepackage{enumitem}
\usepackage{amsmath}     % \left\lbrace and \text{} for the p. 180 braces
\usepackage{fancyhdr}
...
% The CONNECTOR RULE of the displayed schemata: a short thin VERTICAL hairline
% standing in the white space between two successive items of a schema.  It is
% a third kind of rule, distinct from both the divisional rule and the footnote
% separator, and it occurs only on pp. 163 and 180 -- four of them in the
% genealogical tree on p. 163 (measured 4.1--4.2 mm, x-width 1--2 px at 300 dpi)
% and eleven on p. 180 (0.19--0.38 cm tall, 4--6 px wide at 600 dpi).  Set here
% at the p. 163 measurement, dropped half an ex so that it sits in the
% interline space rather than on the baseline.
\newcommand{\connrule}{\rule[-0.9ex]{0.4pt}{4.2mm}}
...
% --- p. 1 ---
\opage{1} % HEAD, p. 1 (L0): "Indledning." --- large LETTERSPACED roman, centred, alone on
...
% --- p. 2 ---
\opage{2} Idet dette Skrifts Opgave er saaledes bestemt, forudsætter
...
% --- p. 3 ---
\opage{3} særegne Bestemthed, vel kunde være Nødvendigheden og Lov\-%
...
% --- p. 4 ---
\opage{4} % EMPHASIS, p. 4, l. 1: the compositor letterspaces only the FIRST element of
...
% --- p. 5 ---
\opage{5} Individ, at Lovene for dets Virksomhed som \emph{menneskeligt},
...
% --- p. 6 ---
\opage{6} Individets Forhold, som et Forhold mellem det Almene og det
...
% --- p. 7 ---
\opage{7} Denne bliver nemlig først at see \emph{som underkastet den
...
% --- p. 8 ---
\opage{8} teleologisk virkende Princip og Princip for den \emph{aandelige}
...
% --- p. 9 ---
\opage{9} derigjennem, at det, der virker som Skjæbne, det de indivi\-%
...
% --- p. 10 ---
\opage{10} % p. 10, ll. 2--8: ABBYY's layer indents these lines, but on the image they run
...
% --- p. 11 ---
\opage{11} sen betingede og bestemte \emph{Tidsudfoldelse} under den \emph{reale}
...
% --- p. 12 ---
\opage{12} gjøre sig gjældende \emph{under psychologiske Udviklings\-%
...
% --- p. 13 ---
\opage{13} Omrids at kunne fastholde Udviklingens Hovedformer, for lettere
...
% --- p. 14 ---
\opage{14} den menneskelige Bevidstheds (Tankes) Udvikling, viser sig
...
% --- p. 15 ---
\opage{15} % p. 15 opens mid-sentence: it continues the paragraph carried over from
...
% --- p. 16 ---
\opage{16} % p. 16--17: L0 display head (transcription.tex §3), large roman, centred,
...
% --- p. 17 ---
\opage{17} nødvendig Væsensudfoldelse er baaret og omfattet af hvad der
...
% --- p. 19 ---
\opage{19} % (the note begun on p. 18 occupies the lower two thirds of this page; it is
...
% --- p. 20 ---
\opage{20} % GREEK, p. 20 (twice), read on the image at 1200 dpi -- neither OCR witness
...
% --- p. 21 ---
\opage{21} hævede Forhold i Videns Idee, ved at lade Andetheden glimte
...
% --- p. 22 ---
\opage{22} og religiøse Opgave og om Salighedstilstanden. Idet vi søge
...
% --- p. 23 ---
\opage{23} nødtvungen factisk Anerkjendelse og Optagelse af
...
% --- p. 24 ---
\opage{24} det Hiinsidige og dens Flugt fra Naturen og Virkeligheden; i
...
% --- p. 25 ---
\opage{25} Telesio og Bruno); --- den samme Grundopfattelse af \emph{Tilvæ\-%
...
% --- p. 26 ---
\opage{26} % PENCIL, p. 26: a "+|" and a cursive word in the left margin beside
...
% --- p. 27 ---
\opage{27} forvandles til Tanke-Væren, der lader Naturen og det Mate\-%
...
% --- p. 28 ---
\opage{28} Den \emph{tredie} philosophiske Hovedepoche, til hvilken der
...
% --- p. 29 ---
\opage{29} % SEAM: p. 29 line 1 is set FLUSH LEFT (measured against the page's own left
...
% =====================================================================
% DISPLAYED SCHEMA, pp. 29--30 -- the three Hovedepocher.  The words were read
% off the image and verified by the pp. 29--42 batch; what follows records WHAT
% IS PRINTED and WHAT CONSTRUCTION WAS CHOSEN for it in the dedicated schema
% pass, which re-measured the arrangement on the page image at 300 dpi.
%
% WHAT IS PRINTED.  Measurements are in pixels of the 300 ppi scan (1 px =
% 0.085 mm); x is measured from the left paper edge.  The BODY MEASURE on p. 29
% runs x 112--1235, centre 673.
%
%   line 1  BOLD, centred, about two ems between the figure and the words:
%             "1.   Den græske Philosophie."                     x 434-- 913
%   line 2  LETTERSPACED roman, centred; the Greek word is in the book's
%           italic Greek fount and is NOT letterspaced:
%             "Ideen aabenbaret i Verdenskunstværket (Κόσμος)."  x 176--1168
%   line 3  LETTERSPACED roman, centred, in parentheses:
%             "(Umiddelbar Enhed af Natur og Aand)."             x 292--1047
%   line 4  BOLD, centred, same two-em gap:
%             "2.   Den nyere Tids Philosophie."                 x 400-- 942
%   line 5  A WIDE SWASH ORNAMENT (a swelled "wave" rule), x 238--1044, i.e.
%           806 px = 6.8 cm = 72 per cent of the measure, its centre (x 641)
%           standing a little left of the measure's (x 673).  It is
%           hairline-thin at both ends and swells into two heavy lobes, one
%           under each of the two columns below, which meet at a small UPWARD
%           CUSP at the centre, under the middle of head 2.  It therefore
%           works as a wide two-column brace whose apex points up to the head.
%           It is NOT a divisional rule and NOT a footnote rule.
%   lines 6--7  TWO COLUMNS of two lines each, side by side.  The first line
%           of the pair FILLS THE WHOLE MEASURE (x 112--1233); the second runs
%           x 206--1232.  ONLY THE FIRST WORD OF EACH COLUMN IS LETTERSPACED
%           ("Natur", "Aand"); everything else in the two columns is ordinary
%           roman -- checked at 600 dpi, "Materie", "Ikke-Aand.", "(Jeg ---
%           absolut Aand)" and "Ikke-Natur = det Ikke-Materielle." are all set
%           close.  The "=" is the TEXT sort (a double stroke), not math; the
%           dash in the right column is an em-dash.  Each column's second line
%           is centred under its first.
%
...
%           "Aand (Jeg --- absolut Aand) = Ikke-Natur = det Ikke-Materielle."
%   line 8  LETTERSPACED roman, centred under BOTH columns:
%             "(abstract Modsætning af Natur og Aand)."          x 244--1083
%
% The schema runs on across the page break: head 3 and its matter stand at the
% top of p. 30 and are set below, inside the same `center' environment, after
% the p. 30 page marker.
%
% ONE READING OF THE CAPTURING BATCH IS CORRECTED HERE.  It reported that the
% displayed block is set to a measure WIDER than the body, its left edge about
% 1.5 cm outside the body's left margin.  IT IS NOT.  On p. 29 the schema's
% widest line (x 112--1233) and the body text immediately above it (x 112--1235)
% share both edges, and every schema line is centred on the body's own axis
% (x 673); on p. 30 the measure is x 240--1355, centre 797, and the schema's
% longest line runs x 240--1351 with the divisional rule centred at x 797.
% The schema is set to the ORDINARY BODY MEASURE, and is set so below.  No word
% of the transcription is changed by this; only the measure.
%
% CONSTRUCTION CHOSEN.  One `center' environment carries the whole schema
% across the page break.  FINAL PASS: the three numerals "1." "2." "3." of
% this schema are ORDINARY ROMAN (median stroke 8--9 px at 900 dpi against
% 11--12 px for the head words, and plainly lighter by eye), so they stand
% OUTSIDE the \textbf{}; the p. 180 schema's "A." and "B." are BOLD and stay
% inside theirs.  The space after a numeral is \enspace throughout the book
% (measured: 87--94 px at 900 dpi against an ordinary word space of 45--48
% on the same line, i.e. two word spaces).  Formerly the heads were
% \textbf{} with \hspace{1.5em} after the
% figure (the printed gap is about two ems).  The letterspaced matter is
% \emph{}, this edition's rendering of Sperrsatz throughout.  The two columns
% are a two-column `tabular' of \makebox[0.5\textwidth]{} cells, so that the
% pair fills the measure as printed and each column's second line is centred
% under its first.  The "=" signs are text, as everywhere in this edition.
%
% THE SWASH ORNAMENT HAS NO LaTeX EQUIVALENT AND IS NOT REPRODUCED.  It is set
% here as $\overbrace{\hspace{0.72\linewidth}}$ -- the nearest honest
% construction, because it has the ornament's topology (two arms spanning the
% two columns below, meeting in a central cusp that points up at the head
% above) and its measured width.  HOW THE PRINTED SORT DIFFERS FROM WHAT IS
% SET: the print is an engraved swash, not a brace.  Its terminals taper away
% to a hairline instead of turning down towards the text; each arm swells to a
% rounded lobe about a third of the way in and thins again towards the middle;
% and the central cusp is a short soft peak, not a brace's beak.  There is no
% vertical stroke anywhere in it.  Anyone who needs the ornament itself must
% go to the scan (PDF 38).
% =====================================================================
\begin{center}
1.\enspace\textbf{Den græske Philosophie.}\\[2pt]
\emph{Ideen aabenbaret i Verdenskunstværket} (\gk{Κόσμος}).\\
\emph{(Umiddelbar Enhed af Natur og Aand).}\\[6pt]
2.\enspace\textbf{Den nyere Tids Philosophie.}\\[2pt]
$\overbrace{\hspace{0.72\linewidth}}$\\[2pt]
\begin{tabular}{@{}c@{}c@{}}
...
\end{tabular}\\[2pt]
\emph{(abstract Modsætning af Natur og Aand).}\\[8pt]
% --- p. 30 ---
\opage{30} % ---------------------------------------------------------------------
% The continuation of the schema, at the head of p. 30.  Measurements in pixels
% of the 300 ppi scan; the body measure on p. 30 is x 240--1355, centre 797.
%
%   line 1  BOLD, centred, ending in a COLON, not a full stop:
%             "3.   Fremtidens Philosophie:"                     x 562--1065
%   lines 2--3  LETTERSPACED roman, on the ordinary body measure, the first
%           line FILLING the measure (x 240--1351) and broken with a hyphen,
%           the turnover (x 326--498) indented about two ems from the left
%           edge of the line above:
%             "Den absolute Subjectivitet som ideelt-reelt Tilværel-"
%             "     sesprincip."
%   line 4  LETTERSPACED roman, centred, in parentheses:
%             "(Modsætningsenhed af Natur og Aand)."             x 409--1185
%   line 5  a SHORT CENTRED DIVISIONAL RULE closing the schema, x 699--896,
%           i.e. 197 px = 1.67 cm, centred on the body measure.  That rule IS
%           text and is set below as \divrule.
%
% THE PRINTED LINE BREAK IN "Tilværel-/sesprincip." IS NOT KEPT.  It is a
% compositor's word-division at the end of a full measure, not a printed
% compound hyphen, so it is resolved with \-% like every other line-break
% hyphen in this edition (BATCH-AGENT §2; the word stays greppable).  The
% printed break and the two-em turnover indent are recorded here instead.
% Contrast p. 180, where "Subject-Objectiviteten" carries a PRINTED compound
% hyphen and the line break there IS kept.
% ---------------------------------------------------------------------
3.\enspace\textbf{Fremtidens Philosophie:}\\[2pt]
\emph{Den absolute Subjectivitet som ideelt-reelt Tilværel\-%
sesprincip.}\\
...
% --- p. 31 ---
\opage{31} % L1 head (transcription.tex §3): large centred head with a SHORT CENTRED
% RULE.  Note that here the rule stands BELOW the head, not above it: this
% head opens p. 31 and there is no rule above it.
% NOT BOLD -- corrected in the final pass, which removed the \textbf{} the
% pp. 29--42 batch put here.  Measured at 600 dpi, stem/x-height:
% head 12/71 = 0.169, body roman of the same page 6/40 = 0.150, and at a
% 35 %-of-ink threshold the two are identical (0.181 against 0.171).  The
% book's actual bold, measured the same way on p. 219, is 0.28.  This head
% is the DISPLAY ROMAN at a display size -- the same fount and size as
% pp. 1, 16--17, 216, 284 and 289 -- and it is unspaced (gaps 1--12 px,
% word spaces 50--74).  See §3.
\begin{center}
{\Large Den græske Philosophies Udvikling.}
\end{center}
\divrule
...
% --- p. 33 ---
\opage{33} Og dette vil med andre Ord sige: saa bliver \emph{Formprinci\-%
...
% --- p. 34 ---
\opage{34} Enhed er der, som allerede ovenfor antydet, givet en \emph{umid\-%
...
% --- p. 36 ---
\opage{36} % p.36 is NOT in RECON.md §4's list of Greek-bearing pages, but it carries
...
% --- p. 38 ---
\opage{38} denne Væren, der ikke kan tilintetgjøre sin Modstander, stiller
...
% --- p. 39 ---
\opage{39} maatte de to Principer atter sondre sig abstract fra hinanden,
...
% --- p. 41 ---
\opage{41} % p.41 is another Greek site not listed in RECON.md §4: \gk{Κόσμος}, acute
...
% p. 42 l. 9: HEAVY `i' -- one of the eleven adjudicated in the final pass;
% see the file header §2 under BOLD AS A SECOND IN-PARAGRAPH EMPHASIS for
% the calibration and the reasoning.  Measured there at 900 dpi: stem 18 px
% against a median of 9 px for the 80 ordinary `i' of this page.  The
% pp. 29--42 batch read it as a stray heavy sort (a foul case) and set it as
% an ordinary `i'; the whole-book pass REVERSES that reading and sets it
% \textbf{}.
Entelechie virker \textbf{i} Stoffet, som Udviklingsprincip fører det til
...
% =====================================================================
% --- p. 43 ---
\opage{43} % p. 43 bears NO note of its own.  Its note area opens WITHOUT a mark and
...
% --- p. 44 ---
\opage{44} % p.44: the printed folio is garbled in the ABBYY layer (it comes through
...
% --- p. 45 ---
\opage{45} % p.45 is one of the pencil-pollution sites of BATCH-AGENT §7.  Three
...
% --- p. 46 ---
\opage{46} vil ogsaa stadfæstes ved den \emph{nyere} Philosophie. Men da
...
% --- p. 47 ---
\opage{47} stemte} Form, saaledes at altsaa det, der i sidste Instants be\-%
...
% --- p. 48 ---
\opage{48} Universelle, det relativt Almene og det Individuelle, sig paa
...
% --- p. 49 ---
\opage{49} Universalitet inciterer Individets Stræben ud over
...
% --- p. 50 ---
\opage{50} ubevidst}, idet hvert af de modsatte Principer, medens de
...
% FINAL PASS: \large added, to match the other three theses and the three
% Hovedepoche heads.  The L2 bold is ONE SIZE UP from the body -- x-height
% 44--46 px at 600 dpi against a body 39--41 on the same pages, a ratio of
% 1.05--1.18, where the body-size bold of L3b and L5 gives 0.88--1.00.  The
% roman numeral is part of the bold (median stroke 17--18 px at 900 dpi
% against 12--13 px for the words) and stays inside the \textbf{}.
\begin{center}
\textbf{\large I. En Udviklingsrækkes Princip præformerer og
...
% --- p. 51 ---
\opage{51} Grundpræg. Saaledes svare de allerede tidligere angivne Be\-%
...
% --- p. 52 ---
\opage{52} i samme Øieblik som denne Betegnelse er given, fordobbler
...
% --- p. 53 ---
\opage{53} % p.53: \gk{Παντα ῥεῖ} -- PRINTED AS SHOWN, with NO accent over either
...
\divrule
% --- p. 54 ---
\opage{54} % L3 HEAD, p. 54 (transcription.tex §3).  Set exactly as its companion
...
% --- p. 55 ---
\opage{55} \emph{Afhængighed af Oldtiden}, trods Bestræbelsen for og den
...
% --- p. 56 ---
\opage{56} Tankeformer fra en \emph{fremmed} Livsanskuelse, optog dem der\-%
...
% --- p. 59 ---
\opage{59} gaaende Tids, men det er først \emph{successivt}, at Modsætningen
...
% --- p. 60 ---
\opage{60} hos \emph{Giordano Bruno}. Han er et Billede paa sin Tid, idet
...
% --- p. 61 ---
\opage{61} givelsens Lidenskab, glemmer den endelige Existentses altid
...
% --- p. 63 ---
\opage{63} % p.62/63: the same defect as p. 57 l. 6--7 -- the run "M a t e r i e n s
...
% --- p. 65 ---
\opage{65} Retning af \emph{Udødelighed}, faae \emph{Reminiscentserne fra
...
% --- p. 66 ---
\opage{66} Væreform Usandt, medens Naturen, der kun er ved denne
...
% --- p. 67 ---
\opage{67} Yttringer} vise tilbage til et \emph{fælleds Udspring: til et
...
% transcribed (BATCH-AGENT §6).
% --- p. 68 ---
\opage{68} Jeget at see det active almene Enhedspunkt, fra hvilket alle
...
% --- p. 70 ---
\opage{70} % p.70, l. 1: the guillemets around «Ting i og for sig» were read at
...
% --- p. 71 ---
\opage{71} % SEAM (batch boundary pp. 57--70 | 71--84): p. 71 line 1 is set FLUSH LEFT
...
% --- p. 74 ---
\opage{74} % p. 74 bears no note and no Greek.
...
% p. 76 note *): the figures "362." are set from a MARKEDLY HEAVIER fount
% than "259." on the same line -- the strokes are about half again as thick,
% checked at 1200 dpi and again at 900 dpi in the final pass.  The pp. 71--84
% batch called it a wrong sort on the ground that bold does not occur inside
% a paragraph.  That ground is gone: bold IS a second emphatic device in this
% book (header §2), and the Register uses bold in exactly this way, for the
% principal page reference of an entry ("Aristoteles . . . 94 ff." on p. 289,
% bold, against "Aristippos . . . 193 ff." roman two lines above).  Three
% consecutive figures cannot be a stray single sort.  Read here as the same
% device -- the second of the two references singled out -- and set \textbf{}.
% Of all the bold sites in the book this is the one whose FUNCTION is least
% obvious; the reading is recorded so that it can be checked.
...
% --- p. 78 ---
\opage{78} % p. 78 bears no note and no Greek.
...
% --- p. 79 ---
\opage{79} % p. 79 bears no note.  Greek: Κόσμος (one word).
...
\divrule
% --- p. 80 ---
\opage{80} % HEAD, p. 80 (L3, transcription.tex §3): the first subdivision under thesis II.
%
...
% body.  It is set \emph{} accordingly.  §3 is now corrected: this level is
% L3a, letterspaced roman at body size.
% *** AND THIS HEAD IS THE ONE EXCEPTION OF THE NINE, kept and not
% normalised.  Eight of the nine L3a heads (50, 54, 65, 94, 127, 137, 145,
% 150) are set as JUSTIFIED PARAGRAPHS -- first line indented by the ordinary
% paragraph indent, last line flush LEFT at the margin.  THIS ONE IS CENTRED:
% measured at 600 dpi, its first line begins at x 552 against a body left
% margin of 544 (no indent), and its short last line runs x 1517--1791, whose
% centre 1654 matches the body centre 1652 to 0.1 % of the measure.  Hence
% the \begin{center} here and its absence at the other eight.  See §3. ***
\begin{center}
...
der som saa\-%
% --- p. 82 ---
\opage{82} % RUN-OVER FOOTNOTE: the note area at the foot of p. 82 opens with unmarked
...
% --- p. 84 ---
\opage{84} % RUN-OVER FOOTNOTE: the note area at the foot of p. 84 opens with unmarked
...
% --- p. 85 ---
\opage{85} % p. 84 ends mid-word "i Opfat-"; this page opens with "telsen".  p. 84's
...
% --- p. 86 ---
\opage{86} af en høieste Idee. De staae ved Siden af hverandre som de
...
% --- p. 87 ---
\opage{87} riens Ikke-Væren derimod er den for Ideen fremmede absolute
...
% --- p. 88 ---
\opage{88} % p. 88 is the worst pencil page in the book (§7).  A two-line cursive
...
% --- p. 89 ---
\opage{89} % p. 89 carries NO footnote call of its own.  Its note area holds only the
...
% p. 89, l. 14: HEAVY `i' -- "men i det Sandselige".  Stem 17 px at 900 dpi
% against a median of 9 px for the 116 ordinary `i' of this page and a
% maximum of 12 px among them.  The pp. 85--98 batch read it as the
% compositor's way of emphasising a single letter, which cannot be
% letterspaced; the final pass confirms that reading over all eleven sites
% (header §2, BOLD AS A SECOND IN-PARAGRAPH EMPHASIS).
...
hængig af Rummet}.
% --- p. 91 ---
\opage{91} % p. 90's note ("Elementernes Grundformer udledes ikke af
...
% --- p. 92 ---
\opage{92} sker sin Modsætning, er Andetheden tilstede som skjult, men
...
% --- p. 95 ---
\opage{95} % p. 94's note closes with a full stop and does not run over.
% p. 95 bears no note.
...
% p. 95, l. 29: TWO HEAVY `i' on one line -- "i det Sandselige, i den
% concrete".  Both measure 19 px at 900 dpi against a median of 9 px for the
% 143 ordinary `i' of this page and a maximum of 11 px among them.  Same
% device as p. 89 l. 14; see header §2.
...
egentlige Betydning.}.
% --- p. 97 ---
\opage{97} % p. 97 carries NO footnote call of its own; its note area holds only the
...
% --- p. 98 ---
\opage{98} % p. 98 bears NO footnote: the text block runs to the foot of the page
...
% --- p. 99 ---
\opage{99} % SEAM p. 98/99.  p. 98 bears NO footnote (confirmed again here: p. 99's note
...
% --- p. 100 ---
\opage{100} tentielle som en Fælledsvirkning af det Bevægendes \gk{ποίησις} og
...
% p. 100: HEAVY `i' -- "Vorden, i det Stofagtige".  Stem 19 px at 900 dpi
% against a median of 10 px for the 95 ordinary `i' of this page and a
% maximum of 13 px among them.  The pp. 99--112 batch read it as a stray
% heavy sort and set it as an ordinary `i'; the whole-book pass REVERSES
% that reading (header §2) and sets it \textbf{}.
Aristoteles's Opfattelse, Bevægelse, Vorden, \textbf{i} det Stofagtige,
...
% --- p. 101 ---
\opage{101} stoteles føier til, tages med i Betragtning. Idet Formen frem\-%
...
% against 10--16 px in the spaced words on the same line; re-checked by eye
% at 900 dpi in the final pass and beyond doubt.
% THIS SITE IS THE KEYSTONE OF THE HEAVY-SORT ADJUDICATION: four consecutive
% bold sorts, two whole words, cannot be a foul case, and they stand inside
% letterspaced matter, where a further letterspacing would have been
% invisible.  It establishes that bold is a SECOND in-paragraph emphatic
% device in this book, which is what the eleven heavy `i' then are.  See
% header §2.  Set \textbf{}.
...
% --- p. 102 ---
\opage{102} % p. 102 l. 1: the letterspaced run that ends p. 101 STOPS at the page break.
...
% --- p. 103 ---
\opage{103} % p. 103 l. 1: the letterspacing stops at the page break -- "hæmmes," is set
...
% --- p. 106 ---
\opage{106} at sætte denne som \emph{reen Energie} og som den af ydre Be\-%
...
% --- p. 107 ---
\opage{107} % p. 107 l. 1: \gk{καλοκἀγαθία} -- the CORONIS over the alpha after the second
...
% --- p. 108 ---
\opage{108} hvad der paa det \emph{Nøieste} hænger sammen dermed, af \emph{Villien
...
% --- p. 109 ---
\opage{109} % NB the % on the \setcounter line: p. 108 ends mid-word ("deel\-agtige") and
...
% --- p. 111 ---
\opage{111} Forstand. Og denne Mangel bliver atter \emph{medbetingende}
...
ning}.
% --- p. 112 ---
\opage{112} Ogsaa paa \emph{Ethikens} Gebet stræber \emph{Aristoteles} ud
...
% --- p. 113 ---
\opage{113} % SEAM p. 112 --> p. 113, checked on the image at 600 dpi.  p. 112 bears NO
...
% --- p. 114 ---
\opage{114} % p. 114: a vertical modern pencil stroke stands in the left margin
...
% --- p. 116 ---
\opage{116} % p. 116 bears no footnote.
...
% p. 119 l. 22: HEAVY `i' -- "en Væren i Verden".  Stem 18 px at 900 dpi
% against a median of 9 px for the ordinary `i' of this page.  (The three
% other high values the sweep returns on this page -- 17, 14, 14 -- are the
% upsilon of \gk{Νοῦς} in the book's heavier cursive Greek, checked by eye,
% not Latin sorts.)  The pp. 113--126 batch read it as a stray heavy sort;
% the whole-book pass REVERSES that reading (header §2) and sets it
% \textbf{}.
lige \gk{Νοῦς} en Væren \textbf{i} Verden, og da, ifølge dens immaterielle
...
% --- p. 121 ---
\opage{121} % p. 121 bears no footnote.  A short modern pencil stroke stands in the
...
% --- p. 122 ---
\opage{122} % p. 122 bears no footnote.  It is the heaviest Greek page of the batch;
...
% --- p. 124 ---
\opage{124} % p. 124 bears no footnote and carries no Greek.
...
% --- p. 125 ---
\opage{125} % p. 125 bears no footnote and carries no Greek.
...
% --- p. 126 ---
\opage{126} % p. 126 bears NO footnote: the text block runs to the last line and
...
% --- p. 127 ---
\opage{127} % SEAM CHECK, p. 126 -> p. 127: p. 126 bears NO footnote (no footnote separator
...
% --- p. 128 ---
\opage{128} % p. 128 bears no note, no Greek and no rule.
...
% --- p. 130 ---
\opage{130} % p. 130 bears no note, no Greek and no rule.
...
% --- p. 131 ---
\opage{131} % p. 131 bears no note and no rule.  Greek: καινὴ κτίσις.
...
% --- p. 132 ---
\opage{132} % p. 132 bears no note and no rule.  Greek: Κόσμος's.
...
% --- p. 134 ---
\opage{134} % p. 134 bears no note, no Greek and no rule.
...
% --- p. 137 ---
\opage{137} % p. 137 bears no note.  No Greek.  The printed folio reads 137 plainly on the
...
% FINAL PASS, CENTRING: this head is set to the FULL MEASURE, all seven lines
% flush at both margins (x 225--2447 at 600 dpi against a body 240--2461),
% and the last line fills the measure too, so the print cannot distinguish
% "justified" from "a centred block whose last line happens to fill".  The
% pp. 127--140 batch therefore set it \noindent, and the pp. 71--84 batch set
% the p. 79 head inside \begin{center}.  The other three theses settle it:
% at pp. 50, 79 and 170 the LAST line is short and centred on the measure
% (p. 170: centre 1631 against a head centre of 1629--1634).  All four are
% therefore encoded \begin{center} here, and the p. 137 uncertainty is
% recorded rather than typeset.
\begin{center}
\textbf{\large III. Det i den historiske Udvikling tidligere Sta\-%
...
% --- p. 139 ---
\opage{139} % p. 139 bears no note and no rule.
...
% --- p. 140 ---
\opage{140} % p. 140 bears NO note, no Greek and no rule.
...
% --- p. 141 ---
\opage{141} % SEAM p.140 -> p.141 checked (BATCH-AGENT §5): p. 140 bears NO footnote and
...
% --- p. 142 ---
\opage{142} % p. 142, l. 5: a small stray dot stands in the left margin beside
...
% --- p. 143 ---
\opage{143} synes at have \emph{assimileret} det Optagne. Og dog viser der
...
% --- p. 144 ---
\opage{144} \emph{Phantasiens Verden}, i hvilken Loven er forsvunden og en
...
% --- p. 145 ---
\opage{145} % p. 145, l. 6: "er" is letterspaced ("e r", gap 16 px) while the flanking
...
% --- p. 146 ---
\opage{146} som al Tilværelse og som evig værende \emph{ved sig selv}, og at
...
% --- p. 147 ---
\opage{147} % p. 147, l. 4 and l. 16: TWO HEAVY `i' -- "dette som Dele i, ikke af," and
% "som værende i Tingene".  Stems 16 px and 19 px at 900 dpi against a median
% of 10 px for the 143 ordinary `i' of this page and a maximum of 12 px among
% them.  The pp. 141--154 batch's argument that these are emphasis and not a
% foul case is the one the final pass adopted for all eleven sites: both
% stand in a contrastive position, and on l. 4 the very word contrasted with
% the `i', namely "af", IS LETTERSPACED, so the two members of one antithesis
% carry the book's two emphatic devices.  See header §2.
% The batch set these two \emph{}; the final pass sets them \textbf{}, as at
% the other nine sites, because the edition records the FOUNT that is printed
% and leaves the function to the header.
...
dette som Dele \textbf{i}, ikke \emph{af}, det Uendelige (nell' infinito, ikke
...
\emph{natura naturans}, som værende \textbf{i} Tingene, bliver nu en
...
% --- p. 150 ---
\opage{150} % p. 150, l. 13: within the Italian «impossibile e nulla a chi non crede» only
...
% --- p. 151 ---
\opage{151} % p. 151, l. 7: a small stray dot stands in the left margin beside "Under den
...
% --- p. 152 ---
\opage{152} % p. 152, ll. 10 f.: the word "begribes" is broken at the line end and only
...
% --- p. 153 ---
\opage{153} % p. 153, l. 12: only the FIRST ELEMENT of the compound is letterspaced --
...
% --- p. 156 ---
\opage{156} Grundbestemmelser sættes ogsaa her som \emph{Phænomen} af det
...
% --- p. 158 ---
\opage{158} % p. 158: no notes, no Greek, no ɔ:, no pencil.
...
% --- p. 160 ---
\opage{160} % p. 160: no notes, no Greek, no ɔ:, no pencil.
...
% --- p. 161 ---
\opage{161} % p. 161: no notes, no Greek, no pencil.
...
% --- p. 163 ---
\opage{163} % =====================================================================
% DISPLAYED SCHEMA, p. 163 -- the genealogical tree of the nyere realistiske
% Philosophie.  The words were read off the image and verified by the batch
% that covered this page; what follows records WHAT IS PRINTED and, at the
% foot of the block, WHAT CONSTRUCTION WAS CHOSEN for it.
%
...
%
% CONSTRUCTION CHOSEN.  A single two-column `tabular' inside a `center', the
% trunk (lines 1--2, their two stems and the crossbar) set as
% \multicolumn{2}{c}{} rows across both columns and the two branches as
% \makebox[0.5\textwidth]{} cells.  Fixed half-measure boxes rather than
% naturally-sized columns, because that is what makes the geometry right: each
% box's centre is the centre of its branch, so the crossbar --
% \rule{0.5\textwidth}{0.4pt}, centred over the pair -- terminates exactly over
% the centre of each branch label, as it does in the print (its termini at
% x 290 and x 897 stand over label centres x 305 and x 899).  The five rules
% are the book's own two kinds: four \connrule (the preamble's connector
% hairline, set at this page's measured 4.2 mm) and one plain horizontal
% \rule for the crossbar.  The turnover "de la nature)." is set in a nested
% left-aligned `tabular' so that it can be indented from the left edge of the
% line above rather than centred under it, as printed.  The negative skip
% \\[-2.3ex] between the lower stem and the crossbar is what makes the two MEET,
% as they do in the print (one blank pixel row between them at 300 dpi); the
% four other rules hang free, as they also do in the print.
%
% THREE DEPARTURES FROM THE PRINT, all consequences of that choice, recorded
% rather than reproduced:
%   1. The two branches are set symmetrically about the measure's centre; in
%      the print they are not (left label centre x 305, right x 899, against a
%      measure centre of x 644), because the right label is much the wider.
%   2. The stem therefore meets the crossbar at the crossbar's own midpoint;
%      in the print it meets it 36 px RIGHT of that midpoint, being centred on
%      the head above it rather than on the bar.
%   3. The crossbar is a level rule.  The printed bar sits very slightly askew
%      in the forme -- its left terminus at y 488, its right at y 482.
% The crossbar's printed width (607 px = 5.1 cm) is not carried over either:
% it is a function of the printed measure and of a 10 pt fount, and here it is
% half of this edition's own measure.  Nothing above changes a word of the
% transcription; all of it concerns arrangement only.
% =====================================================================
\begin{center}
\begin{tabular}{@{}c@{}c@{}}
\multicolumn{2}{c}{Erfaringsphilosophien (Baco).}\\[3pt]
\multicolumn{2}{c}{\connrule}\\[3pt]
\multicolumn{2}{c}{Empirismen (Locke).}\\[3pt]
\multicolumn{2}{c}{\connrule}\\[-2.3ex]
\multicolumn{2}{c}{\rule{0.5\textwidth}{0.4pt}}\\[3pt]
\makebox[0.5\textwidth]{Sensualismen (Condillac).} &
  \makebox[0.5\textwidth]{Den formelle Idealisme (Berkeley).}\\[4pt]
\makebox[0.5\textwidth]{\connrule} &
  \makebox[0.5\textwidth]{\connrule}\\[3pt]
\makebox[0.5\textwidth]{\begin{tabular}[t]{@{}l@{}}
    Materialismen (Système\\
    \hspace*{4em}de la nature).
  \end{tabular}} &
  \makebox[0.5\textwidth]{Skepticismen (Hume).}
\end{tabular}
\end{center}
...
% --- p. 164 ---
\opage{164} % p. 164: no notes, no Greek, no ɔ:, no guillemets.
...
% --- p. 165 ---
\opage{165} % p. 165: no notes, no Greek, no ɔ:, no pencil.
...
% --- p. 166 ---
\opage{166} % p. 166: no notes, no Greek, no pencil.
...
% --- p. 167 ---
\opage{167} % p. 167: no notes, no Greek, no ɔ:, no pencil.
...
% --- p. 168 ---
\opage{168} % SEAM p.168/169 CHECKED (BATCH-AGENT §5.2).  p. 168 bears NO notes at all
...
% --- p. 169 ---
\opage{169} % SEAM 168->169.  p. 168 ends mid-sentence with "...maatte sees som en
...
% --- p. 170 ---
\opage{170} % p. 170 bears no note and no Greek.
%
...
% FINAL PASS: three full-measure lines (x 522--2741 at 600 dpi) and a short
% last line x 1057--2206, centre 1631, against a head centre of 1629--1634 --
% CENTRED.  Encoded \begin{center} + \textbf{\large}, as the other three
% theses now are; the \noindent the pp. 169--182 batch used is gone.
\begin{center}
\textbf{\large IV. To samtidige, coordinerede Udviklingsformers
...
% --- p. 171 ---
\opage{171} % p. 171 bears no note and no Greek.
...
% --- p. 172 ---
\opage{172} % p. 172 bears no note and no Greek.
...
% --- p. 173 ---
\opage{173} % p. 173 bears no note and no Greek.
...
% --- p. 174 ---
\opage{174} % p. 174 bears no note and no Greek.  The word `Modificationer,' is broken
...
% --- p. 176 ---
\opage{176} % p. 176 bears no note (its note area is empty, so p. 175's note does not run
% over) and no Greek.
...
% --- p. 178 ---
\opage{178} % p. 178 bears no note and no Greek.  The letterspaced phrase
...
% --- p. 179 ---
\opage{179} % p. 179 bears no note and no Greek.  It carries the sheet signature "12*" at
...
% --- p. 180 ---
\opage{180} % =====================================================================
% DISPLAYED SCHEMA, p. 180 -- the Genesis of the nyere Idealismes Grundtanke.
% The words were read off the image and verified by the batch that covered this
% page; what follows records WHAT IS PRINTED and, at the foot of the block,
% WHAT CONSTRUCTION WAS CHOSEN for it.
%
...
%   THE SWELL IS NOT REPRODUCED, and nothing is invented to suggest it.  What
% is set below is \divrule, a plain even hairline 2 cm long against the printed
% 2.02 cm -- so the length and the position are right and the SORT is not.  How
% the printed rule differs from what is set: it tapers to nothing at both ends
% and thickens smoothly to about three times the hairline weight at its middle,
% a cast ornament rather than a piece of rule brass.  Anyone who needs it must
% go to the scan (PDF 189).
%
% ---- CONSTRUCTION CHOSEN ----
% Two `center' environments, one for each half of the schema, each holding its
% bold head and then a `tabular' whose FIRST COLUMN is the letterspaced author
% label, set flush right, and whose SECOND COLUMN is the matter, set flush
% left.  That is the printed arrangement: the labels end on a common axis
% (x 437--450 at 300 dpi) and the matter begins on another (x 521--532).
%
% THE BRACES are amsmath's \left\lbrace ... \right. around an `array' of \text{}
% rows, one row per printed line, which is why the preamble loads amsmath.  The
% delimiter then grows with its own group, as the printed brace does: 2 lines
% for Descartes, 3 for Spinoza, 7 for Kant.  Leibnitz and Wolf have no brace in
% the print and get none here; their matter is indented \hspace{0.9em} so that
% it still starts on the common matter axis, the printed brace being about
% that wide (8 px of brace plus a 25 px gap at 300 dpi).
%
% THE LEFT-OPENING BRACE at the foot is \left\rbrace ... \right. around the
% (\lbrace/\rbrace rather than \{ \}, so that check.py's raw brace count,
% which cannot see a TeX escape, still balances.)
% column, and the Fichte array is padded with SIX EMPTY ROWS below its four
% lines.  The padding is what does the alignment: an `array' is centred on the
% math axis, so a four-line block padded to the ten rows of the Kant array is
% centred exactly as the Kant array is, and its first line therefore stands
% level with "Jeget = Selvbevidstheden." and the brace spans the first four
% Kant lines -- which is where the print puts them.  This is done with empty
% rows rather than a \raisebox of some measured length so that it survives a
% change of leading or of body size; \baselineskip is 0 pt inside a tabular
% cell, so a \raisebox written in those units would silently do nothing.
%
% THE ELEVEN CONNECTOR RULES are the preamble's \connrule, each on a row of its
% own, wrapped in \multicolumn{1}{c}{} so that it is centred on the column it
% stands in.  In the print they sit at x 733--742 (section A) and x 709--715
% (section B) at 300 dpi, a little LEFT of the centre of the matter column
% (x ~920 for the widest line) and a little left of the measure's centre
% (x 797); here they are centred on their column, and on the label-plus-matter
% pair for the four that stand outside a brace in section B.  The Schelling and
% Hegel lines are likewise centred on the label-plus-matter pair, not on the
% two-column arrangement, which is what the print does (their centres are
% x 685 and x 696, against x 710 for the connectors beside them).
%
% THE PRINTED LINE BREAK IN "Subject-/Objectiviteten)." IS KEPT, with a real
% hyphen, because "Subject-Objectiviteten" is a PRINTED COMPOUND and the hyphen
% belongs to the word.  Every other line break inside this schema is kept too:
% the schema is displayed matter set line by line, not reflowed prose.  Contrast
% p. 30, where "Tilværel-/sesprincip." is a compositor's word-division and is
% resolved with \-% in the ordinary way.
% =====================================================================
\begin{center}
\textbf{A.\enspace Dogmatisk Idealisme.}\\[8pt]
\renewcommand{\arraystretch}{1.15}%
\begin{tabular}{@{}r@{\hspace{0.35em}}l@{}}
\emph{Descartes:} & $\left\lbrace\begin{array}{@{}l@{}}
      \text{\emph{Jeg} = \emph{activ} Selvbevidsthed.}\\
      \multicolumn{1}{c}{\connrule}\\
      \text{Res cogitans (substantia cog.)}
    \end{array}\right.$\\[3pt]
 & \multicolumn{1}{c}{\connrule}\\[3pt]
\emph{Spinoza:} & $\left\lbrace\begin{array}{@{}l@{}}
      \text{\emph{Substantsen som res cogitans infinita}}\\
      \multicolumn{1}{c}{\text{\emph{og res extensa infinita.}}}\\
      \multicolumn{1}{c}{\connrule}\\
      \text{\emph{Substantsen = Deus = Natura.}}
    \end{array}\right.$\\[3pt]
 & \multicolumn{1}{c}{\connrule}\\[3pt]
\emph{Leibnitz:} & \hspace{0.9em}\begin{tabular}[t]{@{}c@{}}
      \emph{Substantserne} (Monaderne,\\
      \emph{aandeligt} bestemte.)
    \end{tabular}\\[3pt]
 & \multicolumn{1}{c}{\connrule}\\[3pt]
\emph{Wolf:} & \hspace{0.9em}\begin{tabular}[t]{@{}c@{}}
      Monaden som \emph{res} (substantia) \emph{simplex}\\
      (berørende \emph{Atomets} Begreb).
    \end{tabular}
\end{tabular}
\end{center}

\begin{center}
\textbf{B.\enspace Kritisk Idealisme.}\\[8pt]
\renewcommand{\arraystretch}{1.15}%
\begin{tabular}{@{}r@{\hspace{0.35em}}l@{\hspace{0.35em}}l@{}}
\emph{Kant:} & $\left\lbrace\begin{array}{@{}l@{}}
      \text{\emph{Jeget = Selvbevidstheden.}}\\
      \multicolumn{1}{c}{\connrule}\\
      \text{\emph{Selvbevidsthedens rene}}\\
      \text{Form (Selvbevidstheden}\\
      \text{overhovedet.}\\
      \multicolumn{1}{c}{\connrule}\\
      \text{\emph{Homo noumenon.}}\\
      \multicolumn{1}{c}{\connrule}\\
      \text{\emph{Det fælleds intelligible Substrat}}\\
      \multicolumn{1}{c}{\text{\emph{for Naturen og Aanden.}}}
    \end{array}\right.$
 & $\begin{array}{@{}c@{}}
     \left\rbrace\begin{array}{@{}c@{}}
       \text{\emph{Fichte: Jeget som}}\\
       \text{\emph{absolut}}\\
       \multicolumn{1}{c}{\connrule}\\
       \text{(Væren).}
     \end{array}\right.\\
...
\multicolumn{2}{c}{\begin{tabular}{@{}c@{}}
   \emph{Schelling: Det Absolute} (Fornuften, Subject-\\
   Objectiviteten).
   \end{tabular}} & \\[3pt]
\multicolumn{2}{c}{\connrule} & \\[3pt]
\multicolumn{2}{c}{\emph{Hegel: Den absolute Aand} (den logiske Idee).} &
\end{tabular}
\end{center}
\divrule

% --- p. 181 ---
\opage{181} % p. 181 bears no note and no Greek.  spacing.py finds no letterspacing at all
...
% --- p. 182 ---
\opage{182} % p. 182 bears no note and no Greek.  The sentence runs on from p. 181 with no
% paragraph break.
...
% --- p. 183 ---
\opage{183} % SEAM p. 182 -> p. 183 checked (BATCH-AGENT §5).  p. 182 bears NO footnote --
...
% --- p. 184 ---
\opage{184} % p. 184 bears no note.  No Greek.  No ɔ: mark.  No guillemets.  The whole
...
% --- p. 185 ---
\opage{185} % p. 185 bears no note.  No Greek.  No ɔ: mark.  No guillemets.
...
% --- p. 186 ---
\opage{186} % p. 186 bears no note.  Greek: Κόσμος (one word, l. 9; capital kappa,
...
% So it is a short bold centred head under a divisional rule.  Set \textbf{}
% inside \begin{center}; NOT \emph{}.  FINAL PASS: the review item is
% discharged -- §3 now splits the old level L3 into L3a (the nine
% letterspaced-roman paragraph heads) and L3b (these bold centred ones), and
% files 186/192/201 under L3b.  The enumerator "1." here is BOLD (median
% stroke 13 px at 900 dpi against 11 px for the head words, and plainly
% heavier by eye), unlike the "2." of p. 192 and the "3." of p. 201, which
% are roman; the difference is printed and is kept, so this numeral alone of
% the three stands INSIDE the \textbf{}.
\begin{center}\textbf{1.\enspace Den græske Atomlære.}\end{center}
...
% --- p. 189 ---
\opage{189} % p. 189 bears no note.  No Greek.  No ɔ: mark.  The whole page is ONE
...
% --- p. 190 ---
\opage{190} % p. 190 bears no note.  No ɔ: mark.  No guillemets.
...
% ADJUDICATED in the final pass as EMPHASIS, not a foul case; \textbf{} kept.
% See header §2, BOLD AS A SECOND IN-PARAGRAPH EMPHASIS.
...
% --- p. 191 ---
\opage{191} % p. 191 bears no note.  No ɔ: mark.  The whole page is ONE paragraph,
...
% FINAL PASS: the enumerator "2." is ORDINARY ROMAN (median stroke 6 px at
% 900 dpi against 11 px for the head words), so it stands OUTSIDE the
% \textbf{} -- unlike p. 186's "1.", which is bold.  Filed under L3b in §3.
%
...
\begin{center}2.\enspace\textbf{Den Aristippiske og Epikureiske Lystlære.}\end{center}
...
% --- p. 193 ---
\opage{193} % p. 193 bears no note.  No ɔ: mark.
...
% --- p. 194 ---
\opage{194} % p. 194 bears no note.  The whole page is ONE paragraph, continuing p. 193
...
% --- p. 195 ---
\opage{195} % p. 195 bears no note.  No ɔ: mark.  No guillemets.
...
% --- p. 196 ---
\opage{196} % p. 196 bears no note.  No Greek.  No ɔ: mark.  No guillemets.
...
% =====================================================================
% --- p. 197 ---
\opage{197} % No note, no Greek on this page.  Guillemets «Indsigten» point INWARD.
...
% --- p. 199 ---
\opage{199} % GREEK: ἡδὺς βίος is the book's grave/acute calibration pair -- rough
...
% --- p. 200 ---
\opage{200} % No note, no Greek.  NOTE THE COMPOUND EMPHASIS PATTERN that recurs on
...
% --- p. 201 ---
\opage{201} selv som nydende, fandt sig i sin, ved Aandsdannelsen fri\-%
...
% FINAL PASS: §3 now encodes a head that is BOTH bold and letterspaced as
% \textbf{\emph{...}}, in that order, so the letterspacing is in the markup
% after all; this is the ONLY head in the book that is both.  The enumerator
% "3." is ORDINARY ROMAN (median stroke 8 px at 900 dpi against 11 px for
% "Sokrates.", and plainly lighter by eye), so it stands OUTSIDE the
% \textbf{}, as at pp. 192, 231, 236, 237 and 240 -- but not at pp. 186, 252
% and 259, where the enumerator is bold.  See §3 for the whole table.
\begin{center}
3.\enspace\textbf{\emph{Sokrates.}}
\end{center}
...
% --- p. 202 ---
\opage{202} % PENCIL, p. 202: two short vertical strokes in the LEFT margin beside
...
% --- p. 204 ---
\opage{204} % The letterspaced run "negativt Videns Ind-hold." RUNS ACROSS THE PAGE
...
% --- p. 205 ---
\opage{205} % PENCIL, p. 205: several vertical strokes in the RIGHT margin, and a
...
% --- p. 206 ---
\opage{206} stemmelse: \emph{Viden}, være det. Viden maa være en Realitet;
...
% --- p. 207 ---
\opage{207} % PENCIL, p. 207 (a known pollution site): vertical strokes in the RIGHT
...
% --- p. 209 ---
\opage{209} % p. 209 also has a SECOND EXPOSURE (pagemap.py --alt 209 = PDF 220);
...
% --- p. 210 ---
\opage{210} % THE PAGE MAP STEPS HERE: printed 210 = PDF 221, not 219.  Obtained
...
% --- p. 211 ---
\opage{211} % p. 211 bears ONE note, mark *) (\setcounter below).  The note is
% self-contained: it opens with its own mark, so nothing runs over from
% p. 210, and it ends on this page.  Sheet signature "14*" at the foot --
% not transcribed (BATCH-AGENT §6).  No Greek, no ɔ:, no guillemets.
% SEAM: p. 210 ends "...at udtrykke dets \emph{Væsen}; deels" and this page
% opens mid-sentence with "viser det sig" -- NO paragraph break here.
\setcounter{footnote}{0}
viser det sig gjennem de Udsagn af Sokrates om \emph{Guderne},
der, om de end ikke ere at tage i stræng Forstand dogmatisk,
dog antyde et Forhold af Gud til Verden ligt med Sjælens til
Legemet, medens der er en Væsenslighed mellem den menneskelige
Sjæl og den guddommelige. Vel bliver nu den i \emph{Guddommen}
% "Guddommen" IS letterspaced (measured at 800 dpi); spacing.py did not
% flag it -- an instance of the ~75 % recall recorded in BATCH-AGENT §3.
virkelige Viden for \emph{Mennesket} i Usikkerheden til \emph{Uvidenhed};
men der er dog hos Sokrates ingen Skepsis om \emph{Videns Re\-%
alitet i og for sig}, om dens \emph{væsentlige} Forhold til Men\-%
neskenaturen; der hentydes til, at den i \emph{Døden} atter bliver
virkelig gjennem Erindringen, der ikke længer hæmmes af et
\emph{Fordunklende}\footnote{Man sammenligne med Stederne hos Plato ogsaa
de Ord, der i Cyrop. VIII 7, 20 lægges den døende Cyrus i Munden.}, og
gjennem \emph{det Religiøse} faaer den
atter et positivt Forhold til Objectiviteten. Det er altsaa, naar
\emph{Viden i og for sig} har Realitet, men som \emph{menneskelig}
Viden bliver til Uvidenhed, \emph{Existentsen} med dens Relati\-%
vitet og Modsætning, der forhindrer Tanken fra, beherskende
at gjennemtrænge Virkeligheden, og netop derved føres den
Existerende fra \emph{Speculation til Handling}; men \emph{væsent\-%
lig er Tanken Viden}, Eet med sin Gjenstand, som skal
opfattes i Begrebet. \emph{Viden er hos Sokrates i personlig
Form det Samme som Ideen er hos Plato.}

Saaledes har den sokratiske \emph{Uvidenhed}, ligesom den er
\emph{Gjenstand for Viden}, en \emph{Videns Baggrund}, der dog
aldrig kan vinde Skikkelse i Erkjendelsen af det Bestemte.
Den tilsyneladende Modsigelse i en saadan Viden fjernes naar
man erindrer det ovenfor udviklede Grundforhold. Formen,
det Ideelle, er, i Uvidenheden, ikke forsvunden for Bevidst\-%
heden, den har Gyldighed og Virkelighed for denne; den væ\-%
sentlige Viden er derfor midt i Uvidenheden. Men \emph{Formen},
der her er for Tanken i dens oprindelige \emph{Væren ved Siden af}
% "Væren ved Siden af" is letterspaced but "det forudsatte Stof," on the
% next line is NOT (checked at 800 dpi); the emphasis stops at "af".
det forudsatte Stof, --- saaledes opfattes her den lige Oprin\-%
delighed trods Formens Overvægt, --- bliver den \emph{negativt
uendelige}, idet den i Sondringen er \emph{den Intet formende},
% --- p. 212 ---
\opage{212} % p. 212 bears no note.  Greek: αὐτάρκης (one word, l. 31; smooth breathing
% over the υ -- a comma opening to the right -- and an acute over the α,
% both read at 1200 dpi).  No ɔ: mark, no guillemets.
% PENCIL: tesseract picks up stray marks in the left margin of this page
% (it emits ", dette", "/ frivillig", "| gsom", ": græske" as line-initial
% junk).  Nothing that is not printed type is transcribed (BATCH-AGENT §7).
den \emph{formløst uendelige Form}. Den \emph{Viden}, der bliver
--- trods \emph{Uvidenheden}, bliver saaledes en Viden af et for\-%
% PRINTER'S INCONSISTENCY, p. 212: the word "formelt" is broken across the
% line and only its SECOND half is letterspaced.  Measured at 800 dpi:
% "for-" at the end of the line above has body letter-gaps (2--4 px),
% "melt" at the head of this line has 14--18 px.  Transcribed as printed --
% \emph{} on "melt" alone.  (Contrast p. 221, where "Begrænds-/ning" and
% "Tan-/kens" are letterspaced in BOTH halves.)
\emph{melt} Forhold, men en \emph{Viden}, der ikke kan faae Udfyldelse i
Erkjendelsen af det \emph{Virkelige}, og idet den vender sig mod
dette, \emph{kun faaer Uvidenheden til sikkert Indhold}.

Med den fremsatte sokratiske Grundanskuelse af Viden i
dens Forhold til Existentsen, der finder en \emph{Analogie} hos
\emph{græske Digtere} (i Opfattelsen af Døden som forsonende),
og som, trods dens tilsyneladende modernere Form, forklares
ved det udviklede Grundforhold mellem den græske Verdens\-%
anskuelses Momenter, og overalt ligesom bevæger sig paa dens
Baggrund, kunne den \emph{sokratiske Ethiks} væsentlige Be\-%
stemmelser sættes i nøie Forbindelse.

Idet \emph{Viden} er \emph{Væsenets} Udtryk, er \emph{Viden det Her\-%
skende}, over hvilket Lidenskab og Sandselighed ikke har
Magt. \emph{Viden og Handlen ere uadskillelige}; Ingen er
frivillig ond. \emph{Synd er Uvidenhed}; men Uvidenheden er
som en spøgefuld Maske, forsvindende med Existentsen, der,
som det personligt Uforskyldte, tager den upersonlige Skyld
med sig. --- \emph{Og som Viden er Dyden een}; der bliver
hverken en Adskillelse mellem Dydens forskjellige Yttrings\-%
former, eller mellem de forskjellige Individers Dyder; thi det,
der skulde give Dyden Forskjellighed: det Naturlige, enten det
sættes som Handlingens Øiemed, eller som Individets Natur\-%
bestemthed, er ligeoverfor Formen, som Viden repræsenterer,
i Begrebet et Underordnet, der ikke kan bringe Adskillelse ind
i det, hvis Væsen er Enhed. --- Som \emph{væsentligt vidende}
er dernæst Individet \emph{væsentligt fuldstændigt}, og der op\-%
staaer en \emph{dobbelt} Fordring til det. Ligeoverfor det Natur\-%
lige skal det være \gk{αὐτάρκης}, realisere sin Uendelighed i Fri\-%
hed for Afhængigheden af alt det Ydre --- ikke paa \emph{asketisk}
Viis, men ved aandelig Frihed i at kunne nyde og undvære;
saa at denne Autarkie bliver det til Existentsvilkaarene sva\-%
rende Udtryk for det Positive: at den \emph{Vidende} er den \emph{Her\-%
skende}, idet Videns Princip, den fornuftige Tanke, er Alts
% --- p. 213 ---
\opage{213} % p. 213 bears no note.  No Greek, no ɔ: mark.  One guillemet pair,
% «Sam-tales» (ll. 26--27), the normal INWARD pair, and the quoted word is
% itself letterspaced -- read at 400 dpi.
Væsen. \emph{Det, at være den Herskende}, udtrykkes i \emph{Exi\-%
stentsen ved: ikke at lade sig beherske af noget An\-%
det}. --- Ligeoverfor \emph{andre Individer} skal Individet \emph{selv
være uafhængigt og anerkjende dem som uafhængige}.
% Word order checked on the image: "Ligeoverfor andre Individer skal
% Individet selv være uafhængigt" -- ABBYY transposes "selv".
Denne Consequents er nødvendig hvad enten Individerne op\-%
fattes som \emph{uvidende} eller som \emph{væsentlig vidende}. I første
Tilfælde kan den Ene ikke lære den Anden Noget fordi han
selv \emph{Intet} veed, i sidste kan han det ikke fordi den Anden
væsentlig veed \emph{Alt}. Det ene Individs Opgave i Forholdet til
det andet kan kun blive den: at \emph{vække det til ethisk Selv\-%
erkjendelse}, til Bevidsthed, foreløbig \emph{om dets Uvidenhed};
thi dette forudsætter, at det væsentligen \emph{har} hiin Bevidsthed
og har det, der afgiver Maalestokken for Uvidenheden, og der\-%
for væsentligen \emph{ved sig selv} kommer til Bevidsthed. Hiin
Vækken finder Sted idet den indbildte Viden gjøres vaklende,
idet Individet ironisk afklædes, indtil det finder sig ligesom
nøgent overfor sig selv, og nu staaer paa Springet til, \emph{erin\-%
drende} at tage sig tilbage i Ideen, i Væsensforholdet, hvis
ikke denne Erindring \emph{ethisk} afbrydes \emph{ved Handling}. Hiin
Bringen til Selverkjendelse er det \emph{Maieutiske}, Hjælpen til
\emph{at føde sig selv}: den \emph{Gjenfødelse}, Grækenland kjendte.
Forholdet mellem de enkelte \emph{ethiske} Individer bliver \emph{Ven\-%
skabet}, der, efter Sokrates's Opfattelse, forudsætter den gjen\-%
sidige \emph{Autarkie} og \emph{Ligheden} i det, at være \emph{gode}, og hvis
væsentlige Yttringsform er den \emph{gjensidige Tilskynden
til Selverkjendelse} gjennem den ethisk vækkende «\emph{Sam\-%
tales}» Dialektik.

Den gjensidige Uafhængighed og Individets væsentlige
Fuldstændighed fører til samme Consequents som det, at Viden
i ethvert enkelt existerende Individ forvandler sig til Uviden\-%
hed, nemlig til: at \emph{Dyden ikke kan læres}. Sokrates siger det
udtrykkelig, at der ikke kunde findes en Lærer i Dyden, og
hvorledes skulde ogsaa en saadan kunne findes blandt de Uvi\-%
dende? Skal altsaa Dyden blive til i Individet, er Individet,
der er sat som væsentligt vidende, henviist til \emph{sig selv}, til
% --- p. 214 ---
\opage{214} % p. 214 bears no note.  No Greek, no ɔ: mark.  One guillemet pair,
% «Uvidenhed» (l. 32), the normal INWARD pair, the quoted word letterspaced.
gjennem \emph{Selvbesindelsen} at kalde den \emph{væsentlige Vi\-%
den} tillive i den \emph{negative} og \emph{formelle} Skikkelse, i hvilken
den kan haves og i hvilken den kan være Grundlag for det
Ethiskes \emph{negative} Form, for saa, idet den i Forhold til det
Concrete forvandler sig til \emph{Uvidenhed}, at søge en Viden,
der svarer til det \emph{positivt} Ethiske, gjennem \emph{det Religiøse}.
Hiin \emph{Selvbesindelse} er hos Sokrates det Tilsvarende til den
platoniske \emph{Erindring}, gjennem hvilken Viden bliver til som
\emph{positiv Viden af Ideen}.

Disse ere den Sokratiske Ethiks \emph{almene}, væsentlig \emph{for\-%
melle} Bestemmelser. Idet den ethiske Bevidsthed skulde have
et \emph{concret} Indhold, kunde det, efter hvad der ovenfor blev
udviklet om Tankens Grundforhold, ikke skee gjennem en
\emph{Tankeudfoldelse}, og Tanken kunde kun finde Veien \emph{ind
efter}, mod Selvbevidsthedens Uendelighed, det Ethiskes Ud\-%
gangspunkt; men ikke \emph{ud efter} fra Uendeligheden til det
Ethiskes Udfoldelse i Handling: i denne Bevægelse blev for
det \emph{existerende} Individ Viden til \emph{Uvidenhed}. Det Ideelles
Uendelighed (jfr. S. 211 f.) kunde kun aabenbare sig for Aanden
i sin \emph{negative} Virken som bevægende \emph{Magt} i den Dialektik, der
\emph{fortærede} det Endelige, eller i \emph{Ekstasen}, i hvilken Selvet
fortaber sig anskuende i den projicerede Uendelighed, der
fængslende fasttryller Bevidstheden i Ubevægelighed, men ikke
vinder Skikkelse og udfolder sig. Som Fatum over Guderne
stod Uendeligheden for den ironiske Bevidsthed over Endelig\-%
heden som det skikkelsesløse, uudfoldede Mægtige. Forat naae
til \emph{indholdsfuld} ethisk Handlen blev derfor paa dette Stand\-%
punkt det \emph{religiøse Forhold} uundværligt, og gjennem det
faaer \emph{det Gode} Concretion for Sokrates.

Vi see altsaa i Sokrates \emph{Supplementet} til den græske
Philosophies \emph{speculative} Retning. I hans «\emph{Uvidenhed}»,
der dog er som en \emph{hæmmet Viden}, træder en \emph{afgjørende}
Consequents af den græske Grundanskuelse frem, og den ud\-%
folder sig i en Kreds af ethiske Bestemmelser. Som den, for
hvem det Dialektiske \emph{kun} blev tjenende Moment, og som den,
% --- p. 215 ---
\opage{215} % p. 215 bears no note.  No Greek, no ɔ: mark, no guillemets.  The page is
% SHORT (27 lines against the usual 35) and closes the Sokrates section.
% PENCIL: a faint modern mark in the right margin beside ll. 5--6; not
% printed type, not transcribed.
hvis Tanke ender i Videns Negation, stiller Sokrates sig selv
\emph{udenfor Speculationen}; han bliver \emph{væsentlig ethisk-%
% "ethisk-religiøs" is a PRINTED compound hyphen that happens to fall at
% the line break; a real hyphen, not \-.
religiøs Personlighed, ikke Philosoph, fordi han
ikke vil være det}. Men ved sin Methode, ved sin Fordring
af en Viden og sin Opfattelse af denne Viden har han foran\-%
lediget en speculativ Philosophie, og er bleven Incitamentet for
\emph{hele} den sildigere philosophiske \emph{Udvikling} hos Grækerne.
Idet den græske Aand, der, efter sin Natur, ikke vedblivende
kunde nøies med at gjøre Ironiens Bevægelse henimod Uvi\-%
denheden, i Trangen til at fastholde ogsaa den endelige men\-%
neskelige Tankes Harmonie med det Værende, der var brudt
ved den sokratiske Opfattelse, atter, ud fra \emph{Anskuelsen af
den umiddelbare Enhed}, postulerede \emph{Virkelighedens
Lighed med Væsenet}, saa afkastede den sokratiske væ\-%
sentlige Viden \emph{Uvidenhedens} Form og blev til \emph{den exi\-%
sterende Tænkers Viden af Ideen}. \emph{Begrebet}, der hos
\emph{Sokrates} var sat som Tankens \emph{uopnaaede Maal}, og nær\-%
mest seet som det \emph{subjective} Livs Regulator, bliver nu, idet
det sees som \emph{Tankens Væsen og Sandhed} og som \emph{hele
Menneskelivets Norm}, opfattet som \emph{den objective Ver\-%
dens for Tanken tilgængelige Væsen} og som \emph{iogforsig\-%
værende Virkelighed}. Saaledes fremgaaer den \emph{plato\-%
niske Ideelære} af den \emph{sokratiske forsøgte Begrebs\-%
dannelse}, det \emph{ethiske} Standpunkt forvandles til et \emph{specu\-%
lativt}, og dette erkjender i det Standpunkt, af hvilket det er
fremgaaet, det Væsensbeslægtede.

% DIVISIONAL RULE, printed at the FOOT of p. 215, below the last line of
% text and with white space above and below, standing over the L0 head that
% opens p. 216 -- the same placement as the rules at the foot of pp. 15, 53
% and 79.  p. 215 bears no footnote, so this is not the (left-flush)
% footnote separator.  MEASURED at 800 dpi: it is a DOUBLE rule -- two
% parallel hairlines, y 4875--4883 and y 4889--4897, each spanning
% x 1446--2110 (664 px = 2.1 cm), centred on the measure.  Every other
% divisional rule in this batch (under the p. 216 head, at the foot of
% p. 218) is a SINGLE hairline of the same length.  \divrule is used for
% both; the difference of sort is recorded here, not typeset.
\divrule

% --- p. 216 ---
\opage{216} % p. 216 bears ONE note, mark *) (\setcounter below), self-contained.
% No Greek, no ɔ: mark, no guillemets.
\setcounter{footnote}{0}
% HEAD, p. 216 (L0, transcription.tex §3): "Charakteristik af Philosophiens
% historiske Hovedformer.", the third of the book's five L0 heads, centred
% and set on TWO lines exactly as broken here.
%
% NOT LETTERSPACED -- gaps 4--9 px, the body's unspaced value, against
% 36--51 px in the letterspaced L0 heads at pp. 1, 284 and 289.  That much
% of the pp. 211--224 batch's finding stands, and §3 is corrected
% accordingly.
% NOT BOLD, however -- the final pass removed the \textbf{} the batch put
% here.  The batch compared the head only with the body of the same page
% (stem 11--12 px on an x-height of 75 against 6 px on 40) and read the
% residual as weight.  It is not: normalised by x-height the head is
% 0.158--0.162 against a body 0.150, and under a threshold sweep the gap
% closes entirely (0.169 against 0.171 at 35 % of the ink depth), whereas
% the book's real bold sits at 0.27--0.30 at EVERY threshold.  Full
% reasoning and numbers in §3.
\begin{center}
{\Large Charakteristik af Philosophiens\\
historiske Hovedformer.}
\end{center}
% DIVISIONAL RULE, p. 216, printed BELOW the head and above the first
% paragraph: a single short CENTRED hairline, measured at 600 dpi at
% x 1516--1712 (196 px = 0.83 cm), against a text block of x 470--2790.
% This is a rule UNDER a head -- the pattern §3 records for the L1 head on
% p. 31 -- and it stands in addition to the rule at the foot of p. 215.
\divrule

% p. 216: the paragraph opens with an enlarged initial "V" of "Ved" (about
% 1.5 x body height, sitting on the baseline), as on p. 16.  Page-making
% furniture; set as ordinary text.
Ved Slutningen af det Afsnit, i hvilket den almindelige Lov\-%
mæssighed i Hovedepochernes Udvikling blev paaviist, blev\footnote{See
ovenfor S. 30.}
% "S. 30" read on the image at 400 dpi; tesseract gives "S. 80".
den concretere Bestemmelse af de enkelte philosophiske Ho\-%
vedformers Eiendommelighed udsat indtil de følgende Afsnit,
gjennem Behandlingen af væsentlige historiske Phænomener,
havde givet et Materiale til en fuldstændigere Charakteristik.
Paa dette Punkt optage vi nu den Opgave: at bestemme de
enkelte Hovedformer nærmere ud fra de for Charakteristiken
afgjørende Hovedsynspunkter, og idet indenfor den \emph{anden}
Hovedepoche en \emph{Flerhed} af saadanne Former fremtræder, søge
vi tillige, medens vi bestemme deres Eiendommelighed, at paa\-%
vise det \emph{væsentlige Fælleds og Gjennemgaaende} i de
forskjellige og mod hinanden kæmpende Retninger, der samler
dem alle til een Totalitet.

Vi søge da først de afgjørende \emph{Synspunkter for Cha\-%
rakteristiken af de enkelte Former}.

Af de tidligere Udviklinger er det indlysende, at Hoved\-%
synspunktet maa blive \emph{Principets} Bestemmethed. I de
% "Principets" is letterspaced; "Bestemmethed." on the same line is NOT
% (checked at 400 dpi).
enkelte philosophiske Hovedformers Princip fandt vi \emph{det
eiendommelige Udtryk for det Absolute}, saaledes som
dette, i Udfoldningen af sine Momenter, viste sig for den
% --- p. 217 ---
\opage{217} % p. 217 bears no note.  No Greek, no ɔ: mark, no guillemets.
menneskelige Bevidsthed, idet det i begrebsbestemt Form re\-%
sulterede af Tankens Tidsudvikling. I Principet havde vi den
enkelte Hovedforms \emph{Lov}, den bevægende Magt, der beher\-%
skede og bestemte dens Udvikling og i sig bar Spiren til de
enkelte Systemer, gjennem hvilke den udfoldede sit totale
Indhold. Gjennem de enkelte Systemers Principer saae vi
hele Stadiets almene Princip udfolde og klare sig: det havde
gjennem dem sin Vorden i Bevidstheden, sin Culmination, og,
som endnu \emph{ensidigt} Princip, sin Opløsning, gjennem hvilken
det mere Omfattende kom tilsyne, til hvilket det selv forholdt
sig som Moment, og af hvilket det som saadant selv beher\-%
skedes under sin Udvikling og i sin Undergang.

Dette \emph{Hovedsynspunkt} for Charakteristiken fører saa
til andre væsentlige Synspunkter. Ved Principet bestemmes
\emph{Tænkningens Retning} og \emph{Tænkningens Art}, med andre
\emph{Ord: Tankens væsentlige Gjenstand} og \emph{Tænkningens
Grundpræg og Methode}. Principet er Princip for \emph{Til\-%
værelsen som Heelhed, som concret Totalitet}. Men
paa de endnu \emph{fremskridende} Stadier medfører Principets
Bestemthed, som der tillige betegner en \emph{Begrændsning} og
en \emph{Ensidighed}, at \emph{Tilværelsen}, opfattet fra dets Syns\-%
punkt, sees fra \emph{een bestemt Side}, saaledes at denne væ\-%
sentlig hæver sig frem for Tanken og lader Tilværelsens andre
Momenter og Grundforhold sees ligesom \emph{gjennem sig}. I
hver af den philosophiske Udviklings tilbagelagte Hovedstadier
vil derfor en eiendommelig Tankens Retning, mod en eiendom\-%
melig bestemt Tankens Gjenstand kunne paavises. Først \emph{den
Hovedform}, der samler de ensidige Bestemmelser af Principet
til concret, modsigelsesfri Enhed, vil til sin Gjenstand i Vir\-%
keligheden faae Tilværelsen i dens concrete Fuldstændighed.

Ligesom Tankens Retning og Gjenstand, saaledes maa
ogsaa dens \emph{Grundpræg} og \emph{Methode} bestemmes ved Prin\-%
cipet. Principet er, idet det er \emph{Tilværelsesprincip}, tillige i
den Betydning \emph{Erkjendelsesprincip}, at det, som det Grund\-%
% "Grundværende," is NOT letterspaced in either half (checked at 400 dpi),
% though "Erkjendelsesprincip," on the same line is.
værende, der i sig omfatter \emph{Tanken og Tankens Gjen\-%
% NOTE: the letterspaced phrase "Tanken og Tankens Gjenstand" straddles the
% p. 217/218 page break, so there is deliberately NO blank line between the
% text above and the page marker below -- a blank line here would close the
% paragraph inside the open \emph{} group and the file would not compile.
% --- p. 218 ---
\opage{218} % p. 218 bears no note.  No Greek, no ɔ: mark, no guillemets.
stand}, bestemmer \emph{Aandens Væren som erkjendende}, alt\-%
saa \emph{Erkjendelsens Grundforhold}: dets eiendommelige Be\-%
stemthed som \emph{Tilværelsesprincip} bevarer det da ogsaa som
\emph{Erkjendelsesprincip}. Men Erkjendelsesprincipet, taget i
denne Betydning, som det \emph{Erkjendelsens Natur Bestem\-%
mende}, indeslutter Bestemmelsen af Erkjendelsesprincipet i
Betydning \emph{af det, der af Tanken sættes som Theoriens
Norm}, og dette Erkjendelsesprincip bestemmer atter den
\emph{Methode}, i hvilken Erkjendelsens \emph{Væsen} har sit \emph{Virksom\-%
hedsudtryk}. Til \emph{hver} Hovedform af Philosophien vil der
derfor svare et ved det eiendommelige Erkjendelsesprincip be\-%
tinget eiendommeligt \emph{Tænkningens Grundpræg}, og en
eiendommelig udformet \emph{Methode}.

Endelig betinger Principets Eiendommelighed en for hver
enkelt Hovedform eiendommelig \emph{Livsanskuelse}. Det Prin\-%
cip, der som Tilværelsesprincip bestemmer Opfattelsen af Til\-%
værelsesheelheden og derigjennem af Erkjendelsen, maa med
det Samme bestemme Opfattelsen af det \emph{totale} Menneskeliv
i dets udelte Enhed, af dets Indordning i Tilværelsestotaliteten,
af Normen for dets Retning, og af dets derved betingede defi\-%
nitive Opgave.

% THE FOUR-POINT SCHEME, p. 218.  This is a DISPLAYED list, not a run-in
% one: each item stands alone on its own line.  Measured at 600 dpi, the
% four item lines begin at x 526--538 against a left margin of x 476 and a
% paragraph indent of x 639 -- i.e. the items carry a SMALL indent, about a
% third of the paragraph indent -- while the run-over of item 4
% ("Livsanskuelse;") begins at x 628, the full paragraph indent, and the
% closing "og fra disse ..." returns to the flush left margin.  Leading is
% the body's throughout; there is no extra space between the items.  The
% enumerators are "1)" "2)" "3)" "4)".  Set with \\ plus explicit \hspace*
% so the print's two indent depths and its uniform leading are preserved,
% following the p. 186 list.  Nothing here is bold.
De forskjellige Hovedformer af Philosophien ville altsaa
blive at charakterisere\\
\hspace*{1em}1) ved deres \emph{Princip};\\
\hspace*{1em}2) ved deres \emph{væsentlige Gjenstand};\\
\hspace*{1em}3) ved deres \emph{Tænknings Grundpræg og Methode}; og\\
\hspace*{1em}4) ved den til deres Princips Eiendommelighed svarende\\
\hspace*{\parindent}\emph{Livsanskuelse};\\
og fra disse Synspunkter vil deres charakteristiske Eiendomme\-%
lighed i dens \emph{typiske} Udprægning i \emph{væsentlig Fuldstæn\-%
dighed} kunne bestemmes.

% DIVISIONAL RULE, printed at the FOOT of p. 218, below the last line of
% text and with white space above and below, standing over the L2 head that
% opens p. 219.  Single short CENTRED hairline, measured at 600 dpi at
% roughly x 1317--1817 (500 px = 2.1 cm) against a text block of
% x 460--2820.  p. 218 bears no footnote, so this is not the (left-flush)
% footnote separator.
\divrule

% --- p. 219 ---
\opage{219} % p. 219 bears no note.  No ɔ: mark.  One guillemet pair,
% «Tænkning af Tænkningen» (ll. 27--28), the normal INWARD pair, not
% letterspaced -- read at 1200 dpi.  (ABBYY reads the closing mark as «;
% it is ».)
% GREEK on this page, all read at 1200 dpi: Κόσμος five times and εῖδος once.
% Neither witness contains a Greek character; ABBYY gives "K 6 a/tog",
% "Kortbog", "Koopog", "Koer pog".
%
% HEAD, p. 219 (L2, transcription.tex §3): the FIRST of the three
% Hovedepocher (219, 226, 269).  BOLD, centred, one line.  MEASURED at
% 600 dpi: median stem width 11 px against 5 px in the body of this page;
% line height 87 px against 76 px, so the head is barely larger than the
% body but more than twice its weight.  Inter-letter gaps in the head are
% 3--10 px (word spaces 48--50), against 16--24 px in the certainly
% letterspaced body line "Heelhed, var Ideen (Formen) som det Verdenskunst-"
% two lines below, and 3--9 px in unspaced body words.  So: bold, NOT
% letterspaced -- \textbf{}, no \emph{}.  Set \large, as the other L2 heads.
\begin{center}
\textbf{\large Første Hovedepoche: den græske Philosophie.}
\end{center}

% THE FOUR-PART RECURRING SCHEME, pp. 219 and 224.  Unlike the p. 218 list
% and unlike the p. 186 list, items 1.--4. here ARE genuinely RUN-IN: each
% number stands at the ordinary paragraph indent at the head of an ordinary
% justified paragraph, with the text running on after it on the same line
% (checked on the image for 1., 2., 3. and 4.).  The enumerators are
% "1." "2." "3." "4.", not "1)" etc., and the phrase after each is
% letterspaced, not bold.
1. \emph{Principet} for den græske Philosophie, taget i dens
\emph{Heelhed, var Ideen (Formen) som det Verdenskunst\-%
værket} (\gk{Κόσμος}) \emph{Besjælende}. I denne Principets Be\-%
stemmelse have vi fundet Udtrykket for \emph{Aandens} (det Ideales)
\emph{Herredømme}, men \emph{umiddelbare} Herredømme, \emph{over det
Naturlige} (det Reale), i \emph{Skjønhedens} (Harmoniens) Form.
Anskuelsen af Tilværelsen \emph{som \gk{Κόσμος}} udtrykte dette \emph{Umid\-%
delbarhedsforhold}, som vi see afpræge sig i Bestemmelsen
af det herskende Ideelle. I det Ideelles (det Aandeliges) Op\-%
fattelse som \emph{Idee} (\gk{εῖδος}, Form, \emph{Urbillede}) reflecterer sig,
% p. 219: in εῖδος the mark over the iota is a broad tilde with a hook at
% its left; the CIRCUMFLEX is positively there, but a smooth breathing
% cannot be separated from it at 1200 dpi.  Set with the circumflex alone;
% ἶ/ῖ UNDECIDED, and nothing is supplied from knowledge of Greek
% (BATCH-AGENT §4 rule 2).  Compare Νοῦς twelve lines below, where the mark
% is a clean tilde with no hook and no breathing is expected.
gjennem det \emph{Anskuelsesmoment} i Tanken, der træder frem
i denne Betegnelse, det Naturliges \emph{umiddelbare Væren};
og i Anskuelsen af det \emph{intelligible Rum}, i hvilket Ideens
Verden udbreder sig som en ny Verden for Aandens Øie, af\-%
præger sig, ligesom afklaret, det Naturliges \emph{Udvorteshed}.

Endnu hos \emph{Aristoteles} er, trods hans Stræben efter at
see Formen som \emph{Energie} og at drage den ind i det For\-%
mede, dog hiin umiddelbare Væren og hiin Udvorteshed be\-%
varet i Opfattelsen af Formen som det fra Vorden Unddragne,
og i den guddommelige Forms Rumssondring fra \gk{Κόσμος}.
Selv idet \emph{Ideen}, ved at vexle Navn og kalde sig \gk{Νοῦς}, ud\-%
taler sit bag det specifisk græske Navn skjulte \emph{almene} Væ\-%
% "almene" is letterspaced but "sen," at the head of the next line is NOT
% (checked at 1200 dpi); the emphasis stops at the line break.
sen, bevarer den dog sit eiendommelig græske Grundpræg
derved, at den «Tænkning af Tænkningen», i hvilken Gjen\-%
standen bliver selve den Tanke, der er Tilværelsens Væsen og
Energie, som Tankens forskjelsløse og bevægelsesløse Tænken
af sig selv, \emph{kun} udtrykker den \emph{Anskuelsens Hvilen}, i
hvilken \emph{Harmoniens Umiddelbarhed} afspeiler sig.

2. \emph{Den væsentlige Gjenstand}. Denne bliver, naar vi
see hen til det for den \emph{udfoldede} græske Philosophie \emph{Typiske},
at bestemme som det, man kunde betegne som \emph{den ideelle
\gk{Κόσμος}}, idet denne Betegnelse, der nærmest viser hen paa
\emph{Ideernes, Formernes \gk{Κόσμος}}, tillige kan udtrykke deels
% --- p. 220 ---
\opage{220} % p. 220 bears no note.  No ɔ: mark.  One guillemet pair, round the Greek
% quotation in l. 29, the normal INWARD pair.
% GREEK, all read at 1200 dpi: Κόσμος four times, Νοῦς once, and the
% quotation ἡ περὶ ὅλον τὸν κόσμον τάξις.  In the quotation the rough
% breathing over the η is unmistakably a "C" opening to the right; the
% grave over the ι of περὶ and over the ο of τὸν slopes down-left; the mark
% over the ο of ὅλον is that same "C" with an acute beside it.  The rho is
% printed as the variant sort ϱ; plain ρ is typed (BATCH-AGENT §4 rule 4).
\emph{Aandssiden} (Idealitetssiden og det Teleologiske) i \emph{Natur\-%
tilværelsen}, deels det \emph{i en omfattende Heelheds Form
organiserede Menneskeliv}.

Ifølge den umiddelbare Enhed, der var tilstede mellem
Aand og Natur i Verdenskunstværket, blev Tankens Gjenstand
i Naturen væsentligt \emph{det Ideale}, der aabenbarede sig som
\emph{det Herskende}, og \emph{umiddelbart} saaes i det Enkelte som
enkelt Form, i Natursammenhængen som Formernes teleologisk
anlagte, i Menneskeaanden afsluttede Heelhed. Naturopfattelsen
maatte faae den \emph{metaphysiske} Charakteer, som vi tidligere
have fremhævet, idet det, der saaes i Naturen, var \emph{det Ideelle},
der, netop ifølge sit Umiddelbarhedsforhold til det Reale, be\-%
varede sit \emph{abstractere rent metaphysiske} Præg. Og
idet Tanken i den anskuede Natur fulgte Formernes Række
indtil den i \emph{Menneskeaanden} aabenbare Form, saa viste
den \emph{individuelle} Aand, ifølge en tidligere udviklet, i Ideens
almene Væsen liggende Nødvendighed, sig som \emph{umiddelbart
indordnet i og underordnet Aandens \gk{Κόσμος}}, der finder
% p. 220: this Κόσμος IS letterspaced (the Greek letters carry the same
% wide gaps as "Aandens" before it, read at 1200 dpi); the four other
% Κόσμος on pp. 220--221 are not.  The distinction is visible and is
% followed rather than regularised.
sin Skikkelse i \emph{Staten}: Mennesket i det Store. Paa samme
Maade som den enkelte Naturting umiddelbart ligesom saaes
ind i sin almene Form, saaledes sees det enkelte Menneske\-%
individ umiddelbart ind i det aandeligt Almene, saaledes som
dette fremtræder i ydre Virkelighed. Vel lader en gammel
græsk Philosoph Hengivelsen til \emph{Naturanskuelsen} være Aan\-%
dens Maal og Tænkningens Opgave, og fører derved den En\-%
kelte hen til \emph{Naturens}, ikke til \emph{Aandens} \gk{Κόσμος}; men
det, Grækeren søger at anskue gjennem Naturens Phænomener,
er just «\gk{ἡ περὶ ὅλον τὸν κόσμον τάξις}» som en af \emph{Aanden}
(\gk{Νοῦς}) fremgaaende Orden, og gjennem denne søger den
græske Aand sin \emph{egen} \gk{Κόσμος}, der, \emph{som et Aandens Værk}
har Virkelighed i det lovordnede \emph{Statssamfund}.

Men naar saaledes det \emph{Ideelle} blev den græske Tankes
væsentlige Gjenstand, der lod det \emph{Reale} overstraales af sit
Lys, saa ligger der i dets \emph{umiddelbare} Enhedsforhold til
det Naturlige tillige den Consequents, at \emph{Naturelementets}
% --- p. 221 ---
\opage{221} % p. 221 bears no note.  No ɔ: mark, no guillemets.
% GREEK: Κόσμος four times and Νοῦς once, all read at 1200 dpi; none of
% them letterspaced (the letters are tight where the surrounding Danish is
% spaced).
\emph{Umiddelbarhed og Begrændsning uvilkaarligt gjør
sig gjældende i Opfattelsen af den ideelle \gk{Κόσμος}}.
Hvorledes Umiddelbarheden gjør sig gjældende, idet den ideelle
\gk{Κόσμος} sees som en i \emph{ydre} Skikkelse for Anskuelsen værende
\gk{Κόσμος}, er allerede ovenfor viist. Naturelementets \emph{Begrænds\-%
ning}, der er uadskillelig fra dets Umiddelbarhed, og hvis
Indvirkning paa Opfattelsen af det individuelt Aandelige vi
tidligere have angivet, see vi at gjøre sig gjældende som bli\-%
vende Bestemmelse \emph{i det Menneskeliges Totalitetsform,
gjennem det alment Humanes Begrændsethed ved
Statssamfundets naturbestemte Skranker}.

3. \emph{Tænkningens Grundpræg, Methoden}. Idet \emph{Ideen}
er Værens og Videns Princip, og idet den \emph{menneskelige
Aand} erkjendes som \emph{væsenslig} med den, er herved \emph{Tan\-%
kens Grundpræg} bestemt. Den \emph{tænkende} Menneskeaands
\emph{Bevidsthed om sin Væsensenhed med det Ideelle},
der er \emph{Værensprincipet}, det Tilværelsen Besjælende, finder
sit \emph{prægnanteste} Udtryk i den \emph{Aristoteliske Lære} om
den \emph{active} \gk{Νοῦς}, om dennes \emph{umiddelbare} Erkjenden af
de høieste Vidensprinciper, og om dens Tænken af sig selv i
sin Gjenstand; men den aabenbarer sig allerede gjennem den
for den græske Tænkning eiendommelige \emph{Objectivitet}, ved
hvilket Udtryk vi paa den ene Side betegne den individuelle
Tankes reflexionsløse Underordnen under Tankens \emph{almene
Loves} Nødvendighed; paa den anden Side dens reflexions\-%
løse Hengivelse til sin Gjenstand, i hvilken Hengivelse et
Tankens \emph{harmoniske} Forhold \emph{til sit Andet}, som det af
% "Tankens" and "Forhold" on this line are NOT letterspaced, though
% "harmoniske" and "til sit Andet," are (checked at 800 dpi).
\emph{Tankens Princip} umiddelbart Beherskede og Bestemte, af\-%
speiler sig.

Ligesom Tankens Grundpræg i disse Punkter var bestemt
ved Principets Eiendommelighed, saaledes bliver denne ogsaa
normerende idet Tankens Indhold skal træde ud i en Kreds af Be\-%
greber, idet Begreberne skulle stilles i Forhold til hverandre, og
idet Tanken vender sig mod det Reale, forat optage det i sig;
altsaa med eet Ord: under hele \emph{Tankebevægelsen}.

% --- p. 222 ---
\opage{222} % p. 222 bears no note.  No Greek, no ɔ: mark, no guillemets.
Hvad der ved Udviklingen af \emph{Tankens Indhold} og ved
Bestemmelsen af \emph{Begrebernes indbyrdes Forhold} bliver
det Afgjørende, er det i Principets Bestemmelse som Idee frem\-%
trædende \emph{Anskuelseselement}, som vi have seet bevaret selv
hvor hos \emph{Aristoteles} den virksomme Tankes Begreb trængte
sig ind under Begrebet af Formen. Dette Anskuelseselement, i
hvilket Umiddelbarheden af Grundmomenternes Enhed reflec\-%
terer sig, er \emph{blivende} sammensmeltet med den græske
Tænkning og bestemmer den i dens Bevægelse. Vi have seet
dets Indvirkning paa et afgjørende Punkt: ved Bestemmelsen
af Grundmodsætningernes Forhold. Vi saae det, der umiddel\-%
bart var forenet for Anskuelsen, sondres ved den analyserende
Tanke, og idet saa det Sondrede atter gik ind under Anskuel\-%
sens Former, at fæstnes i Sondringen saa at det ikke atter
kunde forenes. Det anskuede Værende og det anskuede Ikke-%
% "Ikke-Værende" is a PRINTED compound hyphen broken at the line end; a
% real hyphen, not \-.
Værende stod hos \emph{Plato} som hvilende, ubevægelige Modsæt\-%
ninger. Og hvad der gjælder om denne Grundmodsætning,
faaer overhovedet Gyldighed ved alle Modsætninger: den græ\-%
ske, til Anskuelsen knyttede, Tanke fæstner dem, idet den
skal bestemme deres Forhold, i ubevægelig Sondring: den
græske Dialektik bliver \emph{Distinctionsdialektik}, og dens
Opgave bliver netop at \emph{hævde} Sondringen, at klare den ud
af det Fordunklende, der kunde synes at lade det Adskilte
forenes, Modsætningerne sammensmeltes.

Og ligesom Anskuelsen indvirker afgjørende i dette For\-%
hold, saaledes maa den virke, naar Tankeindholdet skal udfoldes.
Bestemt af en i Anskuelsen hvilende Tanke blev Begrebet
\emph{ubevægeligt}; det fik ingen indre Udfoldelses Mulighed i sig.
Idet den \emph{platoniske Idee} antog Skikkelse af en \emph{Verden af
Ideer}, var dette ikke ifølge en indre Udfoldelse, men ved
Hjælp af en Induction, der tog sit Udgangspunkt fra den
uforklarede Tilværelsens Mangfoldighed, og Ideeverdenen i
dens Concretion blev en Verden af \emph{hvilende, adskilte
Skikkelser i det intelligible Rums} ligesom \emph{abstracte
Udvorteshed}. Og hos \emph{Aristoteles} saae vi, trods hans
% --- p. 223 ---
\opage{223} % p. 223 bears no note.  No Greek, no ɔ: mark, no guillemets.
Stræben efter at hævde en Tankens indre Bevægelse, den
samme Formens Ubevægelighed.

Idet Anskuelsens Indvirkning i denne Henseende sees
fra det Synspunkt, at hos Grækerne \emph{Negationen}, den fri\-%
gjorte Tankes specifiske Redskab, ved den holdtes ude fra det
\emph{Værende}, føres herved Indvirkningen paa Tankeudfol\-%
delsen tilbage til Enhed med den Indvirkning, der viste
sig i Dialektikens Adskillen. Ved hiin Udelukken af Nega\-%
tionen, ved hvilken Distinctionsdialektikens \emph{Grundsondring}
var sat, var Incitamentet for en Identitetsdialektik tilintetgjort,
den Udviklingsmulighed ophævet, der er en ved Negativiteten
betinget Mulighed til en Sætten af iboende Modsætninger og
en Gjenophæven af dem til Enhed. Hos \emph{Plato} anerkjendtes
Ikke-Væren kun tilsyneladende i Ideen; den blev kun den
sammenlignende Reflexions Bestemmelse, Udtrykket for det i
ydre Relation bragte positivt Værendes \emph{Forskjel}. Hos \emph{Ari\-%
stoteles} er den implicite anerkjendt i Mulighedens Begreb;
men idet Formen ikke berøres af Muligheden, ikke gaaer ind
i Vorden, bliver det Ideelle ogsaa hos ham det rent (positivt)
Værende, og den høieste Forms Energie bliver et blot positivt
og netop derfor formelt og abstract Forhold til sig selv.

I \emph{Tankens Forhold til det Reale} bliver Ideens
\emph{umiddelbare} harmoniske Behersken af Modsætningen be\-%
stemmende. Den finder sit Udtryk i det Reales umiddelbare
Bestemmen ved Begrebet, det Enkeltes ved det Almene, hvilken
vi ogsaa kunde betegne som den \emph{metaphysiske} Bestemmen
af det Reale. Idet Tilværelsens Heelhed sees i Kunstværkets
Lys, som den umiddelbare harmoniske Enhed af Form og Stof,
bliver dette Synspunkt Synspunktet for Opfattelsen af hvad
der faaer Virkelighed indenfor Verdenskunstværket som et i
dettes Heelhed indordnet Led. Det sees fra det Synspunkt,
fra hvilket Harmonien \emph{umiddelbart} viser sig og det Ideelle,
Teleologiske, træder aabenbart frem for Tanken. Men saaledes
sees det kun i den Forklarethed, i hvilken --- ved en Idea\-%
lismens Illusion --- det Reale er blevet et gjennemsigtigt
% --- p. 224 ---
\opage{224} % p. 224 bears no note.  No ɔ: mark.  One guillemet pair, «ideelle Κόσμος»
% (l. 13), the normal INWARD pair.
% GREEK, read at 1200 dpi: Κόσμος once (not letterspaced) and τέλος once
% (acute over the ε, no breathing -- τ is a consonant).
% SEAM: this page ends mid-sentence ("... af det Naturlige i sig, at") and
% p. 225 opens "gjøre sig selv til et individuelt Afbillede af Κόσμος," --
% NO paragraph break at the batch boundary, and NO run-over footnote
% (neither p. 224 nor p. 225 bears a note).
Medium, der gjennem sig \emph{kun} lader Formen tilsyne. \emph{Natur\-%
realitetens Enkelthed} er ligesom løftet et Trin opad og
luttret til \emph{den skjønne Individualitets typiske Form}.
Hvad der sees i det Reale, bliver da \emph{umiddelbart} Formen,
men Formen, saaledes som den, som den \emph{bestemte} Exi\-%
% "bestemte" is letterspaced; "Exi-" at the end of this line and
% "stentses Form," at the head of the next are NOT (checked at 800 dpi).
stentses Form, viser sig i den Sondring, der, som vi ovenfor
saae, var den charakteristiske Form for Ideernes (Formernes)
indbyrdes Forhold. Den reale Tilværelse viser, gjennem en
gjennemsigtig Overflade, i sin Grund det Ideelle som \emph{Ideer},
i den af en Harmoniens Enhed omsluttede Sondringens Form,
de have som Ideeverden, og saaledes falder den dobbelte Be\-%
tydning af den «ideelle \gk{Κόσμος}»: som \emph{Ideeverdenen}, og
som \emph{Naturens Idealitetsside}, sammen.

4. \emph{Livsanskuelsen}. Denne kan med eet Ord betegnes
som \emph{Skjønhedsidealitetens Livsanskuelse}. Denne
dens Charakteer er given ved Grundtankens Bestemthed og
ved den derved betingede Verdensopfattelses Art. Til Begrebet
om Ideen som det Herskende svarede Opfattelsen af Tilvæ\-%
relsen som det i sig afsluttede, altomfattende, guddommelige
Kunstværk, der realiserede Livets Idee (jfr. Plato og Aristote\-%
les), og i hvilket Formen og Materien vare det harmonisk
Værendes kunstnerisk forbundne Elementer. I dette, i sin
Guddommelighed hvilende, Universum vare alle Livsformer
sammenknyttede i en Trinrække, i hvilken intet Led kunde
løsrives fra de andre, men ethvert høiere forholdt sig til det
lavere som til sin nødvendige Forudsætning, medens ethvert
lavere med Nødvendighed stræbte hen mod det høiere som
sit Maal, og \emph{alle lavere} mod \emph{Menneskelivet} som deres
\gk{τέλος}. Men heri ligger saa en dobbelt Consequents for Be\-%
stemmelsen af Livsanskuelsen. \emph{Naturen} maa opfattes som
\emph{Menneskelivets Grundlag}, og Menneskelivets \emph{sædelige
Udvikling} maa sættes som lig med Udviklingen af de \emph{natur\-%
lige} Kræfter; men denne Udvikling maa saa være \emph{med Be\-%
vidsthed ideebestemt}: Menneskets Opgave maa blive den,
gjennem en harmonisk \emph{Formen} af det \emph{Naturlige i sig}, at
% =====================================================================
% BATCH pp. 225--238.  Page map +11 throughout (printed N = PDF N+11),
% obtained from pagemap.py for every page; no offset added by hand.
%
% SEAM, HEAD: p. 225 begins IN MID-SENTENCE.  p. 224 ends
% "...gjennem en harmonisk Formen af det Naturlige i sig, at" and p. 225
% opens "gjøre sig selv til...".  There must be NO paragraph break at the
% splice point (joints.py).  p. 224 bears no note and p. 225 has NO note
% area at all, so nothing runs over into this batch.
%
% SEAM, FOOT: p. 238 ends with the broken word "Consequent-" and p. 239
% opens "sen af Kants Kritik der reinen Vernunft."  The fragment therefore
% ENDS with "Consequent\-%" and the pp. 239-- batch must open with "sen af"
% with no space and no paragraph break.  p. 238 bears no note, so no note
% runs over the foot.
%
% RUN-OVER FOOTNOTE INSIDE THIS BATCH: p. 234's note *) is NOT finished on
% p. 234.  Its tail occupies the first five lines of p. 235's note area,
% ABOVE p. 235's own *), and carries no mark.  Per the header of
% transcription.tex the whole note is set in one \footnote{} at its call on
% p. 234, and a % comment at the p. 235 page break records that it continues
% there in the print.
%
% MEASUREMENTS.  Every emphasis call below was decided by measuring
% inter-letter gaps at 900 dpi (4428x7173 px page; body x-height 60--76 px,
% body stroke 10--11 px).  Ordinary roman runs 2--11 px between letters
% (median 5--8); letterspaced runs 16--30 px (median 18--25).  The two
% distributions do not overlap on any page of this batch.
% =====================================================================
% --- p. 225 ---
\opage{225} % p. 225 bears NO note (no note area, no rule) and carries the sheet
% signature "15" at the foot, not transcribed.
%
% GREEK, the only Greek in this batch: \gk{Κόσμος}, l. 1.  Read at 900 dpi.
% Kappa capital; acute over omicron plainly visible (up-right stroke, not a
% grave and not a circumflex); final sigma is the ς sort.  No breathing.
% It is ALSO LETTERSPACED: its inter-letter gaps are 11--25 px (median 21)
% against 9 px for the same word set unspaced at p. 186 (measured on the
% p. 186 render for the comparison), so it takes \emph{\gk{...}}.
% The letterspaced run of l. 1 begins at "et", not at "til": measured gaps
% are til 3,7 / et 23 / individuelt 18--27 / Afbillede 21--27 / af 25.
gjøre sig selv til \emph{et individuelt Afbillede af} \emph{\gk{Κόσμος}},
til et levende, bevidst Kunstværk, der gjennem \emph{Tanken}, den
\emph{høieste} Livsform, er i Livsenhed med det Tilværelsen be\-%
sjælende Princip.

Idet Tilværelsesheelheden opfattes som uforstyrret og
mangelløs, maa ogsaa \emph{Menneskevæsenet} opfattes som saa\-%
dant. Gjennem Tanken --- gjennem \emph{Erindringen}, som
\emph{Plato} vilde udtrykke det --- maatte Menneskeaanden ved \emph{egen}
% "Kraft" on the next line is NOT letterspaced (gaps 5--10); only "egen" is.
Kraft kunne naae det Høieste (Theorien), naae Actualiseringen
af sit eget fuldstændige Væsen, der ialfald kun var som til\-%
sløret ved Existentsen, ved Forbindelsen med Materien. Om
en \emph{Menneskenaturens Fordærvelse} ved Synden, og om
vidunderlige Indvirkninger af \emph{overnaturlige} Magter paa
Menneskets Handlen, kunde der indenfor den consequent græske
Livsanskuelse ikke blive Tale. ---

Hiin Anskuelse af Verdenskunstværkets Guddommelighed
maatte ogsaa, \emph{consequent} fastholdt, medføre, at det indivi\-%
duelle Livs Maal kunde sættes \emph{indenfor} den af det Gud\-%
dommelige opfyldte nærværende Tilværelse, i hvilken der alle\-%
rede kunde leves et \emph{evigt} Liv i det \emph{Tankens} Liv, der bragte
Aanden i Enhed med den guddommelige Tanke, som var
Tilværelsens iboende Livsprincip.

Hvorledes Opfattelsen af det \emph{ethiske Liv} bestemtere
formede sig ud fra denne Grundanskuelse; hvorledes der i den
laae en Skranke for den ethiske Bevidsthed, en Begrændsning
for Personlighedens uendelige Selvfordybelse, og hvorledes den
maatte føre til en eiendommelig Bestemmen af det ethiske
Individs Forhold til det upersonlige Almene, er paaviist i det
tidligere Udviklede. Der er ogsaa den i al umiddelbar Skjøn\-%
hedsharmonie skjulte Illusion paaviist, der uundgaaelig maa
røbe sig for Tanken, som, idet den gjør den aabenbar, bringer
Splid ind i Bevidstheden, forvandler Skjønhedens Hvile til
Fortvivlelsens Uro, og selv bukker under for Spliden, idet den
ikke mægter at forsone, men kun at adskille. Af denne
Skjønhedsumiddelbarhedens Opløsning fremgaaer nu den nye
% p. 225/226 SEAM: the sentence runs on; NO paragraph break here.
% --- p. 226 ---
\opage{226} \setcounter{footnote}{0}
Form af Tænkning, der paa en anden Grundvold, ud fra an\-%
dre Forudsætninger, begrunder en anden Hovedepoche i Phi\-%
losophiens Historie.

% DIVISIONAL RULE, p. 226: short, CENTRED, white space above and below,
% standing between the end of the paragraph and the L2 head.  Measured at
% 900 dpi: x 2123--2857, i.e. 734 px = 2.07 cm, centre 2490 against a text
% block centre of 2449 (block x 790--4108); a plain even hairline.  It is
% NOT the footnote separator, which stands lower on this page, is left-flush,
% and is LaTeX's job.
\divrule

% L2 HEAD, p. 226 (transcription.tex §3): the SECOND of the three
% "Hovedepoche" heads (219, 226, 269), and THE ONLY HEADING IN THE BOOK THAT
% BEARS A FOOTNOTE MARK.  The mark stands between "Philosophie" and the full
% stop -- the printed line reads "... Tids Philosophie*)." -- so the
% \footnote{} is set at that point and the period follows it.
%
% BOLD, NOT LETTERSPACED.  Measured at 900 dpi:
%                       median stroke   x-height   intra-word letter gaps
%   p. 226 head            20 px          76 px          2--6 px
%   p. 226 body l. 1       11 px          69 px          2--11 px (med. 7)
%   p. 226 body l. 9       11 px          64 px          2--11 px
% stroke/x-height = 0.26 for the head against 0.16--0.17 for the body, and
% the head's letter gaps are no wider than the body's.  So \textbf{}, not
% \emph{}, per BATCH-AGENT §3.  It fills the measure but is set centred.
% FINAL PASS: \large added (x-height 45 px against a body 39, the L2 size);
% and the head's \footnote{} now opens with \normalfont\normalsize, because
% a \footnote inside \textbf{\large ...} inherits the bold and the size and
% would set the whole note bold -- a defect that compiles clean.
\begin{center}
\textbf{\large Anden Hovedepoche: den nyere Tids Philosophie\footnote{\normalfont\normalsize Med Hensyn til \emph{Overgangsperioden} henvise vi til de tidligere
Afsnit. Dens Eiendommelighed bestaaer i en Eftervirkning af det
Antike og en Anticipation af det Moderne, der ikke ere sammen\-%
arbeidede til organisk Enhed; og netop fordi den virkelige indre
Enhed mangler, have vi her ikke nogen selvstændig Grundform af
Philosophien.}.}
\end{center}
% The note's own letterspacing: only "Overgangsperioden" (gaps median 20 px
% in the note's smaller fount, against 3--10 px in the rest of the note).

Naar den \emph{nyere Tids Philosophie} tages som \emph{Enhed}
og \emph{Heelhed}, og Fordringen altsaa bliver, at paavise det, der
som det \emph{gjennemgaaende bestemmende Almene}, som
% "og" between "Enhed" and "Heelhed" is NOT spaced (gaps 8); the compositor
% spaced the two nouns only.
det blivende Grundpræg, faaer Gyldighed for \emph{Alt}, hvad der
sammenfattes derunder, saa viser der sig en eiendommelig
Vanskelighed. De to Hovedrækker, der i den nyere Tids Phi\-%
losophie staae som sideordnede, vise sig ved første Øiekast
som Modsætninger naar der sees hen til Principets Bestem\-%
melse; som grundforskjellige i Tankens Retning og Tænknin\-%
gens Form. De to Hovedformer indenfor den \emph{idealistiske}
Række: den \emph{dogmatiske} og den \emph{kritiske} Idealisme, vise
ligeledes en iøinefaldende Ulighed i Erkjendelsens Fremgangs\-%
maade, i Tænkningens Brug af sine Former. Og indenfor hver
enkelt Række og hvert enkelt Hovedafsnit gjøre fremtrædende
Forskjelligheder sig gjældende. Men dog skjuler sig bag al
den iøinefaldende Forskjel en Overensstemmelse, saa væsent\-%
lig, at den behersker hiin og berettiger os til at sammenfatte
de mange forskjellige philosophiske Phænomener, der i den
nyere Tid ere fremtraadte i sondrede Rækker, i een om\-%
fattende Heelhed.

Den Overensstemmelse, der finder Sted mellem \emph{alle} de
enkelte Hovedformer indenfor den anden Hovedepoche maa,
ifølge Hovedepochens hele Charakteer, ifølge den \emph{dualistiske}
% --- p. 226/227 SEAM: no hyphen; "dualistiske" is complete at the foot of
% p. 226 and "Sondring," (NOT letterspaced, gaps 8) opens p. 227.  The
% sentence runs on: no paragraph break here.
% --- p. 227 ---
\opage{227} Sondring, der charakteriserer Udviklingen i den, være en \emph{for\-%
mel} Overensstemmelse, hvad Bestemmelsen af \emph{Principet},
% "formel" is letterspaced ACROSS the line break: "for-" gaps med. 15,
% "mel" gaps 21, against 6--9 in the surrounding roman.
af \emph{Tankens Gjenstand} og af \emph{Livsopgaven} angaaer, og
kun med Hensyn til \emph{Tænkningens Art} kan den, netop fordi
Formen her er selve Sagen, det Væsentlige, blive en \emph{real}
Enhed. Men den \emph{formelle} Lighed i Bestemmelsen af hine
% "Enhed." at the head of the line is NOT spaced (gaps 9); only "real" is.
Punkter bliver en saadan, der viser hen til de \emph{reale} Modsæt\-%
ningers \emph{gjensidige Suppleren}, altsaa til en \emph{Enhed}, der
% "Modsætningers" is NOT spaced (gaps 7/6) although "reale" before it and
% "gjensidige" after it are.
omfatter det Sondrede.

Overensstemmelsen viser sig først og afgjørende ved
Betragtningen af \emph{Principet}, og den blev med Hensyn til
dette allerede i det Væsentlige paaviist ved Bestemmelsen
af Hovedepochernes almindelige Forhold til hverandre. Vi
saae der denne Epoche bestemt ved det Grundforhold i
Videns Idee, i hvilket Modsætningen mellem Videns subjective
Idealitetsbestemmelse og Objectivitetens Realitetsbestemmelse
gjorde sig gjældende \emph{som} Modsætning, og denne Modsætning
% "som" alone is letterspaced here (gaps 21) while "Modsætning," beside it
% is not (gaps 7).  Odd but printed; confirmed on the image at 900 dpi.
saae vi reflectere sig i \emph{den endelige Bevidstheds Dua\-%
lisme}, og som \emph{principiel Modsætning} mellem \emph{Tanken}
og \emph{Tankens Andet} bestemme den \emph{Enhedsstræben}, der
% "og som" between "Dualisme," and "principiel" is NOT spaced (gaps 4/9);
% neither is "mellem" (7) nor the "og" before "Tankens Andet" (9).
med Nødvendighed fremgik \emph{af Selvbevidsthedens Enhed.}
Principet bliver derfor, under \emph{alle} Hovedformer af den nyere
Philosophies Udvikling, \emph{ensidig} bestemt ved den ene af
Grundmodsætningerne, og derfor saaledes, at det, idet det som
Princip tænkes som høieste \emph{omfattende} Enhed, kun har
Modsætningens sammenhørende Led \emph{som uforenede} i sig,
og altsaa kun viser sig som \emph{omfattende} Enhed fra det Syns\-%
punkt, hvor Modsætningens ene Led ligesom \emph{dækker det
andet}; medens dog dette kun er skjult, men ikke tilintet\-%
gjort eller virkelig forenet med hint.

Med Hensyn til Tankens \emph{væsentlige Gjenstand}, viser
Overensstemmelsen sig i den formelle Lighed, at det i Prin\-%
cipet ensidigt fastholdte Moment: Naturen eller Aanden, en\-%
sidigt drager Tanken til sig. Men idet Modsætningen uund\-%
gaaeligt hævder sig, faaer Tanken til Gjenstand \emph{den duali\-%
% --- p. 228 ---
\opage{228} \setcounter{footnote}{0}%
stisk adskilte, men i Deltheden ved det ene Mo\-%
ments Overvægt bestemte Tilværelsestotalitet.} Hvad
% The letterspaced run "den dualistisk adskilte ... Tilværelsestotalitet."
% RUNS ACROSS THE PAGE BREAK: every word of p. 228 ll. 1--2 down to
% "Tilværelsestotalitet." is spaced (gaps 20--25) and "Hvad" after it is not
% (gaps 5).  The \setcounter line carries a % so that "duali-" and "stisk"
% join with no space (TRANSCRIPTION-PLAYBOOK §6).
enten Udgangspunktet tages fra Naturen eller fra Selvbevidst\-%
heden, saa bliver Grundforholdet det samme: paa Grund af
den oprindelige principielle Modsætning er Dobbeltheden bli\-%
vende medens Enheden vel tilstræbes, men \emph{kun tilsynela\-%
dende} naaes, idet den factisk i dualistisk Sondring blivende
Tilværelse ensidig sees i sit ene Grundmoments Lys\footnote{Dette gjælder ogsaa om de Former, hvor en \emph{Parallelisme} sta\-%
tueres (Spinoza, Schelling), hvor altsaa \emph{Enheden} sees \emph{bag ved
de tilsyneladende parallele} Modsætninger. Ogsaa her er nem\-%
lig Parallelismen ikke fuldstændig: det \emph{ideelle} Moment har Over\-%
vægten og Enheden sees definitivt fra dets Synspunkt.}.
% This is the note the Register cites as "Attributernes Parallelisme: 228,
% Anm."  Read word by word at 900 dpi; ABBYY's "livor" for "hvor" and
% tesseract's silence on the letterspacing were both corrected on the image.

Hvad dernæst \emph{Livsanskuelsen} angaaer, saa er Forhol\-%
det med Hensyn til denne analogt med Forholdet med Hensyn
til Tankens Gjenstand. I den umiddelbare Fremtrædelsesform
synes Realismens og Idealismens Livsanskuelser at staae imod
hinanden \emph{kun} som Modsætninger. Naar \emph{Realismens} Livs\-%
% "Livsanskuelse" is NOT spaced ("Livs-" gaps 9, "anskuelse," 7) although
% "Realismens" before it is (22).  Checked twice at 900 dpi.
anskuelse, der begrændsede Menneskelivets Skueplads til Na\-%
turen, satte det ved Erkjendelsen af Naturen og ved «Lydig\-%
heden» mod den erhvervede Herredømme over Naturens Kræf\-%
% Guillemets «Lydigheden»: the normal INWARD pair, read at 900 dpi.
ter, og en ved dette Herredømme fremmet Nyden af sig i sin
naturbestemte atomistiske Enkelthed som Menneskets Maal;
saa satte den \emph{dogmatiske Idealisme}, for hvilken Substant\-%
sen i sin Uendelighed var det Absolute og derfor Uendelig\-%
heden Menneskets Livselement, Maalet i en Erkjendelse af
Substantsens Væsen, der fører til Bevidstheden om den evige
Enhed med den og til den frie Hengivelse til Tilværelsens
evige Orden\footnote{Ogsaa hos \emph{Leibnitz} træder, under Opfattelsen af Monadernes \emph{selv\-%
stændige} Udvikling, dette Grundforhold frem, idet Monadernes
Udvikling, som bestemt ved den \emph{omfattende Harmonie}, afspei\-%
ler \emph{Universet}.}, og den \emph{speculative Idealisme}, der lader
Ideens evige Proces omfatte al Virkelighed, sætter Maalet i et
Tankeliv i Evighedens Element, i hvilket den individuelle Aand,
% "i hvilket", not "i hvilken": ABBYY reads "hvilken", the image reads
% "hvilket" plainly at 900 dpi.
% p. 228/229 SEAM: the sentence runs on; NO paragraph break here.
% --- p. 229 ---
\opage{229} \setcounter{footnote}{0}
hvis Naturbestemthed kun er et Usandt, absorberes i det Ab\-%
solute. Her er Modsætningen mellem de to Rækker umiddel\-%
bart iøinefaldende; men under den skjuler sig den formelle,
en real Sammenhæng antydende Lighed, at baade Realismens
og Idealismens Livsanskuelse, i Overensstemmelse med Prin\-%
cipet, begrændsede Menneskelivets Opgave til \emph{een Side af
Tilværelsen}, til \emph{een Værensform}; men saaledes, \emph{at i
denne Begrændsning det Udelukkede uimodstaae\-%
ligt trængte sig ind} og gjorde Livet uconsequent, uroligt
% "til" between "Tilværelsen," and "een Værensform" is NOT spaced (gaps 2).
og usikkert. Dette viser sig selv der, hvor den høieste Quie\-%
tismens Ro synes tilveiebragt: hos \emph{Spinoza}. For ham bliver
den høieste Opgave en \emph{intuitiv Viden af Gud}, i hvilken
Aanden veed sig som en \emph{evig Tankemodus} i Enhed med
ham. Men denne Viden, der til sit Støttepunkt kun faaer
\emph{den abstract-uendelige} Substantses Tanke, kan ikke
rumme Bevidsthedens \emph{concrete} Indhold, der i sin Bestemt\-%
hed sees som et Begrændset og \emph{Endeligt} og derfor bortsky\-%
% "et Begrændset" is NOT spaced (gaps 8/7) although "Endeligt" is (23).
des. Men Endeligheden, der ikke kan skaffes bort fordi den
betinger Tankens Bevægelse, trænger sig ind i Bevidstheden,
ikke blot i det endelige Vidensindholds, men i \emph{Sandselig\-%
hedens}, Lidenskabens Form, og Livets Opgaaen i hiin Er\-%
kjendelse bliver i hvert Øieblik forstyrret.

Gaae vi fra Betragtningen af Livsanskuelsen over til Be\-%
tragtningen af \emph{Tænkningens Art}, saa er Overensstemmel\-%
sen her mere skjult, og vanskeligere at opdage gjennem den
umiddelbart fremtrædende, tilsyneladende uforenelige Forskjel.
Dog bliver den ved en skarpere Analyse kjendelig og viser sig
i sin Begrundethed ved den for alle Formerne fælleds \emph{duali\-%
stiske} Sondring i Principet.

Sammenholde vi først \emph{Realismen} og den \emph{dogmatiske
Idealisme}, saa er, trods Forskjellen i Opfattelsen af Begre\-%
bernes Oprindelse og af Erkjendelsens Kilde, \emph{det} fælleds for
dem, at Bestemmelsen af Tankerne i deres indbyrdes Forhold\footnote{Naar den dogmatiske Idealisme taler om \emph{ideæ innatæ}, saa sees
disse som forefundne i deres Udvorteshed og Sondring.}
% "ideæ innatæ" is Latin set in the same upright roman as the Danish and
% LETTERSPACED (gaps 27 and 19 against 5--9 in the rest of the note), so it
% takes \emph{} and NOT \textit{} (BATCH-AGENT §3).
% p. 229/230 SEAM: the sentence runs on; NO paragraph break here.
% --- p. 230 ---
\opage{230} \setcounter{footnote}{0}
og Anvendelsen af dem er \emph{Forstandsdistinctionens}, og
i denne Forstandsdistinction, der endeliggjør idet den adskiller,
og som fastholder Modsætningen som uophævet og uforsonet,
reflecterer sig den i Principet udtalte Dualisme. Hos Grækerne
var det Principets \emph{Umiddelbarheds}bestemthed, der betin\-%
% MEASURED SPLIT INSIDE A COMPOUND.  The gaps across this word at 900 dpi
% run 19,23,22,21,1,24,22,1,21,24,20,1,19,22,23,22,23 for "Umiddelbarheds"
% and then 10,7,8,5,6,16,4,9,9 for "bestemthed," -- i.e. the compositor
% letterspaced the qualifying element only and set the second element plain.
% The same pattern recurs at pp. 232 ("Tilværelses|principet"), 232
% ("Grundværende" spaced, "Bestemte" not) and 238 ("Substants|begrebet").
gede \emph{Tankens} blivende Enhed med den \emph{kunstneriske An\-%
skuelse} og derigjennem den Begrebsdistinctionens Art, der
kunde betegnes som en \emph{Anskuelsesdistinction}. I de
nævnte Former af den nyere Philosophie er det \emph{Dualismen},
der, idet den sætter Spaltningen mellem Tanken og Tankens
Andet: det Reale, lader dette, som Gjenstand for Tanken, ikke
blive gjennemtrængt og tilegnet, men kun ligesom udenfra
formet af denne, og derfor overføre sin \emph{Udvorteshedsson\-%
dring} paa Realbegreberne, og gjennem dem paa Begreberne
overhovedet.

Sammenstille vi saa den \emph{kritiske}\footnote{At den \emph{kritiske} Idealisme i dens \emph{første} Former, hos \emph{Kant} og
\emph{Fichte}, i sin Bestemmelse af Begreberne, naar der kun tages Hen\-%
syn til hvad der virkelig realiseres, ikke til hvad der antydes og
tilstræbes, med Hensyn til Forstandsdistinctionen stemmer overens
med Dogmatismen, vil blive paaviist i de efterfølgende Afsnit.} \emph{Idealisme} i dens
Udvikling til en \emph{speculativ} Form og til en immanent Be\-%
grebsdialektik med de andre Hovedformer af den nyere Philo\-%
sophie, saa viser det sig, at \emph{Identitetsdialektiken}, idet
den, ifølge Standpunktets Ensidighed, ifølge Dualismens skjulte
Indvirken, bliver \emph{abstract Begrebsdialektik}, paa ethvert
Punkt skjuler \emph{Forstandsdistinctionen} under Enheden.
Det i den \emph{abstract-logiske} Tanke ikke Optagne: det \emph{Reale}
i dets \emph{Virkelighedsform}, gjør sig, hvad der nedenfor ud\-%
førligere vil blive paaviist, netop som ikke optaget i Dialekti\-%
kens Strømning, gjældende i \emph{sin Udvorteshed} og indvirker
paa en dobbelt Maade paa Begrebernes indbyrdes Forhold.
% PENCIL: a short diagonal stroke crosses the type between "udtrykke" and
% "Forhold." in the last two lines of this page (tesseract renders the first
% as "udtryk/koe").  It is not printed type and is not transcribed.
Deels komme nemlig de Begreber, der skulle udtrykke det
Reale som saadant, til at staae i et Udvorteshedsforhold til
% p. 230/231 SEAM: the sentence runs on; NO paragraph break here.
% --- p. 231 ---
\opage{231} hverandre, naar det Reale ikke er opløst i Tanken. Deels
maae de logiske Begreber, hvis indre Bevægelse vilde forud\-%
sætte et Forhold til den reale, levende Virkelighed, factisk
blive staaende \emph{adskilte} fra hverandre, idet deres For\-%
bindelse kun tilveiebringes ved et ifølge Principet \emph{uberet\-%
tiget} heterogent Element: et fra Anskuelsen af det Reale ucon\-%
seqvent optaget Bevægelseselement, der bliver uundværligt forat
virkeliggjøre den «rene Tankes» Dialektik, og Forbindelsen alt\-%
saa bliver illusorisk.
% There is NO dash after "illusorisk." -- the faint marks tesseract reads as
% "— - ;" are show-through from the verso, checked at 900 dpi.

I den \emph{aabenbare eller skjulte Forstandsdistinc\-%
tion} mødes saaledes de forskjellige Hovedformers Tænkning.
% "den" before "aabenbare" is NOT spaced (gaps 9); the run begins at
% "aabenbare".
Realismens og Idealismens forskjellige Tankeretning: mod Na\-%
turen og mod Aanden, ophæver \emph{ikke} Overensstemmelsen i
hiint Punkt, for hvilken Tænkningens Forhold til et dualistisk
bestemt \emph{Realt} overalt bliver afgjørende.
% PENCIL: a small stroke stands above the line between "Realt" and
% "overalt"; it bridges the word space and is what made the automatic
% gap measurement merge the two words.  Not printed type; not transcribed.

I en Eiendommelighed, der viser tilbage til den gjennem\-%
gaaende Fremtræden af Forstandssondringen, mødes ligeledes
de forskjellige Hovedformer, nemlig deri, at de alle have \emph{Tviv\-%
len}, Tankens \emph{kritiske} Forhold til sit Indhold, til \emph{Udgangs\-%
punkt}. De begynde alle med Bevidstheden om Nødvendig\-%
heden af, ved en forudgaaende Tvivl at sikkre den methodiske
Løsning af Philosophiens Opgave, og denne Bevidsthed danner
en Modsætning til den blinde om end begeistrede Tillid, som
Aandens Tro paa sin uendelige Magt, og det harmoniske
Forhold til Naturen, gav Overgangsperiodens digteriske Tænk\-%
ning. ---

Efterat vi saaledes have charakteriseret den \emph{nyere Phi\-%
losophie} ved det i dens \emph{særegne} Former \emph{Almene}, blive
nu de forskjellige Hovedformer, der ligge som Muligheder i
den, enkeltviis at bestemme.

% DIVISIONAL RULE, p. 231: short, CENTRED, generous white above and below.
% Measured at 900 dpi: x 1741--2339, i.e. 598 px = 1.69 cm, centre 2040.0
% against a text-block centre of 2036.5 (block x 340--3733).  A plain even
% hairline.  This page bears no note, so it cannot be the footnote separator.
\divrule

% L3 HEAD, p. 231 (transcription.tex §3, the A./B. level that sits ABOVE the
% a./b. level and the Greek α./β. level -- NOT renumbered or harmonised).
%
% THE TWO HALVES OF THIS HEAD ARE SET IN DIFFERENT FOUNTS, measured at
% 900 dpi by median black-run width per glyph:
%     "A."                             15 px  (p25 7, p75 17)
%     "Den realistiske Philosophie."   23 px  (p25 16, p75 24)
%     body capital "P" of "Principet"  14 px
%     body capital "I" of "Idet"       13 px
% So "A." is ORDINARY ROMAN, at exactly the weight of the body capitals, and
% only "Den realistiske Philosophie." is BOLD.  Neither half is letterspaced
% (gaps 6--10).  The same split is printed at pp. 236 ("B.") and 237 ("a.").
% Centred, alone on its line.
\begin{center}
A.\enspace\textbf{Den realistiske Philosophie.}
\end{center}

% THE FOUR-PART SCHEME IS RUN IN, NOT SET ON ITS OWN LINES.  "1. Principet."
% stands at the head of the indented paragraph and the text continues on the
% same line ("... Principet.  Idet vi bestemme dette, ...").  ABBYY's layout
% breaks it onto a separate line; tesseract and the image agree that it is
% run in.  The same holds for 2. (p. 232), 3. (p. 233) and 4. (p. 235).
% The wording is LETTERSPACED, NOT BOLD: median stroke 12 px for
% "Principet." against 11 px for the rest of the same line, and gaps 20 px
% against 4--9 px.  So \emph{}, not \textbf{}.  The numeral "1." itself
% shows only the gap before its period and is left outside the \emph{}.
1. \emph{Principet.} Idet vi bestemme dette, opfatte vi det
nærmest under Synspunktet af \emph{Tilværelsesprincip}, men
% p. 231/232 SEAM: the sentence runs on; NO paragraph break here.
% --- p. 232 ---
\opage{232} lade saa, ifølge den ovenfor paaviste Sammenhæng mellem
Tilværelsesprincipets og Erkjendelsesprincipets Bestemmelse,
Tilværelsesprincipet ogsaa vise sig som \emph{Princip for Erkjen\-%
delsen}.

Idet Realismen opfatter det Absolute under den ud fra
den dualistiske Modsætning til Idealiteten bestemte Realitets
Synspunkt, seer den det som \emph{Natur}, og lader, som vi have
paaviist, i denne dualistiske Sondring, Naturen reduceres til
\emph{den bevægede Materie}, under hvis almene Begreb det
\emph{Aandelige} bringes ind med kun \emph{quantitativ} bestemt Eien\-%
dommelighed, til hvilken de \emph{qualitative} Differentser, der
uafviseligt gjøre sig gjældende, søges førte tilbage.

Idet saa det som det \emph{Grundværende} Bestemte opfattes
% "Bestemte" is NOT spaced (gaps 3--11) although "Grundværende" is (18--25).
som bestemmende den erkjendende Aands Væren, altsaa Er\-%
kjendelsens Natur og Grundforhold; idet med andre Ord, \emph{Til\-%
værelses}principet sees som \emph{Erkjendelsesprincip}, saa bliver
% Another measured split inside a compound: "Til-" (21) and "værelses"
% (18--24) are letterspaced, "principet" (2--10) is not.
for Realismen Erkjendelsesprincipet, taget i denne Betydning,
\emph{den bevægede Materie som frembringende Sands\-%
ningen}, af hvis uvilkaarlige Omformninger \emph{Tankens} Former
som ved en mechanisk Proces skulle udvikle sig i det \emph{pas\-%
sive}, med det Materielle væsenslige Subject. I denne Be\-%
stemmelse af Erkjendelsesprincipet, der er \emph{Materialismens},
er den Tanke \emph{helt} udtalt, der laae skjult i Realismens Be\-%
gyndelsesform, i det Stadium, vi betegnede som \emph{Erfarings\-%
philosophie}.
% "Erfaringsphilosophie" has a LOWER-CASE p on the page (ABBYY reads a
% capital); checked at 900 dpi.

2. \emph{Tankens væsentlige Gjenstand} er paa dette Sta\-%
dium, hvor Principet er opfattet som et i den \emph{umiddelbare
Realitetsform} Værende, \emph{umiddelbart} bestemt ved Prin\-%
cipet og identisk med dette. Realismens væsentlige Gjenstand
er \emph{den som Materie bestemte Natur}, Naturen seet fra
\emph{Mechanismens} Standpunkt. Hvorledes Realismens Bestem\-%
melse af Naturen udviklede sig af den \emph{Baconiske} Philoso\-%
phies Princip, er i det Foregaaende (S. 160 ff.) udviklet. Be\-%
tydningen af Realismens Accentueren af Naturen er ligeledes
tidligere antydet. I Modsætning til Fortidens \emph{theologiske}
% p. 232/233 SEAM: the sentence runs on; NO paragraph break here.
% --- p. 233 ---
\opage{233} Opfattelse af Naturens Væsenløshed som skabt og som Skue\-%
plads for Underet; i Modsætning til \emph{Phantasiens} og den i
Phantasien endende \emph{metaphysisk-abstracte} Naturopfat\-%
telse, der skjult og indirecte statuerer Væsenløsheden af det
Naturlige som Naturligt, hævder Realismen, gjennem Fordrin\-%
gen paa en \emph{physisk} Naturopfattelse, paa en Opfattelse af
Naturen ved den selv, gjennem Accentueringen af de virkende
Aarsager, \emph{Naturens Væsentlighed}. Idet den opfatter Na\-%
turen som det \emph{Lovbegrundede} og i sine \emph{egne, uforan\-%
derlige} Love Hvilende, som det, der er \emph{Erkjendelsens og
Sandhedens Kilde}, og som kun tilegnes \emph{ved Erfaringen}
% "Love Hvilende," is NOT spaced (gaps 5/8) although "uforanderlige" before
% it is.
gjennem Aandens Receptivitetsforhold, udtaler den \emph{med Be\-%
vidsthed} Naturens Væsensgyldighed \emph{som Natur} i dens \emph{eien\-%
dommelige} Væren. Men gjennem hine Bestemmelser af Na\-%
turen som \emph{Tankens væsentlige} Gjenstand og som en \emph{Er\-%
kjendelseskilde} har den tillige, indirecte og ubevidst, ud\-%
talt Naturens \emph{Væsensenhed} med \emph{den} erkjendende \emph{Aand},
% Printed as measured: "den" (24) and "Aand," (21) are letterspaced but
% "med" (10) and "erkjendende" (9) between them are not.
der selv i Materialismen hævder sig i sin Eiendommelighed,
og derigjennem dens Enhed med det Fornuftiges Idealitet, som
den udtrykker i Realitetens Objectivitetsform.

3. \emph{Tænkningens Grundpræg, Methoden.} Idet Til\-%
værelsesprincipet, som Princip for Erkjendelsens Væren, be\-%
stemmer den med Bevidsthed satte Norm for Erkjendelsen:
Erkjendelsesprincipet i \emph{afledet} Betydning, saa bliver dette
indenfor Realismen \emph{Erfaringsprincipet}; dens charakteristi\-%
ske Methode bliver \emph{Erfaringens Methode}. Subjectet, der
opfattes som passivt, bliver kun erkjendende gjennem en Paa\-%
virkning af det Reale (Materielle), fra hvilket det, som et me\-%
chanisk Bevæget, modtager en meddelt Bevægelse, der, idet
den fortsætter sig i Subjectet, frembringer Erkjendelsens For\-%
mer. Subjectets Forhold til Gjenstanden bliver da et \emph{rent
Receptivitetsforhold}, og dette Forhold er \emph{Erfaringens
i dens Exclusivitet}. Gjennem Erfaringen afspeiler det Ob\-%
% The one-letter word "i" carries no measurable internal gap; it stands
% between two certainly letterspaced words and is included in the run.
jective sig i sin Objectivitet i Subjectet. I Erfaringsforholdet
ligger ikke blot et \emph{Udvorteshedsforhold} mellem Subject
% p. 233/234 SEAM: the sentence runs on; NO paragraph break here.
% --- p. 234 ---
\opage{234} \setcounter{footnote}{0}
og Gjenstand; men Gjenstanden er, ifølge det Reales Grund\-%
bestemthed, sat med \emph{Udvorteshedens Endelighedsson\-%
dring}. Erfaringsforholdet faaer derfor, consequent, den nær\-%
mere Bestemmelse, at det bliver \emph{den receptive Tankes
Forhold til det i Udvorteshedssondring værende
Reale.} Dette bliver, som det, der i sin Individualitet er det
egentlige Værende og Virkelige, idet Tankedannelsen foregaaer
i Subjectet, og idet Abstractionen og Inductionen skride fremad,
sammenfattet under \emph{Almeenbegreber}, der \emph{som} almene kun
% "som" alone is spaced (24) here; "der" (5) and "almene" (8) are not.
have \emph{subjectiv} Gyldighed, og under \emph{Love}, der kun ligesom
midlertidig laane Almindelighedens Form fra Subjectet. Idet
Realismen ved sit Erfaringsprincip udelukker det et \emph{Ideelt} for\-%
udsættende \emph{Aprioriske} med dets Almeengyldighed og Nød\-%
vendighed, fører den uundgaaeligt, hvad den historiske Udvik\-%
ling stadfæster, til en \emph{Skepsis}, der sætter det Sandsynlige
paa Sandhedens Plads og lader Erkjendelsen tabe sig i det
Relative. Men fra dette Punkt maatte Erfaringsprincipet, efter
sin strængeste Consequents, føre endnu videre, og gjennem
den \emph{absolute} Fornægtelse af det Ideelle og Almene, ved at
ende i det absolute Udvorteshedsforhold mellem det \emph{atomi\-%
stiske} Individuelle, tilintetgjøre al Erkjendelse. Men ligesom,
ifølge Sagens Natur, Realismens \emph{Tilværelsesprincip} uvil\-%
kaarligt reflecterede det udelukkede Moment, og under Forsø\-%
get paa en Gjennemførelse lod det komme frem gjennem In\-%
consequentser, saaledes lader ogsaa \emph{Methoden} et fra det
Ideelle nedstammende Apriorisk, og med det et Væsentligt og
Alment, der giver et Tilknytningspunkt for Erkjendelsen, faae
en Plads, og i den «inductive Methode» opereres der paa et\-%
hvert Punkt kun ved Hjælp af aprioriske Elementer\footnote{Den \emph{consequente} Gjennemførelse af den dualistiske Sondring af
det Ideelle og det Reale maatte udelukke \emph{enhver} Idealitetsbestem\-%
melse, altsaa al Bestemmelse ved det \emph{Almene}, altsaa al Mulighed
til en \emph{Erkjendelse}, fra det \emph{Reale}. Og naar nu denne det
Reales Bestemthed skulde overføres paa \emph{Aanden}, saa vilde det
være enstydigt med at Aanden ved sig selv skulde \emph{negere sig
selv}, ved en Erkjendelsesact forvandle sig til et Ikke-Erkjendeligt
og Ikke-Erkjendende. Det er derfor uundgaaeligt, at der bøies af
fra den yderste Consequents, og at det \emph{Ideelle}, der skulde for\-%
trænges, maa gjøre sig gjældende gjennem \emph{Inconsequentser}.}.
% RUN-OVER FOOTNOTE.  In the print this note breaks after "...saa vilde det"
% at the foot of p. 234 and its remaining five lines stand at the TOP of
% p. 235's note area, above p. 235's own *) and carrying no mark of their
% own.  The whole note is set here in one \footnote{} per the practice
% recorded in transcription.tex §4; see the % comment at the p. 235 marker.

% --- p. 235 ---
\opage{235} \setcounter{footnote}{0}
% NOTE AREA OF p. 235: its first five lines ("være enstydigt med at Aanden
% ... gjennem Inconsequentser.") are the TAIL OF p. 234's note *) and carry
% no mark; they are set above p. 235's own *).  They have been appended
% inside the \footnote{} at the call on p. 234 and are NOT repeated here.
4. \emph{Livsanskuelsen}, der svarer til denne philosophiske
Hovedform, kan betegnes som den \emph{praktiske Realismes},
ethisk: som den \emph{atomistiske Egoismes}. Den har ligesom
sit uskyldige, neutrale Udtryk hos \emph{Baco}, idet han gjør Er\-%
kjendelsen af Naturen, og det herved betingede Herredømme
over Naturen, Erkjendelsens praktiske Anvendelse til Menne\-%
skets Nytte, til Menneskelivets Opgave. Naturen er her vel
sat som vort Livs Skueplads, som væsentlig Gjenstand for vor
Interesse; vort Livs Maal er sat indenfor Naturens Grændser,
vor Opgave som en naturlig; men det Ideelle i Erkjendelsen
og den praktiske Realisme i Anvendelsen ere ligesom i et Øie\-%
bliks Ligevægt. Dog viser selve Nævnelsen af det \emph{Nyttige}
hen paa den Side, hvor Overvægten vil komme tilsyne, og
Erkjendelsen, det Ideelle, bliver allerede her ligesom kun et
uundgaaeligt, men ikke selvgyldigt, Element i Livsopgaven.
Og idet Standpunktets Consequentser udvikle sig, i den anty\-%
dede Retning, bestemmes ogsaa Livsanskuelsens Form saaledes,
at det \emph{ethisk Neutrale} drages ind under \emph{ethiske} Bestem\-%
melser. I Tilværelsens \emph{atomistiske} Sondring, der er den
uundgaaelige Consequents af, at al Idealitetsbestemmelse er
fjernet fra det Reale, ligger \emph{Egoismens} Consequents\footnote{\emph{Egoismen} bliver Consequentsen trods den \emph{Nødvendighed}, der
omfatter og behersker Atomerne; thi Nødvendigheden, der bestem\-%
mer disse, bestemmer dem \emph{som} Atomer, saaledes at i Bestemmet\-%
heden deres atomistiske «\emph{Gravitation mod sig selv}», --- Ego\-%
ismens Udtryk --- bevares.}, der
% PRINTED "Bestemmetheden", with a double m, where the book's usual form is
% "Bestemtheden"; confirmed at 900 dpi ("Bestemmet-" at the line end).  Not
% on the author's Trykfeil leaf; transcribed as printed.
vel giver Plads for en vis Elasticitet, for en hensyntagende,
beregnende, og ligesom udvidet Egoisme, men dog i Principet
bestandig \emph{kun} bliver Egoisme, og det en Egoisme, hvis Maal
er \emph{sandselig Nydelse}. I den \emph{franske Materialisme} og
% p. 235/236 SEAM: the sentence runs on; NO paragraph break here.
% --- p. 236 ---
\opage{236} i dens umiddelbare Forberedelser træde disse Consequentser
frem, og det er kun en Inconsequents, om end, som tidligere
paaviist, en uundgaaelig, naar Dyd og Menneskelighed faae en
Plads indenfor en Livsanskuelse, hvis Grundpræg, ifølge Prin\-%
cipet, kun kunde være Selvkjærlighedens. Det er en Inconse\-%
quents, analog med den, der lader den Aand, der selv ordner
sig ind under \emph{Nødvendighedens} Magt, begeistret kæmpe
en \emph{Frihedskamp}, og forkynde \emph{Perfectibiliteten} trods de
\emph{altomfattende} Naturloves \emph{Uforanderlighed}. Det er en
% "Naturloves" between the two spaced words is NOT spaced (gaps 6).
Inconsequents, der, ifølge Modsætningernes Væsenssammenhæng
i det Absolute, overalt maa gjøre sig gjældende, hvor den ene
abstract søges udelukket. Det \emph{Væsentlige}, der fornægtes,
tvinger sig frem og gjør Livet uroligt og uconsequent.

% DIVISIONAL RULE, p. 236: short, CENTRED, white above and below.  Measured
% at 900 dpi: x 2172--2766, i.e. 594 px = 1.68 cm, centre 2469 against a
% text-block centre of 2446 (block x 784--4108).  This page bears no note.
\divrule

% L3 HEAD, p. 236, the B. of the A./B. pair (231, 236).  Same split fount as
% p. 231, measured at 900 dpi: "B." median stroke 14 px, level with the body
% (11 px) and with the body capitals; "Den idealistiske Philosophie."
% median stroke 18 px -- BOLD.  Neither half letterspaced (gaps 5--13).
\begin{center}
B.\enspace\textbf{Den idealistiske Philosophie.}
\end{center}

Medens den \emph{realistiske} Philosophie hævder sig som en
\emph{nødvendig} Tankens Udviklingsform, baade naar den sees fra
Ideeforholdets Synspunkt, og naar den sees fra den individu\-%
elle Tankeudviklings Betingelsers Synspunkt, saa viser den dog,
baade gjennem Bestemmelsen af Tilværelses- og af Erkjendel\-%
% "Tilværelses- og" is a SUSPENDED hyphen, printed, and stays as printed.
sesprincipet, ud over sig selv, og viser hen til \emph{Idealismen},
foreløbig som til et \emph{Supplement} for sig.

Utilstrækkeligheden af Realismens \emph{Tilværelsesprincip}
viser sig hvor Standpunktets yderste Consequentser drages: i
\emph{Materialismen}. Aanden, der skulde absorberes af Materien,
hævdede sig i sin Eiendommelighed gjennem sine for Materia\-%
lismen ligesaa uforklarlige som ubenægtelige Grundbestemmel\-%
ser, og gjorde sig i denne Emancipation fra Materien gjældende
i sin principielle Selvstændighed.

Utilstrækkeligheden af Realismens \emph{Erkjendelsesprincip}
har viist sig ved de Inconsequentser, der bleve uundgaaelige
forat redde en Erkjendelses Mulighed. Det Reale, i hvilket
Idealitetsmomentet var tilhyllet, lod ingen Tilknytning for Er\-%
% p. 236/237 SEAM: the word "Erkjendelsens" is BROKEN ACROSS THE PAGE.
% --- p. 237 ---
\opage{237} kjendelsens, ved det Almene og derfor ved Idealitetens Gyl\-%
% "Idealitetens", not ABBYY's "Idealistens"; read at 900 dpi.
dighed betingede, Mulighed blive tilbage. Erkjendelsens \emph{Nød\-%
vendighed} fremtvang den uvilkaarlige og ubevidste factiske
Anerkjendelse af det Aprioriske, ved hvilken det tilhyllede
Ideale viste sig kun at være skjult, men ikke tilintetgjort.
Og allerede i selve den principielle Fordring af Erfaringen,
i det \emph{Erfaringsprincip}, der, som \emph{Princip, ikke} kan
begrundes \emph{ved Erfaringen}, ligger en Henviisning til en
\emph{anden} Sphære, hvor det Begrundende maatte søges.

Gjennem selve Forsøget paa at udvikle en consequent
Realisme bryder saaledes det \emph{Ideale} frem som et Uafviseligt.
Ifølge sin \emph{Væsentlighed} gjør det sig gjældende som Grund\-%
lag og Udgangspunkt for en \emph{eiendommelig} Tænkningens
Hovedform ved Siden af Realismen, og vi betragte først denne
Form \emph{som Enhed} saaledes som den fremtræder i sin \emph{Paral\-%
lelisme} til den \emph{realistiske} Udviklingsrække.

% NO DIVISIONAL RULE ABOVE THIS HEAD.  The interval between the last body
% line (y 3266) and the head (y 3475) at 900 dpi carries nothing but the
% descenders of the line above: the row-ink profile falls to zero from
% y 3292 to y 3473.  Reported because BATCH-AGENT §6 says a rule stands
% above "almost every" display head; this one has none, and neither does any
% other head in the a./b. sequence so far as this batch reaches.
%
% L3 HEAD, p. 237, the "a." of the a./b. pair (237, 240).  Same split fount
% as pp. 231 and 236, measured at 900 dpi: "a." median stroke 11 px, exactly
% the body's 11 px -- ORDINARY ROMAN; "Den nyere idealistiske Philosophie
% som Enhed." median stroke 18 px -- BOLD.  Neither half letterspaced
% (gaps 5--8).
\begin{center}
a.\enspace\textbf{Den nyere idealistiske Philosophie som Enhed.}
\end{center}

Hvad der sammenknytter de forskjellige, tidligere angivne,
Former af den nyere Idealisme med hverandre, er først og
væsentlig den \emph{fælleds} Opfattelse af det \emph{Absolute}, ifølge
hvilken dette viser sig under \emph{Idealitetsmomentets} Bestem\-%
melse, saaledes, at det Ideale skjuler over det Reale, men
dog lader dette reflecteres i sig. Selv hvor Principets Be\-%
stemmelse tilsyneladende deler ligeligt mellem det Reale og det
Ideale, ja endog kan antyde en Tilbøielighed til at lægge Over\-%
vægten over paa det første, der bliver dog det Reales Bestem\-%
melse saaledes behersket af det Ideelle, at dette viser sig som
det Overgribende og som det, der \emph{definitivt} afgiver Væsens\-%
bestemmelsen for det Absolute; og ved Udviklingsrækkens Af\-%
slutning træder dette Grundforhold frem i sin Klarhed.

Tankens \emph{væsentlige Gjenstand} bliver derfor, trods
% "Tankens" here is NOT letterspaced (gaps 5,5,10,2,7,5,8) although the same
% word is spaced at p. 233.  Checked twice on the image at 900 dpi.
den Tiltrækningskraft, som Naturen, som tidligere fremhævet,
udøvede paa Tanken, det \emph{Ideelle}, \emph{Aanden med dens
aprioriske Indhold}. Philosophien blev bestemt som dette
% p. 237/238 SEAM: the sentence runs on; NO paragraph break here.
% --- p. 238 ---
\opage{238} \emph{Aprioriskes Videnskab} (Kant, Hegel) og dens Indbold
% PRINTER'S ERROR, transcribed as printed: "Indbold" for "Indhold".  The
% glyph is unambiguously a b (closed bowl on the baseline with the ascender
% to its left), read at 900 dpi; both OCR witnesses read "Indbold" too.  Not
% on the author's Trykfeil leaf.
som congruerende med dettes.

I \emph{Methoden}, i hvilken netop Forskjellene indenfor Idea\-%
lismens Enhed fremtræde, bliver Overensstemmelsen ikke saa
umiddelbart synlig. Men deels indeholder det, der allerede i
det Foregaaende blev udviklet angaaende den \emph{hele} nyere Phi\-%
losophies Enhed, en Enhedsbestemmelse for den deri indbefat\-%
tede Idealisme, der vil faae sin klarere Belysning ved den
nærmere Bestemmelse af de enkelte Former; deels faaer Idea\-%
lismen, i \emph{sin Modsætning til Realismen}, en Fælledseien\-%
dommelighed i det i Tænkningens Art, der er betinget ved
% "i det i Tænkningens Art" is printed with the two i's; not emended.
Anerkjendelsen af det \emph{Ideelle} og det \emph{Aprioriske}. Ved
denne Anerkjendelse betinges det nemlig, at \emph{Tankens Vei}
væsentlig bliver Veien fra det Almene ned til det Enkelte, og
at Deductionen og Syllogismen faae væsentlig Betydning, og
ved den betinges den \emph{eiendommelige Opfattelse af
Tænkningens Functioner} i deres Forhold til det Reale,
der finder sit prægnanteste Udtryk i \emph{Hegels} Opfattelse af Be\-%
grebets, Dommens og Slutningens reale Betydning. Selv hvor,
som hos \emph{Kant}, Tankens Former holdes skarpest sondrede fra
det Objective, \emph{antydes} dog Enheden gjennem Bestemmelsen
af det centrale Begreb (see ovenfor S. 69).

Søge vi endelig den \emph{Livsanskuelse}, der svarer til det
i Idealismens Former Gjennemgaaende, saa finde vi som saa\-%
dan den Anskuelse, der sætter Subjectets Opgaaen i det ideelle
evige Absolute som Livsmaalet. Denne Opfattelse af Livsop\-%
gaven bliver udtalt hvor \emph{Substants}begrebet \emph{consequent}
% A fourth measured split inside a compound: "Substants" gaps 18--30,
% "begrebet" gaps 3--11.
gjennemføres, i \emph{Spinozismens} Fordring af den enkelte Tanke\-%
modus's Opgaaen i Substantsens Uendelighed. Den træder frem
i \emph{Fichtes} Bestemmen af Jegets Forhold til den Proces, i
hvilken det som individuelt Jeg omsluttes af det sættende og
sig realiserende absolute Jeg, bagved hvilket der, under Tan\-%
kens Udvikling, ligesom samler sig \emph{en Værens Baggrund},
mindende om Spinozismens bevægelsesløse Substants. Men
Fichte udtrykker, som vi tidligere have paaviist, Consequent\-%
% --- p. 239 ---
\opage{239} % SEAM, pp. 238/239.  p. 238 ends its last line with "Consequent-" and p. 239
% opens "sen af Kants ..." -- i.e. the word "Consequentsen" is broken ACROSS
% THE BATCH BOUNDARY.  The pp. 225--238 batch must end its fragment
% "Consequent\-%" (no newline after) or the joined file will print
% "Consequent- sen".  p. 238 bears NO footnote, so nothing runs over into
% p. 239's note area; p. 239 bears no note either.
% p. 239: no Greek, no ɔ: mark, no bold sorts in the paragraph.
% GUILLEMETS: one pair, «klare og tydelige» -- read at 1200 dpi, opening «
% and closing », the book's normal INWARD pair.  Not regularised, none needed.
% Letterspacing measured per word as the 75th percentile of the inter-blob
% gaps at 300 dpi: spaced words run 7--10 px, ordinary roman 2--5 px.
sen af \emph{Kants} Kritik der reinen Vernunft. Og gaae vi fra
\emph{Fichte} til de Systemer, der udtrykke Kantianismens sidste
Consequentser, saa see vi, efterat \emph{Schelling} i en nuanceret
Form har kaldt Spinozismens Grundtanke igjen tillive, hiin
Opfattelse af Livsanskuelsen træde frem hos \emph{Hegel} i den Be\-%
tragtning af den individuelle Aand i dens Forhold til den ab\-%
solute Aands Proces, der lader hiin i dens naturbestemte
Enkelthed blive til et, ligesom det Naturlige overhovedet,
Usandt og kun Forsvindende, og Ideens tidløse evige Proces
% The hyphen ending the next line is the PRINTED SUSPENDED hyphen of
% "Tids- og Endelighedsbestemmelser", not a word-break hyphen; it is kept as
% printed and must NOT be turned into \- (the line break is coincidental).
blive den sande Værens \emph{eneste} Form, i hvilken alt i Tids-
og Endelighedsbestemmelser værende Liv skal opløses af Tan\-%
ken, og til hvilken den endelige Aand skal søge tilbage gjen\-%
nem den Contemplation, der lader den blive Moment i den
absolute Videns tidløse Bevægelse. ---

Indenfor den nyere Idealisme sondre sig nu, ifølge det tid\-%
ligere Udviklede, nærmest \emph{to Hovedformer}: den \emph{dogma\-%
tiske} og den \emph{kritiske}; men indenfor den sidste gjøre vi
atter en Sondring mellem to Former, af hvilke vi bestemme
den første som \emph{kritisk, men med Retningen mod det
Speculative,} den anden som \emph{speculativ,} men saaledes, at
den i sig bærer, om end som skjult, det ved Principbestem\-%
% p. 239 l. 22: "i ensidig Forstand kritiske Element".  Measured: "ensidig"
% 8 px, "Forstand" 3 px, "kritiske" 8 px.  The compositor spaced the two
% adjectives and left "Forstand" between them unspaced; that is what is set
% here.  The preceding "i" is a single sort and carries no measurable gap, so
% it is left outside the emphasis rather than guessed into it.
melsens Ensidighed betingede, i \emph{ensidig} Forstand \emph{kritiske}
Element.

Den \emph{første} Form, i Tiden og efter Begrebet, bliver den,
hvor Tanken, i Tillid til den «klare og tydelige» Erkjendelses
Magt, uden at gjøre sig Muligheden af en \emph{objectiv} Erkjen\-%
delse til Problem, vender sig mod Gjenstanden forat bemæg\-%
tige sig den, og denne Idealismens Form er den \emph{dogmati\-%
ske} Idealisme.

% DIVISIONAL RULE at the FOOT of p. 239, below the last line of type, with
% the L3 head standing at the TOP of p. 240 -- the pp. 15 and 79 pattern
% (BATCH-AGENT §6).  It is NOT a footnote separator: p. 239 bears no note,
% and it is CENTRED, not left-flush -- x 559--800 at 300 dpi (span 241 px =
% 2.04 cm, centre 679.5) against a type area 120--1237 whose centre is 678.5.
% It is INK, not the modern pencil this copy carries: minimum grey 90 against
% a paper of 206, i.e. 116 levels below the paper, where the header records
% ink at ~106 levels below and pencil at ~33.  (The show-through of p. 240's
% text in the same white space bottoms out at 183.)  Its 4 px rise from left
% to right over 241 px matches the skew of the printed lines on this page
% (the last line's baseline rises 4 px over 200 px), so the slight slope is
% the scan's, not a hand's.  This site is not in RECON.md's rule list.
\divrule

% --- p. 240 ---
\opage{240} % p. 240 bears no note.  No Greek in the running text (see the head below).
% No ɔ: mark, no guillemets, no bold sorts inside the paragraphs.
%
% L3 HEAD, p. 240 l. 1.  MEASURED at 300 dpi, median horizontal dark-run
% (stem width) with mean in brackets:
%   * "Den idealistiske Philosophies særegne Former."  6.0 px [5.35], n=1442.
%   * the enumerator "b."                              3.0 px [3.10], n=59.
%   * body roman of the same page (whole text block)   3.0 px [3.31], n=19411.
% So the head proper is BOLD, ~1.8x the body; the enumerator "b." is set in
% the ORDINARY ROMAN and is not bold.  NOT letterspaced: inter-letter gaps
% 1.5--4.0 px, against 7--9 px in the Sperrsatz on the same page.  CENTRED
% and short: x 398--1238 (centre 818) in a type area 243--1365 (centre 804),
% against a full body measure of ~1120 px.
% transcription.tex §3 files this head under L3 and describes L3 as
% "LETTERSPACED ROMAN at body size, set as a justified PARAGRAPH -- not bold
% and not centred".  THAT DESCRIPTION DOES NOT FIT THIS HEAD on any of the
% three counts.  It is set here exactly as the pp. 186/192/201 heads already
% in this file, which the pp. 183--196 batch measured the same way and
% reached the same conclusion about.  FINAL PASS: the review item is
% discharged -- §3 now splits the old L3 into L3a (nine letterspaced-roman
% justified paragraphs) and L3b (these bold centred heads), and files this
% head under L3b.  The enumerator "b." is roman and stays outside the
% \textbf{}; the space after it is \enspace throughout the book.
\begin{center}b.\enspace\textbf{Den idealistiske Philosophies særegne Former.}\end{center}

% L4 HEAD, p. 240 l. 2.  MEASURED at 300 dpi:
%   * "Den dogmatiske Idealisme."   median stem 4.0 px [4.33], n=933
%   * body roman, same page          median stem 3.0 px [3.31]
%   * the L3 bold head above         median stem 6.0 px [5.35]
% i.e. it is HEAVIER than the body (1.3x) but nothing like the bold head
% (1.8x); x-height 23 px, identical to the body's 23 px, so the extra weight
% is not a size effect.  NOT letterspaced: gaps 2.0--4.0 px.  CENTRED:
% x 544--1091 (centre 818) in the type area 243--1365.  §3 called L4
% "letterspaced (NOT bold)"; the letterspacing half of that was wrong and §3
% is now corrected.
% FINAL PASS, at 600 dpi and normalised by x-height: this head r = 1.15--1.16
% against the body of the same page, where the display roman runs 0.92--1.19
% and the bold 1.32--2.02.  Its x-height is 44 px against a body 40--41, i.e.
% it is the ROMAN one size up, which accounts for the 1.3x raw stem the
% pp. 239--252 batch measured without normalising.  NOT bold.  The β head on
% p. 250 measures the same (r = 1.12--1.16, gaps 4.5 px).
%
% THE ENUMERATOR α.  Read on the image at 1200 dpi (4x interpolation over the
% 300 ppi original).  It IS a Greek alpha, and on the image it is set in the
% BOOK'S CURSIVE (italic) GREEK FOUNT: a slanted single-storey alpha, the
% loop on the left and the stroke on the right falling to the baseline, the
% same letterform as the running Greek elsewhere in the book -- NOT an
% upright roman alpha.  Its x-height is 15 px against the head's 23 px, and
% its median stem is 4.0 px.  Per this batch's brief it is set UPRIGHT here,
% as a plain Unicode α outside \gk{} (it is an outline enumerator, not
% running Greek), and the discrepancy with the image is recorded here rather
% than silently resolved either way.  The β on p. 250 is the same fount.
\begin{center}α.\enspace Den dogmatiske Idealisme.\end{center}

1. \emph{Principet.} Principets Bestemmelse og Principets
Genesis er allerede i Almindelighed bleven angivet i det Fore\-%
gaaende (S. 172 ff.). Det blev der viist, hvorledes den dogma\-%
tiske Idealismes Tænkning, idet den søger det \emph{væsentlig
Værende,} bestemmer det som \emph{Substantsen,} hvis Begreb
fra Begyndelsen af omfattede det Aandelige og det Naturlige
og begges aandelige Princip. Begrebet om Substantsen, det
væsentlig Værende i dets Nødvendighed og som Erkjendelsens
væsentlige og umiddelbare Gjenstand, bestemte sig saa i den
consequente Gjennemførelse af det, idet den \emph{væsentlige
nødvendige Væren} viste sig som en Væren \emph{ved sig selv}
(per se), og som saadan som \emph{evig, uendelig} og \emph{een} (jfr.
\emph{Spinozas} Ethik, 1. Bog), til Begrebet om \emph{en abstract
Enhed med Uendelighedens Bestemmelse,} og blev i
denne Opfattelse Udtrykket for det Absolute.

I Bestemmelsen af det i Tilværelsen Væsentlige havde
Tanken oprindelig taget sit Udgangspunkt fra \emph{Idealitetssi\-%
den,} fra Subjectet som \emph{tænkende,} og kun overført \emph{dets} Be\-%
stemmelse paa det Guddommelige: den \emph{uendelige} Substants
er derfor, efter sin Genesis, oprindelig \emph{det som Tanke be\-%
stemte Jegs uendelige Væsen.} Idet \emph{Descartes} havde
bestemt Jeget ved det første Visse, ved \emph{Tanken,} saaledes at
denne Bestemmelse \emph{udtømte} dets Væsen, maatte Jegets In\-%
% p. 240 l. 26: the print reads "i Tankens .Almindelighed," -- a full stop
% stands on the baseline between "Tankens" and "Almindelighed", with
% "Almindelighed" set hard against it and no word space after it.  Read on
% the image at 300 and 1200 dpi; both witnesses see it (ABBYY "Tankens
% .Almindelighed", tesseract "Tankens.Almindelighed").  Transcribed AS
% PRINTED; the sense wants no stop here.
dividualitet forsvinde i Tankens .Almindelighed, og det tæn\-%
kende Jeg, der tænkte sig selv, maatte tænke sig gjennem
sin \emph{Tankes uendelige Væsen,} den uendelige \emph{tænkende}
Substants. Men den uendelige Substants, der saaledes nær\-%
mest var \emph{Tankens} uendelige Væsen, blev, idet den i sit
uendelige Væsens \emph{Enhed} skulde sammenfatte det hos Descar\-%
tes dualistisk Sondrede, tillige \emph{Naturens} uendelige Væsen:
\emph{Descartes's rent aandelige} Gudsbegreb forvandlede sig
hos \emph{Spinoza} til Begrebet om den guddommelige Substants,
% --- p. 241 ---
\opage{241} % p. 241 bears no note.  No Greek, no ɔ: mark, no guillemets, no bold sorts.
% Latin, roman and unspaced as everywhere in this book: "idea corporis",
% "idea ideæ" (checked at 300 dpi: gaps 3--4 px, i.e. ordinary roman).
% The "16" at the foot is the sheet signature; not transcribed.
der omfattede \emph{Aandens og Naturens} Grundbestemmelser, og,
idet dens Begreb ubevidst paavirkedes af den Idealismen modsatte
Retnings Princip, i sin Værens Form paa væsentlig Maade be\-%
stemtes ved \emph{Naturens} Væren, saa at ogsaa \emph{Naturens Navn}
kunde blive synonymt med Guds, medens paa den anden Side
Naturens Væsen fra Begyndelsen af var bestemt ud fra Tan\-%
kens Abstraction og Tænkningens Attribut blev det overgri\-%
bende.

I \emph{Substantsbegrebet} ligger der fra Begyndelsen af en
\emph{Flerhed af Muligheder} for den nærmere Bestemmelse, og
disse Muligheder have fundet deres historiske Udtryk i de for\-%
skjellige Systemer, for hvilke Substantsens Begreb er det høie\-%
ste. Denne Flerhed af Muligheder træder frem naar Forholdet
mellem \emph{Idealitets-} og \emph{Realitetsmomentet,} og naar For\-%
holdet mellem \emph{hvilende Væren} og \emph{Activitet} (Causa\-%
litet) skal bestemmes. Ved Bestemmelsen af det \emph{første}
Forhold kan nemlig enten den \emph{Idealitetens} Bestemmelse,
der er Begrebets Udgangspunkt, hæves saa afgjørende frem,
at det Absolute, eller det Værende overhovedet, bestemmes
% p. 241 l. 21: the print spaces only the first element of the compound --
% "R e a l i t e t s bestemmelsen" -- measured at 300 dpi, "Realitets"
% 8--9 px between letters, "bestemmelsen" 2--3 px.  Set as printed.  The
% same split occurs at p. 242 l. 9 (Activitetsbestemmelsen), p. 243 l. 20
% (Substantsbegrebet), p. 245 l. 3 (Erfaringsgjenstandens) and p. 248 l. 29
% (Monadebegrebet).
\emph{exclusivt} ved den, og \emph{Realitets}bestemmelsen kun bli\-%
ver \emph{Endelighedens} eller \emph{Phænomenets (Descartes,
Leibnitz);} eller det al Væren omsluttende \emph{uendelige Væ\-%
rendes} Bestemmelse kan søges holdt i \emph{Ligevægt} af Idealitet og
Realitet, men saaledes, at i Virkeligheden Idealitetsmomentet dog
gjør sin oprindelige Overvægt gjældende, hvilket kan skee deels
gjennem Bestemmelsen af Naturbegrebet, idet Naturen kun bliver,
hvad den er i Tankens Bestemthed, og idet Aandens Activi\-%
tetsbestemmelse overføres paa Udstrækningen; deels gjennem
den Fordobbling i Tanken, ifølge hvilken den ikke blot er idea
corporis, men idea ideæ, og saaledes viser sig som det Over\-%
gribende (Spinoza). Men den strænge og \emph{consequente} Gjen\-%
nemførelse af Substantsbegrebet er \emph{den,} der fastholder den
\emph{sidste} Mulighed og lader det uendelige Værende omfatte Til\-%
værelsesmodsætningerne i sin Enhed. \emph{Descartes's} Opfattelse
af det Guddommelige som det rent Aandelige og dog som
% --- p. 242 ---
\opage{242} % p. 242 bears no note.  No Greek, no ɔ: mark, no guillemets, no bold sorts.
% STRAY INKED MARK, p. 242 l. 3: between "Phænomenalt" and "fører" stands a
% short horizontal stroke, 10 px wide and 4 px tall at 300 dpi, sitting ON
% THE BASELINE (rows 399--402 against a baseline of 402--403 and an x-height
% top of 383), not at the mid-height where a hyphen would be.  Both witnesses
% read it as a hyphen ("Phænomenalt-fører" / "Phænomenalt -fører"); the sense
% wants nothing there.  It is not an identifiable sort in an identifiable
% position, so NOTHING is put into the text for it and the measurement is
% logged here instead.  Three further marks of the same family occur in this
% batch -- p. 245 l. 15, p. 250 l. 10, p. 251 ll. 25 and 31 -- and they are
% logged the same way.  See the report.
Aarsag til Naturen indeslutter umiddelbart en Selvmodsigelse,
og \emph{Leibnitzes} Forsøg paa at reducere det Materielle til et
blot Phænomenalt fører, idet den dualistiske Modsætning bli\-%
ver uovervunden, til at ophæve den Uendelighedens og den
altomfattende Værens Bestemmelse, hvilken Begrebet om Sub\-%
stantsen \emph{som} Substants indeslutter.

Ved Bestemmelsen af det \emph{andet} af de ovenfor fremhæ\-%
vede Forhold kan enten, idet \emph{Activitets}bestemmelsen i det
Ideelle accentueres, et \emph{Differentiationens} Princip bringes
ind, der giver \emph{Individualiteten} en blivende og \emph{væsentlig}
Betydning, saa at den ene og uendelige Substantses Begreb
omsættes i Begrebet om de \emph{uendelige Monader;} --- eller
ogsaa kan den \emph{hvilende Værens} Bestemmelse saaledes ac\-%
centueres, at den \emph{abstracte Uendelighed og Enhed} bli\-%
ver herskende og det \emph{Individuelle} kun \emph{et Forsvindende.}
Men ogsaa her er kun den \emph{sidste} Mulighed den, der ud\-%
trykker det \emph{consequent} gjennemførte \emph{Substantsbegreb.} I
Substantsens Begreb maa, ifølge den for dette Standpunkt
charakteristiske Sondring af Modsætningerne, der ogsaa fast\-%
holdes \emph{indenfor} Substantsens Enhed, Begrebet af \emph{hvilende}
og derfor \emph{abstract Væren} træde frem i Forgrunden. Idet i
Sondringen Modsætningerne afsluttede sig i sig, fik Realitets\-%
momentet (Naturmomentet) fra Begyndelsen af den \emph{passive
Værens} Bestemmelse; det sattes i Udvorteshedens, Udstræk\-%
ningens umiddelbare, abstracte Væren. Og Idealitetsmomentet
(Aandsmomentet), der i den oprindelige Sondring fik \emph{Acti\-%
vitetens} Bestemmelse fordi fra det Tankebevægelsen udgik,
antog, under ubevidst Paavirkning af Modsætningen, i den
fæstnede Sondring, der begrændser Activitetsbestemmelsen,
uvilkaarligt og ligesom umærkeligt det Præg, at Activitetens
Bestemmelse kun saaes som \emph{Attribut ved et i sin fær\-%
dige Væren forudsat Subject,} --- men ikke som fra
Grunden af omsættende denne Væren i Energie. Og idet saa\-%
ledes de to Modsætninger mødes i denne \emph{Værens} Bestem\-%
melse, faae vi, naar de, uden ved en indre Bevægelse at for\-%
% --- p. 243 ---
\opage{243} % p. 243 bears no note.  No Greek, no ɔ: mark, no guillemets, no bold sorts.
% Latin "modus", roman and unspaced.  "16 *" at the foot is the sheet
% signature; not transcribed.
bindes til Enhed, sammensluttes i et Omfattende, for dette
Begrebet om den \emph{absolut uendelige} Substants, der i \emph{sin
neutrale færdige Uendelighed} ligesom kun har en \emph{Ten\-%
dents} til at actualisere sig som \emph{Causalitet} og i \emph{qualita\-%
tiv different Bestemthed,} men i samme Øieblik synker
tilbage, hvilende, i sig selv og absorberer de kun acciden\-%
telle, forsvindende Bestemmelser. \emph{Spinozas} Substantsbe\-%
greb, der absorberer det Individuelle i sin uendelige En\-%
hed, og ikke giver nogen Plads for en \emph{Selvbevidsthed} i
egentlig Forstand, er derfor det \emph{consequente} Udtryk for den
\emph{dogmatiske Idealismes} Grundbegreb. \emph{Leibnitzes} Mo\-%
nadebegreb er, som allerede tidligere fremhævet, trods sit ri\-%
gere Indhold, dog, strængt taget, kun det \emph{postulerede} Sup\-%
plement til Spinozismens Grundbegreb; det staaer ligesom paa
Grændsen mellem Dogmatismen og Kriticismen, og vilde, naar
det gjennemtænktes, ved det \emph{Selvbevidsthedens} Begreb,
som det viser hen til, og som Leibnitz opfatter efter dets Eien\-%
dommelighed, føre til Kriticismen og til Opgivelsen af \emph{Sub\-%
stants}begrebet.

2. \emph{Den væsentlige Gjenstand.} Paa den dogmatiske
Idealismes Standpunkt, i dets \emph{consequente} Gjennemførelse,
falder Princip og Tilværelse \emph{umiddelbart} sammen. Principet
er her et Tankens Udgangspunkt, fra hvilket Tanken ikke
kommer bort. Fra den abstracte Uendelighed fører ingen Ud\-%
foldelsens Energie ud til en i sin Endelighed værende Virke\-%
lighed: Alt, hvad der synes at bestaae i endelig Væren, strøm\-%
mer uopholdeligt tilbage til Substantsen, og den Tanke, der i
et forsvindende Øieblik søger at fastholde det, følger dets
Strømning og svinder selv ind i det Uendelige. Det selvbe\-%
vidste Jeg forsvinder i det substantielle Værende, netop fordi
den enkelte Tænkningens modus, ligesom den uendelige Sub\-%
stants, kun faaer Tanken \emph{til Prædicat,} ikke i sit Væsen
gaaer sammen med Tænkningens Activitet, eller begribes i den
Fordobbling, i hvilken Selvbevidstheden viser sig som det ac\-%
tive Selvs \emph{væsentlige} Bestemmelse. Den enkelte tænkende
% --- p. 244 ---
\opage{244} % p. 244 bears no note.  No Greek, no ɔ: mark, no bold sorts.
% GUILLEMETS: one pair, «eminenter» (broken «emi-/nenter»), read at 1200 dpi
% -- opening «, closing », the normal INWARD pair.  Not regularised.
modus har da ikke noget Udgangspunkt for en indre Bevæ\-%
gelse og Bestemmelse, intet Selvhævdelsens Centrum i sig selv.
I Substantsen, hvor Tænkningens Attribut, naar der gjordes
Alvor af Bestemmelsen: \emph{tænkende} Substants, vilde forvandle
\emph{Substantsen} til \emph{Subjectivitet} og derved til \emph{Energie,} faaer
Virksomheden Bestemmelsen af \emph{den til den metaphysiske
Grund sublimerede Naturcausalitet,} og bliver, ifølge det
ovenfor Anførte, under denne Opfattelse som det uendelige Væren\-%
des tidløse og negativitetsløse Causalitet, kun en forsvindende, virk\-%
ningsløs Virksomhed. Væsentlig Gjenstand for Tanken er saaledes
hos Spinoza \emph{kun Substantsen selv;} det, der i den Forstand er
\emph{det Substantielle i Alt,} at dette \emph{Alt} kun er \emph{det Sub\-%
stantielle.} Og om Substantsen som væsentlig Gjenstand
gjælder da den Bestemmelse, der med Hensyn til den nyere
Philosophie som Heelhed blev sat som Fælledsbestemmelse for
Tankens væsentlige Gjenstand: at den er en \emph{dualistisk son\-%
dret, ved det ene Moments Overvægt bestemt To\-%
talitet.}

Ogsaa hvor Grundbegrebet opfattes i andre Nuancer, som
hos \emph{Descartes} og \emph{Leibnitz,} bliver den \emph{væsentlige Gjen\-%
stand} paa samme Maade at bestemme som \emph{det Substan\-%
tielle,} saaledes, at i denne Bestemmelse, trods Naturmomen\-%
tets ligesom uimodstaaelige Fremtrængen, \emph{Idealitetssiden}
definitivt hævder sig sit Principat, enten, som hos \emph{Descar\-%
tes,} ved at blive udelukkende Bestemmelse for det Guddom\-%
melige, og det, der kan omfatte Naturens Væren, taget «emi\-%
nenter», eller, som hos \emph{Leibnitz,} ved at udtrykke det Væ\-%
sentlige, der begrunder Phænomenet. Men det Substantielles
Begreb bliver hos disse Tænkere ikke, som hos \emph{Spinoza,} ved
en skarp Sondring løsnet fra det i Tilværelsen Endelige.

3. \emph{Tænkningens Grundpræg, Methoden.} Til \emph{Sub\-%
stantsen} som Grundbegreb svarer den \emph{Methode,} som vi i
dens Consequents finde udpræget hos dette Standpunkts Hoved\-%
repræsentant, og hvis charakteristiske Grundtræk blive følgende.
Udgangspunktet for Tænkningen, der, som oprindelig forhol\-%
% --- p. 245 ---
\opage{245} % p. 245 bears no note.  No Greek, no ɔ: mark, no guillemets, no bold sorts.
% STRAY INKED MARK, p. 245 l. 15: between "giver" and "Philosophien" stands
% the same kind of short baseline stroke as at p. 242 l. 3 -- both witnesses
% read it as a hyphen ("-Philosophien"), the sense wants nothing there, and
% it is not an identifiable sort in an identifiable position.  Nothing is put
% into the text for it; see the comment at p. 242.
% p. 245 l. 3--4: "E r f a r i n g s g j e n -" is letterspaced (8 px) and
% "standens" on the next line is NOT (3 px) -- the emphasis stops at the
% line break.  Set as printed; see the note at p. 241 l. 21.
dende sig til Idealiteten, til sig selv, er \emph{apriorisk} Tænkning,
og \emph{bliver} det ogsaa hvor den bestemmer \emph{Erfaringsgjen}\-%
standens Væsen, er et \emph{umiddelbart Vist,} til hvilket det
øvrige Tankeindhold knyttes, \emph{ikke} ved en \emph{dialektisk Ud\-%
vikling,} men ved \emph{Slutninger i mathematisk} Form.
Forat disse Slutninger skulle kunne komme istand, bliver et
Materiale optaget, et Apparat \emph{postuleret,} der i Form af
\emph{Axiomer, Definitioner} o. desl. \emph{uden Deduction} anti\-%
ciperer de \emph{væsentlige} Bestemmelser. Og idet Begreberne
ikke vindes ved en Deduction, men postulatsviis optages, faae
de Gyldighed for Tanken i en \emph{Forstandsdistinction,} i
deres fixerede Adskillelse, og i denne benyttes de i det For\-%
standsraisonnement, der indklædes i den mathematiske Form.
Denne Tænkningens Art giver Philosophien Præget af en
Forstandsmetaphysik, hvis Endeliggjørelse af Tanken maa føre
til, at Standpunktet afslutter i det Uphilosophiske.

Det i denne Tænknings Form Eiendommelige viser sig
let som bestemt ved Forholdet til \emph{Substantsen,} der som
Tilværelsens uendelige Væsen ogsaa er Tankens uendelige
Væsen og ved sin Virksomhedsform maa bestemme Tænknin\-%
gens Form.

Den menneskelige Tankes Enhed med Substantsen gjør
denne til Gjenstand for en \emph{umiddelbar Vished,} og denne
\emph{Intuitionens} Vished danner Tænkningens Baggrund. Idet saa
Tanken med uafviselig Nødvendighed gjør den Bevægelse hen
imod det Concrete, ved hvilken alene den kan hævde sig som
Tanke, og ved denne nødvendige Tankebevægelse ligesom
løsner sig fra hiin Hvilen i sin Grund, saa bliver det, at
Substantsen faaer Naturcausalitetens Nødvendighedsform til
Form for sin postulerede Activitet, bestemmende for den
\emph{Tankesammenknytningens Nødvendighed,} der faaer
sit Udtryk i den mathematisk stringente Beviisform. Og det,
at Substantsen \emph{ikke} har et \emph{indre} Udviklingsprincip i sig,
bliver bestemmende for det, at hiin stringente Sammenknytten
dog kun sammenknytter \emph{et allerede Givet og Forudsat,}
% --- p. 246 ---
\opage{246} % p. 246 bears no note.  No Greek, no ɔ: mark, no guillemets, no bold sorts.
% Latin "sub specie æterni", roman and unspaced (gaps 3--3.5 px), as
% transcription.tex §2 records for all Latin in this book.
ikke udfolder et Nyt. Hvad den sammenknytter, maa altsaa
komme til at staae i et \emph{Udvorteshedsforhold} til hinanden,
svarende til Udvorteshedsforholdet mellem de ved Causalitets\-%
forholdet forbundne modi. Men denne Tankesammenknytning
har hiin umiddelbare intuitive Viden af Substantsen til sin
Baggrund, og viser, idet alle Begreberne \emph{postuleres} som
Bestemmelser i \emph{Substantsen,} overalt tilbage til denne.
Skjøndt hvert enkelt Element i Tankebevægelsen maa være
postuleret, bliver det dog, gjennem det supponerede Forhold
til Substantsen, i hvilken Alt drages ind idet det sees sub
specie æterni, Gjenstand for en Intuitionens Vished: Aandens
umiddelbare Enhed med den Substants, der absorberer den,
udelukker enhver Skepsis om det klart Tænkte, ogsaa i For\-%
holdet til Naturen, med hvilken Aanden mødes i Substantsens
Enhed.

Begrebernes Udvorteshedsforhold til hverandre ophæves
ikke derved, at de bestemmes som fremgaaende af Aanden,
som \emph{aprioriske.} Vel hævder den \emph{dogmatiske} Idealisme,
gjennem Bestemmelsen af det Ideelle som activt, den tænkende
Aand som en \emph{oprindelig} Erkjendelsens Kilde; men Sub\-%
stantsbegrebets Eiendommelighed forhindrer den fra at gjen\-%
nemføre Tanken. Selv hvor som hos \emph{Leibnitz} den aande\-%
lige Monade bliver \emph{Udviklingsprincip,} og \emph{Selvbevidst\-%
hedens} Væsen bliver gjort til Gjenstand for en skarpere
Analyse, bliver den \emph{immanente Udviklings} Form dog
ubegreben, og Tanken sees ikke som gjennem en saadan Ud\-%
foldelse sluttende sig sammen til Enhed. Derfor bliver ogsaa
hos \emph{Leibnitz} Udvorteshedsforholdet bevaret og Tankebevæ\-%
gelsens Form bliver hos ham ikke væsentlig forskjellig fra
Spinozas.

Idet det Indhold, der sættes som fremgaaende af Aanden,
ikke erkjendes i denne Fremgaaen, bliver Aandens frembrin\-%
gende Virksomhed i Forhold til det enkelte, bestemte Begreb
strængt taget \emph{kun Postulat,} og det er forsaavidt en uund\-%
gaaelig Consequents, at det Aprioriske opfattes i Givethedens
% --- p. 247 ---
\opage{247} % p. 247 bears no note.  No Greek, no ɔ: mark, no guillemets, no bold sorts.
% Latin: "ideæ innatæ" IS letterspaced here (8--9 px) and takes \emph{} with
% the rest of its phrase, per transcription.tex §2 -- letterspacing, not the
% fount, is what marks it.  "natura naturata", "natura naturans" and "res
% particularis æterna" further down are roman and UNSPACED (gaps 2--3 px)
% and take nothing.  Two em-dashes in the last paragraph.
Form som en \emph{Beholdning af ideæ innatæ,} adskilte fra
hverandre.

I denne Sondring blive Tankerne, der faae Gyldighed i
deres Udvorteshedsforhold til hverandre, som umiddelbart er
begrundet i Substantsbegrebets Eiendommelighed, og definitivt i
den oprindelige \emph{abstracte} Modsætning mellem Tanke og
Udstrækning, \emph{endelige} og \emph{abstracte.} Med disse endelige
og abstracte Tanker maa den dogmatiske Idealismes Tænkning
forholde sig til sin Gjenstand, ligesom bevægende sig om den,
uden at kunne gjøre denne Bevægelse til Gjenstandens egen
Bevægelse, uden at kunne drage den ind i en Begrebets
Strømning, i hvilken de endelige, abstracte Tanker gik sammen
til en indholdsrig, concret Tankeenhed. Og det bliver, paa
Grund af denne Begrebernes Charakteer, en nødvendig Conse\-%
quents, at under denne Tænkningsforms Udvikling, Uendelig\-%
hedstanken, der fra Begyndelsen af skjulte over Endeligheden,
maa trænges tilbage, og det Endelige faae Overmagt. Denne
Consequents træder frem i Tiden efter \emph{Leibnitz} i Philoso\-%
phiens Trivialisering, og den dogmatiske Idealisme ender i
det Uphilosophiske.

4. \emph{Livsanskuelsen.} Dennes Eiendommelighed er allerede
angivet; den er indesluttet i Fordringen af den individuelle
Aands Opgaaen i Substantsen gjennem en Erkjendelse af Til\-%
værelsens evige Enhed med denne. Denne Aandens Gaaen op
i Enheden med Substantsen --- en Enhed, der, seet fra det
Eviges Synspunkt, ikke er en Stræbens Maal, men et evigt
Værende --- forestiller \emph{Spinoza} sig som den enkelte Tanke\-%
modus's Viden af sig som Led i Totaliteten af det, der med
evig Nødvendighed fremgaaer af Substantsens virksomme Væsen,
i den natura naturata, der evigt sættes af den evige natura
naturans, og Aanden skal i denne Viden være sig Gjenstand
som en res particularis æterna. Idet den veed sig som be\-%
stemt ved Substantsens Nødvendighed, saa skal Enheden med
Substantsen forvandle denne Nødvendighed, der fra det Ende\-%
liges Synspunkt fik den strænge Naturnødvendigheds Præg,
% --- p. 248 ---
\opage{248} % p. 248 bears TWO notes, *) and **), both self-contained (neither runs over
% onto p. 249, whose first line is body text).  The footnote separator above
% them is LEFT-FLUSH -- x 256--505 at 300 dpi against a type-area left edge
% of 256 -- and is therefore LaTeX's \footnoterule, not transcribed.
% No Greek, no ɔ: mark, no bold sorts.  Latin "res particularis æterna",
% roman and unspaced.
% GUILLEMETS: one pair, «Imaginationen», read at 1200 dpi -- opening «,
% closing », the normal INWARD pair.  Not regularised.
% The notes are set smaller than the body; measured separately from it, and
% nothing in either note is letterspaced.
\setcounter{footnote}{0}%
til den Væsenets indre Nødvendighed, der for Spinoza er Eet
med Friheden. I denne Anskuelse foresvæver der Spinoza et
Billede af en Substantsens Fylde, af en Aandsorganisme, den
evigt sætter og sammenslutter i sig. Men i denne Opfattelse
ligger der en Modsigelse med hans Forudsætninger. Begrebet
om en res particularis æterna, om en i \emph{Enkelthedens} Form
værende \emph{evig} Tankemodus, indeslutter modstridende Elemen\-%
ter\footnote{Jfr. min Monographie om Spinoza, S. 63.}; thi Evighedens Bestemmelse er for Spinoza uforenelig
med Begrændsningens og Enkelthedens. I Evighedsenheden
med Substantsen maatte derfor den enkelte modus, den indivi\-%
duelle Aand, forsvinde; dens Erkjendelses høieste Act, dens
høieste Selvbekræftelse, vilde blive en Tilintetgjørelse af det
tænkende Individ. Consequent kan Spinoza kun opfatte Sub\-%
stantsen i \emph{abstract Uendelighed} og med en \emph{illusorisk}
Bevægelse, og i Erkjendelsen af denne Substants kan Tanken
ikke finde Hvile, fordi den er det, der negerer den Tanke,
som kun har Realitet ved Existentsadskillelsen mellem det
Endelige og det Uendelige\footnote{Jfr. sammesteds, S. 62, 175 ff.}. Det Endelige, hvis Væren for
«Imaginationen» efter de spinozistiske Forudsætninger bliver
uforklarlig, trænger sig uimodstaaelig ind paa Tanken, og idet
den endelige Aand i sin Individualitet hviler paa en Natur\-%
concretion, saa gjør denne sig gjældende i Bevidstheden, der
ligesom med Magt unddrager sig Absorptionen.

Dette er Livsanskuelsens Charakteer hos \emph{Spinoza,} og
saaledes udpræget er den det \emph{correcte} Udtryk for Grund\-%
begrebets Consequentser. Hos \emph{Leibnitz,} hos hvem Sub\-%
stantsbegrebet ved et Postulat forvandles til \emph{Substantsernes}
Begreb, til \emph{Monade}begrebet, bliver vel Livsanskuelsen modi\-%
ficeret i Overensstemmelse hermed: individuelle frie Substantser
danne i Harmonie en Heelhed, i hvilken hver enkelt, som en
Verden for sig, ud fra sit eget Livscentrum frit skal udfolde
en Uendelighed. Men det er allerede ovenfor (S. 228) blevet
% --- p. 249 ---
\opage{249} % p. 249 bears no note.  No Greek, no ɔ: mark, no guillemets, no bold sorts.
% Latin "monas monadum", roman and UNSPACED (gaps 3--4 px) against the
% letterspaced "Centralmonade," immediately before it -- so it takes nothing.
% MODERN PENCIL on this page, none of it transcribed (§7): a two-word cursive
% marginal note in the RIGHT margin beside l. 11 (outside the type area), and
% pencil underlinings beneath the printed "Monadebegrebet" (l. 12) and
% "Centralmonade," (l. 13).  Both underlined words are already letterspaced
% in the type; the underlinings are a reader's, not the printer's.
% A faint low dot stands between "Forhold" and "til" in l. 11 (minimum grey
% ~150 against a paper of 181 and type of 25 -- far lighter than the marks
% logged at pp. 242 and 245); nothing is set for it.
antydet, at denne Opfattelse viser tilbage til hiin, idet Mona\-%
dens Udvikling, hvis Form ikke begribes, i Kraft af en \emph{præ\-%
stabileret} Harmonie afspeiler \emph{Universet.} Og naar det
har viist sig, at Leibnitzes Substantsbegreb, med sit rigere
Indhold, kun blev et \emph{postuleret} Supplement til Spinozas, og
ikke vilde kunne gjennemtænkes uden at føre til Kriticismen,
paa hvis Grændseskjel det staaer, saa maa ogsaa den derved
betingede Livsanskuelse hvile paa en usikker Grund og ikke
kunne undgaae Inconsequentser. Vanskeligheden i denne
Henseende ligger hos \emph{Leibnitz} i Gudsbegrebets Forhold til
den individuelle Monades. \emph{Monadebegrebet} overføres paa
\emph{Gud,} der bliver \emph{Centralmonade,} monas monadum, og af
ham skulle de endelige Monader være \emph{skabte} og Harmonien
\emph{forud ordnet.} Men idet Gud saaledes som Monade kommer
til at staae \emph{ved Siden af} de andre Monader, saa vil paa den
ene Side Guds \emph{Almagt} tilintetgjøre de enkelte Monaders Grund\-%
bestemmelse: den \emph{selvstændige} Activitet, medens paa den
anden Side Monadens anden Grundbestemmelse: \emph{Individua\-%
liteten,} kommer i Strid med Gudsbegrebets \emph{Uendelighed.}
Naar Forholdet mellem den uendelige, guddommelige Monade
og de endelige, skabte Monader gjennemtænktes, vilde det
enten føre tilbage til Spinozismens Livsanskuelse eller fremad
til et Standpunkt, høiere end Leibnitzes.

% DIVISIONAL RULE, p. 249, between the two paragraphs -- short and CENTRED
% (x 556--805 at 300 dpi, span 249 px = 2.11 cm, centre 680, against a type
% area 119--1233 whose centre is 676), with white space above and below.  Not
% the footnote separator: it stands in mid-page and p. 249 bears no note.
% Minimum grey 79 against a paper of 206 -- ink, not pencil.
% This site is not in RECON.md's confirmed divisional-rule list.
\divrule

I den \emph{dogmatiske Idealisme} er det, der giver den
\emph{Dogmatismens Charakteer,} den \emph{Videns Umiddelbar\-%
hed,} der er begrundet, ikke i Tilværelsesmodsætningernes
\emph{umiddelbare Harmonie,} men deels i den \emph{individuelle}
Tankes, en Skepsis udelukkende, \emph{umiddelbare Enhed med
den absolute, uendelige Substants,} deels deri, at under
Momenternes Modsætning Accenten saaledes, bevidst eller ube\-%
vidst, er lagt paa det \emph{Ideelle,} at dette, som det Absolutes
primitive Væsensbestemthed, i sin Overgriben over Modsæt\-%
% --- p. 250 ---
\opage{250} % p. 250 bears ONE note, *), self-contained (p. 251 opens with body text and
% its own note).  Its separator is LEFT-FLUSH (x 232--809 against a type-area
% left edge of 238) and is not transcribed; the OTHER rule on this page, at
% mid-page above the β head, IS the divisional rule and is set below.
% No Greek in the running text, no ɔ: mark, no guillemets, no bold sorts.
% Two em-dashes.  A faint low dot stands between "til" and "Tvivl" in l. 10
% (5 x 6 px, minimum grey 93 against a paper of 181 and type of 25); it is
% not an identifiable sort in an identifiable position and nothing is set for
% it -- see the comment at p. 242 l. 3.
\setcounter{footnote}{0}%
ningen implicite har \emph{Totalitetens} Bestemthed i sig, og
umiddelbart udtrykker Modsætningens Væsen i sine Former\footnote{Jfr. den dogmatiske Idealismes Lære om \emph{Tankeevidentsen} som Sandhedserkjendelsens Kriterium.}.
Men idet Dualismen i Virkeligheden er uophævet, selv der
hvor Modsætningerne danne Paralleler indenfor den uendelige
Substantses Enhed, saa maa Modsætningen gjøre sig gjældende
\emph{som} Modsætning, og den \emph{umiddelbare Vished} maa vige
Pladsen for en \emph{Tvivl} om Muligheden af en Erkjenden af det
fra Tanken sondrede \emph{Objective,} --- hvilken Tvivl saa ogsaa,
idet Tankens \emph{Endelighed} træder frem i Sondringen, fører
til Tvivl om Muligheden af en Erkjendelse af \emph{Principet} i
\emph{dets Uendelighed.} Fra Tvivlen om Erkjendelsen af det
\emph{Objective,} det \emph{Reale,} gaaer den Form af Idealismen ud,
der afløser den dogmatiske: den \emph{kritiske} Idealisme.

% DIVISIONAL RULE above the L4 head, p. 250 -- short and CENTRED
% (x 681--920 at 300 dpi, span 239 px = 2.02 cm, centre 800, against a type
% area 238--1362 whose centre is 800).  Not the footnote separator, which is
% the left-flush rule at the foot of this page.
\divrule

% L4 HEAD, p. 250.  MEASURED at 300 dpi, exactly as the α head on p. 240:
%   * "Den kritiske Idealisme."   median stem 4.0 px [4.09], n=876
%   * body roman, same page        median stem 3.0 px [3.09], n=1739
%   * the L5 bold head on p. 252   median stem 6.0 px [5.38]
% So the two L4 heads agree with each other (4.0 px) and are distinct from
% both the body (3.0) and the bold display heads (6.0).  NOT letterspaced:
% inter-letter gaps 2.0--3.5 px.  CENTRED: x 539--1044 (centre 792).
% THE ENUMERATOR β.  Read at 1200 dpi: a Greek beta with the descender and
% the two lobes of the book's CURSIVE (italic) Greek fount, slanted, the same
% fount as the α on p. 240 and not an upright roman beta.  Set upright here,
% outside \gk{}, per this batch's brief; the image is recorded, not resolved.
\begin{center}β.\enspace Den kritiske Idealisme.\end{center}

Idet vi skulle bestemme Idealismens anden Hovedforms
Væsen, viser der sig, indenfor det, vi tidligere, med eet Navn,
have betegnet som den \emph{kritiske} Idealisme, en Sondring mellem
\emph{en i strængere Betydning kritisk} Form og \emph{en specu\-%
lativ Form paa Kriticismens Grundlag.} Vi begynde
derfor med at angive disses almindelige Forhold til hinanden.

De skille sig fra hinanden umiddelbarest ved deres \emph{Me\-%
thode,} idet \emph{Kriticismens} fremtrædende Grundpræg er \emph{Tanke\-%
sondringen,} Begrebernes Adskillelse i Reflexionsbestemtheden,
Tankens og dens Formers Adskillen fra Gjenstanden og dens
Indhold; --- \emph{Speculationens} derimod: \emph{Begrebernes dia\-%
lektiske Forening, Tankens Enhed med Sagen} i dens
nødvendige Selvudvikling. Til denne Forskjel svarer en lig\-%
nende i Bestemmelsen af den \emph{væsentlige Opgave for Er\-%
% --- p. 251 ---
\opage{251} % p. 251 bears ONE note, *), self-contained (p. 252 opens with body text and
% bears no note).  The emphasis \emph{Erkjendelsen} and the word itself run
% ACROSS the page marker: "Er-" is the last word of p. 250 and "kjendelsen"
% the first of p. 251, so the page marker, these comments and the
% \setcounter line all carry a trailing % to keep TeX from inserting a space
% inside the word.
% No Greek, no ɔ: mark, no bold sorts.
% NO GUILLEMETS ANYWHERE ON THIS PAGE.  The ABBYY layer renders the last word
% of the note "«karakteriseres"; read on the image at 300 and 1200 dpi the
% print has NO guillemet and reads "charakteriseres." with ch- and a full
% stop -- the book's ordinary spelling, not a k- variant.  The « and the
% missing period are both OCR noise.  Nothing to log as a defect here.
% STRAY INKED MARKS, p. 251, two of them, neither set in the text:
%   * l. 25, after "til" (before "Antydningen"): a dot 6 x 4 px at mid
%     x-height, minimum grey 81 against a paper of 181 and type of 38.
%     ABBYY reads it "til'", tesseract "til:".
%   * l. 31, between "tilbageblivende" and "Forstandsdistinction": a stroke
%     7 x 4 px sitting ABOVE the x-height (rows 1761--1764 against an
%     x-height top of 1766 and a cap top of 1757), minimum grey 105.
% Same family as pp. 242, 245 and 250; see the comment at p. 242 l. 3.
\setcounter{footnote}{0}%
kjendelsen}\footnote{Bestemmelsen af den væsentlige \emph{Gjenstand} for Erkjendelsen og Sammenhængen mellem Forskjellen i Bestemmelsen af denne og i Bestemmelsen af Erkjendelsens væsentlige \emph{Opgave}, vil fremgaae af det Efterfølgende, hvor de to Former af den kritiske Idealisme charakteriseres.} og af \emph{Grundbegrebet.} \emph{Kriticismens} Pro\-%
blem er fra Begyndelsen af et \emph{erkjendelsestheoretisk}
Problem: Undersøgelsen af \emph{det i Subjectets Erkjendelse
Aprioriske i dets Forhold til det Empiriske;} dens
\emph{Grundbegreb} er \emph{Jeget,} der af sin Individualitetsform lader
Begrebet om Subjectivitetens \emph{almene} Væsen og derigjennem
det \emph{Absolutes} Begreb udvikle sig. Den \emph{speculative} Ide\-%
alismes Problem er væsentlig et \emph{ontologisk} Problem, dens
Grundbegreb Begrebet om den \emph{absolute Aand,} i hvilken
\emph{Naturen} er et \emph{ophævet} Moment.

Men gjennem den kritiske og den speculative Idealismes
Sondring træder deres \emph{Enhed} afgjørende frem. Den viser sig
deels gjennem deres \emph{fælleds} Modsætning til den \emph{dogmatiske}
Idealisme, deels ved en Betragtning af dem i og for sig i
deres Udfoldelse fra deres idealistiske Udgangspunkt. Det er
allerede tidligere blevet paaviist, hvorledes den \emph{kritiske} Phi\-%
losophie bar \emph{Retningen imod det Speculative} i sig. I
Hævdelsen af det \emph{Aprioriske} laae allerede Tendentsen til at
see Anskuelsesformerne og Begreberne, som fremgaaende af
den active Aands Grundsynthese, i indre Enhed, og det laae
nær, fra denne Enhed, gjennem Accentueringen af Aandens
Activitet (jfr. \emph{Fichtes} Forhold til \emph{Kant}), at naae idetmindste
til \emph{Antydningen} af \emph{Grundmodsætningernes Enhed.}
Af Problemet om \emph{Erkjendelsen} udfoldede sig, som ligeledes
er bleven paaviist, det \emph{ontologiske} Problem, og \emph{Jegets}
Begreb udviklede sig til Begrebet om hvad der som \emph{fælleds
intelligibelt Substrat} omfatter \emph{Aand og Natur} i Enhed.
Og paa den anden Side have vi seet det \emph{kritiske} Element
gjennem den tilbageblivende Forstandsdistinction at gjøre sig
% --- p. 252 ---
\opage{252} % p. 252 bears no note.  No Greek, no ɔ: mark, no guillemets, no bold sorts
% inside the paragraphs.
% p. 252 l. 1: the print sets a visible gap (14 px at 300 dpi) between
% "Begrebsbestemmelse" and its comma; the comma is transcribed hard against
% the word, as the sense and every other comma on the page require.
gjældende i Speculationens Begrebsbestemmelse, Dualismen
ubevidst at trænge sig ind mellem Tanken og Tankens Andet,
mellem Aanden og Naturen. Det \emph{kritiske} og det \emph{specu\-%
lative} Element ere altsaa \emph{begge} tilstede i \emph{begge} Former,
og Forholdet imellem de to Elementer et saadant, som det be\-%
tinges derved, at dette Stadium af Idealismen \emph{med Bevidsthed}
tager sit Udgangspunkt i \emph{Subjectet,} hvorved Overvægten
maa lægges over paa det \emph{Subjective,} saaledes opfattet, at
det i \emph{sin Modsætningsform} bliver sat som i \emph{sin virk\-%
somme Enhed overgribende over det Objective} og
omfattende dette. Dette maa nemlig føre til, at det \emph{specu\-%
lative} Element efterhaanden, saaledes som det under den
historiske Udvikling er skeet, faaer Overvægten over det \emph{kri\-%
tiske,} dog uden at kunne fortrænge dette.

Dette \emph{kritiske} Element, der er betinget ved \emph{Grund\-%
modsætningernes oprindelige Stilling til hinanden}
i den \emph{nyere Idealisme,} træder frem i \emph{alle} dennes Former.
Det gjør sig gjældende i den \emph{Forstandsdistinction,} der i
den \emph{dogmatiske} Idealisme sondrer Tilværelsens Modsætninger
absolut og lader Tankerne fæstne sig i deres Adskilthed; det
gjennemtrænger i \emph{Kriticismen} Løsningen af Erkjendelses\-%
problemet, og standser ligesom med Magt paa ethvert Punkt
den antydede Forening af de sondrede Former; det gjør sig
i den \emph{speculative} Idealisme gjældende i den bevarede Dua\-%
lisme og i den under Identitetsdialektiken skjulte blivende
Sondring af Begreberne.

% DIVISIONAL RULE above the L5 head, p. 252 -- short and CENTRED
% (x 672--922 at 300 dpi, span 250 px = 2.12 cm, centre 797, against a type
% area 245--1358 whose centre is 801).  p. 252 bears no note, so this is not
% a footnote separator.
\divrule

% L5 HEAD, p. 252.  MEASURED at 300 dpi:
%   * "Den kritiske Idealisme i snævrere Betydning."  median stem 6.0 px
%     [5.38], n=1368
%   * the enumerator "I."                             median stem 7.0 px
%     [6.37], n=35  -- i.e. the enumerator IS bold here, unlike the roman
%     "b." of the L3 head on p. 240
%   * body roman, same page                           median stem 3.0 px [3.48]
% So BOLD, ~1.8x the body, exactly matching the L3 head on p. 240 (6.0
% [5.35]).  NOT letterspaced: inter-letter gaps 2.5--3.5 px.  CENTRED:
% x 379--1208 (centre 794) in a type area 245--1358 (centre 801).
%
% THE "I." HERE BEGINS A DIFFERENT SEQUENCE from the "I.--IV." theses at
% pp. 50, 79, 137 and 170.  This I. and the II. at p. 259 enumerate the two
% forms of the KRITISKE Idealisme under the β head of p. 250; they are not a
% continuation of, and must not be renumbered or harmonised with, the four
% theses.  transcription.tex §3 records the same.  A later reader meeting a
% second "I." will otherwise take it for a duplicate.
\begin{center}\textbf{I.\enspace Den kritiske Idealisme i snævrere Betydning.}\end{center}

1. \emph{Principet} i den kritiske Idealisme maa, efter det
paa et tidligere Sted Udviklede, bestemmes \emph{definitivt} ud
fra det i \emph{Kritik der Urtheilskraft} angivne Synspunkt. Af
\emph{Jeget,} der i sin Activitet som det Aprioriskes Udgangspunkt
% SEAM, pp. 252/253.  p. 252's last line ends "Udgangspunkt" with no hyphen
% and the sentence runs on into p. 253 ("blev Centralpunktet i Kritik der
% reinen Vernunft, ..."), so the pp. 253--266 batch must NOT open a new
% paragraph, and joints.py must not be allowed to leave the blank line that
% splice.py's marker replacement puts here.  p. 252 bears no footnote, so
% nothing runs over into p. 253.
% --- p. 253 ---
\opage{253} % SEAM CHECK (BATCH-AGENT §5.1): p. 253 carries NO footnote and no note area at
% all -- no footnote separator rule, no unmarked matter at the head of the note
% region.  p. 252 (previous batch) likewise shows no note in either witness.
% The p. 252/253 seam is therefore clean: nothing runs over.
% p. 253 also opens mid-sentence, continuing p. 252's "... som det Aprioriskes
% Udgangspunkt" -- do NOT let splice.py leave a paragraph break here.
blev Centralpunktet i Kritik der reinen Vernunft, og endnu
% p. 253: "Kritik der reinen Vernunft" is set in the SAME upright roman as the
% Danish -- no Fraktur, no italic.  Checked at 600 dpi.  Not letterspaced here.
som udviklet til det Absolutes Form (hos \emph{Fichte}) bevarede
\emph{Jegets Navn}, udviklede sig, gjennem Begrebet om \emph{Jegets
almene Væsen}, og gjennem \emph{Antydningen} af dettes Enhed
med det \emph{Objectives almene Væsen}, Tanken om det for
% p. 253: the run of letterspacing begins at "Aand", not at "for" --- "det for"
% is tight at 1200 dpi (spacing.py flags "for" at 1.30; a short-word false
% positive).
\emph{Aand og Natur fælleds intelligible Substrat}, der vel
skulde unddrage sig al bestemt \emph{Erkjendelse}, men dog ikke
blot blev staaende som en \emph{nødvendig} Tanke og som den
\emph{høieste} Tanke, men \emph{indirecte} bestemtes saaledes, at Foreningen
af Modsætningerne maatte tænkes bestemt \emph{ved Idealitetens
Overvægt}: en Overvægt, til hvilken allerede Betegnelsen:
\emph{intelligibelt} Substrat viste hen.
% p. 253: "Substrat" after "intelligibelt" is NOT spaced (1200 dpi), although
% "intelligible Substrat" ten lines above IS.

I dette Princip er den \emph{kritiske} Sondring af Tilværelsens
Grundformer traadt tilbage for den \emph{antydede speculative}
Enhed, om end kun saaledes, at denne atter, paa den \emph{bestemte
Tænknings}, paa den egentlige \emph{Erkjendelses}, Gebet maa vige
for det kritiske Element. Og i den i \emph{Tilværelsesprincipet}
antydede Enhed ere ogsaa, idet Principet efter sit Væsen tillige
er \emph{Erkjendelsesprincip}, Consequentser indesluttede for den
ved dette Princip bestemte Tanke, der blive udviklede i den
\emph{speculative} Idealisme.

Ved det Udgangspunkt i \emph{Jeget}, i \emph{Selvbevidstheden},
fra hvilket den \emph{kritiske} Idealisme naaer hen til sin Bestemmelse
af Principet, vises vi tilbage til den \emph{dogmatiske} Idealismes
Udgangspunkt. Men ved den Vei, disse to Former tage
fra det fælleds Udgangspunkt, skille de sig væsentlig fra hinanden.
Hos \emph{Descartes} er, idet han gaaer ud fra Jeget, dog
% p. 253: "cogito ergo sum" is upright roman, exactly like the Danish, and is
% NOT letterspaced -- checked at 600 dpi.  No \textit{}.
strax ved Begyndelsespunktet i cogito ergo sum Væren sat i
Ligevægt med Tanken, og under Substantsbegrebets Udvikling
i den dogmatiske Idealisme falder Eftertrykket saaledes paa
Væren, at den bliver Subjectet, for hvilket Tanken kun er et
tilføiet Prædicat, og som uendelig hvilende Væren absorberer
den hos \emph{Spinoza} Selvbevidstheden. I den \emph{kritiske} Idealisme
derimod faaer Tanken, Idealitetsmomentet i dets Activitet,
Overvægt over Væren, Realitetsmomentets Reflex; den
% --- p. 254 ---
\opage{254} \setcounter{footnote}{0}
bliver i sin Bestemthed ved Activiteten, ved Subjectivitetens
Form, det sig selv sættende, sin Modsætning ideelt omfattende
Princip.

2. Tankens \emph{væsentlige Gjenstand} bliver, naar Synspunktet
tages fra Kriticismens historiske Udgangspunkt, med
dens eget Udtryk at bestemme som det \emph{Aprioriske}\footnote{Jfr. \emph{Kants} Bestemmelse af Philosophien som det Aprioriskes Videnskab.}. Dette
% p. 254 note: printed mark *).  "Kants" is letterspaced; "Videnskab" is NOT
% (the apparent gap in "Vid-en" is an artefact -- "skab." on the turn-line is
% tight at 600 dpi).
Aprioriske, der er det, der constituerer Erkjendelsen \emph{som} Erkjendelse,
% p. 254: the letterspacing is on "som" ALONE.  "Er-" at the end of the line is
% tight at 1200 dpi and "kjendelse," on the next line is tight; a first reading
% at 600 dpi that took "Er-" for spaced did not survive the 1200 dpi check.
d. v. s. giver den Almeengyldighed og Nødvendighed,
medens det Empiriske kun bliver Erkjendelsens Betingelse,
opfattes saa, om det end, som formende det Givne, faaer et
Forhold til det Objective, som \emph{Former}, der \emph{kun} tilhøre det
\emph{ideelle}, active \emph{Subject}, hvis Virksomheds Art de udtrykke,
og netop dette exclusive Forhold til Subjectet skal betinge det
Aprioriskes Betydning for den almeengyldige og nødvendige
Erkjendelse. Idet nu hos \emph{Kant} Bestemmelsen af Grundbegrebet
gjennemløber den tidligere paaviste Udvikling, saa bliver
det vel, gjennem Principets \emph{definitive} Bestemmelse, antydet,
at de aprioriske, Subjectet tilhørende, Bestemmelser ikke blot
som et Fremmedt anvendes paa Naturphænomenerne, men at
de ere et Subjectivt-Objectivt, efter deres Væsen gyldige for
Naturen som for Aanden (jfr. \emph{Kants} Bestemmelser om det
Teleologiske), og herigjennem antydes et \emph{concretere} Tankens
Indhold. Men hvor Kriticismen ikke blot antyder, men skarpere
bestemmer det Aprioriskes Forhold til det Empiriske, der
bliver hiin oprindelige Opfattelse af det Aprioriske kun som
\emph{subjective Former} fastholdt, og netop i den ligger en væsentlig
Forskjel mellem Kriticismen og den Speculation, der
udvikler sig af Kriticismen. Naar det Aprioriske saaledes
bestemmes blot som Former, saa maa den aprioriske Erkjendelse
bevare \emph{Abstractionens} Præg, og dette Præg maa bestemme
Opfattelsen af \emph{Naturen}, forsaavidt den gaaer ind
under Philosophien. Det Aprioriske er det, der «gjør Phæno
% --- p. 255 ---
\opage{255} \setcounter{footnote}{0}
% p. 255 is one of the eleven two- and three-note pages on which spacing.py's
% single-page fit must be split at the footnote rule (BATCH-AGENT §3); the
% letterspacing below was in every case confirmed on the image at 600 dpi and,
% for the line-end cases, at 1200 dpi.
menernes Indbegreb til Natur»\footnote{Jfr. \emph{Kants} Udsagn: Naturen er kun ved den transscendentale Apperception.}: Naturens almindelige Love,
af hvilke de særegne \emph{ikke} skulle kunne udledes (jfr. Kritik der
reinen Vernunft), bestemmes ved det. Men disse almindelige
Love udtrykke kun det Formelle og Abstracte; Naturen sees
% p. 255: a small round brown speck stands in the space between "den" and
% "abstract-ideelle" (both witnesses render it, ABBYY as "den* abstract",
% tesseract as "den”abstract").  It is not type -- lighter than the ink and of
% no letter shape -- and is not transcribed.
gjennem dem som bestemt ved den abstract-ideelle Subjectivitets
Lov. Hvad Naturen er \emph{for} Tanken, er den kun \emph{ved}
Tanken; denne forholder sig i Naturen som Natur i Kantisk
Betydning directe \emph{til sig selv}, om end, gjennem «Indbegrebet
af det phænomenale \emph{Givne}», et objectivt Correlat til Fornemmelsen
gjør sig gjældende i sin Selvstændighed\footnote{Jfr. Kritik der reinen Vernunft (transscendentale Analytik) og Metaphysische Anfangsgründe der Naturwissenschaft.}.
% p. 255, THE BOOK'S GERMAN TEST CASE, RE-CONFIRMED: the Kant title
% "Metaphysische Anfangsgründe der Naturwissenschaft" in the **) note is set in
% the SAME upright roman antiqua as the surrounding Danish.  Read at 600 dpi:
% ordinary roman M, A, g, ü with a plain two-dot umlaut, and serifs identical to
% the Danish above it.  There is NO Fraktur anywhere on the page, and no italic.
% "Kritik der reinen Vernunft" in the same note is likewise roman.  Neither
% title is letterspaced, so neither takes \emph{}; per transcription.tex §2 and
% BATCH-AGENT §3, foreign matter never takes \textit{}.

Hvad der gjælder om \emph{Kant}, gjælder i endnu høiere Grad
om \emph{Fichte}, hos hvem Jegets Forhold til sig selv i sin Activitet
er et Forhold, i hvilket Naturen kun kaster en ligesom
forsvindende Skygge ind.

3. \emph{Tænkningens Grundpræg, Methoden.} Naar vi
% p. 255: the run-in numbered head "3. Tænkningens Grundpræg, Methoden." is
% LETTERSPACED roman at body size, not bold: measured letter gaps 10--16 px at
% 600 dpi against 5--9 px in the neighbouring roman, and stem width 7 px, the
% body value (the bold L5 head on p. 259 measures 12 px).  The numeral "3." is
% NOT spaced and is left outside the \emph{}.
betegne Kriticismens Methode som \emph{Forstandsdistinctionens},
altsaa ved den Betegnelse, som vi have anvendt som
den \emph{fælleds} Betegnelse for hele den nyere Philosophie, og
netop seet som begrundet i det \emph{kritiske} Element i alle dens
Former, i hvilket Dualismen i Principet reflecterer sig, saa
tage vi hiin Betegnelse i \emph{prægnant} Betydning i dens Anvendelse
paa den Form af den nyere Philosophie, der ved selve
sit Navn viser sig som den \emph{egentlige Repræsentant} for
hint \emph{kritiske} Element. Dette træder her frem i sin skarpest
udprægede Form, idet Sondringen mellem Tankerne, som baade
den realistiske Philosophie og den dogmatiske Idealisme gjorde
gjældende med mere eller mindre klar Bevidsthed, i Kriticismen
træder frem som \emph{fuldstændiggjort} ved og i sidste
Instants hvilende paa \emph{Grundmodsætningen mellem Subjectivt
og Objectivt}. \emph{Descartes} gjør vel, fra Begyndel
% --- p. 256 ---
\opage{256} sen af, med Bevidsthed den skarpe Sondring mellem Aand og
Natur, men denne Sondring forhindrer hos ham ikke den dogmatiske
Forudsætning af Muligheden af en Erkjendelse af det
Objective. Først idet Modsætningen faaer Formen af en Modsætning
% p. 256: a short horizontal tick is inked at the foot of the final "t" of
% "Subjectivt", and a dot stands above "og" on the same line; both are lighter
% than the ink and of no letter shape.  Not type; not transcribed.
mellem Subjectivt og Objectivt, kan Kriticismens Grundproblem
træde frem og Forstandssondringen overalt gjennemføres.
Denne \emph{abstracte} Sondring af det Subjective og det
Objective gjør \emph{Kant}, om end gjennem en \emph{Feilslutning},
strax ved Fornuftkritikens Begyndelse, og af denne Sondring,
der indeslutter den skarpe Adskillelse mellem Tankens \emph{Form}
og \emph{Indhold}, mellem \emph{Apriorisk} og \emph{Empirisk}, bliver det Følgen,
at Væren undslipper Tanken, at det Iogforsigværende som
et Bestemmelsesløst sondrer sig fra den paa det \emph{theoretiske}
Gebet, og at dette Iogforsigværende og det Ubetingede, der er
dets prægnanteste Udtryk, end ikke der, hvor dets Væren faaer
\emph{Kjendsgjerningens Vished}, bliver en Gjenstand for Erkjendelse.
Og idet det Iogforsigværende, i og udenfor os, i
hvilket Modsætningernes Enhedsgrund vilde være at søge, for
Erkjendelsen bliver et Bestemmelsesløst, saa maa der, ligesom
der sondredes mellem Numenets og Phænomenets Sphære, og,
i Overensstemmelse hermed, mellem Frihedens og Naturens,
paa lignende Maade factisk sondres mellem de enkelte Grundbegreber
og Anskuelser, skjøndt de alle postuleres at gaae tilbage
til \emph{Selvbevidsthedens Enhed}, og skjøndt \emph{Kant} i
Tredelingerne af Kategorierne antyder Trehedens to første Leds
Forbindelse i det tredie.

Gjennem alle disse Sondringer, ved hvilke Kant vil sikkre
Philosophien en, om end begrændset Eiendom (see ovenfor S.
66, 72), gjør den Tankens Form sig gjældende, der beherskes
af Modsigelsesgrundsætningen, men som dog ikke kan fjerne
Modsigelsen. Modsigelsen anerkjendes af Kriticismen indirecte
som væsentlig i Antydningerne af det Speculative, og den
\emph{bliver i den menneskelige Bevidsthed}, der rummer alle
disse Modsætninger, og i selve det \emph{menneskelige Væsen},
der skjøndt det af sit Centrum udfolder de aprioriske Former
% --- p. 257 ---
\opage{257} som \emph{sine} eiendommelige Former, dog ikke gjennem dem skal
komme i et Bevidsthedsforhold til sig selv, men, afspeilet i
disse Former, forvandle sig til et Phænomenalt.

Hos \emph{Fichte} træder den \emph{kritiske} Methode frem i en
% p. 257: no comma after "Form" before "idet"; both witnesses agree, and the
% image at 600 dpi shows none.  Transcribed as printed.
eiendommelig nuanceret Form idet fra Begyndelsen af den kritiske
Grundsondring er gjennembrudt derved, at \emph{Jeget} er sat
som det \emph{ene} Værende, Objectet ligesom kastet ind i Jeget;
men med det Samme det Kritiske er bragt tilbage ved det
% p. 257: a short horizontal stroke is inked above the space between
% "Uundværlighed" and "for"; not type, not transcribed.
\emph{uforklarede} Objectives Uundværlighed for Jeget. I hiin Bestemmelse
af Jeget, ved hvilken det, trods Bibeholdelsen af
dets Navn, er forvandlet til et \emph{Absolut}, ligger det Speculative
skjult, der udvikler sig af Videnskabslærens Standpunkt;
men i Bestemmelsen af Udgangspunktet som \emph{Jeg}, som reen
% p. 257: a small horizontal mark stands above the "o" of "Reflexions-"; it is
% not a Danish diacritic and not a Greek accent, and is read as an ink speck.
% The printed word is "Reflexionsbestemmelsen"; only "Reflexions" is
% letterspaced, "bestemmelsen" is tight at 1200 dpi.
\emph{Selvbevidsthed}, er i \emph{Reflexions}bestemmelsen den skjulte
Modsætning til det Objective bevaret, og den Tænkning, der ud
fra hiin Forudsætning bestandig paany støder paa et Ikke-Jeg,
der vides som forsvindende og dog aldrig vil forsvinde, udvikler
en \emph{Reflexionsvirtuositet}, der i uendelig Gjenspeiling
lader Modsætningernes Tanker brydes i hinanden, men
uden at kunne samle dem til Enhed, og som, idet den Opgave
stilles, at deducere Kategorierne som nødvendige Bestemmelser
ud fra Jeget, kun tilveiebringer en tilsyneladende Deduction
ud fra Bevidsthedens Reflexionsstandpunkt, ikke en
immanent Udvikling ud fra den sande Enhed af det Subjective
og det Objective.

4. \emph{Livsanskuelsen}, der svarer til dette Standpunkt,
bliver at bestemme gjennem en Sammenholden af \emph{Kants}
% p. 257: "Kritik der praktischen Vernunft" -- German set in the same upright
% roman as the Danish; "praktischen" alone is letterspaced.  600 dpi.
Kritik der \emph{praktischen} Vernunft, seet i Belysning af hans
Kritik der \emph{Urtheilskraft}, med \emph{Fichtes praktiske Videnskabslære},
seet i Belysning af hans \emph{sildigere} Standpunkt
og dets Opfattelse af \emph{Religionen}.

Idet \emph{Kant} i sin Kritik d. pr. Vernunft accentuerer \emph{Friheden}
som vor Aands \emph{Væsen} og vor Handlings Lov, som
\emph{Eet med Fornuften} som lovgivende og selvbestemmende,
som det Absolute i Subjectet, tager han, medens han gjennem
% p. 257: sheet signature "17" at the foot; not transcribed (BATCH-AGENT §6).
% --- p. 258 ---
\opage{258} disse Bestemmelser antyder den concrete, sig realiserende Idee,
ligesom i Kr. d. r. Vernunft sit Udgangspunkt fra \emph{Jeget}, og
først idet dette, som hørende til en dobbelt Verden, gjennem
den abstracte Modsætning mellem Frihed og Natur, fører ud
i en uendelig Skullen, i en Kamp med det Naturlige, i hvilken
Maalets Opnaaelse, Realisationen af det høieste Gode,
bliver tvivlsom, i en Kamp med Drifter og Tilbøieligheder,
der aldrig afsluttes, statuerer han, for at løse denne Tilværelsens
Uro, gjennem et praktisk Postulat, ikke blot en Udødelighed,
men en Maalet betryggende \emph{Gud}. I Kritik d. pr. V.
staaer Jeget i Frihedens og Selvstændighedens Lys, gjennem
Handling i uendelig hvileløs Kamp stræbende mod Realisationen
af sin Væsensbestemmelse; dets ethiske Handlen har \emph{Selvhævdelsens}
Form. Men Loven, der opfyldes, Væsenet, der
virkeliggjøres, viser \emph{ud over Individet} til Menneskets \emph{almene}
Væsen i dets \emph{antydede} Enhed med det \emph{Objectives}
Væsen, og idet det i Forestillingen om en Naturen og Friheden
harmoniserende Gud antydede Begreb af det \emph{subjectivt-objectivt
Absolute} (det «fælleds intelligible Substrat for
Natur og Aand») hæves frem, er Begrebet givet om det, i
hvis Proces det \emph{individuelle} Jeg gaaer ind som et \emph{behersket}
Moment, der omsluttes af og optages i det \emph{Absolutes}
Livsproces.

Saaledes viser Forholdet sig allerede tydeligere hos \emph{Fichte}.
I \emph{Videnskabslærens} Ethik træder tilsyneladende Jegets
\emph{Selvhævdelse} frem i sin intensiveste Form. Den ved Friheden
frembragte Uafhængighed af Naturdriften: Selvvirksomheden
for Selvvirksomhedens Skyld, bliver Løsenet, og denne
Selvvirksomheds Subject er Jeget som individuelt. Men allerede
her sættes det som \emph{Ideal} forestillede \emph{Maal} som den al
\emph{Individualitet ophævende Tilbagevenden til Subject-Objectet},
det \emph{absolute} Jeg, og gjennem Jegets frie
Handlen fuldbyrder sig det som den \emph{moralske Verdensordens}
evige Lov virksomme Guddommelige, der kun har Væren
% p. 258, HEAVY SORT, last line of the page: the opening word "i" of
% "i denne Realisationsproces" is set from a markedly heavier fount than its
% neighbours.  MEASUREMENT (BATCH-AGENT §3, step 2), all at 600 dpi with a
% fixed threshold of 150 on the greyscale render: the candidate's stem, taken as
% the median longest horizontal dark run over the middle 45--70 % of the glyph's
% vertical extent, is 13 px, and the whole glyph is 18 px wide.  The
% certain-roman vertical stems on the SAME line (n, d, l, u, b, h in "denne",
% "Realisationsproces", "idet", "Philosophie") measure 6--9 px, and nine certain
% roman "i" on p. 258 l. 5 ("i en uendelig Skullen, i en Kamp ...") measure
% 12--20 px overall width with 8--12 px stems at 1200 dpi, i.e. 6--10 px at 600.
% The certainly bold L5 head on p. 259 measures a median stem of 12 px at the
% same dpi and threshold.  The candidate therefore measures at the bold value
% and at roughly 1.8x the roman (20 px against a page median of 10 px in the
% final pass's 900 dpi sweep, the largest of the eleven).  ADJUDICATED as
% EMPHASIS, not a foul case; \textbf{} kept.  See header §2.
\textbf{i} denne Realisationsproces. Og idet \emph{Fichtes} Philosophie ud
% --- p. 259 ---
\opage{259} vikler sig til sin \emph{sildigere} Form, i hvilken \emph{Gud}, det Absolute,
faaer den \emph{absolute Værens} Bestemmelse, og i \emph{Bestemmelsen
af uforanderlig Isigværen} løfter sig op over
Bestemmelsen af \emph{absolut Viden}, saa bliver det \emph{høieste}
Maal for Jeget ikke længere \emph{Moralens} uendelige rastløse
\emph{Stræben}, men \emph{Religionens Væren i det Guddommelige},
der betinges \emph{ved Jeghedens Selvhengivelse}, og opfattes,
ikke som Kamp og Handling, men som en \emph{salig anskuende
Nyden}.

Saaledes fører baade hos \emph{Kant} og \emph{Fichte} den i Reflexionssondringen
grundende uendelige \emph{Stræbens Uro} tilbage
til \emph{en antydet Hvile i det Absolute}, Jegets \emph{Selvhævdelse}
% p. 259: "til" before "en antydet Hvile" is NOT spaced (1200 dpi); the run
% begins at "en".
til \emph{Selvhengivelse}; men i det Absolute, der hos Kant med
% p. 259: a short horizontal stroke is inked above the space after
% "Erkjendelsen" (ABBYY renders it as an apostrophe); not type, not transcribed.
Bevidsthed unddrages Erkjendelsen, og hos Fichte kun paa
ubegribelig Maade reflecterer sin Væren i Viden, har Jeget
kun det \emph{Bestemmelsesløse}, i hvilket det med sin Bestemthed
og sine indre Modsætninger kun vil kunne finde en \emph{illusorisk}
Befrielse fra Uroen i den drømte Forsoningens Hvile.

% DIVISIONAL RULE, p. 259: short, CENTRED hairline with white space above and
% below, standing between the paragraph above and the II. head below.  Measured
% at 600 dpi: the rule runs x 1063--1305 against a text block of x 240--2530 --
% rule centre 1184, block centre 1385 -- and is 1.02 cm long.  It is NOT the
% footnote separator: p. 259 bears no footnote, and the separator is left-flush.
\divrule

% HEAD, p. 259 (L5, transcription.tex §3): "II. Den speculative Idealisme.",
% the SECOND member of the I./II. sequence begun at p. 252 under the L4 head
% "β. Den kritiske Idealisme" (p. 250).  This is a DIFFERENT sequence from the
% I.--IV. theses at pp. 50/79/137/170 (transcription.tex §3); it is neither
% renumbered nor harmonised.
%
% MEASURED, NOT ASSUMED (BATCH-AGENT §6).  All figures at 600 dpi, fixed
% threshold 150, same script for head and controls:
%   this head           stem width median 12 px (range 9--18); intra-word
%                       letter gaps median 5 px (range 1--9)
%   ordinary roman body line on p. 259
%                       stem width median  7 px; letter gaps median 7 px
%   letterspaced body run on p. 259 ("absolute Værens")
%                       stem width median  6 px; letter gaps 10--25 px
% So the head is BOLD (stem 1.7x the roman) and is NOT letterspaced (its gaps
% are at or below the ordinary roman value, and nothing like the 10--25 px of
% real Sperrsatz).  x-height 43--48 px against 42--49 px in the body: the head
% is at BODY SIZE, so no \large.  \textbf{} and not \emph{}.
% The enumerator "II." is bold with the rest, and is followed by a gap of
% 117 px against a word space of 52--53 px elsewhere in the head, i.e. rather
% more than two word spaces -- set \quad, matching the treatment of the p. 252
% head "I.\quad Den kritiske Idealisme i snævrere Betydning." in the
% pp. 239--252 fragment.
\begin{center}\textbf{II.\enspace Den speculative Idealisme.}\end{center}

Den Form af den nyere idealistiske Philosophie, som vi
betegne som \emph{speculativ} Idealisme, repræsenteres af de Systemer,
for hvilke, til Adskillelse fra Kriticismens principielle
Sondring af Modsætningerne, Modsætningernes Væsensenhed,
Enheden af det Subjective og det Objective, af Aand og Natur,
blev Løsenet. Disse Systemer ere, naar vi see bort fra
% p. 259, PRINTER'S DEFECT: the comma after "Hegelske" is set at x-height
% instead of on the baseline -- a raised or mis-registered comma sort, read at
% 2400 dpi.  ABBYY renders it as an apostrophe, tesseract as a colon.  The sort
% is a comma and is transcribed as one; the fault is in its position.
de ufuldkomne Tilløb, det \emph{Schellingske} og det \emph{Hegelske},
det første dog indenfor en vis Begrændsning, der sildigere
vil blive udviklet.

Idet vi see denne Form af Idealismen, hvis Forberedelser
indenfor den i snævrere Betydning \emph{kritiske} Idealisme vi allerede
% p. 259: sheet signature "17*" at the foot; not transcribed.
% --- p. 260 ---
\opage{260} i det Foregaaende have paaviist, som Afslutningen af den
hele idealistiske Udviklingsrække, og som en Afslutning, fremgaaet
af en sammenhængende, indenfra bestemt Udvikling,
maae vi vente at see dens Grundtanke antydet allerede \emph{ved
Udviklingens Begyndelse}, ved den \emph{dogmatiske} Idealismes
% p. 260: the letterspacing stops at the line break -- "dogmatiske" is spaced
% but "Idealismes" is NOT: both "Idea-" at the line end and "lismes" on the
% turn-line are tight at 1200 dpi.  (At 600 dpi "Idea-" reads as spaced; it is
% not.  Compare p. 253, where "ved Idea-/litetens" IS spaced throughout, checked
% the same way.)
første Grundlæggelse.

En saadan Antydning af den speculative Idealismes Grundtanke
finde vi ogsaa, som tidligere berørt, allerede hos \emph{Descartes}.
Hans Tænkning tager sit Udgangspunkt fra \emph{Jeget},
og Tanken faaer ligesom først Fodfæste i cogito ergo sum.
Men Jeget, der kun bestemmes ved den \emph{abstracte Tanke},
% p. 260: the "=" between «Jeg er» and «Tanken er» is an equals sign in running
% text (two parallel horizontal strokes at 600 dpi), not a dash and not maths.
saa at «Jeg er» bliver = «Tanken er», har i Visheden om
sig kun Tankens Vished om \emph{Tankens Væren}, og nærmere:
om den individualitetsløse, \emph{almene} Tankes Væren. Idet Jeget
falder sammen med Tanken, bliver, ifølge Tankens Natur: at
være Tænken af det Almene, Evige, Uendelige --- hvori ligger,
at Uendeligheden er Tankens Væsen --- \emph{Uendelighedens
Tanke} Jegets \emph{første positive Tanke}, det Endelige kun
en secundær, der tænkes ved en Begrændsning af hiin. I Tanken
om det Uendelige, om Gud, tænker Jeget da oprindelig
sit eget Væsen, først i den tænker det det, og den menneskelige
Tanke er i den i Enhed med det tænkende Uendelige.
Denne Consequents er berørt i Descartes's Opfattelse af det
Endelige som Negation af det positive Uendelige, og i hans
dybere, men ikke fastholdte, Opfattelse af Guds Væren i vor
Tanke i den med ham identiske Gudsidee, og af vor Tænken
af denne i den samme Act, ved hvilken vi tænke os selv; ---
og den bliver klarere udtalt i Descartes's Skole (jfr. navnlig
\emph{Geulincx}). Til denne Tanke om Gud som vor Tankes første,
med vort tænkende Væsen identiske Gjenstand, knytter sig saa
Forestillingen om Tankens Modsætning: Materien som det,
der \emph{eminenter} er i Gud, ved hvis skabende Virksomhed Naturen
er. Idet Gud sættes som \emph{Aarsag} til Materien, men
Aarsagen kun kan være Aarsag til det, der er begrundet i
dens Natur, saa maa han, om end bestemt som \emph{aandeligt}
% --- p. 261 ---
\opage{261} \setcounter{footnote}{0}
Væsen, have dens Bestemtheds Grund i sig, og dette er det, der
antydes, idet Muligheden af, at Materien eminenter skulde
kunne være i Gud, udtales\footnote{Jfr. \emph{Geulincx} og \emph{Malebranche}.}.
% p. 261 note: the mark is *) -- CONFIRMED on the image at 600 dpi; the ABBYY
% layer's "')" is a mis-read of the asterisk.  Both proper names are
% letterspaced.  The note is a single line, ends in a full stop, and does NOT
% run over: p. 262 has no note area at all and its first line continues the body
% ("finder en Forklaring i det ...").  Recon's clearance of p. 261 is confirmed
% independently here.

I disse Bestemmelser af den endelige Tankes Forhold til
det uendelige tænkende Væsen, der opfattes som Activitet, og i
Antydningen af Modsætningernes Enhed gjennem Materiens
Væren eminenter i det aandelige Væsen, der er dens Grund,
anticiperes det, der er charakteristisk for den \emph{speculative}
% p. 261: "charakteristisk", not ABBYY's "Charakteristik"; read at 600 dpi.
Philosophie; der sigtes hen mod Begrebet af \emph{det, Modsætningerne
i sig forenende og ved sig bestemmende
sande Uendelige}; men det Antydede fastholdes ikke idet
det Endelige atter løsner sig fra det Uendelige i en ubegreben
% p. 261: a short horizontal stroke, ~55 px long at 1200 dpi, is inked in the
% space between "skarp," and "uforenelig", set at the top of the x-height.  It
% is far too short for an en (83 px) or an em (167 px) and is not a
% word-forming mark; read as a rising space or stray sort and not transcribed.
% Both witnesses render it as a hyphen ("skarp,-uforenelig").
selvstændig Væren, og idet Naturen i sin Virkelighedsform
sættes ligeoverfor Aanden i skarp, uforenelig Modsætning. Kun
i sin Væren \emph{eminenter} bliver Naturen forenelig med Aanden,
ligesom i \emph{Hegels} System kun den \emph{forflygtigede} Natur forenes
med den.

Hvad der er antydet hos Descartes, kommer frem i udviklet
Form hos \emph{Spinoza}, og hans Systems Grundlag er speculativt.
Men den ovenfor angivne Mangel, der gjør Tanken
ubevægelig, og lader Modsætningerne falde uafledte udenfor
hinanden, reducerer det Speculative til det \emph{abstracte Grundlag}
og forhindrer dets Gjennemførelse.

Ogsaa \emph{Leibnitz berører} det Speculative, men naaer
ikke videre. Hos ham er det Speculatives Tilknytningspunkt
givet i Begrebet om \emph{Monadens uendelige Udviklingsmulighed}
ud fra sit Virksomhedscentrum, og i Opfattelsen af
dens ideelle Activitet som al Tilværelses Grund. Men hiin
% p. 261: a faint grey vertical pencil stroke stands immediately before the "i"
% of "ikke" (ABBYY reads "jikke", tesseract "iikke").  Modern pencil, deleted
% per BATCH-AGENT §7; a further site for the pencil log.
Udviklings \emph{Tankeform} bliver ikke funden, og Grundmodsætningerne,
hvis Forhold Leibnitz bestemmer gjennem \emph{Reflexionskategorier},
komme til at staae ved Siden af hinanden,
uden at Materien, der skulde være det Phænomenale,
% --- p. 262 ---
\opage{262} finder en Forklaring i det som det ene Væsentlige bestemte
Aandelige. ---

Gaae vi fra denne Paaviisning af den speculative Idealismes
Antydninger i de tidligere Tankeformer over til \emph{Charakteristiken}
af den, saa bliver dens

1. \emph{Princip} at bestemme som den \emph{absolute Idee}, der
bliver synonym med den \emph{absolute Aand}, opfattet som \emph{naturløs
Aand}.

Paa det \emph{Schellingske} Absolute synes den sidste Bestemmelse
ikke at blive anvendelig: Identitetsphilosophien synes,
sammenholdt med \emph{Fichtes} Videnskabslære, netop at
gjenindsætte det Naturlige i dets væsentlige Betydning for det
Absolutes Begreb, og at hævde det objectivt Fornuftige en
idetmindste parallel og ligeberettiget Væren ved Siden af det
subjectivt Fornuftige. Men Naturens Ligeberettigelse med Aanden
er usikker og ligesom kun midlertidig. Dette antydes allerede
derigjennem, at Schelling fra Begyndelsen af construerer
Naturen ud fra den \emph{abstracte Selvbevidstheds} Former,
og reducerer dens concrete Væren til disse: --- en Opfattelse,
% p. 262, UNDER-INKED SORT: the third letter of "den" before "Schellingske"
% prints only as a bare stem -- 24 px wide at 1200 dpi with a fixed threshold,
% against 72--80 px for a certain "n" and ~48 px for a certain "r" on the same
% line.  There is no ink at all where the rest of the letter would stand, so the
% scan gives no positive signal for either reading; per TRANSCRIPTION-PLAYBOOK
% §2 rules 2 and 3 the sense-reading "den" is set.  ABBYY reads "det",
% tesseract "der"; neither is grammatical here.
fra hvilken den Schellingske Naturphilosophie i Virkeligheden
\emph{aldrig} kommer bort. Og det bliver klart under Udviklingen
af den Schellingske Philosophie i dens ældre Skikkelse, idet
% p. 262: a light brown speck follows "træder"; lighter than the ink and of no
% punctuation shape.  ABBYY reads a comma, tesseract a full stop; neither is
% transcribed.
\emph{Idealitets}momentet bestandig træder mere overveiende frem,
saa at Naturens Ligestilling med Aanden maa vige for dens
Underordning under denne, og Idealitetsbestemmelsen og den
\emph{postulerede} Bestemmelse af det \emph{absolute Subject} bliver
den constitutive for det Absolute, uden at dog derved Realitetsmomentet
bliver forklaret. Det \emph{Underordningsforhold},
som \emph{Schelling} paa sine sildigere Udviklingstrin statuerer,
bliver \emph{fra Begyndelsen} af udtalt af \emph{Hegel}. Hos ham bliver
Naturen \emph{Moment} i den Proces, gjennem hvilken \emph{Ideen
evig} bestemmer sig som \emph{Aand}, og, som Moment, bliver Naturen
sat ved Ideen og ophævet i Aanden. Og idet Ideens og
Aandens Begreb hos \emph{Hegel} falde sammen, da allerede Ideen
% p. 262: no comma after "Begreb" before "og Realitet"; the mark ABBYY reads
% there is a speck.  600 dpi.
bestemmes som Enhed af Begreb og Realitet (Objectivitet), saa
% --- p. 263 ---
\opage{263} er Naturen et \emph{forsvindende} Moment, der, ved ikke at give
Aanden en rigere Bestemthed, viser sig som manglende selvgyldig
Væsentlighed. \emph{Dens Væren er kun at forsvinde},
% p. 263: "selv-/gyldig Væsentlighed" is NOT letterspaced -- tight at 600 dpi on
% both halves of the broken word and on "Væsentlighed"; the emphasis begins at
% "Dens".
og den Aand, der i sin Væren forudsætter og sætter Naturen,
forudsætter den kun \emph{som et Intet}, og er derfor \emph{naturløs
Aand}; den udtrykker, som Enhedsprincip, den \emph{ensidige} Enhed,
% p. 263: "En-/hed" (the broken "Enhed") is NOT spaced -- "En" is tight at
% 1200 dpi; the run ends at "ensidige".
ligeoverfor hvis Abstraction det Naturlige i dets Virkelighed
uundgaaeligt maa hævde sig. Dette fremgaaer endnu
tydeligere naar vi bestemme

2. Den \emph{væsentlige Gjenstand}. Denne bliver \emph{Ideeprocessens
Bestemmelser}, der falde sammen med \emph{Systemet
af det Logisk-Metaphysiske i dets Abstraction}.
For den \emph{Hegelske} Philosophie ere de logisk-metaphysiske
Tankebestemmelser, som Ideebestemmelser, ikke et med
Hensyn til Virkelighed Mindre end Naturen og det endelige
naturbestemte Aandslivs concrete Former, men tværtimod et
Mere: det Andet er kun hvad det er ved hine Tankeformer.
Og hvad særlig det Naturlige i dets tilsyneladende Rigdom, i
dets concrete Mangfoldighed angaaer, saa er det kun en
\emph{usand, kun for Mennesket}, den endelige Aand, \emph{værende
Existentsform}, der \emph{kun} faaer Sandhed \emph{som Aand}, i sin
Ophævelse som Naturligt; Naturens Rigdom og Concretion er
netop kun et Skin, er netop kun Tegnet paa dens Afmagt til
at fastholde Begrebet. I \emph{Væren} søger Tanken \emph{kun sig selv};
den seer fra Begyndelsen af kun Væren som \emph{tænkt Væren};
i Naturen seer den kun den til abstracte Tankebestemmelser
forflygtigede, ophævede Natur, kun Naturens abstracte Aandsside.
Saaledes forholder Tanken hos \emph{Hegel} sig overalt væsentlig
til Aand, men det Aandelige, der er dens Gjenstand,
er et saadant, der i sin ved det Naturliges Forflygtigelse betingede
Abstraction ikke magter den reale Virkelighed.

Hvad der gjælder om Hegels Philosophie, faaer ogsaa,
trods Naturphilosophiens Fremtræden indenfor Identitetsphilosophien,
sin Anvendelse paa \emph{Schellings}, naar der sees hen til
hans oprindelige Opfattelse af det Naturlige som Selvbevidst
% --- p. 264 ---
\opage{264} hedsprocessens Momenters Reflex, og til den ovenfor berørte
Udvikling af hans Grundbegreb i idealistisk Retning.

3. \emph{Tænkningens Grundpræg, Methoden}. Den til den
speculative Idealismes Grundbegreb svarende Methode bestemme
vi som den \emph{dialektiske} Methode, saaledes, at der ved Dialektiken
tænkes paa det indenfor dette Standpunkt \emph{Tilstræbte:
Identitetsdialektiken}. Vi betragte altsaa den af \emph{Hegel}
hævdede Methode, i hvilken det ideelle Princips Subjectivitetsbestemthed
afspeiler sig, som det \emph{typiske} Udtryk for dette
Standpunkts Tænkeart, medens vi i den \emph{Schellingske} «Construction»
see Udtrykket for den fra det \emph{Spinozistiske} Substantsbegreb
nedarvede Mangel i Bestemmelsen af det Absolute,
idet dette hos Schelling, skjøndt det skal være Modsætningernes
Enhed, kun bliver død, hvilende Enhed, der kun ved
et Postulat faaer Subjectivitetens Bestemmelse, og, som det
bestemmelsesløse Tomme, ogsaa kun ved Postulater kan bringes
til en illusorisk Concretion.

Den \emph{Hegelske Methodes Opgave} er: ved Tankens,
med Sagens Bevægelse identiske, immanente Bevægelse at
fremstille den fremskridende Udvikling, i hvilken der fra det
abstracteste Begreb, gjennem Modsætningernes Udfoldelse og
deres Sammenknytning til en høiere Enhed, naaes til de concretere,
tilsidst til det absolut concrete, til den absolute Idees
Begreb, der sammenknytter alle Begreber, i deres ophævede
Modsætning, i sin høieste, levende Enhed. Denne Dialektik
skal til sit Medium have den «\emph{rene Tænken}», fra hvilken
ethvert Anskuelseselement skal være udelukket, fordi det, som
reflecterende det Naturliges Udvorteshed, bærer den usande
Sondring i sig. I den rene Tankes Element skal al Udvorteshed
være forsvunden, Tankens Strømning skal ikke hæmmes
af noget fremmedt Element, og den concrete Idee skal uhindret
fuldbyrde sin Proces gjennem sine Momenters Sætten og
Ophæven. I Ideebevægelsens tidløse Proces er al Virkelighed
indesluttet: Begrebsdialektiken er umiddelbart Tilværelsesdialektik,
Tankernes System er Virkelighedens.
% --- p. 265 ---
\opage{265} \emph{Men under denne Identitetsdialektik skjuler},
som tidligere berørt, \emph{den kritiske Sondring sig, Forstandsdistinctionen
viser sig uovervunden}. Idet det
\emph{Reale} i sin \emph{Virkelighedsform} staaer som uforeneligt med
det abstracte logiske Begreb, der netop er formet ud fra den
oprindelige Modsætning mellem Ideelt og Realt, og idet denne
Uforenelighed er anerkjendt af \emph{Hegel}, hvor han statuerer
Naturens Afmagt til at fastholde Begrebet, saa maa det Reales
% p. 265: a brown speck stands before the opening guillemet of «rene Tankes»
% (both witnesses render it: ".«rene").  Not type, not transcribed.
Incommensurabilitet med den «rene Tankes» Begreber ikke
blot indvirke paa de logiske Begrebers Forhold til Virkeligheden,
men ogsaa paa hine Begrebers indbyrdes Forhold. Den
rene Tanke, Logikens Element, bestemtes som den fra al Anskuelse
befriede Tanke. Netop i denne Frihed skulde den i
sig have den uendelige Bevægelighed, der er Betingelsen for
Begrebernes Sammenslutning til Enhed. Men idet Forholdet
til det Reale i dets reale Existentsform er afskaaret derved,
at den logiske Tankebevægelse som isoleret skal afslutte sig i
sig med Bortskyden af Anskuelsens Element, saa er med det
Samme det Forhold til Tilværelsens levende Virkelighed afskaaret,
der skulde indeholde Dialektikens Incitament, være
Betingelsen for, at den ved sin Natur i Relation til Tilværelsens
Totalitet værende menneskelige Tanke, der kun forholder
sig til sig selv gjennem sit Andet, kan virkeliggjøre den indre
Enhed mellem sine Former, der i deres indbyrdes Forhold
maae reflectere Forholdet til Tankens Andet. Idet Tankebevægelsen,
under hiin Forudsætning, skal komme igang, skeer
det kun, idet paa skjult og inconsequent Maade Anskuelseselementet,
der afspeiler Forholdet til Tankens Andet, optages.
Men den Enhed af Begreberne, der tilveiebringes gjennem en
Dialektik, bragt i Bevægelse ved et saaledes optaget Element,
der principielt er sat som uberettiget, kan kun blive illusorisk.
Og idet Dialektiken fra de rene \emph{logiske} Begreber skrider
fremad til \emph{Naturtilværelsens} Begreber, saa faae disse det
Reales Begreber, om de end nok saa meget forflygtiges til
Reflexer af de logiske Former, dog, ved Siden af det abstracte
% --- p. 266 ---
\opage{266} logiske Element, et fra det Reale overført Element, i hvilket
det med Tanken incongruente og i Tanken uopløste Reales
Sondring afspeiler sig, og ved denne Bestemthed ved det fremmede
Reale vise Begreberne sig som kun bragte til en illusorisk
Enhed; Sondringen gjør sig atter gjældende.

4. \emph{Livsanskuelsen}, der svarer til den speculative
Idealismes Standpunkt, fremgaaer med Nødvendighed af Opfattelsen
af Principet: \emph{den absolute Idee}. Idet Ideen, den
% p. 266: a brown speck follows the comma after "Udtryk"; not type.
«rene Tankes» høieste Udtryk, i sig omfatter al Tilværelse og
er \emph{Enhed af Tanke og Væren}, saa er det en ligefrem
Consequents, at den \emph{evige} Ideens Proces, i hvilken Naturen
og den endelige Aands-Tilværelse, hvis Selvstændighed kun er
et Skin, evigt ere optagne som \emph{ophævede} Momenter, sættes
% p. 266: "al" is letterspaced BOTH times in "al Virkelighed og al Sandhed"
% (confirmed at 1200 dpi), while "Virkelighed" and "Sandhed" are not, and while
% "al Tilværelse" four lines above is tight.  spacing.py flags none of them.
som omfattende \emph{al} Virkelighed og \emph{al} Sandhed. Der er da
ingen anden sand Tilværelse end den, der er i den metaphysiske
Evigheds Element; Alt, hvad der har sin Væren i Tidens
og Rummets Former, er et \emph{kun} Usandt, der med evig Nødvendighed
ophæves i det Absolute. Derfor maae Menneskelivets
Naturformer og Naturindhold sættes som et kun Uberettiget
og Usandt, og Menneskeaandens eneste sande Væren
maa blive \emph{Tankens}, fra Individualitetens Begrændsning befriede,
contemplative Liv i \emph{Evighedens} Element: den Tankens
Form, i hvilken det Absolute vides som det i sin evige
Proces ene Værende, og det tænkende Subject veed sig som
Moment i det Absolute, i hvis Viden om sig Menneskets Viden
om det Absolute og om sig som Moment i det Absolute er
optagen. Ligeoverfor Anskuelsen af Ideens evige metaphysiske
Proces, der ikke er at opfatte som en Stræben hen imod et
Maal, men som et evigt Værendes Selvbekræftelse, i hvis Nødvendighed
derfor enhver Modsætning evig er hævet, tabe Personlighedens
Virkelighedsproblemer: Problemerne om den endelige
frie Villie, der i Existentsen forholder sig til en Evighedens
Opgave: til det Ethisk-Religiøse; om dens Selvbestemmelse
til denne Opgaves Løsning; om dens Forsømmen af
Opgaven; om dens Skyld og dens Udsoning af Skylden; om
% SEAM CHECK (BATCH-AGENT §5.2): p. 266 carries NO footnote, so nothing can run
% over into p. 267.  p. 267's own note opens with its own mark *) ("Til den
% Schellingske Identitetsphilosophie ...") and is self-contained.  p. 266 ends
% mid-sentence on "om", continued on p. 267 by "dens concrete Virkeliggjørelse
% af Væsenet ..." -- the next batch must not let a paragraph break appear here.
% SEAM (BATCH-AGENT §5.1) CONFIRMED CLEAN AT THE TOP: p. 266 bears no note and
% p. 267's *) is self-contained, so nothing runs over.  BUT the join is
% MID-SENTENCE ("... om Skylden; om | dens concrete Virkeliggjørelse ..."), and
% transcription.tex has a BLANK LINE immediately above the pp. 267--280 marker,
% which splice.py does not remove.  That blank line becomes a paragraph break
% the book does not have -- run joints.py after the splice and delete it.
% --- p. 267 ---
\opage{267} \setcounter{footnote}{0}%
dens concrete Virkeliggjørelse af Væsenet som naturligt-aan\-%
deligt, deres Skarphed og deres Vægt. Overfor den evige og
evigt færdige Processes Nødvendighed bliver Endelighedens
Bevægelse, selv hvor den afslutter sig i det Onde, kun en Spøg,
der evigt er tagen tilbage i det Absolutes Proces, i hvilken al
Alvoren falder. Gjennem Tanken om det Absolute kan In\-%
dividet tage sig ud af Endelighedens Modsigelser, der ingen
Betydning kunne faae for dets Evighedsliv, og i den indivi\-%
dualitetsløse, contemplative Enhed med det Absolute har det
Absolutionen for al Ufuldkommenhed og Forsoningen med sit
Væsen. Den Bestemmelse, der gives af Menneskeaandens
sande Værensform, indeslutter den Fordring, at Mennesket i
den naturbestemte Tilværelse skal leve i den rene Aandeligheds
Abstraction, at det indenfor den endelige Existents skal leve
kun i Evighedens Element, saa at altsaa Tiden i Tiden skal
tages tilbage i Evigheden, saa at Tidsforholdet ikke faaer
Virkelighedsbetydning for en ethisk Beslutning til Handling,
for en religiøs Beslutning til Selvhengivelse. Tanke er Hand\-%
ling, det at begribe det Absolute den absolute Handling. Livet
bliver en Aandelighedens Illusion: det ethiske og religiøse
Virkelighedsforhold bliver sublimeret til et tidløst Tankeforhold,
i hvilket det tænkende Subject forsvinder.

Men denne Livsanskuelse\footnote{Til den \emph{Schellingske} Identitetsphilosophie med dens bestemmel\-%
sesløse Begreb af det Absolute, og med dens Fordring paa Enhed
med dette gjennem en intellectuel Anskuelse, svarer væsentlig den
samme Livsanskuelse. Kun træder her Ligheden med \emph{Spinoza}
endnu umiddelbarere frem.}, hvis relative Berettigelse lig\-%
ger i dens Hævden af det Ideelle og Evige, i dens Opfattelse
af Menneskets Væsensenhed med det Absolute, i hvis Livs\-%
proces det skal gaae ind, bærer væsentlig den samme Mod\-%
sigelse i sig, som blev paaviist ved \emph{Spinozas}, og kaster
Aanden, der higer efter Evighedens Enhed, urolig tilbage i en
Virkelighed, der ikke som forsonet er optagen i Enheden.

% p. 267: a short CENTRED divisional rule stands at the FOOT of the page, BELOW
% the footnote (measured at 300 dpi: y=2130, x=544--794, i.e. 250 px = 2.1 cm,
% centre 669 against a text-block centre of 677).  It is NOT the footnote
% separator, which is on the same page at y=1828, x=113--344, LEFT-FLUSH against
% a text-block left edge of 113.  p. 268 carries no rule at all, so this is the
% rule that divides the "4. Livsanskuelsen" section from the recapitulation
% opening p. 268 -- another instance of BATCH-AGENT §6's "divisional rules do
% not always stand above their head" (cf. pp. 15, 79).  It is ink, not
% show-through: min grey 85 / mean 171, identical to the footnote separator on
% the same page (97/171) and to the divisional rules on pp. 269 (85) and 279
% (88), against min grey 183 for the paper immediately above it.
\divrule

% --- p. 268 ---
\opage{268} \emph{I den speculative Idealisme}, saaledes som den væ\-%
% p. 268: "I den" is NOT spaced -- measured letter gaps 6 px against 24--28 px
% in "speculative Idealisme," at 1200 dpi.  Set \emph{} from "speculative".
sentlig er repræsenteret ved \emph{Hegel}, er den \emph{nyere Idealis\-%
mes anden Hovedform} afsluttet, og denne Idealismes \emph{sidste}
% p. 268: LINE-END TEST (1200 dpi).  "sidste" at the end of l. 3 IS letterspaced
% (gaps 24--28 px) but "Maal er naaet" opening l. 4 is NOT (gaps 4--12 px):
% the spacing stops dead at the line break.  Cf. l. 10--11, where
% "Selvbevidsthedens transscendentale | Enhed," carries ACROSS the break.
Maal er naaet, ved hvilket den danner \emph{Parallelen til Rea\-%
lismens dogmatiske Afslutning}. Den naturløse Aand
er Modstykket til Materialismens aandløse Natur.

Og ved dette den nyere Idealismes Slutningspunkt ere
de Tanker udfoldede, som vi hos \emph{Kant} have fundet i Spiren.
Det Begreb, der var den Kantiske Fornuftkritiks Centralpunkt:
Begrebet om den \emph{Selvbevidsthedens transscendentale
Enhed}, der antydedes at omslutte det Objective, er udfoldet
til Begrebet om den \emph{absolute Aand}, der i sin evige Proces
omfatter Naturen og de endelige Bevidstheder. Og naar Kant
opfattede hiin Selvbevidsthedens Enhed som \emph{Grundsynthe\-%
sen}, der er alle Synthesers Kilde, og, idet han accentuerede,
hvad Idealismen, idet den bliver sig Erkjendelsens Væsen be\-%
vidst, idet den har Bevidstheden om sin Viden, \emph{definitivt}
maa hævde: \emph{Aandens Spontaneitet}, gjorde det saaledes,
at han angav denne Spontaneitets \emph{ved Aandens Væsen
bestemte Resultat}; saa bliver dette Resultat, som \emph{Kant}
bestemmer som Summen af det Aprioriskes \emph{adskilte} Former,
hvis Fremgaaen af Aanden han udsiger, men ikke paaviser,
for \emph{Hegel} til det af Ideen omsluttede \emph{System} af Tanker, i
Enhed med Virkelighedens. Og idet Synspunktet forlægges fra
Selvbevidstheden til det Absolute, bliver for \emph{Hegel} Tankesy\-%
stemets Udfoldelse al Virkeligheds evige Udfoldelse af den som
Subjectivitet bestemte absolute Idee, hvis Processes væsentlige
Form sees som afprægende sig i den dialektiske Methode, der
udfolder Ideesystemets virkelighedsgyldige Momenter: de ob\-%
jective Tanker, for Bevidstheden.

Men naar saaledes i den Hegelske Philosophie den nyere
Idealisme har naaet sit Maal og udfoldet sit Anlæg, saa har
det allerede viist sig, at det Begreb, ved hvilket dette System
vilde udtrykke Tilværelsesmodsætningernes concrete Enhed:
Enheden af Tanke og Væren, af Aand og Natur, kun udtrykte
% --- p. 269 ---
\opage{269} en ensidig opfattet Enhed: en saadan, i hvilken Realitetsmo\-%
mentet, Naturmomentet, var forflygtiget, og overfor hvilken det
derfor maatte gjøre sig gjældende i sin Virkelighedsform. Den
Modsigelse, der, paa Grund af Dualismens Bliven, gjennem\-%
trænger hele den nyere Philosophie i dens Udvikling, er altsaa
% p. 269: BOTH witnesses read an acute here -- ABBYY "dén", tesseract "deén" --
% and the sense would carry one (the emphatic demonstrative).  It is NOT type.
% Measured at 1200 dpi: the mark over the "e" bottoms out at grey 155, against
% grey 20--22 for the letter strokes beside it and grey 32 for the darkest speck
% in clean paper on the same page; the genuine grave of "Système" (p. 280)
% bottoms out at 47.  A faint stain or show-through, not an accent sort.
% Transcribed as plain "den".
endnu tilstede ved dette Slutningspunkt, ligesom den var til\-%
stede ved den realistiske Philosophies Afslutning, og herved
stilles der da paany en Fordring til Philosophien: Fordringen
paa, ad en anden Vei end de hidtil betraadte at forsøge Løs\-%
ningen af den Opgave, der betegner Philosophiens ved Men\-%
neskeaandens Væsen bestemte Grundopgave: Erkjendelsen af
Tilværelsens Enhed gjennem Erkjendelsen af Principet som det,
der i sin concrete Enhed sammenslutter Tilværelsens Grund\-%
modsætninger. Og den hele forudgaaende historiske Udvikling
viser hen til den Vei, ad hvilken Løsningen skal forsøges, og som
\emph{Fremtidens Philosophie} maa følge, naar den vil over\-%
vinde den Dualisme, som de forudgaaende Hovedepocher ikke
mægtede at betvinge.

% p. 269: short CENTRED divisional rule above the head (300 dpi: y=1306,
% x=538--819, centre 678 against a text-block centre of 675).
\divrule

% p. 269: L2 head, MEASURED, not taken from transcription.tex §3.  At 1200 dpi
% the head's median stroke width is 24 px (IQR 20--28) against 12 px (IQR 12--16)
% for the body roman on the same page -- exactly 2x, i.e. BOLD.  Its inter-letter
% gaps are 4--12 px (1--3 px at 300 dpi), nothing like the 24--36 px of the
% Sperrsatz in the body of this very page.  So \textbf{}, NOT \emph{}.
% FINAL PASS: \large added -- x-height 46 px at 600 dpi against a body 39,
% the L2 size, as at pp. 50, 79, 137, 170, 219 and 226.
\begin{center}
\textbf{\large Tredie Hovedepoche: Fremtidens Philosophie.}
\end{center}

Den nye Grundform af Philosophien, der gjennem det
forudgaaende Udviklingstrins Mangler viser sig i sin Nødven\-%
dighed, bliver, ifølge det allerede tidligere Paaviste (jfr. S. 28 ff.)
at opfatte som den philosophiske Udviklings \emph{sidste} Hoved\-%
epoche, og som den, der i sig bærer Spiren til en \emph{uendelig}
Tidsudvikling. Hint fremgik af Sammenstillingen af de for\-%
skjellige Muligheder for Bestemmelsen af Tilværelsesmodsæt\-%
ningernes Forhold; dette af den Betydning, der tillægges Er\-%
kjendelsen af den reale Virkelighed i dens Concretion. Naar
Forholdet mellem Tilværelsens Grundmodsætninger har viist
sig, hverken at kunne bestemmes ved den umiddelbare Enhed
% --- p. 270 ---
\opage{270} eller ved den abstracte Modsætning, bliver kun een Bestem\-%
melse af Forholdet mulig: Bestemmelsen af det som \emph{Mod\-%
sætningsenhedens}. Og idet denne Bestemmelse udtrykker,
at det Reale og det Ideale, Naturen og Aanden, medens de hævde
% p. 270: "som" IS letterspaced here (1200 dpi: s--o 33 px, o--m 33 px, against
% intra-word gaps of 4--16 px and word spaces of 48--100 px on the same line;
% "Modsætninger," immediately after is tight, 4--16 px).  spacing.py flags
% nothing here.  The emphasis of a bare "som" is odd, and l. 27--28 of this page
% has "og som" plainly unspaced, so the reading is recorded as measured and not
% argued for.
sig i deres Eiendommelighed \emph{som} Modsætninger, sammensluttes
til concret Enhed i Principet, hvilket kun kan tænkes, naar
Principets Væsen er Modsætningens Kilde, Principet altsaa, i
sin Bestemthed som \emph{absolut Subjectivitet}, er \emph{idealt-%
realt} Princip, saa er ved hiin Bestemmelse af Grundmodsæt\-%
ningernes Forhold det \emph{Reales Væsentlighed} hævdet, og
Erkjendelsen af det i dets uendelige Rigdom gjort til Philoso\-%
phiens Opgave.

Som den sidste mulige Grundform af Philosophien maa
den \emph{tredie} Hovedepoche i Fuldstændighed realisere den Phi\-%
losophiens Opgave med Hensyn til Erkjendelsens Indhold og
Form, der deelviis og paa den i Isolationen mulige Maade er
bleven realiseret i de historisk foreliggende Hovedformer. Den
maa da i sig sammenfatte og bevare det i de forudgaaende
Former Væsentlige og Blivende; thi kun under denne Betin\-%
gelse vil det i de forudgaaende Former ikke Realiserede kunne
realiseres i den. Og idet den i sig sammenfatter hint væsent\-%
lige og blivende Resultat af de forudgaaende Udviklingstrin,
vil den ikke kunne forholde sig til det Forudgaaende og For\-%
% p. 270: a small brown speck follows "Udfoldelse" on this line; it is not type
% (foxing colour, no ink density) and is not transcribed.
beredende abstract som Modsætning eller som Udfoldelse af en
enkelt Retning, altsaa ikke kunne gjentage de tidligere Sta\-%
diers Forhold til hinanden; men den maa forholde sig til det
historisk Forudgaaende som \emph{alle Modsætningernes En\-%
hed} og som \emph{alt} det Forudgaaendes \emph{Fuldendelse}. --- Dette
Forhold til de tidligere Stadier maa nærmest vise sig i Be\-%
stemmelsen af

% p. 270: the four-part scheme is RUN-IN here, not displayed: "1. Principet."
% opens an ordinary indented paragraph and the sentence continues on the same
% line.  MEASURED at 1200 dpi: "Principet." has letter gaps 12--32 px with a
% stem width of 12 px, identical to the body roman on the same line -- so it is
% LETTERSPACED, not bold.  The numeral's own "1"--"." gap is 28 px (against 12 px
% for "2." on p. 272, 20 px for "3." on p. 273 and 8 px for "4." on p. 276);
% the numeral is left plain here, as it plainly is in the other three.
% A second brown speck sits between "Principet." and "Idet"; not type, not
% transcribed (tesseract reads it as a hyphen).
1. \emph{Principet.} Idet det var den \emph{Dualisme}, der ved
den anden Hovedepoches Slutning atter traadte frem som
uovervunden, der gjorde Fordringen af en \emph{ny} Philosophiens
Grundform uafviselig, saa maa den nærmeste Opgave for denne,
% p. 270: PARTIAL LETTERSPACING, measured at 1200 dpi: in "Modsætningsenheden"
% the first element is tight (gaps 8 px) and only "enheden," is spaced (32 px).
% The break falls at the compound seam, so only the spaced element is marked.
% Cf. p. 271 "Aandsbegrebet", where the spaced element is the FIRST one.
der skal hævde Modsætnings\emph{enheden}, blive den: at omforme
% --- p. 271 ---
\opage{271} det Naturbegreb og det Aandsbegreb, i hvilke Dualismen op\-%
rindelig havde fundet sit Udtryk, og gjennem hvilke den ved\-%
blev at hævde sig. I Stedet for det \emph{Naturbegreb}, der var
dannet ud fra Forudsætningen af Naturens abstracte Modsæt\-%
ning til Aanden, og som kun bestemte Naturen ved det, der
betegner det Reale ligesom i dets største Afstand fra det
Ideelles Enhed og Energie: ved den \emph{passive Udstrækning},
maa der træde et Begreb, der ikke opfatter Naturen efter dens
fattigste og abstracteste Bestemmelse, men fra det Punkt, hvor
den, i sine \emph{høieste} Phænomener, rigest og klarest udtrykker
% p. 271: "Le-" at the end of the next line is NOT spaced (gaps 14 px against
% 28--32 px in "Organismens,"): the run stops before "Levendes".
sit \emph{fuldstændige} Væsen: fra \emph{Organismens}, fra det Le\-%
vendes Synspunkt. Og idet den her viser sig som bestemt
ved et \emph{Ideelt}, for hvilket de materielle Elementer blive tje\-%
nende Momenter, og Enheden med Aandens Bestemmelse alt\-%
saa kommer tilsyne, saa maa atter den Ensidighed undgaaes,
der lader det Reale forflygtiges, dets Væreformer forsvinde i
Aandens. Det Naturlige maa opfattes i sin \emph{væsensgyldige}
Materialitet; det Reales Grundbestemmelser: Rummets, Tidens
og Materiens Bestemmelser, maae sees i deres Væsentlighed, i
deres afgjørende Betydning for Tænkeligheden af det Ideales
Grundbestemmelse: Energiens Bestemmelse.

Til Naturbegrebets Omdannelse vil saa en analog Om\-%
% p. 271: PARTIAL LETTERSPACING at the compound seam, measured at 1200 dpi:
% "Aands" is spaced (gaps 28--32 px), "begrebet" is tight (8--12 px).  The
% mirror image of p. 270's "Modsætningsenheden".
dannelse af \emph{Aands}begrebet svare. I Stedet for det Begreb
af Aanden, der satte den som løsreven fra alle det Reales Be\-%
stemmelser og som det Naturliges abstracte Modsætning, som
gaaende op i Tanke og Bevidsthed, maa et Begreb substitueres,
dannet i Overensstemmelse med den \emph{Erfaring}, der viser
Aandens individuelle Existents som baaren af og i Tidsudvik\-%
% p. 271: a small comma-shaped speck stands between "Grundlag," and "og" on the
% next line; not type, not transcribed.  "og dens" is NOT spaced (gaps 4--12 px).
lingen betinget ved \emph{et realt organisk Grundlag}, og dens
bevidste Aandsliv som gaaende i Enhed med et \emph{ubevidst
sjæleligt}, ved hvilket det forbindes med Naturtilværelsens
objective Fornuft; saaledes at altsaa Aanden i sin \emph{Frihed} og
\emph{Idealitet} sees i \emph{sin Sammenhæng med Naturen}, som
dennes Fuldendelse, og som staaende i \emph{væsentligt} Forhold
til det \emph{Reale}. Ud fra en uhildet Erfaring vil der, hvad enten
% --- p. 272 ---
\opage{272} \setcounter{footnote}{0}%
Naturen eller Aanden tages til Udgangspunkt, naaes til den
samme væsentlige Enhed af Idealitetens og Realitetens Be\-%
stemmelser under deres Modsætning som ved den logiske Ana\-%
lyse af Begrebsforholdene.

En saadan Omformning af Grundmodsætningernes Begre\-%
ber, ved hvilken hvert af disse sees som visende hen paa Mod\-%
sætningens og som bærende det i sig, indeslutter Bestemmel\-%
sen af \emph{Principet} som den \emph{concrete Enhed}, der omfatter
\emph{begge} Modsætninger, som Tilværelsens \emph{idealt-reale En\-%
hedsprincip}. Som Modsætningernes \emph{virksomme Enhed}
% p. 272: "Som Modsætningernes" is NOT spaced (gaps 8--14 px at 1200 dpi) while
% "virksomme Enhed" is (28--34 px); spacing.py's RUN "Som virksomme Enhed" is
% therefore wrong about "Som".
kan Principet kun tænkes naar det bestemmes som \emph{absolut
Subjectivitet}, og Subjectiviteten sees da som det, der kun
har Virkelighed i sit \emph{nødvendige og oprindelige} Forhold
til Andetheden, til Subjectivitetens \emph{Modsætning}: det \emph{Ob\-%
jective, Reale}. Begge Bestemmelser faae, som nødvendige,
% p. 272: "det" between "Modsætning:" and "Objective," is NOT spaced (8 px);
% the two neighbours are (28--36 px).  Transcribed as measured.
\emph{lige Oprindelighed}; men \emph{Begrebsprioriteten} tilkommer
det \emph{Ideelle}, idet Subjectiviteten bliver \emph{Forklaringsgrund
for sig selv og for Objectiviteten}, der er ved \emph{den}, men
% p. 272: "der er ved" is tight (8--12 px) and "den," is spaced (32 px); "men"
% and the "er" opening the next line are tight.  Measured, not regularised.
er \emph{som dens egen Værens Betingelse}. Fra dette Syns\-%
punkt sees altsaa det Reale ikke som et forsvindende Moment,
men som det, der i \emph{sin eiendommelige Værensbestemt\-%
hed} er \emph{blivende} Betingelse for det Ideales Virkelighed: det
sees som det, der sættes forat idealiseres, men som det, der
% p. 272: HEAVY SORT.  The standalone "i" between "idealiseres" and "sin" is set
% from a markedly heavier fount than its neighbours.  Measured at 1200 dpi, stem
% run through the x-height: this "i" 28--32 px (median 28); the "i" of "sin" on
% the same line 12--16 px (median 16); the "l" of "idealiseres" on the same line
% 16 px.  The ratio (2x) is the same as the bold L2 head on p. 269 (24 px against
% a 12 px body roman).  ADJUDICATED as EMPHASIS, not a foul case; \textbf{}
% kept.  See header §2.
idealiseres \textbf{i} sin \emph{reale} Virkelighed.\footnote{Jævnfør, med Hensyn til den nærmere Udvikling af dette, mit Skrift:
Om det Religiøse i dets Enhed med det Humane, S. 65 ff. 86 ff.} Principet bliver, idet det
sees i sin \emph{Idealitet}, med det Samme seet som det den
\emph{reale} Tilværelse Begrundende og teleologisk Bestemmende;
det bliver seet som den \emph{Tilværelsens Fornuft}, ved hvilken
Tilværelsen \emph{er} som \emph{real}, og bestaaer i \emph{real} Virkelighed.

% p. 272: the second member of the four-part scheme is run-in like p. 270's, but
% here the numeral is followed by ORDINARY text -- there is no head-word at all,
% and nothing on the line is letterspaced except "væsentlige Gjenstand" two
% lines down.  Measured: "2." gap 12 px; "Af denne Bestemmelse af Principet
% følger saa den" all 4--12 px.
2. Af denne Bestemmelse af Principet følger saa den
Bestemmelse af den philosophiske Erkjendelses \emph{væsentlige
Gjenstand}, at denne ikke længere, som paa tidligere Ud\-%
% --- p. 273 ---
\opage{273} viklingsstadier, hvor Principet opfattedes ufuldstændigt og en\-%
sidigt, kan blive en enkelt, ensidigt udsondret Grundform af
Tilværelsen, eller Tilværelsen i en Abstractionens Bestemmet\-%
hed, ved hvilken den svinder ind i Principets abstracte Væren;
men at den maa blive \emph{den i relativ Selvstændighed
udfoldede concrete ideal-reale Tilværelsesheelhed i
dens Livssammenhæng med det begrundende og or\-%
ganiserende Princip}. I denne Bestemmelse af den væ\-%
sentlige Gjenstand ligger saa paa den ene Side, at der maa
falde et væsentligt Eftertryk paa det Enkelte, Concrete, Indi\-%
viduelle, der i sin \emph{reale} Væren træder ud i relativ Selvstæn\-%
dighed, og, som \emph{integrerende} Led i Totaliteten, i sin spe\-%
cifiske Eiendommelighed faaer væsentlig Betydning for Heel\-%
heden, og derfor ikke bliver at see som et blot forsvindende
Moment, men hævder sig i sit Væsens Eiendommelighed. Men
paa den anden Side ligger der deri, at de reale, individuelle
Existentser, netop som Led i den udelelige \emph{Totalitet}, maae
sees som dem, der ikke kunne udskilles i Isolation, men paa
ethvert Punkt, selv der, hvor en Selvhed, gjennem Selvbevidst\-%
hed og Villie, bevidst modsætter sig Principet, maae aaben\-%
bare Livssammenhængen med Heelheden og med dens ideelle
Enhedsprincip. I Forholdet til sin Gjenstand faaer da Tanken
paa ethvert Punkt et \emph{concret Virkelighedsindhold}, og
hævder overalt Virkeligheden som Virkelighed; men Virkelig\-%
hedsindholdet viser sig overalt for Tanken i Belysning af det
concrete \emph{Enhedsprincip}, der gjennemklarer det og idealise\-%
rer det, men ikke fortærer dets Virkelighed.

% p. 273: the third member of the scheme is again RUN-IN and again worded
% differently from the recon list: it reads "3. Den philosophiske Methode, der
% skal svare til den ...", not "3. Tænkningens Grundpræg, Methoden."  Measured:
% "3." gap 20 px, "Den philosophiske" 8--12 px, "Methode," 32 px.  Only
% "Methode," is letterspaced.
3. Den philosophiske \emph{Methode}, der skal svare til den
angivne Bestemmelse af Principet i dets Forhold til den or\-%
ganiserede Tilværelse, i hvilken \emph{Realitetssondringen} og
den \emph{ideelle Enhed} begge komme til deres Ret, maa blive
% p. 273: "Methode" here is NOT spaced (8 px) though "dialektisk" (28 px) and
% "fuldstændige" (28 px) on the same line are.  Measured, not harmonised with
% the line above.
en \emph{dialektisk} Methode i dette Begrebs \emph{fuldstændige} Be\-%
tydning. Den kan ikke blive en Distinctionsdialektik, der overseer
Væsensenheden, den indre Uendelighed, ikke heller en abstract
Identitetsdialektik, der nøies med at udfolde et System af ab\-%
% --- p. 274 ---
\opage{274} stracte aprioriske Begreber, som ikke rumme det Reale; men
den maa blive en \emph{Identitetsdialektik}, der udfolder \emph{et Sy\-%
stem af Virkelighedsbegreber}, som har \emph{det Apriori\-%
skes Nødvendighed og Gjennemsigtighed}. Opgaven
bliver altsaa \emph{en Assimilation af det Reale i dets væ\-%
sensgyldige Eiendommelighed}, hvilken Assimilation
falder sammen med \emph{det Aprioriskes Udfoldelse i Con\-%
cretion}. Veien hertil bliver en Forbindelse af \emph{den sande
Erfaringens Methode} ɔ: af den ved Anerkjendelsen af det
% p. 274: the ɔ: id-est mark, read on the image at 1200 dpi (a reversed c
% followed by a colon).  Neither witness has it: ABBYY gives "o:", tesseract "o:".
Aprioriske fuldstændiggjorte og corrigerede Erfaringsmethode, med
\emph{Dialektikens immanente Begrebsudvikling}. Idet hiin
Erfaringsmethode fører til det Punkt, hvor det \emph{Reale}, gjennem
den concrete Erkjendelse af dets Væsensbestemmelser og væsentlige
Relationer, begribes som \emph{ideelt Moment} i en \emph{organisk
Totalitet}, vil paa dette Punkt dens Resultat kunne \emph{om\-%
sættes i dialektisk Tankeudvikling}, assimileres af den
\emph{aprioriske} Tankeudfoldelse, og gjennem et bestandigt Vexel\-%
virkningsforhold af Tanken og det Reale i dets levende Virke\-%
lighed forvandles til et af Subjectet ideelt Frembragt, idet det
Empiriskes Givethed omsættes i \emph{total Production af det
formede Indhold} ud fra den subjective Tankes indre Uen\-%
delighed. Saaledes faaer det Reale Ret til at gjøre sig gjæl\-%
dende efter sin Eiendommelighed for i \emph{concret} Skikkelse at
optages i Begrebet. Begrebernes Enhed bliver da en saadan,
i hvilken Virkeligheden rummes: den faaer ikke blot en Con\-%
cretion indenfor det Abstracte, men en Concretion, i hvilken
det Reales Væsen er bevaret. Begrebernes dialektiske Enhed
hviler paa Virkelighedserkjendelsen, og idet den \emph{aprioriske}
Erkjendelse træder i Forhold til den \emph{empiriske}, bliver, \emph{ifølge
den efter sit Væsen universelle Menneskeaands
Forhold til Tilværelsens Totalorganisation}, Grænd\-%
% p. 274: "Grænd-|sen" is NOT spaced (10--12 px) although the long run before it
% is (28--36 px); the run ends at "Totalorganisation,".
sen mellem dem en \emph{bestandig bevægelig} Grændse, idet
det Aprioriske ud fra sig successivt assimilerer det Empiriske,
der erkjendes efter sin concrete Virkelighed. Under Dialek\-%
% p. 274: PRINTER'S DEFECT.  The last line of the page ends "... den fremskri"
% with NO division hyphen (confirmed at 1200 dpi at the right edge of the type
% area), the word continuing "dende," at the head of the next line.  The word is
% joined here, since a reflowed transcription cannot represent the omission;
% the omitted hyphen is logged, not supplied silently.
tikens Udvikling af sit concrete Indhold vil da den fremskridende,
men ikke fuldbyrdede Tilegnelse af det Reale træde
% --- p. 275 ---
\opage{275} frem i den Form, at det endnu ikke Tilegnede saaledes i Ud\-%
viklingen knyttes til det Tilegnede, at de Punkter hæves frem,
i hvilke Idealiteten ligesom lyser igjennem og som derved
pege hen paa den mere omfattende Tilegnelse. Den philoso\-%
phiske Fremstilling maa da omgive sit formede Indhold med
et endnu ikke i dens Form omdannet Stof, men med et saa\-%
dant, der røber Muligheden af en Tilegnelse, idet det allerede
ligesom er gjennemskinnet af enkelte Straaler af den forkla\-%
rede Tanke.

Hvad der bestemmer Philosophiens \emph{Methode} paa dette
Standpunkt, bestemmer tillige dens \emph{Forhold til de sær\-%
skilte, reale Videnskaber}. Idet Principets Bestemmelse
indeslutter Anerkjendelsen af det \emph{Reales Væsentlighed},
maa Philosophien staae i et bestandigt Forhold til dette Reales
Totalitet. Men derved vil den være i et bestandigt Forhold
til de \emph{særskilte} Videnskaber, der forholde sig hver til sit
Gebet af det Værende, og gjennem deres eiendommelige Me\-%
thoder forberede det reale Erkjendelsesstof for en philosophisk
Erkjendelse. De særskilte Videnskaber, der have deres, ved
deres Stofs Eiendommelighed betingede, eiendommelige Form
og eiendommelige Methode, faae i deres Eiendommelighed en
\emph{Selvstændighed}, som Philosophien maa anerkjende; men
denne Selvstændighed kan kun blive en \emph{relativ} Selvstændig\-%
hed. Relativiteten er begrundet i den kun \emph{partielle} og \emph{re\-%
lative} Realisation af \emph{Videnskabens Begreb} i de særskilte
Videnskaber, der forholde sig til et Partielt, og hvile paa For\-%
udsætninger, de ikke selv begrunde. Idet Philosophien i Kraft
af sit \emph{Princip}, der ikke er et kun relativt eller forudsat; i
Kraft af sin \emph{Gjenstand}, der ikke er en begrændset, men en
\emph{universel}; i Kraft af sin \emph{Videns Art}, der ikke blot for\-%
holder sig til Gjenstanden, men til selve Viden og dens Grund\-%
forhold, og i Kraft af sin \emph{Methode}, der omfatter Erfarings\-%
methoden, Fælledsbestemmelsen for de enkelte reale Viden\-%
skabers eiendommelige Methoder, som Moment i sig, og ud\-%
folder Erkjendelsens Indhold til en \emph{principbestemt Enhed},
% p. 275: sheet signature "18*" stands at the foot; not transcribed (§6).
% --- p. 276 ---
\opage{276} bliver den \emph{adæquate} Realisation af Videnskabens Begreb,
...
Erkjendelse tilegner sig og assimilerer gjennem en Methode,
der ikke er fremmed for hine Videnskabers, men som netop
har tilegnet sig denne som Moment. I dette Forhold viser
Philosophien sig i sin indre Enhed med de enkelte Videnska\-%
ber, og netop idet de udvikle sig i deres hele Eiendommelig\-%
hed, blive de sig deres \emph{fælleds Opgave} bevidste og arbeide
bevidst hen mod en \emph{Videnskabens Enhed}, der realiseres
gjennem den philosophiske Erkjendelse. De reale Videnskabers,
\emph{af Philosophien besjælede}, totale Organisation som en
saadan Videnskabernes Enhed, i hvilken de enkelte Led have
deres \emph{relative} Selvstændighed, maa blive Maalet for Philo\-%
sophiens og de særskilte Videnskabers historiske Udvikling.

% p. 276: the fourth member of the scheme, run-in like the others.  Measured:
% "4." gap 8 px (plain), "Livsanskuelsen." 28 px (letterspaced), stem width the
% same as the body roman -- so \emph{}, not \textbf{}.
4. \emph{Livsanskuelsen.} Søge vi til Slutning den Livs\-%
anskuelse, der vilde svare til Opfattelsen af Principet i denne
Epoche, saa føre Principets Bestemmelser umiddelbart til Be\-%
stemmelsen af dens Eiendommelighed. --- Er Principet bestemt
som det \emph{ideal-reale} Princip, altsaa som det Princip, for
hvilket det Reale har \emph{væsentlig} Betydning, saa maa ogsaa
% p. 276: the SECOND "væsentlig", on the next line, is NOT spaced (8 px against
% 28 px on this one).  Two occurrences three lines apart, one spaced, one not.
Livsanskuelsen hævde det Reale en væsentlig Betydning for
Menneskelivet, og stille den Fordring, at Mennesket skal ud\-%
vikle sig \emph{efter sit Væsens Fuldstændighed som na\-%
turligt-aandeligt}, og gjennem en saadan Udvikling, i hvil\-%
ken det reale Naturgrundlag bevidst gjennemformes, give Vil\-%
lien et Virkelighedsindhold.

% p. 276: "ideal-realt" here is NOT spaced (12 px), though "ideal-reale" nine
% lines above is (28--32 px).
Er Principet som ideal-realt bestemt som det, der be\-%
% --- p. 277 ---
\opage{277} grunder en \emph{organisk} ideal-real Tilværelsestotalitet, i hvilken
Menneskevæsenet er indordnet som \emph{integrerende} Led i den
\emph{relative Selvstændighed}, som Realitetsbestemmelsens Væ\-%
sentlighed gjør mulig, og som Mennesket, ifølge sit Væsens
Universalitet, kan blive sig bevidst, saa maa Livsanskuelsen
udtrykke dette, idet den stiller den Fordring, at Mennesket,
idet det hævder sig i sin fuldstændige, naturlig-aandelige Væ\-%
rens Eiendommelighed, i sin Selvhævdelse skal blive sig be\-%
vidst og ville sig som \emph{Led} i den Totalitet, i hvilken det er
harmonisk indordnet, og som bestemt ved det Princip, der er
\emph{Totalitetens}, altsaa \emph{ud fra sin Selvstændighed frit
bestemme sig til Selvhengivelse}. Dette vil med andre
Ord sige: det skal, idet det hævder sig i sin eiendommelige
Væren, hævde sig \emph{ethisk} i denne. Det \emph{Ethiske} bliver alt\-%
saa Fordringen til Menneskelivet, og dette Ethiske bestemmes
saaledes, at det kan omfatte Menneskelivet i dets concrete
Virkelighed.

Er endelig Principet bestemt som det, der, i sin Sammen\-%
slutten af Modsætningerne til concret Enhed, \emph{udelukker al
Dualisme} fra sig, saa maa Livsanskuelsen udtrykke dette
ved at opfatte det i Væsensenhed med Principet værende
Menneskevæsen som det fra al Dualisme befriede, ved at see
det som uadsplittet, i indre Enhed med sig selv i sine Grund\-%
functioners Harmonie. Af det som sand Enhed bestemte Princip,
til hvis Enhed der kun kan naaes, naar det, der er det phænome\-%
nologiske Udgangspunkt for Erkjendelsen af det: den endelige Til\-%
værelse og den endelige Menneskeaand, ingen uophævelig Modsæt\-%
ning gjemmer i sig, kan der ikke, naar Tanken fra dets Begreb,
der, idet det er naaet, hævder sig for Tanken ved sig selv, og bærer
Visheden i sig som Begrebet om det i sig Begrundede, stiger
nedad til den definitive Erkjendelse af det ved Principet Væ\-%
rende, udledes nogen blivende Dualisme i dette. \emph{Principets
Enhed betrygger Menneskevæsenets og hele Tilvæ\-%
relsens Enhed, viser enhver dualistisk Adsplittelse
i hint som i denne som usand}.
% --- p. 278 ---
\opage{278} \setcounter{footnote}{0}%

Hiin Enhed i Menneskevæsenet, der allerede er seet fra
den Side, at det Naturlige i Mennesket ikke dualistisk sondres
fra det Aandelige, bliver her at see fra den Side, at den ud\-%
trykker \emph{Menneskeaandens} indre Enhed med sig selv i sine
Grundfunctioners Harmonie. Hiin Livsanskuelse maa da hævde
denne Harmonie, idet den hævder Enhedsforholdet mellem
\emph{Erkjendelse} og \emph{Handling} i deres \emph{sande} Form, mellem
\emph{Viden} og \emph{Tro}, mellem det \emph{Ethiske} og det \emph{Religiøse}.
Den maa hævde Enhedsforholdet mellem \emph{Erkjendelse og
Handling}, fordi den Villen, der, som ethisk, er Væsensvillen,
overalt maa være ledet af den Erkjendelse, i hvilken Væsenet
bliver sig aabenbart, og fordi derfor den sande Erkjendelse
maa blive Betingelse for den klart bevidste Villen af det
Ethiske, og for dettes concrete Udfoldelse til en Heelhed, der
omfatter den virkelige Personligheds Liv.\footnote{Jævnfør: Om det Religiøse i dets Enhed med det Humane, S. 5---29.} Den maa hævde
Enhedsforholdet mellem \emph{Viden} og \emph{Troen} i dens Sandhed,
fordi det, der ifølge en Væsenets Trang sættes som Troens
Gjenstand, ikke vil kunne modsige den Viden, der er Væsenets
sande Udtryk; og fordi Viden i sig bærer Troens Bestemmelse
ligesom den i sig bærer Hengivelsens, og derved viser sig at
være i væsentlig Enhed med det Religiøse efter dettes sande
Betydning.\footnote{Sammesteds, S. 43---51.} Den maa endelig hævde Enheden mellem det
\emph{Ethiske} og det \emph{Religiøse}, fordi Villien efter sin Sandhed,
som ethisk Villen, bærer det Religiøses Væsensbestemmelser:
Troen og Hengivelsen, i sig; fordi det ethiske Valg, ved hvilket
Personligheden constituerer sig som ethisk, kun bliver til idet
Forholdet sættes til det Absolute; og fordi de ethiske Forhold
overalt indeslutte dette Forhold, saa at det religiøse absolute
Forhold bliver det, der omfatter alle de relative Forhold, er
iboende i dem og, idet det giver dem absolut Betydning, gjen\-%
% --- p. 279 ---
\opage{279} \setcounter{footnote}{0}%
nem dem udfolder et concret Indhold og vinder Virkeligheds\-%
skikkelse.\footnote{Sammesteds, S. 51---56.}

Idet Livsanskuelsen hævder den indre Enhed mellem Men\-%
neskeaandens Grundvirksomheder og Grundforhold, hævder den
det, der er Betingelsen for den Aandens \emph{Forsoning} i sig,
der ikke kan tænkes, naar Aanden er deelt af modstridende
Magter; thi den i sig adsplittede Bevidsthed maa da i ethvert
Øieblik være sønderreven af Tvivlen om Erkjendelsens Sand\-%
hed, om Handlingens Gyldighed. Den hævder hvad der er
Betingelsen for et i Ordets fulde Betydning \emph{menneskeligt}
Liv, for det sande \emph{humane} Liv, i hvilket \emph{alle} Aandslivets
Kræfter virke uforstyrrede, i bevidst, villet Harmonie. Og
dette humane Liv er, som \emph{ethisk} Liv, overalt i Enhed med
det \emph{Religiøse}; Aandens Forsoning i det er \emph{religiøs For\-%
soning}.

% p. 279: short CENTRED divisional rule between the two paragraphs (300 dpi:
% y=1158, x=538--788, centre 663; the footnote separator lower down on the same
% page is left-flush at x=109--360 against a text-block left edge of 106).
\divrule

I Oversigten over Udviklingen i Philosophiens Historie ere
vi standsede ved de Systemer, der afslutte den nyere Philo\-%
sophies realistiske og idealistiske Udviklingsrække, og vi have
betegnet den Grundform af Philosophien, der maa afløse den
anden Epoche, som \emph{Fremtidens Philosophie}, altsaa som
% p. 279: PARTIAL LETTERSPACING within a single word, and not at a compound
% seam: in "ikke" the gaps run 32, 28, 4 px -- spaced for the first two, tight
% for the last -- while "endnu" before it is fully spaced (32 px).  The whole
% word is marked and the irregularity logged rather than split arbitrarily.
\emph{endnu ikke} historisk virkeliggjort. Et Blik paa de philoso\-%
phiske Phænomener, der ere yngre end hine afsluttende Sy\-%
stemer, vil vise det Berettigede i denne Opfattelse.

De philosophiske Forsøg, der efter \emph{Système de la na\-%
ture} ere fremkomne, med \emph{Realismen} til Grundlag og Ud\-%
gangspunkt, ere enten, som \emph{Materialismen} i \emph{vor Tid}, blotte
\emph{Gjentagelser} af det forrige Aarhundredes Tanker, uden no\-%
% p. 279: another partial: in "Tankeindhold;" the first four gaps are 28--32 px
% and the rest 4--16 px.  Whole word marked, irregularity logged.
get virkeligt nyt \emph{Tankeindhold}; eller det er \emph{Consequentser},
% --- p. 280 ---
\opage{280} der \emph{kun} i det \emph{Negative}, i Polemiken mod den ved \emph{Kant}
hævdede aprioriske Tænkning, adskille sig fra deres Forgæn\-%
gere, og endnu gaae ind under \emph{samme Princip}. De Former
af \emph{Sensualisme}, der ere komne tilsyne i vort Aarhundrede
i \emph{England} og \emph{Frankrig}, gaae ind under \emph{samme} Grund\-%
form som \emph{Realismen overhovedet}, og de undgaae kun den
Consequents, der uundgaaelig maa ramme ethvert paa Ato\-%
mismens Grundlag hvilende System: \emph{Erkjendelsens Til\-%
intetgjørelse gjennem det Almenes Tilintetgjørelse},
ved \emph{vilkaarlig} at holde Tanken borte fra de afsluttende Be\-%
greber, og ved vilkaarlig at indskyde et \emph{relativt Alment}.
Og trods al den tilsyneladende Consequents, med hvilken det
Aprioriske søges fjernet selv fra det Mathematiskes Omraade
(\emph{Mill}), gjør det sig dog uvilkaarligt gjældende, og hos denne
Retnings betydeligste Repræsentant, hos \emph{Comte}, bliver det indi\-%
recte anerkjendt gjennem Forandringen af Methoden i hans
andet Hovedværk: \emph{Système de politique positive}, der har
Menneskelivet til Gjenstand.

De philosophiske Forsøg, der have \emph{Idealismen} til Grund\-%
lag, ere, hvor de ikke ere simple \emph{Fortsættelser}, enten kun
\emph{eklektiske} Forsøg, som den Philosophie, der i Frankrig selv
benævner sig som \emph{Eklekticisme}, som den \emph{italienske} idea\-%
listiske Philosophie i vort Aarhundrede, og som den \emph{Bostrøm\-%
ske} Philosophie i vort Naboland; eller de betegne en \emph{Reac\-%
tion} mod den \emph{Tankens Monisme}, der har fundet sit con\-%
sequenteste Udtryk i den \emph{Hegelske} Panlogisme, men en Re\-%
action, der dog ikke har kunnet føre til at skabe Systemer,
der kunde faae blivende historisk Betydning som Udtryk for
en høiere Tænkningens Grundform.

Blandt disse philosophiske Forsøg have \emph{de} været af rin\-%
gere Betydning, i hvilke Reactionen mod det Hegelske System
antog Formen af en \emph{Hævden af det religiøse Forestil\-%
lingselement mod den begribende Tankes Suprema\-%
tie}. I disse halvtheologiske Omformninger har der været for
% END-OF-BATCH SEAM (BATCH-AGENT §5.2): p. 280 carries NO footnote, so nothing
% can run over into p. 281.  The page ends MID-SENTENCE on "for", continued on
% p. 281; the pp. 281--288 batch must not let a paragraph break appear here, and
% the blank line above its own marker in transcription.tex must be removed.
% --- p. 281 ---
\opage{281} % SEAM CHECK (BATCH-AGENT §5.1): p. 280 carries NO footnote at all -- the whole
% lower third of the page below "har der været for" is blank white, verified on
% the image -- so nothing can run over into p. 281, and p. 281 opens with no
% unmarked note matter.  p. 280 ends mid-sentence on "har der været for",
% continued here by "megen Uklarhed og Hildethed"; the pp. 267--280 batch and
% joints.py must not let a paragraph break appear at this seam.
% p. 281 carries no footnote, no Greek and no ɔ: mark.
% p. 281, l. 6: a round rust-brown blot (foxing/stain, ~34 px across at 600 dpi)
% stands in the word-space between "været" and "en"; ABBYY reads it as "*" and
% tesseract as a comma.  Not printed type; not transcribed.
megen Uklarhed og Hildethed, til at noget Heelt og Blivende
kunde fremgaae deraf.

Større Betydning have de philosophiske Forsøg havt, i
hvilke Reactionen mod Tankens Monisme traadte frem gjen\-%
% p. 281, l. 5: "Værens" and "Erfaringens" are separately letterspaced but the
% "og" between them is NOT, and "Betydning." is not either.  Measured letter
% gaps at 600 dpi: Værens 15--21 px, Erfaringens 15--21 px, og 7 px,
% Betydning. 1--10 px.
nem en Hævdelse af \emph{Værens} og \emph{Erfaringens} Betydning.
Der har i dem været en Bevidsthed om Manglen i den ideali\-%
stiske Philosophies Udvikling fra det Punkt af, hvor hos \emph{Kant}
% p. 281, l. 7: "Kant" is letterspaced and the spacing RUNS TO THE LINE END --
% the final "t" is carried 28 px clear of the "n" by the justification, so the
% word occupies x 2233--2434 where an unspaced "Kant" would be ~150 px.
Realitetsmomentet, der oprindelig fik sin Betydning som Er\-%
kjendelsens objective Forudsætning, traadte i Skygge for Ideali\-%
tetsmomentet. Og betegnende for den Styrke, med hvilken
denne Reaction har gjort sig gjældende i den efterhegelske
% p. 281, ll. 12 and 15: "Væren"/"Værens og Erfaringens" here are NOT
% letterspaced (Væren 205 px, Værens 236 px, against the spaced Værens of
% l. 5 at 291 px).  Only the first statement of the phrase, at l. 5, is spaced.
Tid, er det, at Bestræbelsen for at hævde Væren og Erfarin\-%
gen, ogsaa, om end i uklare og usikkre Former, kommer til\-%
syne indenfor den halvtheologiske Philosopheren.

I Bestræbelsen for at hævde Værens og Erfaringens Be\-%
tydning mødes Mænd, der, med den Kantiske Idealisme til
fælleds Udgangspunkt, ere naaede til de meest modsatte Con\-%
% p. 281, l. 18: "de betydeligste" is NOT letterspaced (de 69 px, betydeligste
% 420 px, all inter-letter gaps <13 px at 600 dpi); only "Schelling" is.
sequentser. Vi nævne kun de betydeligste: \emph{Schelling} (i
hans Philosophies sidste Form), \emph{Herbart} og \emph{Feuerbach}, og
paapege med Hensyn til dem hvad der berettiger vor Dom om
de sidste Former af Philosophien.

I \emph{Schellings nyere Philosophie}, der, om end paa\-%
virket ved theologiske Motiver og i Formen stærkt bestemt
ved Forholdet til Mystiken, naturligt er fremvoxet af hans æl\-%
dre, ved Bestræbelsen for at løse det Problem, denne ikke
% p. 281, ll. 26--27: the letterspacing of "Endeligheden" CARRIES ACROSS the
% line break -- "Ende-" (gaps 14--21 px) and "ligheden" (gaps 14--23 px) are
% both spaced.  Same at ll. 33--34, "positive Philoso-/phies".
magtede: Muligheden af den \emph{Reflexion}, for hvilken \emph{Ende\-%
ligheden} skulde blive til, --- er den Mangel paapeget, der
% p. 281, l. 27: the dash measures 73 px at 600 dpi, within the parenthetical
% range recorded in §2; set --- per §2, which allows no en-dash sort.
laae i den forudgaaende Tænknings ensidige Udvikling. Men
istedenfor at søge Correctivet for den ensidige Idealisme i den
% p. 281, ll. 30--32: the compositor's letterspacing here is PATCHY and is set
% exactly as measured, not regularised.  Measured letter gaps at 600 dpi
% (spaced = 14--26 px, unspaced = 1--12 px):
%   l. 30  reale 18--21 | Væren 15--19 | og 7 | i - | den 19--20 |
%          Erfaring, 1--10 | der/forholder/sig/til/dennes all <13
%   l. 31  Virkelighed, 17--25 | søger 2--10 | Schelling 1--12 |
%          Correctivet 14--24 | i - | en 8--10 | Phantasiens 15--25
%   l. 32  Væren 1--10 (NOT spaced) | og/i/en <13 | «apriorisk» 13--20 |
%          Erfaring, 17--21
% So "den" stands spaced alone before an unspaced "Erfaring,", and "Væren" is
% spaced in l. 30 but not in l. 32.  Nothing is emended.
% l. 29: the first "i den" is NOT spaced (den 111 px against the spaced den of
% l. 30 at 133 px).
\emph{reale Væren} og i \emph{den} Erfaring, der forholder sig til dennes
\emph{Virkelighed}, søger Schelling \emph{Correctivet} i en \emph{Phantasiens}
Væren og i en \emph{«apriorisk» Erfaring}, der svarer til den, og
% p. 281, ll. 32--33: both guillemet pairs point INWARD, «…», read at 1200 dpi.
den \emph{«frie Tænken»}, der skal blive den \emph{positive Philoso\-%
phies} Organ, bliver i sin Vilkaarlighed og Lovløshed kun et
% SEAM p. 281 --> p. 282: the sentence runs on ("... kun et / Navn for den
% Phantasien tjenende Tanke"), so there is deliberately NO blank line above this
% page marker.  A blank line here would give TeX a \par the book does not have.
% --- p. 282 ---
\opage{282} \setcounter{footnote}{0}%
% p. 282 bears ONE footnote, marked *) after "Grundbegreb" in l. 6; it is
% self-contained (two lines, ending "for S. 155.") and does not run over onto
% p. 283.  No Greek, no ɔ: mark.
% p. 282, l. 1: "kun" IS letterspaced (gaps 14--16 px at 600 dpi, width 134 px
% against the unspaced "kun" of p. 281 l. 18 at 116 px) while "gjennem" beside
% it is not (gaps <13 px).
Navn for \emph{den Phantasien tjenende Tanke}, og dens Be\-%
vægelse foregaaer \emph{kun} gjennem \emph{Postulater}.

Hos \emph{Herbart} er der overalt rige Tankefermenter tilstede,
og der er overalt Eiendommelighed og Skarphed i hans Opfat\-%
% p. 282, l. 6: "Grundbegreb*)" itself is NOT letterspaced (gaps 1--12 px at
% 600 dpi, width 524 px) although "ved Feilslutninger vundne" before it is
% (gaps 14--24 px).  Confirmed on the image at 1200 dpi.
telse af Problemerne. Men Ensidigheden i hans \emph{ved Feil\-%
slutninger vundne} Grundbegreb\footnote{Om \emph{Dualismens} Indvirkning paa Bestemmelsen af dette, jfr. oven\-%
for S. 155.}, der \emph{udelukker al
% p. 282, l. 6: a small round rust-brown speck (foxing) stands in the
% word-space between "Grundbegreb*)," and "der"; ABBYY and tesseract both read
% it as a stray comma or full stop.  Not printed type; not transcribed.
% p. 282, ll. 6--7: "fra det" between "Udvikling" and "Reale," is NOT spaced
% (gaps <13 px), so the emphasis breaks there.
Udvikling} fra det \emph{Reale}, og kun indrømmer en \emph{imagi\-%
nær} Selvopholdelse imod en \emph{imaginær} Forstyrrelse, fører
deels til \emph{ufrugtbare Abstractioner}, deels til \emph{Fictioner},
og deels, navnlig i Psychologien og den rationelle Theologie,
til \emph{uopløselige Modsigelser}, der forhindre en Gjennemfø\-%
relse af Grundtanken og Uddannelsen af et System med \emph{bli\-%
vende} Betydning.

Hos \emph{Feuerbach} endelig er Kritiken af Panlogismens
Ensidighed skarp og fiin, og der vises med klar Bevidsthed
hen til Correctivet. Men idet Feuerbach med ubøielig Energie
forfølger den Vei, han anviser, undgaaer han ikke, trods al
den Forskjel, der bliver mellem hans Philosophie, der ikke
% p. 282, ll. 19--20: "Kant" spaced; "vulgære" NOT spaced (gaps <13 px);
% "Materialisme," spaced across the line break (Ma- 147 px with a 17 px gap,
% terialisme, gaps 14--22 px).
helt kan glemme hvad \emph{Kant} har sikkret, og den vulgære \emph{Ma\-%
terialisme}, at mødes med denne i Umuligheden af at for\-%
klare Aandslivets og allerede det organiske Livs Kjendsgjer\-%
ninger, og at løse de Problemer, disse bringe frem, og han
% p. 282, ll. 23--24: the run "al Tankes og Erkjendelses Undergang" is spaced
% throughout, including the two-letter words: al 63 px (unspaced "al" on l. 17
% of the same page measures 48 px) and og 82 px (unspaced og = 72 px).
ender i en \emph{Individualisme}, der bliver \emph{al Tankes og Er\-%
kjendelses Undergang}.

Saaledes stadfæster dette Blik paa de betydeligste philo\-%
sophiske Phænomener, der ere fremkomne efter Afslutningen
af den nyere Philosophies to Hovedformer, det ovenfor Udtalte:
at der endnu kun foreligge Tilløb til og Antydninger af en
ny Philosophiens Grundform, men ingen virkelig Realisation af
en saadan. De Tendentser, der beherske Forsøgene paa at
uddanne en ny Philosophie, ere enten uklare eller modsigende
og derfor uigjennemførlige. Rettest vilde maaskee disse For\-%
% SEAM p. 282 --> p. 283: a WORD is broken across the page break ("For-/søg"),
% so no blank line above this marker and no blank line below the comment block.
% --- p. 283 ---
\opage{283} % p. 283 is a SHORT PAGE: six lines of text and nothing else.  It closes the
% third Hovedepoche and the body of the argument.  NO divisional rule stands at
% its foot -- the faint short rule visible in the blank lower half of the image
% is show-through from p. 284's rule on the verso of the same leaf, confirmed by
% the mirrored text that accompanies it.  No footnote, no Greek, no ɔ: mark.
søg opfattes, i Analogie med de afsluttende Former af den
græske Philosophie, som betegnende den anden Hovedepoches
% p. 283, l. 3: "Opløsning." is the page's ONLY letterspacing (gaps 15--23 px
% at 600 dpi, width 435 px); "løse," in l. 4 is not (gaps <13 px).
\emph{Opløsning}. Men den Opgave, som de forgjæves stræbe at
løse, viser Betragtningen af Philosophiens historiske Udvik\-%
ling som den, der nødvendig maa løses, og i selve Udviklingen
ligger Veien antydet, der ene kan føre til Maalet.

% --- p. 284 ---
\opage{284} % HEAD, p. 284 (L0, transcription.tex §3): "Slutning." -- the last of the five
% L0 heads and the final division of the text.  Centred, alone on its line.
%
% MEASURED AT 600 dpi, and the measurement SETTLES the question the pp. 211--224
% batch left open at p. 216 ("§3 should be corrected once p. 284's Slutning.
% has been measured too"):
%
%   (a) IT IS LETTERSPACED, decisively.  The nine glyphs of "Slutning." occupy
%       x 1215--1979 (764 px) with inter-letter gaps of 46, 52, 49, 43, 46, 44,
%       44 px.  The p. 216 L0 head "Philosophiens" -- same fount, same size --
%       puts THIRTEEN glyphs into 761 px with gaps of 6, 9, 7, 7, 7, 8, 8, 2, 9,
%       6, 8, 6, 6 px.  A factor of six.
%
%   (b) IT IS ONE FOUNT WITH THE OTHER DISPLAY HEADS.  Comparing the letter
%       'n' at 600 dpi (x-height px / median stem px / ratio):
%         p. 284 "Slutning."        72 / 13 / 0.181
%         p.   1 "Indledning."      71 / 13 / 0.183
%         p. 216 "Charakteristik…"  71 / 12 / 0.169
%         p.  31 "Udvikling."       70 / 13 / 0.186
%         body roman, same pages    42 /  6 / 0.143
%       All of them are ONE fount at ONE size; they differ only in
%       letterspacing.  That much the final pass confirms, over all six L0/L1
%       heads and at four thresholds.
%
%   (c) BUT THAT FOUNT IS NOT THE BOLD.  This batch inferred from (b) that
%       every display head is bold, because 0.18 is above the body's 0.14.
%       THAT INFERENCE WAS WRONG, and the final pass reverses it: the batch
%       had no BODY-SIZE bold to calibrate against.  The book's real bold --
%       the L2/L3b/L5 heads, e.g. p. 219 or p. 252 -- measures 0.27--0.30,
%       twice the roman, at every threshold from 35 % to 65 % of the ink
%       depth; the display heads measure 0.16--0.18 at 45 % and are
%       INDISTINGUISHABLE from the body roman at 35 % (0.169--0.181 against
%       0.171), so the residue is a thin-stroke threshold artefact and not
%       weight.  The \textbf{} this batch put on pp. 284 and 289 has been
%       removed; p. 1's {\Large\emph{Indledning.}} was right as it stood.
%       §3 now records the method and the numbers.
\begin{center}
{\Large\emph{Slutning.}}
\end{center}
% DIVISIONAL RULE, p. 284, printed BELOW the head, not above it: a single short
% CENTRED hairline, measured at 600 dpi at x 1490--1680 (190 px = 0.80 cm)
% against a text block of x 490--2700, standing at y 1460 between the head
% (bottom y 1343) and the first text line (top y 1604).  The same pattern §3
% records for p. 31 and the pp. 211--224 batch found at p. 216 (196 px there).
\divrule

% p. 284: the paragraph opens with an enlarged initial "H" of "Hvad" (about
% 1.5 x body height, sitting on the baseline), as at pp. 1, 216 and 288.
% Page-making furniture; set as ordinary text.
% p. 284 carries NO letterspacing anywhere in its 25 lines, no footnote, no
% Greek and no ɔ: mark.  (spacing.py's flags on "Hvad" and on "af Detail-" were
% checked at 600 dpi and 1200 dpi and are serif-splitting noise: all gaps <13 px.)
Hvad der i dette Skrift er blevet udviklet angaaende den
fremskridende Tankebevægelse i Philosophiens Historie, og an\-%
gaaende Tankebevægelsens Forhold til de enkelte Hovedstadiers
centrale Tanker, og, i sidste Instants, til det Tankens Princip,
der sammenfatter disse i sin høieste Enhed, indeslutter Conse\-%
quentser med Hensyn til den Maade, paa hvilken et mere om\-%
fattende og dybere indtrængende Studium af Philosophiens Hi\-%
storie maa anlægges, og til et saadant Studiums Betydning
for den philosophiske Tankefrembringelse.

Et grundigere Studium af Philosophiens Historie maa,
naar den foreløbige Orientering er tilveiebragt, der bedst kan
erhverves gjennem en saadan kortere Fremstilling, der giver
saa Meget af Detaillen, at de almindelige Bestemmelser faae
Indhold, og at de væsentlige Tankeformer kunne blive tydelige
i deres Udfoldelse; men som ikke heller giver Mere af Detail\-%
len, eller giver det paa anden Maade, end at Overskuelsens
% p. 284, l. 17: the dash measures 74 px at 600 dpi.
Klarhed kan bevares; --- nærmest concentreres paa saadanne
prægnante Værker, i hvilke en stor Udvikling grundlægges el\-%
ler naaer det Høidepunkt, ud fra hvilket en ny Udvikling for\-%
beredes, og fra hvis Synspunkt derfor en viid Omkreds klart
overskues. Saadanne Værker have en afgjørende Betydning
ikke blot for Forstaaelsen af det enkelte Udviklingsstadium,
men ogsaa for Forstaaelsen af den historiske Tankeudvikling
i dens Heelhed. I saadanne Værker, i hvilke den philosophi\-%
ske Tænkning viser sig primitiv, skaberisk og fremtidsrig, maa
% SEAM p. 284 --> p. 285: the sentence runs on ("... fremtidsrig, maa / den
% levende og virksomme Aand gribes"), so no blank line above this marker.
% --- p. 285 ---
\opage{285} % p. 285 runs as ONE unbroken paragraph -- no line on the page is indented --
% continuing the paragraph begun on p. 284 and continued onto p. 286.
% No footnote, no Greek, no ɔ: mark.
den levende og virksomme Aand gribes, der styrer og beher\-%
sker Udviklingen, i dem maa det Ord findes, der som et Lø\-%
sen aabner Adgangen til Forstaaelsen af en afsluttet Heelheds,
% p. 285, ll. 4--5: "typiske" is letterspaced across the line break (ty- 110 px
% with a 14 px gap; piske gaps 14--21 px), while the same word UNSPACED stands
% at l. 20 ("meest typiske Udprægning", 231 px, gaps <13 px).
et heelt Udviklingsstadiums, Tanker. Idet de blive det \emph{ty\-%
piske} Udtryk for Udviklingsstadiets Eiendommelighed, faaer
det ved dem sit Enhedsmærke: det viser sig gjennem dem som
en Totalitet i Forhold til de mangfoldige Phænomener, det
saaledes omfatter, at dets styrende, centrale Tanke gjennem\-%
% p. 285, l. 9: the dash measures 75 px at 600 dpi.
trænger og sammenslutter dem; --- og det viser sig som eien\-%
dommeligt udpræget ligeoverfor de andre Hovedformer af Tan\-%
ker, og som i denne eiendommelige Udprægethed, der bestem\-%
mer alle dets Grundtræk, klart udsondrende sig fra disse. Men
netop den Betydning, saadanne Værker have for Forstaaelsen
af det enkelte Udviklingstrin, hvis Væsen de prægnantest ud\-%
trykke, giver dem tillige en Betydning for Forstaaelsen af den
almene, totale Tankebevægelse. Netop idet gjennem dem de
eiendommelige Hovedformers Sondring viser sig i sin Bestemt\-%
hed og Tankens store Grundformer med plastisk Klarhed træde
frem i deres udprægede Eiendommelighed, aabenbarer sig, gjen\-%
nem det Særegnes meest typiske Udprægning, den almene
Livssammenhæng, Forholdet til den levende, universelle Tanke\-%
energies Enhed, der individualiserer sig i de enkelte Former og
sammenslutter dem alle. I det \emph{typiske} Individuelles høieste
% p. 285, l. 24: "begrundende" is NOT letterspaced although it looks so beside
% the spaced "Almene": measured at 600 dpi begrundende runs 430 px with gaps
% 1--15 px, "Almene" 316 px with gaps 16--22 px (an unspaced "Almene" on l. 20
% of the same page measures 233 px).  Printed "begrundende", not "begründende"
% as ABBYY reads it.
Udfoldelse bliver det begrundende \emph{Almene} aabenbart, idet
det ligesom bryder igjennem Individualitetens Begrændsning.
Den Tilværelsens Lov, der fører alt Endeligt tilbage til dets
Udspring, der lægger Opløsningens Spire i Udviklingens, der
i selve Individualitetens concreteste Bestemmelser, i det tilsyne\-%
ladende meest i sig Afsluttede, holder Tilknytningen til det
Almene virksom, lader de udprægede Tankeindividualiteter,
netop idet de udtale det forbindende Ord for en heel Udvik\-%
ling, vise sig som organisk forbundne Led i en større Heelhed,
og den lader gjennem deres individuelle Væsens Udfoldelse en
Udvikling af det Almene komme tilsyne, der paa een Gang er
fremskridende i Fylde og stigende i Intensitet.

% --- p. 286 ---
\opage{286} % p. 286: no footnote, no Greek, no ɔ: mark.  The Latin and German titles
% (de augmentis scientiarum, novum organon, Kritik der reinen Vernunft,
% Grundlegung zur Metaphysik der Sitten, Kritik des Urtheilskraft, Wissenschaft
% der Logik) are set in the SAME upright roman as the Danish and are NOT
% letterspaced -- checked at 600 dpi, all inter-letter gaps <13 px.  Only the
% philosophers' names and the epoch-labels attached to them are spaced.
% PENCIL/SPECK CHECK (BATCH-AGENT §7; p. 286 is on the known-pollution list):
% no pencil is visible on this page at 600 dpi.  What damages the OCR here is
% three small ink specks in word-spaces -- after "at" in l. 23 (ABBYY "at_"),
% between "risk" and "Studium" in l. 18, and after "støtter" in l. 25 -- plus
% the descender of "sig." in l. 19.  None is printed type; none is transcribed.
Saadanne philosophiske Værker, der kunne faae den angivne
Betydning for et Studium af Philosophiens Historie, ere, inden\-%
for den \emph{græske} Philosophies Omraade, \emph{Platos} Værker, og
% p. 286, l. 4: the apostrophe and the "s" of "Aristoteles's" fall inside the
% letterspaced run, read at 1200 dpi.
\emph{Aristoteles's} logiske, metaphysiske, ethiske og generelle
physiske Værker; indenfor \emph{Overgangsperiodens Giordano
Brunos} metaphysiske Skrifter; indenfor den nyere \emph{Realis\-%
mes Bacos} de augmentis scientiarum og novum organon,
% p. 286, l. 7: "organon" is printed lower-case (tesseract's "Organon" is
% wrong), confirmed at 1200 dpi.
samt \emph{Lockes'} Afhandling om den menneskelige Forstand;
indenfor den \emph{dogmatiske Idealismes Spinozas} Ethik;
indenfor den \emph{kritiske Kants} Værker, navnlig hans Kritik
der reinen Vernunft, hans Grundlegung zur Metaphysik der
% p. 286, l. 12: "Kritik des Urtheilskraft" is printed thus, with "des" where
% German requires "der"; read at 1200 dpi on both witnesses' agreement.  The
% author's error, not the compositor's, and not on the Trykfeil leaf.
% Transcribed as printed per the repo's diplomatic stance.
Sitten og hans Kritik des Urtheilskraft; indenfor den \emph{specu\-%
lative Hegels} Wissenschaft der Logik. Paa disse Værker
maa et dyberegaaende Studium fortrinsviis rettes; ved et gjen\-%
taget, indtrængende Studium af dem vil en grundig, sammen\-%
sluttet Erkjendelse af Philosophiens historiske Udvikling blive
mulig. Og paa Studiet af disse kan et mere udstrakt histo\-%
risk Studium, i Omfang bestemt ved det individuelle Øiemed,
sikkert støtte sig.

Et saaledes anlagt og ved Bevidstheden om Udviklingens
Love ledet Studium vil nødvendigen faae sin Betydning for
% p. 286, l. 22: "Tankeudvikling." is letterspaced (gaps 14--26 px at 600 dpi)
% but "philosophiske" immediately before it is not (gaps 2--12 px); ABBYY's
% "Tanke ud vi kling" is the tell.  Likewise l. 23 "Kritik," spaced (gaps
% 16--23 px, 284 px wide, against the unspaced "Kritik" of l. 10 at 186 px),
% and l. 28 "negativ" spaced (gaps 15--22 px) with "Retning," unspaced
% (gaps 1--11 px).
den philosophiske \emph{Tankeudvikling}. Det faaer først en saa\-%
dan Betydning derved, at det uddanner en \emph{Kritik}, der er
ledet ved Totalitetsanskuelsen, og ved Bevidstheden om Udvik\-%
lingslovene og Udviklingsbetingelserne, og som støtter sig paa
et omfattende og grundigt Kjendskab til de historiske Phæno\-%
mener. Denne Kritik virker i Forhold til den egne Tanke\-%
frembringelse nærmest i \emph{negativ} Retning, som afværgende det
Uigjennemførlige, som forhindrende en Betræden af Veie, der
allerede have viist sig, ikke at kunne føre til Maalet, altsaa
som afskjærende de ufrugtbare Anachronismer, paa hvilke et\-%
hvert Afsnit i Philosophiens Historie, der følger efter en Cul\-%
mination, frembyder Exempler, ogsaa vor Tid, hvis philosophi\-%
ske Reformationsforsøg, navnlig de, der optræde med meest
Larm og flest Reclamer, saagodtsom alle ere famlende Gjen\-%
% SEAM p. 286 --> p. 287: a WORD is broken across the page break
% ("Gjen-/tagelser"), so no blank line above this marker.
% --- p. 287 ---
\opage{287} % p. 287 runs as ONE unbroken paragraph -- no line on the page is indented.
% No footnote, no Greek, no ɔ: mark.
tagelser af det Uigjennemførlige, der har været og kun fornyes
fordi man ikke kjender den historiske Udvikling. Men Kri\-%
tiken fører saa ogsaa, gjennem Forholdet til det historisk
Givne, idet den her gaaer i Enhed med den levende begri\-%
% PRINTER'S ERROR, p. 287, l. 5: the page prints a COMMA between "positivt"
% and "Resultat" -- "til et p o s i t i v t, Resultat, derved" -- where the
% sense requires none.  Confirmed at 1200 dpi; both OCR witnesses read it.
% Not on the author's Trykfeil leaf.  Transcribed as printed.
bende Reproduction, til et \emph{positivt}, Resultat, derved, at un\-%
der Opfattelsen af det Historiske Synet paa det skaberiske
Tankeprincip, paa den philosophiske Tankes høiere tilskyndende
Magt, dens universelle Entelechie, bliver det Første og Væ\-%
sentlige; den kritiske Prøven det Secundære, der aldrig over\-%
vælder hint, men i enhver Paaviisning af en ved Ensidighed
bevirket Forfeilen af Maalet tillige viser hen paa den nye Livs\-%
spire, den levende Mulighed, der voxer frem af den ufuld\-%
komne, men i sin Ufuldkommenhed af Ideen besjælede Stræben.
% p. 287, l. 14: a fine horizontal hairline stands over the "e" of "det",
% joining the ascender of the d to the stem of the t (tesseract reads "d&ét").
% It is an ink flaw or a hair in the forme, not a printed accent; the word is
% "det" and is transcribed so.
Idet Kritiken ikke standser i det Negative, men viser hen
paa dette Positive, kommer, netop ad dens Vei, den, der sysler
% p. 287, ll. 16--17: "Tanke." is so widely letterspaced that its letters clear
% one another by up to 30 px at 600 dpi -- the word occupies x 217--487 where an
% unspaced "Tanke." would run ~205 px.
med det Historiske, i Forhold til en \emph{levende, nærværende
Tanke}. Idet han overalt i det enkelte historiske Phænomen
seer den \emph{almene} Tankebevægelse, der i sin lovbestemste
Fremskriden samler alt det Enkelte til en \emph{articuleret En\-%
hed}, træder han i Forhold til et ikke blot Fortiden Tilhørende, og
føres fra det historisk Givne, fra det umiddelbart som Stof
Foreliggende, over i en fremskridende og under denne Frem\-%
skriden ved Indsigten i det Historiske betrygget Tankefrem\-%
bringelse. Reproduktionen af det Tænkte fører, idet det sees
som fremgaaende af en \emph{levende Enhed}, med indre Nødven\-%
dighed til Productionen af Tanker, i hvilke det Forbigangne
udfoldes. Den menneskelige Tankes Liv er gjennem den hi\-%
storiske Udviklings forskjellige Stadier det ene og samme Liv;
dets Forbigangenhed, som under Udviklingen bestandig ophæ\-%
vedes i et Nærværende, bliver ogsaa i den levende Reproduc\-%
tion ophævet i et saadant, og af dette energiske Nærværende,
der i sig rummer den universelle Tankes Livsenhed, vil
Tankens formende Livsdrift lade nye livskraftige Tankespirer
skyde frem.

% DIVISIONAL RULE at the FOOT of p. 287, below the last line of the Slutning
% and above the blank remainder of the page.  It is NOT the ordinary single
% hairline of §2: measured at 600 dpi it is a DOUBLE rule -- a heavier line with
% a thin hairline immediately below it -- spanning x 999--1605 (606 px = 2.6 cm)
% and centred on the measure (x 200--2745).  It is the widest and the only
% doubled divisional rule met in this batch; it closes the body of the Slutning
% and stands above the small-type author's note that fills p. 288.  Set with
% \divrule, the preamble's short centred rule, with the doubling recorded here.
\divrule

% --- p. 288 ---
\opage{288} % p. 288 IS THE LAST PAGE OF THE BODY.  The whole page is ONE paragraph -- the
% author's closing bibliographical note -- set in a SMALLER FOUNT than the body:
% x-height 36--37 px at 600 dpi, against 42 px in the body of pp. 281--287 and
% 32 px in the footnote fount of p. 282's note.  An intermediate size, not the
% footnote size; recorded here with \small, the only place in the edition where
% a size change is printed.  There is no heading and no rule on the page (the
% rule belonging to this matter stands at the foot of p. 287), no footnote, no
% Greek and no ɔ: mark.
% The paragraph opens with an enlarged initial "D" of "Det", as at pp. 1, 216
% and 284; page-making furniture, set here as an ordinary D.
% LETTERSPACING on this page, established by measuring every line's gap
% distribution at 600 dpi: only lines 2--3 and line 10 depart from the page's
% norm (90th-percentile gap 20--22 px against 11--12 px on all 22 other lines).
% They are the lecture title «over Udviklingsgangen i Philosophiens Historie»
% and the name Spinoza.  «christelige Philosophie» in l. 20 is NOT spaced.
% GUILLEMETS: both pairs on this page point INWARD, «…», read at 1200 dpi --
% including the opening « of l. 2, which ABBYY reads as ».
{\small
Det Æmne, jeg i dette Skrift har behandlet, har jeg tidligere gjort
til Gjenstand for en Universitetsforelæsning \emph{«over Udviklings\-%
gangen i Philosophiens Historie»}, der blev holdt i Efter\-%
aarshalvaaret 1864 og en Deel af Foraarshalvaaret 1865, og som,
med nogen Bearbeidelse i det Enkelte, blev gjentagen i Efteraars\-%
halvaaret 1867 og Foraarshalvaaret 1868. Fra disse Forelæsnin\-%
ger er det væsentlige Indhold af det foreliggende Skrift hentet.
% p. 288, ll. 8--9: "Epoche-" is a SUSPENDED hyphen (Epoche- og
% Periodeinddelingen), printed and kept as printed per §2; it is not a
% line-break hyphen and is not resolved.
Opfattelsen af Articulationen af Philosophiens Historie, af Epoche-
og Periodeinddelingen, er allerede i Korthed fremsat i Fortalen til
% p. 288, l. 10: the numeral-range dash in "3---12" measures 68 px at 600 dpi,
% and that in "1849---1850" (l. 18) 55 px, against 73--75 px for the
% parenthetical dashes of pp. 281, 284 and 285 -- but this page is set in a
% smaller fount, and per §2 the scan establishes no distinct en-dash sort.
% Every dash is transcribed ---.
min Afhandling om \emph{Spinoza}, S. 3---12, og har endnu tidligere
været gjennemført i de Forelæsninger over Philosophiens Historie,
jeg fra 1849 af har holdt ved Universitetet. I disse Forelæsninger
ere ogsaa allerede de enkelte historiske Afsnit, der i dette Skrift
ere udførte nærmere forat oplyse Udviklingens Love, behandlede
væsentlig paa samme Maade som her. Saaledes er f. Ex. Afsnittet
om Sokrates, med idethele kun formelle Forandringer, taget fra
min Forelæsning over den græske Philosophie i Vinterhalvaaret
1849---1850; de øvrige Afsnit af den græske Philosophie fra en
Forelæsning over samme Æmne fra 1857; Afsnittene om den
% PRINTER'S ERROR, p. 288, ll. 20--21: "Forelæsning" is broken across the line
% break with NO hyphen -- the page prints "fra en Fore" at the end of l. 20 and
% "læsning fra 1853." at the head of l. 21.  A dropped hyphen; confirmed at
% 1200 dpi, and both OCR witnesses read the break unhyphenated.  The word is
% joined here without supplying the hyphen the compositor omitted.
«christelige Philosophie» og om Overgangsperioden fra en Fore%
læsning fra 1853. Men de Resultater, til hvilke mit Studium af
Philosophiens Historie allerede for længere Tid siden har ført mig,
har jeg søgt stadfæstede ved en gjentagen Syslen med Kilderne, og
jeg har derved i dette Skrift kunnet give dem en præcisere Form
og tildeels kunnet belyse dem fra nye Synspunkter.

% p. 288, last line: the author's signature "Forf." (= Forfatteren), closing the
% Slutning and the body of the book.  IT IS TEXT and is kept.  Measured at
% 600 dpi: it occupies x 2399--2554 against a text measure of x 533--2745, so it
% stands about 190 px (0.80 cm, ~9 % of the measure) SHORT of the right-hand
% edge -- set toward the right, but NOT truly flush to it; the \hspace below
% records that inset.  It is set in the SAME small roman as the rest of the
% page: NOT letterspaced (inter-letter gaps 6, 5, 2, 4 px) and NOT bold
% (cap 'F' height 46 px and median stem 7 px, against 48 px and 6 px for the 'F'
% of "Forelæsning" in l. 19 of the same page, and identical 'o' dimensions --
% height 35 px, side-wall thickness 6 and 8 px against 6 and 7 px -- so the 1 px
% stem difference is within noise).
% NOTHING STANDS BELOW IT: no rule, no ornament, no printer's mark, no sheet
% signature; the lower third of the page is blank, and p. 289 begins the
% Register.
\begin{flushright}
Forf.\hspace{0.8cm}
\end{flushright}
...
% --- p. 289 ---
\opage{289} % =====================================================================
%  REGISTER, printed pp. 289--294 (= PDF 300--305).  Transcribed COLUMN BY
%  COLUMN off the images at 400 dpi, with the column split taken from
%  `pdftotext -bbox-layout` (the vertical rule sits at x ~ 178--180 pt on
%  every one of the six pages, and the word boxes separate cleanly at it);
%  every figure, every Greek word and every doubtful point re-read on the
%  image at 600 or 1200 dpi.
%
%  WHAT THE PRINT DOES AND THIS TRANSCRIPTION DOES NOT REPRODUCE:
%   * TWO COLUMNS to a page, separated by a thin vertical rule running the
%     full height of the column block.
%   * A small BOLD flush-right head "Side." over every column -- twelve of
%     them, one in each column of each of the six pages.
%   * LEADER DOTS from the headword to a right-aligned figure whenever an
%     entry carries a SINGLE reference ("Agricola. . . . . . . 62."); an
%     entry with several references has none ("Anaxagoras 38, 53, 189,
%     210.").  Some entries fill the measure and take a single leader dot
%     ("Baco . 156 ff., ...", "Patristisk Philosophie. 138 ff.",
%     "Bostrømsk Philosophie . 280."); that dot is a leader like the
%     rest and is dropped with them.
%   * The oversized initial capital.  There is exactly ONE in the whole
%     Register -- the "A" of "Agricola", the first word of the list.
%     Measured at 400 dpi its cap height is 49 px against 35 px for the
%     "A" of "Anaxagoras" and 39 px for the "A" of "Anaximander" on the
%     next two lines, i.e. about 1.4x, the same enlarged-initial device
%     the book uses at the head of pp. 1, 216, 284 and 288.  RECON.md §10
%     reports an oversized initial at the head of EVERY alphabet group
%     ("a two-line A for Agricola, B for Baco, and so on"); that is WRONG,
%     and the measurement refutes it: Baco's B is 42 px against Berkeley's
%     41; Campanella's C is 42 against Cardano's 41; Damascius's D is 40
%     against Dante's 38.  Only the first entry of the list is enlarged.
%   * The COLUMN-BREAK line divisions, and the compositor's line-break
%     hyphens.  Because the Register is reset here as one reflowed
%     single-column list, broken words are simply rejoined (Erfarings-\
%     principet -> Erfaringsprincipet) rather than kept as \-% ; there is
%     no printed line structure left to record.  The Greek word broken as
%     "Κόσ-|μος" on p. 289 is rejoined the same way.
%   * The compositor's loose spacing BEFORE a colon ("Grundbegreb : 252 f.",
%     "krates : 203"), which is line-fitting and not orthography; colons
%     are set tight throughout here.
%   * SHEET SIGNATURES at the foot: "19" under p. 289 and "19*" under
%     p. 291.  Not transcribed (transcription.tex §2).
%
%  WHAT IS KEPT: the BOLD page figures, which mark the principal treatment
%  of a topic; the --- separators between run-on sub-entries; f., ff.,
%  Anm. (= the reference is to a FOOTNOTE on that page) and S. exactly as
%  printed; the blank line between alphabet groups, as a vertical space.
%
%  \regsub{...} below sets a sub-entry block: run-on prose, indented one
%  em from the main entry, exactly as printed.  The list itself gives the
%  main entries a hanging indent, so that a main entry's continuation
%  lines fall to the right as they do on the page.
% =====================================================================

% HEAD, p. 289 (L0, transcription.tex §3): "R e g i s t e r ." centred,
% alone on its line, with a short centred rule beneath it.
%
% MEASURED AT 600 dpi, and it agrees with what the pp. 281--288 batch
% established at p. 284:
%   (a) IT IS LETTERSPACED.  The nine glyphs occupy x 1069--1798 with
%       inter-letter gaps of 39, 39, 35, 36, 38, 38, 37 px.  The p. 216 L0
%       head, same fount and size unspaced, runs 2--9 px.
%   (b) IT IS THE DISPLAY FOUNT, and is indistinguishable from the
%       p. 284 head "Slutning."  x-height 75 px against 72 px there;
%       vertical-stem mode 12 px against 12--13 px there (run histogram
%       over the middle 20 % of the x-height band: 12 px x 51, 13 px x 38
%       here; 13 px x 25, 12 px x 24 at p. 284).  Body roman on the same
%       page is 8 px at x-height 45.  The two L0 heads are one fount.
% The batch inferred from (b) that the fount is BOLD and set \textbf{\emph{}};
% the final pass reverses that -- see the note at the p. 284 head and §3 --
% and the head keeps only its \emph{}, the display roman being the roman.
\begin{center}
{\Large\emph{Register.}}
\end{center}
% DIVISIONAL RULE, p. 289, printed BELOW the head: a single short CENTRED
% hairline, measured at 400 dpi at x 158.8--182.9 pt (24.1 pt = 0.85 cm)
% against a text block of x 35--326 pt, standing between the head and the
% first entry.  The same pattern as pp. 31, 216 and 284.
\divrule

\begingroup
% A run-on sub-entry block, indented one em from the main entry.
\newcommand{\regsub}[1]{\par{\advance\leftskip 1em\relax\noindent #1\par}}
\begin{list}{}{%
...
% p. 289: the "A" of "Agricola" is the enlarged initial described above;
% set as an ordinary capital.
\item Agricola 62.
\item Anaxagoras 38, 53, 189, 210.
\item Anaximander 51.
\item Apologeterne 139.
\item Aristippos 193 ff.
  \regsub{Hans Lystlæres Forhold til den græske Ethik: 195 ff.}
\item Aristoteles \textbf{94 ff.}
  \regsub{Hans Philosophies almindelige Charakteer: 95 ff. --- Hans
  Forhold til den græske Philosophie overhovedet: 33, 96 f., 120 f. ---
  Hans Forhold til Plato: 41 ff., 95 f., 97, 112, 127, 190. --- Hans
  Tænknings Art: 105, 222 f. --- Hans Opfattelse af Grundbegrebet
  (Formens Begreb): 41, 95 f. --- Forholdet mellem Form og Stof: 41 f.,
  82, Anm., 95, 99 ff., 103, 118. --- Dualismen hos Aristoteles: 117 ff.
  --- \gk{Οὐσία}: 121, 123. --- Den høieste Form, den guddommelige
  \gk{Νοῦς}: 33, 100 f., 105 f., 119, 219. --- Stoffet: 97 ff. ---
  Bevægelsen: 99. --- Vordens Begreb: 97 ff. --- Udviklingens Begreb: 96.
  --- Universet som \gk{Κόσμος}: 119 f., 126 f. --- Uendeligheden i Rum
  og Tid: 192, Anm. --- Det Organiske: 96. --- Naturerkjendelsen: S. 122
  ff., 125 f. --- Erfaringen: 97, 121, 124. --- Det Enkelte: 97, 120 ff.
  --- Tilværelsens Concretion: 102 ff. --- Anthropologien, dens
  Eiendommelighed, dens Mangler: 107 f., 110 f. --- \gk{Νοῦς παθητικός}:
  108 f. --- \gk{Νοῦς ποιητικός}: 109 f. --- Villien: 111. --- Det
  Ethiske: 106 f., 110 ff. --- Dyden: 111 f. --- Begrebet
  \gk{μεσότης}: 113 ff. --- Historiens Udvikling: 192.}
% p. 289: the GREEK of this page, all read at 1200 dpi.  Οὐσία -- smooth
% breathing (a comma opening right) over the υ, acute over the ι; the
% same reading the body carries at pp. 97 and 121.  Νοῦς -- circumflex
% over the υ and NO separate breathing visible, as at p. 88 and as the
% edition sets it throughout.  Κόσμος is broken across the two lines of
% the column as "Κόσ-|μος" and is rejoined here; acute over the ό.
% παθητικός, ποιητικός, μεσότης -- acute in each case, seen rising to the
% right.  Theta is typed plain (its curled form in this cursive fount is a
% glyph variant, not a distinct sort -- BATCH-AGENT.md §4).
\item Aristoteles's Skole 190.

\vspace{0.5\baselineskip}
% --- p. 290 ---
\opage{290} \item Aristotelikerne i Overgangsperioden 61 f.
\item Atomisterne 38, \textbf{186 ff.}
  \regsub{Atomlærens Forhold til den græske Naturopfattelse: 188 f. ---
  Dens Forhold til den moderne Naturopfattelse: 187 f. --- Dens
  metaphysiske Charakteer: 188 f.}

\vspace{0.5\baselineskip}
\item Baco \textbf{156 ff.}, 180, 187, 235.
  \regsub{Hans Opfattelse af Naturen og Materien: 157. --- Dualismen hos
  ham: 157 f. --- Erfaringsprincipet: 158 f.}
\item Berkeley 162 f.
\item Bostrømsk Philosophie 280.
% p. 290: "Bøhm, Jacob" as printed (not Bøhme).  The alphabet of this
% Register puts ø immediately after o: Bostrømsk, Bøhm, Bruno.  That is
% consistent, not a fault -- neither "ø after z" nor "ø = oe" would give
% the printed order.
\item Bøhm, Jacob 62.
\item Bruno, Giordano 59, \textbf{60 f.}, 62, \textbf{146 f.}, 157.

\vspace{0.5\baselineskip}
\item Campanella 62, 149.
\item Cardano 62.
\item Charron 63.
% p. 290: "Christendommen" stands BEFORE "Christelig Philosophie", which
% reverses the alphabet (Christel- < Christen-).  Printed thus; not
% normalised.  This is the only break in the alphabetical order in the
% six pages.
\item Christendommen 21, 129 f.
  \regsub{Dens Livsanskuelse: 21. --- Dens Forhold til Tænkningen: 22 f.,
  137 f. --- Dens Genesis: 130.}
\item Christelig Philosophie 23, \textbf{137 ff.}
  \regsub{Dens to Former: 138.}
\item Comte 280.
\item Condillac 163.

\vspace{0.5\baselineskip}
\item Damascius 129, 136.
\item Dante 58, Anm.
\item Demokrit 187, Anm.
\item Descartes 151, 173, 176, 180, 187, 244, 253, 255.
% p. 290, right column, l. 5 of the Descartes sub-entry: a faint thin
% vertical hairline stands between "177," and "Anm."  It is far lighter
% than the type of the same line at 1200 dpi and takes no serif; not
% transcribed.  (Cf. the heavier stroke at "Det Aprioriske" below.)
  \regsub{Hans Substantsbegreb: 173, 240 f. --- Antydninger af det
  Speculative hos ham: 177, Anm., 260 f.}
% p. 290: "Dogmatisk Idealisme:" -- this is the ONLY main entry in the
% Register that carries a colon after the headword.  Confirmed at 400 dpi;
% printed thus, not normalised.
\item Dogmatisk Idealisme: 66, 151 ff., 172 ff., 180, \textbf{240 ff.}, 253.
% p. 290: where the colon of "Det Aprioriske:" belongs there stands
% instead a plain vertical stroke.  Measured at 1200 dpi it is a single
% CONTINUOUS run of ink 96 px tall (5.8 pt) and 15 px wide with a slight
% top serif, whereas a colon of this fount on the same page ("Ideæ
% innatæ:", two lines below) is two separate round dots spanning 4.3 pt.
% It is therefore not a colon -- most likely a space or quotation quad
% risen to type height and taking ink.  Transcribed as what is printed,
% i.e. with no colon; nothing is supplied.
  \regsub{Dens Grundbegreb: Substantsbegrebet: 172 ff., 240 ff. --- Dens
  Tænknings Eiendommelighed: 229 f., 236, \textbf{244 ff.}, 250. --- Det
  Aprioriske 244, 246. --- Ideæ innatæ: 229, Anm., 247. ---
  Tankeevidentsen: 250, Anm. --- Livsanskuelsen: 228 f.}
\item Duns Scotus 140, 141.

\vspace{0.5\baselineskip}
\item Efterhegelsk Philosophie 281 f.
% p. 290: "Elekticisme" as printed -- a dropped k, for "Eklekticisme".
% Read at 400 dpi and again at 1200; the word has no k between the E and
% the l.  The alphabet confirms the compositor's intention: the entry
% stands BEFORE "Eleaterne", which is where Eklekticisme belongs
% (Ekl- < Ele-) and where Elekticisme does not.  Printed reading kept.
\item Elekticisme 280.
\item Eleaterne 37, 52, 86, 189.
\item Empedokles 38.
\item Empirisme 160.
\item Epikur \textbf{190 ff.}, \textbf{199 ff.}
  \regsub{Hans Atomlære: 190 ff. --- Hans Lystlære: 199 ff. ---
  Antydning af det alment Humane: 201.}
\item Epikureismen 44.
\item Epocherne i Philosophiens Historie 16 ff., \textbf{28 ff.}
\item Erfaringsphilosophie 160, 232.

\vspace{0.5\baselineskip}
\item Feuerbach 281 f.
\item Ficino, Marsiglio 61, 64.

\vspace{0.5\baselineskip}
% --- p. 291 ---
\opage{291} \item Fichte 68, 78, 155, 178, 180, 230, 238, 251, 255.
  \regsub{Begrebet om Jeget som absolut: 180, 253. --- Naturen: 255. ---
  Methoden: 257. --- Livsanskuelsen: 258 f.}
\item Fremtidens Philosophie 27, \textbf{269 ff.}

\vspace{0.5\baselineskip}
\item Geulincx 260 f.
\item Græsk Philosophie \textbf{18 ff.}, \textbf{31 ff.}, \textbf{219 ff.}
  \regsub{Dens Plads i Philosophiens Historie: 18 ff. --- Dens
  Grundtanke: 31 f., \textbf{41}, 219 f. --- Dens Opfattelse af Universet
  som \gk{Κόσμος}: 18 f., \textbf{32 f.} --- Dens Tænknings Forhold til
  Anskuelsen: \textbf{84}, 105, \textbf{221 ff.}, 230. --- Dens
  Naturerkjendelse: 124 f., 220, 223 f. --- Dens Opfattelse af det
  Ethiske: 107, 225. --- Dens Livsanskuelse: 224 f. --- Dens
  Udviklingsgang: 31 ff. --- Første Periode: 35 ff., 50 ff. --- Anden
  Periode: 39 ff. --- Tredie Periode: 43 ff. --- Den græske Philosophies
  nødvendige Opløsning: 20, 45.}

\vspace{0.5\baselineskip}
% p. 291: "Hans Panlogisme 65, 238, 263." -- NO colon after the
% sub-topic, where every other sub-topic in the Register has one.
% Confirmed at 400 dpi.  Printed thus.
\item Hegel 65, 155, 178 f., 180.
  \regsub{Hans Forhold til Kant: 71, 268. --- Hans Forhold til den nyere
  Idealisme overhovedet: 268 f. --- Hans Opfattelse af Philosophien: 238.
  --- Hans Panlogisme 65, 238, 263. --- Grundbegrebet: 262 f. ---
  Naturopfattelsen: 155, 263. --- Dialektikens Art: 264 ff. ---
  Livsanskuelsen: 239, 266 f. --- Dualismen i hans System: 268 f.}
\item Hegesias Peisithanatos 197 ff.
\item Heraklit 38, 52 f.
\item Herbart 155, 281 f.
\item Hobbes 162, Anm.
\item Hume 162 f., 172.

\vspace{0.5\baselineskip}
\item Idealistisk Philosophie i den nyere Tid 26 f., 65 f., \textbf{236 ff.}
  \regsub{Opfattelsen af Principet: 237; --- af Tankens væsentlige
  Gjenstand: 237. --- Methoden: 238. --- Livsanskuelsen: 238. --- Den
  idealistiske Philosophies Hovedformer: 239.}
\item Italiensk Philosophie i vort Aarhundrede 280.

\vspace{0.5\baselineskip}
\item Jonierne 37.

\vspace{0.5\baselineskip}
\item Kant \textbf{65 ff.}, 178, 180, 230, 251, 281.
  \regsub{Hans Philosophies almindelige Charakteer: 72. --- Dens
  Betydning: 66 f., 78 f. --- Dens Forhold til de efterfølgende
  Systemer: 78 f. --- Kritik der reinen Vernunft: 66 f.
% --- p. 292 ---
\opage{292} % p. 292's left column opens INSIDE this sub-entry, at "66 f. ---
% Sondringen mellem ..."; the marker stands at that point.
  --- Sondringen mellem Apriorisk og Empirisk: 238, 251, 254, 256. ---
  Centralbegrebet: den transscendentale Apperception: 67 f., jfr. 253 f.,
  268. --- Kritik der praktischen Vernunft: 70 f. --- Kritik der
  Urtheilskraft: 72 ff. --- Begrebet om det fælleds intelligible Substrat
  for Natur og Aand: 75 f. --- Den intellectuelle Anskuelse: 78. ---
  Kants Opfattelse af Naturen: 69, 71, 154, 254 f. --- Mechanisme og
  Teleologie: 178. --- Dualisme: 154. --- Livsanskuelse: 257 f.}
\item Kritisk Idealisme 66, 154 ff., 177 f., 180.
  \regsub{Dens Forhold til den dogmatiske Idealisme: 177, 249 f. --- Dens
  Udgangspunkt: 177. --- Dens to Former: 239. --- Den kritiske Idealisme
  i snævrere Betydning: \textbf{252 ff.} --- Dens Forhold til den
  speculative Idealisme: 250 f. --- Dens Grundbegreb: 252 f. --- Dens
  Tænknings Eiendommelighed: 230, 251, 255 ff. --- Modsætningen mellem
  Subjectivt og Objectivt: 256. --- Dens Livsanskuelse: 228 f., 257.}

\vspace{0.5\baselineskip}
% p. 292: the Leibnitz sub-entry ends "248 ---", with a dash and no
% closing full stop -- the only sub-entry block in the Register that does.
% Verified at 400 dpi: nothing stands to the right of the dash, and there
% is no point after the 248.  Printed thus.
\item Leibnitz 152 f., 176, 180, 228, Anm., 241, 261.
  \regsub{Monadebegrebet: 152, 241 f. --- Monaderne og Gud: 248 f. ---
  Aand og Legeme: 241. --- Selvbevidstheden: 243. --- Monadernes
  Udvikling: 246. --- Forholdet til Spinoza: 241 f., 244, 248 ---}
\item Locke 160 f., 163.
\item Lystlæren 192 ff.

\vspace{0.5\baselineskip}
\item Malebranche 261.
\item Materialismen \textbf{162 ff.}, 232, 235, 236.
  \regsub{Dens Opfattelse af Bevægelsen: 164 f. --- Dens Forklaring af
  Bevidstheden: 165 ff. --- Dens Inconsequentser: 167 f., 171 f. ---
  Materialismen i vor Tid: 279 f.}
% p. 292: "21. 23." -- a FULL STOP between the two figures where every
% other entry has a comma.  Read at 400 dpi: the mark is round and sits on
% the baseline with no tail, against the plainly tailed commas of "61 f.,
% 64." and "128, 138." elsewhere on the same page.  Printed thus.
\item Middelalderen 21. 23.
  \regsub{Dens Charakteer: 21. --- Dens Opløsning: 23.}
\item Mill 280.
\item Montaigne 63.

\vspace{0.5\baselineskip}
\item Nicolaus Cusanus 61, 63 f.
\item Nyere Tids Philosophie 25 f., 150, \textbf{226 ff.}
  \regsub{Dens Opgave: 150 f. --- Dens almindelige Charakteer: 25, 150
  ff. --- Dens Hovedformer: 26 ff., 170 ff. --- Det for disse Fælleds:
  \textbf{226 ff.} ---
% --- p. 293 ---
\opage{293} % p. 293's left column opens INSIDE this sub-entry, at "Dens Paavirkning
% ved det Middelalderlige"; the marker stands at that point.
  Dens Paavirkning ved det Middelalderlige: 150 ff.}
\item Nyere Tid, dens Charakteer 23.
\item Nyplatonismen 44, \textbf{127 ff.}
  \regsub{Dens Stilling til den græske Philosophie overhovedet: 127 f.,
  132 ff. --- Dens Forhold til Christendommen: 129 ff. --- Dens
  Tænknings Eiendommelighed: 134 ff.}
\item Nypythagoræerne 44.

\vspace{0.5\baselineskip}
\item Overgangsperioden 24 f., \textbf{54 ff.}, \textbf{145 ff.}, 226, Anm.
  \regsub{Dens Nødvendighed: 54. --- Dens Forhold til Oldtidens,
  Middelalderens og den nyere Tids Tænkning: 55 ff., 146 f., 226, Anm.
  --- Dens Grundtanker: 57 ff., 145 ff. --- Dens Naturopfattelse: 148 f.
  --- Dens Udvikling: 59.}

\vspace{0.5\baselineskip}
\item Paracelsus 62.
\item Patristisk Philosophie 138 ff.
% p. 293: "44. 128, 138." -- a FULL STOP after the first figure, as at
% "Middelalderen 21. 23." on p. 292.  Read at 400 dpi against the tailed
% comma of "128," on the same line.  Printed thus.
\item Philo 44. 128, 138.
\item Pico af Mirandola 61, 64.
% p. 293, Plato sub-entry: "80 f,," -- a COMMA where the point of "f."
% belongs, followed by the separating comma, so that two commas stand
% together.  Read at 400 dpi; both marks are tailed.  Printed thus.
% p. 293: "Den ethiske Opgave: 93." -- the mark after 93 is a heavy round
% ink blot sitting slightly below the baseline; read as the full stop the
% pattern of the block requires, but it is a blot, not a clean point.
\item Plato \textbf{80 ff.}
  \regsub{Hans Philosophie som typisk Udtryk for den græske Tænkning: 80
  f,, 91 f., 94. --- Dens almindelige Charakteer: 80. --- Dens Betydning:
  80. --- Platos Forhold til Sokrates: 203, 207; --- til Aristoteles: 41
  ff., 95 ff., 97, 112, 127; --- til Lystlæren: 198. --- Den platoniske
  Tænknings Eiendommelighed, Dialektikens Charakteer: 91 ff., 222. ---
  Den platoniske Philosophies Grundbegreb: Ideen: 41, 80 f., 85 f. ---
  Ideernes indbyrdes Forhold, Ideeverdenen: 85, 222. --- Materien: 42,
  Anm., 81 f., 86 ff. --- Forholdet mellem Ideen og Materien: 41, 81 ff.,
  93. --- Sandseverdenen: 81, 87, 91. --- \gk{Κόσμος}: 82 f., 91 f. ---
  Verdenssjælen: 88 f. --- Demiurgen: 87. --- Verdenstilblivelsen: 93 f.
  --- Naturaarsagerne: 93. --- Naturerkjendelsen: 96, Anm., 122, 127. ---
  Dualismen: 92 f. --- Sjælen og Legemet: 93. --- Sjælens Dele: 108. ---
  Den ethiske Opgave: 93. --- Det Historiske: 192.}
\item Platonikerne i Overgangsperioden 61 f., 64.
\item Plotinos 128, 129, 133.
\item Pomponazzo 62.
\item Pythagoræerne 37.

\vspace{0.5\baselineskip}
% p. 293, Realistisk Philosophie, l. 2 of the sub-entry: a small stray
% mark, about the size of a comma, stands below the colon of
% "Tilværelsesprincipet:".  It is not on the baseline and is not a
% punctuation sort of this fount; not transcribed.  RECON.md §9 lists
% p. 293 among the pencil-polluted pages.
\item Realistisk Philosophie i den nyere Tid 26 f., 156 ff., 171 ff., \textbf{231 ff.}
  \regsub{Dens Opfattelse af Tilværelsesprincipet: 156, 231 f. ---
  Naturopfattelsen: 156 ff., 163, 232 f. --- Erkjendelsesprincipet: 232,
  236. --- Erfaringsprincipet: 233, 237. --- Tænkningens
  Eiendommelighed: 160 ff., 229 f.
% --- p. 294 ---
\opage{294} % p. 294's left column opens INSIDE this sub-entry, at "--- Skepsis:
% 234."; the marker stands at that point.
  --- Skepsis: 234. --- Livsanskuelsen: 168, 172, 228, 235. --- Forhold
  til Idealismen: 171 f., 236 f.}

\vspace{0.5\baselineskip}
\item Sanchez 63.
\item Schelling 76, 78, 155, 178 f., 180, 228, Anm., 239.
  \regsub{Det Absolutes Begreb: 262. --- Opfattelsen af Naturen: 262 f.
  --- Methoden: 264. --- Livsanskuelsen: 267. --- S.'s sildigere
  Philosophie: 281 f.}
\item Scholastisk Philosophie \textbf{140 ff.}
  \regsub{Dens Forhold til det Antike: 140 ff.}
\item Sensualismen 161.
  \regsub{Sensualismen i vor Tid i England og Frankrig: 280.}
\item Skepticismen 44, 162 f.
% p. 294, Sokrates sub-entry, l. 3: a small round ink blot stands between
% "201 f.;" and the following dash.  Not a sort of this fount; not
% transcribed.  Small marks of the same kind sit below the colons of "Det
% Gode:" and "Autarkien:" further down the same column.
\item Sokrates 40 f., \textbf{201 ff.}
  \regsub{Hans Forhold til det eiendommeligt Græske: 201 f.; --- til det
  Moderne: 202; --- til Sophisterne: 209; --- til Anaxagoras: 210; ---
  til den speculative Philosophie: 203, 214 f. --- Hans Uvidenhed: 201,
  203, 209. --- Hans Dialektik: 207. --- Hans Forhold til det Religiøse:
  207 f. --- Hans ethiske Standpunkt: 212 f. --- Dyden: 206, 212 f. ---
  Det Gode: 206. --- Det Nyttige: 208. --- Autarkien: 208, 213. ---
  Venskabet: 213.}
\item Sophisterne 39, 209.
\item Speculativ Idealisme \textbf{259 ff.}
  \regsub{Dens Forhold til Kriticismen: 250 f. --- Dens Grundbegreb: 262
  ff. --- Dens væsentlige Gjenstand: 263. --- Dens Tænknings Art: 230,
  264 ff. --- Dens Livsanskuelse: 266.}
\item Spinoza 151, 180.
  \regsub{Substantsbegrebet: 152, \textbf{173 ff.}, 240 f. ---
  Substantsen som Causalitet: 244. --- Gud, Naturen: 174 ff. ---
  Attributerne: 173. --- Attributernes Parallelisme: 228, Anm. --- Idea
  ideæ: 241. --- Selvbevidstheden: 243, 253. --- Intuitiv Viden af Gud:
  229, 238, 247 f. --- Livsopgaven: 247 f. --- Det speculative Element i
  Spinozismen: 261, 267.}
\item Stoikerne 44, 138, 190.
% p. 294: "Système de la Nature" is set in the same upright roman as the
% Danish, like every other foreign phrase in this book (transcription.tex
% §2); it takes no \textit{}.
\item Système de la Nature \textbf{163 ff.}, 172, 279.

\vspace{0.5\baselineskip}
\item Telesio 62, 148, 149.
\item Thales 51.
\item Theodoros 197 f.
\item Theosopherne i Overgangsperioden 61, 63.
\item Thomas Aquinas 140.

\vspace{0.5\baselineskip}
\item Udviklingen i Philosophiens Historie i dens Lovbestemthed
  \textbf{2 ff.}, \textbf{46 ff.}
...
\vspace{0.5\baselineskip}
\item Østerlandsk Philosophie 18 f.

\end{list}
\endgroup

% p. 294, below the last entry: a short CENTRED DOUBLE RULE -- two
% parallel hairlines with a hair of white between them -- closing the
% Register and the book.  Measured at 400 dpi: x 160.8--228.8 pt (68.0 pt
% = 2.40 cm) against a text block of x 61--326 pt, i.e. centred on the
% measure; the two hairlines stand at y 512.1--513.0 and y 513.3--514.2 pt.
% Nothing else is printed below it; the lower third of the page is blank.
% It is text.  Per the policy already settled at the foot of p. 215 and
% again on the Trykfeil leaf (PDF 306), a double rule is carried by
% \divrule and the DOUBLING is recorded here rather than typeset; \divrule
% is 2 cm and the difference of length is likewise recorded, not set.
\divrule
...
% --- p. 295 ---
\opage{295} % =====================================================================
%  TRYKFEIL OG RETTELSER -- the author's own errata leaf.
%  RECORDED HERE, NOT EXECUTED.
% =====================================================================
%  The unnumbered leaf at PDF 306, which pagemap.py handles as internal
%  printed p. 295.  It is the last printed leaf of the volume: PDF
%  307--313 are blank binder's leaves and the verso (PDF 307) is blank.
%
%  *** NONE OF THE NINE CORRECTIONS IS APPLIED TO THE BODY OF THIS
%  EDITION. ***  Per transcription.tex §5 and BATCH-AGENT §8, the body
%  keeps the PRINTED reading in every one of the nine places --
%    p.  18 l.  2   "fra Philosophiens"      (leaf: "for")
%    p.  31 l. 26   no comma after "Høieste" (leaf: set one)
%    p.  37 l.  5   "Vordens"                (leaf: "Vorden")
%    p.  46 l.  4   "blive given"            (leaf: "først blive given")
%    p.  48 l. 20f. "gjennem sit partielt realiserede \gk{τέλος}"
%    p.  50 l.  2   "hævde sig"              (leaf: "hævdes")
%    p.  99 l.  7   "\gk{ἐγεργείᾳ}"          (leaf: "\gk{ἐνεργείᾳ}")
%    p. 110 l. 29f. "Forbrydelse"            (leaf: "Fordybelse")
%    p. 113 l. 33   "Bostemmelse"            (leaf: "Bestemmelse")
%  -- each with a % comment at its site pointing to this leaf.  The leaf
%  is set here exactly as the author printed it, so that the edition
%  RECORDS his corrections without EXECUTING them: a reader who wants
%  Brøchner's second thoughts has them here, and a reader who wants the
%  1869 text as it left the press has that in the body.
%
%  ---------------------------------------------------------------
%  ONE ERROR IN THE LEAF ITSELF, RECORDED AT THE ENTRY BELOW
%  ---------------------------------------------------------------
%  The last entry cites p. 113 LINE 35.  The printed "Bostemmelse"
%  stands on LINE 33 -- "Begrebet \gk{μεσότης} som Bostemmelse for den
%  ethiske Dyd." -- and p. 113 carries exactly 35 lines of type, l. 35
%  being "Opfattelse af Dyden som Væsenets uforstyrrede Virksomhed og",
%  which contains no such word.  The author's own line count is two out.
%  See the % note at the p. 113 site and the repeat of this note at the
%  entry below, so that a later reader hunting l. 35 does not conclude
%  that the transcription is wrong.
%
%  ---------------------------------------------------------------
%  THIS COPY CARRIES A MODERN READER'S PENCIL APPLYING TWO OF THEM
%  ---------------------------------------------------------------
%  In the type area of p. 18 a modern hand has struck "fra" through and
%  written "for" above it; on p. 110 "Forbrydelse" is struck through.
%  Both are that reader working from this very leaf.  Per BATCH-AGENT §7
%  nothing that is not printed type is transcribed anywhere in this
%  edition, so the pencil appears nowhere -- neither in the body (which
%  keeps "fra" and "Forbrydelse") nor here.  It is mentioned only so
%  that someone comparing this transcription against the Royal Danish
%  Library scan is not surprised by ink that is not the printer's.
%
%  ---------------------------------------------------------------
%  READINGS SETTLED ON THE IMAGE (600 and 1200 dpi)
%  ---------------------------------------------------------------
%  The scan of this leaf is a single 300 ppi RGB JPEG (pdfimages -list:
%  1503x2392, no 1-bit mask), so 600 dpi is 2x and 1200 dpi is 4x
%  interpolation -- it helps the eye and invents no evidence.
%
%  1. THE SECOND ENTRY READS p. 31, NOT 81.  ABBYY gives "81".  At 1200
%     dpi the first digit is an open-sided 3: two bowls opening left,
%     no closed upper counter, identical in shape with the 3 of "37,"
%     on the next line, and quite unlike the two fully closed counters
%     of the 8 in "48," four lines below.  The body site is p. 31 l. 26,
%     where no comma follows "Høieste" -- as the recon pass found.
%
%  2. p. 50 reads "hævde sig", not ABBYY's "bævde": at 600 dpi the
%     letter carries a full ascender and a shoulder, an h.
%
%  3. THE GREEK.  Neither OCR witness reads a single Greek character on
%     this leaf: ABBYY gives "riXos" for \gk{τέλος} and
%     "iyiQyeia, læs: iviqyeia" for the p. 99 pair.  All four Greek
%     words were read off the image at 1200 dpi and nowhere else.
%
%     \gk{τέλος}, twice in the p. 48 entry: tau, epsilon with an ACUTE
%     (the stroke rises to the right), lambda, omicron, FINAL sigma.
%     No breathing, none wanted.
%
%     THE p. 99 ENTRY PRINTS THE PAIR THE SAME WAY ROUND AS THE BODY
%     PAGE -- the faulty reading first with a GAMMA, the correction
%     second with a NU.  Settled mechanically rather than by eye, from
%     the descender profile at 1200 dpi.  Glyph segmentation across the
%     x-height band, in px from the start of each word:
%       \gk{ἐγεργείᾳ}  ἐ 46--86  γ 98--162  ε 174--218  ρ 230--286
%                      γ 302--366  ε 378--418  ι 430--466  ᾳ 478--542
%                      comma 554--574
%       \gk{ἐνεργείᾳ}  ἐ 8--48   ν 60--112  ε 128--164  ρ 176--232
%                      γ 248--312  ε 324--364  ι 380--408  ᾳ 420--488
%                      stop 500--520
%     Ink below the baseline: FOUR descenders in the first word, at
%     px 99--122, 220--261, 302--331 and 481--516, i.e. under letters
%     2, 4, 5 and 8; THREE in the second, at px 171--207, 248--277 and
%     425--460, i.e. under letters 4, 5 and 8 only.  Letter 2 of
%     \gk{ἐνεργείᾳ} sits wholly on the baseline.  That is the same
%     discrimination the pp. 99--112 batch made on the body page, where
%     p. 99 l. 7 prints a γ with a full descender identical to the
%     following γ against the descenderless ν of \gk{δυνάμει}.
%
%     BOTH WORDS CARRY THE IOTA SUBSCRIPT.  The fourth descender of the
%     first word (px 481--516) and the third of the second (px 425--460)
%     both fall under the final alpha and are plainly separate from the
%     comma / full stop that follows.  So \gk{ἐγεργείᾳ} carries one too,
%     not only \gk{ἐνεργείᾳ}.
%
%     BOTH carry a SMOOTH breathing over the initial epsilon: a
%     comma-shaped hook opening to the right with a white counter at its
%     left, not the closed "C" bowl of the rough (calibrated against the
%     rough breathing of ἑαυτῷ at p. 99 ll. 15 f.).  BOTH carry an ACUTE
%     over the iota (the mark's top lies right of its foot).  Darkest
%     pixel 21--25 of 255 at 1200 dpi for breathing and acute alike:
%     solid type, not an artefact.
%
%     A faint mark stands over the third letter (ε) of \gk{ἐνεργείᾳ}.
%     It is NOT an accent and is not transcribed: its darkest pixel is
%     102/255 with no pixel below 90, against 21--25 for the certain
%     breathing and acute in the same word.  Foxing or a light offset.
%
%  ---------------------------------------------------------------
%  THE TYPOGRAPHY OF THE LEAF, MEASURED
%  ---------------------------------------------------------------
%  THE HEADING IS BOLD AND NOT LETTERSPACED, so it takes \textbf{} and
%  must NOT take \emph{} (BATCH-AGENT §3).  At 600 dpi:
%    - inter-letter gaps in the heading 1--5 px (median 3), against
%      3--10 px (median 6) in the list's own roman below it, which is the
%      SMALLER fount -- so normalised the heading's letters are set
%      tighter, not looser.  Word gaps in the heading 47 and 43 px.
%    - stroke weight: heading "o" side walls 10--11 px, top and bottom
%      arcs 8--9 px, x-height 50 px; the list's roman "o" side walls
%      6 px, arcs 2--4 px, x-height 33 px.  Stroke contrast 1.2:1 in the
%      heading against 2.0:1 in the roman, and wall/x-height 0.22
%      against 0.18 -- an almost monoline slab-serifed display face,
%      which is what reads as "bold" here.  Plain stems 9--10 px in the
%      heading against 5--7 px in the roman.
%    - size: heading cap height 64 px against 48 px in the list = 1.33x.
%      {\large\textbf{}} over a \small list reproduces that ratio.
%
%  THE LIST IS SET SMALLER THAN THE BOOK'S TEXT PAGES: x-height 31 px
%  against 39 px on p. 288 (PDF 299), and line pitch 92 px against
%  108 px, both at 600 dpi.  The print's ratio is thus about 0.8;
%  \small in this 12 pt preamble gives 0.92.  \footnotesize would match
%  the ratio and is unreadable at this measure, so \small is used and
%  the discrepancy recorded rather than hidden.
%
%  THE SHORT RULE UNDER THE HEADING: measured at 600 dpi at
%  x 1304--1501, y 1678--1685 -- 197 px = 0.83 cm long, core thickness
%  3 px (0.13 mm), centre x 1402.5 against the list's own centre
%  x 1401 (list x 294--2509).  Centred.  This is exactly the length of
%  the rule under the p. 216 head (196 px = 0.83 cm at 600 dpi), i.e.
%  the book's standard rule-under-a-head, so \divrule is right for it;
%  \divrule is 2 cm and the difference of length is recorded here, not
%  typeset, as at the p. 215 and p. 216 rules.
%
%  THE TERMINAL DOUBLE RULE: TWO parallel hairlines, y 3122--3128 and
%  y 3133--3139, each spanning x 1085--1692 (607 px = 2.57 cm) at
%  600 dpi, core thickness about 3 px each, 11 px (0.47 mm) between
%  their centres.  Rule centre x 1388, i.e. 13 px -- 0.6 % of the
%  measure -- left of the list's centre; centred within the impression's
%  tolerance.  It stands 415 px (1.76 cm) below the last entry.  The
%  book sets a double rule elsewhere (foot of p. 215: two hairlines of
%  2.1 cm); there too \divrule carries it and the doubling is recorded
%  rather than typeset, and that policy is followed here.
%
%  LAYOUT.  Four columns and a text column, measured at 600 dpi:
%    - "Side" x 294--418 and "Linie" x 610--754 on the first line only;
%      from the second entry on, each is replaced by a DASH, and the
%      dashes are CENTRED under the words they stand for (Side centre
%      356, dashes centre about 352; Linie centre 682, dashes centre
%      about 678) -- hence the c columns below.
%    - the dashes measure 58--66 px against a cap height of 48 px, so
%      about 0.9 em: EM-dashes, set --- per BATCH-AGENT §2, which sets
%      every dash in this book as ---.
%    - the page figure right-aligns on its comma (comma at x 562--572 on
%      every line), the line figure on its colon (colon at x 877--897).
%    - the p. 48 entry's first line runs the FULL measure, x 322--2509,
%      against a book measure whose right edge falls at x 2510--2512 on
%      PDF 300, 302 and 304; its turnover is indented to x 894 (600 px =
%      2.54 cm from the list's left edge at x 294) and is set flush
%      right at x 2507.  The transcription re-wraps; the printed break
%      "gjen-|nem" is a word-break hyphen and is resolved with \- per
%      BATCH-AGENT §2.  The text column is set p{0.63\textwidth} so that
%      that entry still takes exactly two lines, as it does in the
%      print, with the turnover indented; nothing else depends on the
%      figure, and it may be adjusted if the fount changes.
%
%  FRENCH SPACING, RECORDED AND NOT REPRODUCED.  The compositor sets a
%  thin space before every colon: 19 px between the line figure and its
%  colon on line 1 ("2" ends x 862, colon x 881--891), 11 px in
%  "efter:", 14 px in "læs:" -- against 3--8 px for an ordinary
%  inter-letter gap and 36--47 px for a word space on the same lines.
%  No thin-space convention is used anywhere else in this edition, so
%  the colons are set tight here and the fact is logged.
%
%  THE LEAF IS SUNK.  First ink at y 1484 at 600 dpi against y 316--329
%  on PDF 300, 302, 304 and 305 -- about 1165 px = 4.9 cm below the
%  normal top of the type area -- and the double rule leaves the lower
%  4.9 cm of the page blank.  That is page-making furniture and is not
%  reproduced: \clearpage only, with no artificial \vspace.
%
%  NOTHING ELSE IS ON THE LEAF.  A full-page ink sweep at 600 dpi finds,
%  besides the heading, the short rule, the ten lines of the list and
%  the double rule, only eight specks of 8--39 dark pixels each (at
%  y 888, 1268, 1726, 1972--1979, 2542--2545, 3187, 3679, 4568, 4727).
%  Two of them fall where they might be mistaken for type and are NOT
%  transcribed: a speck at x 1451--1458 in the white line above
%  "sættes", and a 4 px speck at x 633--637 just left of the second dash
%  in the p. 113 entry.  A third artefact: the first dash of the p. 46
%  entry prints broken in the middle (x 324--373 plus x 377--389), but
%  its total span of 65 px matches the other dashes' 58--66 px, so it is
%  a light or damaged impression of the same sort, not a shorter one.
%  There is NO folio, NO sheet signature, NO catchword and NO press
%  figure on this leaf.
% =====================================================================
...
\begin{center}
{\large\textbf{Trykfeil og Rettelser.}}
\end{center}

% The short centred rule under the heading; 0.83 cm on the image, see above.
\divrule

\begin{center}
\small
\setlength{\tabcolsep}{0.55em}
\begin{tabular}{@{}c r c r p{0.63\textwidth}@{}}
Side & 18,  & Linie & 2:     & fra Philosophiens, læs: for Philosophiens.\\
---  & 31,  & ---   & 26:    & efter: Høieste sættes (,).\\
---  & 37,  & ---   & 5:     & Vordens, læs: Vorden.\\
---  & 46,  & ---   & 4:     & blive given, læs: først blive given.\\
% The printed line breaks after "gjen-"; the hyphen is a word-break hyphen
% and is resolved here, the transcription re-wrapping on its own measure.
---  & 48,  & ---   & 20 f.: & gjennem sit partielt realiserede \gk{τέλος},
                               læs: gjen\-%
nem sit (individuelle) \gk{τέλος}, der partielt realiserer det.\\
---  & 50,  & ---   & 2:     & hævde sig, læs: hævdes.\\
% The γ/ν pair, read at 1200 dpi; see the note above.  The faulty reading
% stands first, the correction second, exactly as at the p. 99 body site.
---  & 99,  & ---   & 7:     & \gk{ἐγεργείᾳ}, læs: \gk{ἐνεργείᾳ}.\\
---  & 110, & ---   & 29 f.: & Forbrydelse, læs: Fordybelse.\\
% AS PRINTED: the leaf cites LINE 35, but the printed "Bostemmelse" stands
% on LINE 33 of p. 113 ("Begrebet \gk{μεσότης} som Bostemmelse for den
% ethiske Dyd."), the page carrying exactly 35 lines of type.  The author's
% own line count is two out.  The leaf's figure is kept as printed.
---  & 113, & ---   & 35:    & Bostemmelse, læs: Bestemmelse.
\end{tabular}
\end{center}

% The terminal DOUBLE rule, two hairlines of 2.57 cm 0.47 mm apart, closing
% the volume; \divrule per the policy recorded above and at p. 215.
\divrule
