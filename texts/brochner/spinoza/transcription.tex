...
%  in the body of this file exactly TWICE [now FOUR: two more italic sorts
%  were found in notes on pp. 69 and 70, collation 2026-09-23; see §9] -- both are the print's own
...
%  the hyphen is 29--33 px. The Indhold's page ranges are ALSO em rules (---):
%  55--62 px against a 34 px figure pitch (collation 2026-09-23, two readers;
%  the recon pass had called them en rules).
%
%  ---------------------------------------------------------------
%  5. FOOTNOTES
%  ---------------------------------------------------------------
...
%
%  ---------------------------------------------------------------
%  9. ACCURACY  (full collation, 23 September 2026)
%  ---------------------------------------------------------------
%  Accuracy: full collation of all 188 printed pages, plus the title page
%  and the Indhold, against the scan (sha256 5bb8c033..., PDF = printed
%  + 10, folio read on every page). Every page was read at the image,
%  word against word, by one of 19 collators (10 pages each, 4 waves), and
%  letterspacing was measured by gap at 600 dpi on every line. Every
%  proposed change other than a routine emphasis correction -- all
%  printer's-error claims, all punctuation, all structure, and every item a
%  collator held in doubt -- was put to a BLIND second reader before being
%  applied. The two disagreed once (p. 95, `ab' in `cognitio ab experientia
%  vaga'), and the file was left as it stood.
%
%  Transcriber's errors corrected: 20 words or marks --
%    p. 13 `Bd. 1.' -> `Bd. I.' (a roman I; the earlier note here was wrong);
%    p. 66 `skjærpes' -> `skærpes'; p. 72 `sui,' -> `sui;';
%    p. 75 `»Quare' -> `„Quare' (so the pp. 75-76 `mixed pair' was not one:
%      withdrawn, and the quote tallies move to »« -1, „“ -1);
%    p. 80 two dropped dashes (`; --- ligeledes'); p. 82 `i det hele' ->
%      `idethele' (printed idet-|hele at a line end); p. 95 n. 3
%      `Foreningens' -> `Forenings'; p. 119 n. 3 `tidligere“.' ->
%      `tidligere.“'; p. 124 n., p. 125 n. seven dropped dashes; p. 143
%      `saa at' duplicated across the page turn, deleted; p. 147 comma and
%      pp. 148, 152 full stops dropped after a footnote call.
%  Also: the Indhold's thirteen page ranges are em rules, not en rules.
%  Printer's errors the file had silently corrected, restored as printed:
%    4 -- p. 50 n. `Aanden, Her'; p. 63 `Aarsagerne Række' (line-final, no
%    room for the s); p. 87 n. the stray `)' after `præf.'; p. 143
%    `uadskilllelig'. Comments added to 13 misprints already transcribed
%    as printed (pp. 57, 60, 92, 119, 122, 138, 144, 148, 156, 158, 159,
%    166, 184).
%  Emphasis: 231 corrections to letterspacing (110 runs added, 116 extents
%    corrected, 5 removed), most of them short words (kun, som, er, Gud) and
%    Latin terms missed by the transcription and its earlier audit passes,
%    and `og' wrongly swept into a run between two spaced words. Several
%    earlier `%' notes about spacing were wrong about the page and have been
%    rewritten (pp. 5, 128, 129-30, 132, 142, 152, 166, 171, 172).
%  Genuine italic sorts: two more found, p. 69 n. `possibilis' and p. 70 n.
%    `endelig', set \textit{} like the Greek and p. 171's `i' (§3).
%  Structure: 9 paragraph breaks restored (pp. 87, 88, 110, 111 x2, 113,
%    124, 126, 153); 4 page markers moved to the true turn (pp. 47, 48, 113,
%    131); p. 137's quotation „Thi Aandens Frihed ...“ moved out of the body
%    into p. 136's note 2, whose run-on it is (note type); the footnote
%    counter reset added at p. 120.
%  Typesetting, not text: 91 printed line-end hyphens that had been carried
%    as bare `-' (pp. 88-131 above all) now reflow as \-%, and at the 30
%    page turns falling inside a word the space after \opage{N} has been
%    removed. Before this, the PDF printed `Ud førlighed', `udtryk ker' and
%    the like at 83 places. Two such breaks joined by editorial judgement:
%    `Selvtilintetgjørelse' and `Smertefølelse' (printed Selv-|, Smerte-|).
%  Held, not changed: p. 112 n. 1 a damaged, unidentifiable digit before
%    `7.' (see the site); p. 111 `har uadæquate ɔ:' (no noun, as printed);
%    punctuation specks at pp. 89 n., 156 n., 163, 168, 169 and 174, left as
%    the file had them; p. 172 `som', p. 185 `at' and p. 186 `i' left
%    outside the adjacent spaced runs.
%  ocrdiff (embedded Google/ABBYY layer, --frag per batch): 44 candidates,
%    0 real, 44 the witness's fault. On this scan the tool found nothing;
%    everything above was found by reading the page.
%  Rate: 24 transcriber's word or punctuation errors (20 corrected + 4
%    silently corrected misprints) over 188 pp. is about 0.13/page, 7 of them
%    word-level (0.04/page). Emphasis was far worse, at about 1.2/page.
%  Post-repair re-sample (seed `brochner/spinoza|post': pp. 2, 25, 74, 83,
%    91, 142, a fresh reader): 0 errors, 0 emphasis, 0 paragraph
%    disagreements.
%
% =====================================================================
...
%  display lines are letterspaced, hence \emph{} -- except `EN MONOGRAPHIE',
%  which is set solid (collation 2026-09-23: 3-7 px gaps at 75 px caps, like
%  the solid KJÖBENHAVN., against 19-25 px in the spaced lines; two readers).
%  Note ö, not ø, in
...
%  full stops; ranges use an em rule (collation 2026-09-23; earlier said en);
%  NO ENTRY ENDS IN A FULL STOP (the
...
% --- p. 1 ---
\opage{1} % The BSB ink stamp stands at the head of this page; it is not text.
...
% --- p. 2 ---
\opage{2}førlighed, at der kan aabnes et dybere Indblik i disses indre
...
% --- p. 3 ---
\opage{3}afhandlinger til de enkelte Fremstillinger vil Sammenhængen mellem det
...
første Begyndelse og indtil vore Dage sig kun i \emph{to} store,
...
% --- p. 4 ---
\opage{4} Grundvold, Naturen, for i en høiere Region at finde en Forsoning, den,
...
% --- p. 5 ---
\opage{5} til det menneskelige og naturlige Livs Former og Kræfter, dens
...
Christendommen, ikke er udgaaet \emph{af denne}, men kun
% `af denne' is spaced as one run, like the `af' inside the next run
% (a-f gap 11 px against 0-3 px in solid `af'; collation 2026-09-23,
% two readers -- the earlier note here said the reverse).
...
% --- p. 6 ---
\opage{6} tilberedt gjennem de sidste græske Philosopher: det var en storartet
...
% --- p. 7 ---
\opage{7} Sammenhæng. Men den dualistiske Forudsætning maa bestemme denne Enhed
...
% --- p. 8 ---
\opage{8} sidste Maal antydet; men en Rest af et rationalistisk Element hos ham,
...
% --- p. 9 ---
\opage{9} Abstraction, og Tanken hævder den Betydning, den havde som
...
% --- p. 10 ---
\opage{10}vikling havde den samtidige Idealisme ikke naaet. Den
...
% --- p. 11 ---
\opage{11}ning. Han sætter endnu ikke alt Objectivt ind i Selvbevidstheden,
...
% --- p. 12 ---
\opage{12} første Hovedstadium (som Dogmatisme). \emph{Kant} og \emph{Hegel}
...
neueren Philosophie Bd.\ I.), \emph{Cousin} (fragments de philosophie
% `Bd. I.': a roman capital I, flat-topped with bracketed serifs and no
% flag, shorter than the `1' of `1854' two lines below (collation 2026-09-23, two readers
% at 1000-1200 dpi; an earlier note here read it as a digit one).
...
% --- p. 14 ---
\opage{14} undgaae Misforstaaelser og en ufuldstændig Opfattelse, da at lade
...
% =====================================================================
% --- p. 15 ---
\opage{15} \chapter*{Benedict Spinoza}
...
% --- p. 16 ---
\opage{16} % blank verso of the divisional half-title.
...
% --- p. 17 ---
\opage{17} % The head (`Indledning.' letterspaced bold, the subtitle `Spinozismens
...
% `20-22' is a hyphen, not the em rule the Indhold uses for page ranges
...
% --- p. 21 ---
\opage{21}vandling under Meddelelsen paaviser han ikke, og den kan ikke paavises ud
fra hans Principer.
...
% --- p. 23 ---
\opage{23} som skabte af Gud, existerende, ligesom de tænkende, alene ved hans
...
bliver den mechaniske Nødvendighed, den abstract opfattede Naturs Lov,
% The scan shows `Natnrs'. Sense-reading set under the book's JBIG2 rule
% (header §6): a one-letter non-word in a symbol-matching scan is not
% evidence of a wrong sort. (Collation 2026-09-23; earlier transcribed
% `Natnrs' as printed.)
...
% `Tanke' is set solid; the spacing begins at `udviklingens' (600 dpi,
% collation 2026-09-23, two readers). As printed.
Tanke\emph{udviklingens mathematiske} Form\footnote{Ikke just den
...
% --- p. 26 ---
\opage{26} medens Tankens Udgangs- og Slutningspunkt bliver \emph{den umiddelbare
...
% --- p. 28 ---
\opage{28} og denne enfoldige, redelige, trods den theologiske Antipathie med den
...
% --- p. 29 ---
\opage{29} Sønnens Biographer, faaer man Indtrykket af en klar Forstand og Uhildethed
...
% --- p. 30 ---
\opage{30} tilbage, Menigheden søgte at kjøbe ham tilbage ved et aarligt Stipendium
...
% --- p. 31 ---
\opage{31} Om denne Deel af Spinozas Liv er Colerus udførligere, idetmindste
...
% --- p. 33 ---
\opage{33} lidet tilfredse Raad, den heidelbergske Professor \emph{Ludvig
Fabritius}.
...
% --- p. 38 ---
\opage{38} Afhandlingen \emph{de intellectus emendatione} er, efter Udgiverens
...
og dens Egenskaber.
% --- p. 39 ---
\opage{39} Efter Planen for Afhandlingen (S. 371) stod det endnu tilbage, først at
...
% --- p. 40 ---
\opage{40} ere omtalte andensteds (saaledes af forskjellige christelige Dogmer, af
...
% --- p. 41 ---
\opage{41} Spinozas \emph{Compendium grammatices linguæ hebrææ}, der danner et
...
% --- p. 43 ---
\opage{43} kan udvikles fuldstændig efterat det metaphysiske Grundlag, der er
...
% --- p. 44 ---
\opage{44} Det, der i Indledningen blev udviklet angaaende Spinozas Forhold til
...
Væsen, fra det Uendelige, og kan kun begynde med dette fordi \emph{kun} det
...
\emph{nødvendig}, sat med dens Væsen: Per \emph{causam sui} intelligo id,
...
Væsen er \emph{Evighed}: per \emph{æternitatem} intelligo ipsam existentiam,
...
% Only `Tal' is spaced; `bestemmelse' is solid (600 dpi, two readers). As printed.
Samme; thi som blot \emph{Tal}bestemmelse har Enheden ikke Anvendelse
...
af Uendeligheden, idet Alt, hvad der udtryk\-%
% --- p. 47 ---
\opage{47}\setcounter{footnote}{0}%
ker Væsen og ikke indeslutter Negation, henhører til den (Eth. I, def. 6.
explic.).
...
--- --- percipit, skulde gjøre Attributet til en \emph{blot subjectiv}
...
det, der \emph{er i sig}, og som det, der \emph{begribes ved sig}.}. Misforstaaelsen
...
Substantsen: notandum, me per \emph{attributum} intelligere omne id, quod
...
begrebet kommer imellem\footnote{Man seer af Ep. 2 ff. ogsaa, at de 4 første
...
\emph{Substantsen} som det, quod in se est et per se concipitur, hoc est,
...
udsiger allerede Definitionen af \emph{Gud} som den af uendelige mange
Attributer \emph{bestaaende} Substants (\emph{constantem} infinitis
...
Dei \emph{attributis} esse. Beviserne for Sætningerne 10, 19 og 20 i Ethikens
...
19. demonstr. paa Attributet som det, quod divinæ substantiæ \emph{essentiam}
...
og derfor kan der i Coroll. 2 til denne Sætning sluttes: Deum \emph{sive}
...
% `for' is solid, `malis' spaced (600 dpi, two readers). As printed.
for\emph{malis}\footnote{Formalis betyder hos Spinoza og den Tids Philosopher
...
absolute \emph{indeterminatum}. Ep. 41.} er \emph{Gud} ɔ: substantia constans
infinitis attributis, quorum unumquodque æternam et infinitam
essentiam exprimit (def. 6). I Begrebet: \emph{Gud}, har Substantsens Begreb
...
% AS PRINTED (collation 2026-09-23, second reader agrees): «optaget i Aanden, Her kommer»
...
% --- p. 51 ---
\opage{51} flere Attributer i Substantsen; man kommer da i ethvert Tilfælde til at
...
komme til Attributernes \emph{Flerhed}. Det absolut uendelige, absolut
...
Affirmative i dem kun er Substantsen, \emph{rent som saadan}, der er
ligegyldig imod om den er Aand eller Materie\footnote{Efter Spinozas
Opfattelse affirmerer Attributets \emph{Eiendommelighed} sig netop umiddelbart
for Tanken.}, og for hvilken de derfor begge, \emph{forsaavidt} de ere
et \emph{Bestemt}, fra hinanden \emph{Forskjelligt}, og
forsaavidt der kun sees paa denne Bestemthed \emph{som} Bestemthed,
...
universalis,
% --- p. 53 ---
\opage{53} egentlig en subjectiv Imagination (jfr. Cap. 5), eller Differentsen
...
Forenelighed i Substantsen, hvis Væsen \emph{kun} udtrykkes i Attributerne,
...
\emph{Causalitetsbegrebet}. Substantsen er causa sui; dette er
...
Synonymer bruges: \emph{modi, modificationes, affectiones}. Fra \emph{modus}
adskiller Spinoza \emph{accidens} (Cog. met. I, c. 1), der hos Descartes er
ensbetydende dermed og af Spinoza selv endnu bruges saaledes i Ep. 4.
Betegnelsen \emph{affectio} bruges i Cog. met. I, c. 3 som Synonym med
\emph{Attribut}.}. »Per \emph{modum} intelligo substantiæ affectiones, sive id
...
bestemmes som værende \emph{i et Andet} og \emph{begreben ved et Andet}, er
den bestemt som \emph{sat ved et Andet}, ved det, i hvilket den er og ved
...
Bestemmelsen: Enkelthed og Begrændsning fører os til Problemet om
\emph{det Uendeliges Forhold til det Endelige}, og Spørgsmaalet kunde
nærmest synes at maatte
% --- p. 55 ---
\opage{55} stilles saaledes: hvorledes det Uendelige hos Spinoza frembringer en
...
ikke kan være Tale om en Overgang fra hint til dette, \emph{at det
Endelige ikke er som Endeligt}, at Intet er uden Substantsen,
og at selve Erkjendelsen af det Endeliges Intethed ikke vorder ---
altsaa ikke er underkastet Endelighedens Form; --- men \emph{er
evigt}\footnote{Med Hensyn til Problemet om \emph{de endelige Ting} jfr. Eth.
...
\emph{sui}, i hvilket Causaliteten ligesom er bøiet tilbage i sig selv; dels
...
paa et Andet, men som \emph{causa immanens}\footnote{„\emph{agentem} sive
...
% `Eth. I, 25': the scan shows a digit `1' for the roman I; sense-reading
% kept (JBIG2 rule, header §6; collation 2026-09-23).
Noget tilfælles med de endelige Ting.“ Ep. 4. jfr. Eth. I, 25. Coroll.
(Prop. 10 i Ethikens 2den Bog, der angaaer den \emph{enkelte} modus, faaer
ikke Anvendelse her.)} ---
% --- p. 56 ---
...
eller \emph{følger} bestandig med samme Nødvendighed af Guds uendelige
Natur, \emph{paa samme Maade}, paa hvilken det fra Evighed til Evighed
...
% `Eth. I, 7': the scan shows a digit `1' for the roman I; sense-reading
% kept (JBIG2 rule; collation 2026-09-23).
...
maatte opfattes som det Begrændsedes, % `egen, Negation': the comma is as printed (collation 2026-09-23, two readers).
altsaa som sin egen, Negation
(Ep. 50). Derfor er Begrændsning udelukket ved Substantsens Begreb; og
naar den sees fra et hypothetisk
% --- p. 58 ---
\opage{58} antaget Endeligts Synspunkt, bliver den, som noget blot Negativt, et
Forsvindende.

Bestemmelsen \emph{Flerhed} ligger ikke i Tingens Definition, men
...
altsaa Endelighed. --- Bestemmelserne \emph{Delelighed, Tid, Tal, Maal}, der
...
\emph{Attributet}; det, der er \emph{umiddelbart frembragt af Gud} (immediate
a Deo productum), og \emph{det, der er frembragt mediante infinita
quadam modificatione} (jfr. Ep. 65 og Spinozas Svar i
% --- p. 59 ---
\opage{59} Ep. 66, hvor der som Exempler paa den anden Art anføres, under
...
Ved Bestemmelsen modus ere vi saaledes endnu ikke traadte ud af
...
Uendelige, dog ved sin Uensartethed med dette skal blive \emph{udenfor} det
...
Opfattelsen af modus \emph{under Evighedens og Nødvendighedens Form} (sub
specie æternitatis et necessitatis), og endelig Opfattelsen af modi
som en Helhed, som \emph{universelt Individuum}, som \emph{natura
naturata}.

Stiller man Spørgsmaalet saaledes: hvorledes kunne Modificationer ɔ:
% `Begrebet;' below: the semicolon is as printed (collation 2026-09-23, two readers).
...
Lemma 7, schol.; jfr. Cap. 4). Dette universelle Individuum er
...
det ikke sige, at det \emph{er sammensat}, man kan ikke tillægge det \emph{Dele};
...
siges at forsvinde, men, seet i sin Sandhed, kun \emph{at være
forsvundet}.
...
sig i sin Enkelthed; men dens Maal er Selvbekræftelse. Det, \emph{at
philosophere}, har kun Realitet ved Enkeltheden, kun derved, at
...
\emph{Erindringen} om Endeligheden; denne maa atter tvinge sig frem og
...
Inconsequentsen frem naar der tales om en \emph{Forskjellighed i det
Endelige}; thi den gamle aristoteliske Sætning: \textit{τοῦ μὴ ὄντος
οὐκ ἐστιν εἴδη}, beholder bestandig sin Gyldighed.
% The scan shows `οὔκ ἐστιν' -- breathing+acute on the υ of οὐκ, breathing
% only on ἐστιν -- i.e. the acute of ἔστιν one word to the left. Set as
% οὐκ (sense) with ἐστιν as printed; under the JBIG2 rule (§6) a single
% accent in this scan is not evidence of the compositor (collation 2026-09-23, two readers).
% Greek, printed in
...
Forskjelligheden sættes som en \emph{Grad}forskjel i det Endeliges Væren; men
...
% AS PRINTED: `Aarsagerne' without the genitive s -- line-final in the
% print, flush to the measure, no room for a failed sort (collation 2026-09-23, two readers).
om: \emph{Ordenen} i Naturen, Aarsagerne Række, ordo et constitutio totius
naturæ, series et ordo causarum, der adskilles fra leges naturæ
æternæ; thi i denne \emph{Række} hævde de enkelte Ting en Betydning \emph{som}
enkelte. Det er en Inconsequents naar der af de virksomme evige Love
udledes som Følge et uendelig \emph{Mangfoldigt} (Eth. I, 16). Det er en
...
tillægges modi \emph{som} modi positiv Betydning for dette fyldige Begreb,
...
at den \emph{menneskelige} Tanke tænker Substantsbegrebet. Den
...
Endelige et hypothetisk Udgangspunkt, der bestandig \emph{kun} forlades; men
...
\emph{Uendeligheden} og \emph{Causaliteten}, maae blive de afgjørende for
...
Vi saae, at \emph{Substants}begrebet i sin Fuldstændighed faldt sammen
med \emph{Guds}begrebet, og at \emph{Gud} bestemtes som det absolut uendelige
...
\emph{fri} Nødvendighed (libera necessitas, Ep. 62),\footnote{Idet denne
...
skærpes ved det, der var Spinozas Tankes dybere Grund.
...
\emph{Nødvendighed} og \emph{Umulighed} (skjøndt det sidste, som blot Negation,
...
% p.69: `natura naturata' prints in plain roman here (600 dpi). So do the
% p. 68 instances (collation 2026-09-23); an earlier note here called them spaced.
...
bliver i de endelige Ting en \textit{endelig} Bestemmethed, altsaa ikke
...
% --- p. 71 ---
\opage{71} % No footnote on this page (confirmed on image).
...
Aand (jfr. Cap. 5), nemlig \emph{Udstrækning} og \emph{Tænkning}, fordi kun
...
% `princ.': the scan shows `priuc.'; sense-reading (JBIG2 rule; collation 2026-09-23).
...
\emph{operandi} potentia (f. Ex. Eth. III, 28. dem. Ep. 45).
...
saaledes ogsaa dens \emph{Uendelighed} og \emph{Udelelighed} (jfr. Eth. I, 8.
...
385; princ. phil. Cart. II, 5. schol. --- Om Begrebet: \emph{det tomme
...
kun adskilles modaliter, ikke realiter. Vandet f. Ex. \emph{som} Vand
...
bliver Vandet til og forgaaer \emph{som} Vand; men forsaavidt det er
...
% This quotation opens with a low „ and closes with a high “ -- an
% ordinary pair (collation 2026-09-23, two readers). The file earlier had » here and
% logged a mixed pair at pp. 75-76; there is none.
}.
...
\emph{Udstrækningens ɔ: Materiens Enhed med Gud}, og \emph{de endelige
...
Eftertryk paa Ordene: rerum \emph{varietas}. Tænkes nemlig de enkelte
...
\emph{Hvile}\footnote{Dette: Bevægelse \emph{og} Hvile, maa tages copulativt,
...
universi}. --- De \emph{endelige} Modi ere \emph{Legemerne}. \emph{De deduceres
ikke af hine}; thi en saadan Deduction er, efter hvad der i Cap. 2
...
coguntur. Ep. 15.}; thi netop Begrebet \emph{variatio} kan ikke
udledes af det Uendelige. De endelige Legemer \emph{forudsættes
hypothetisk}, saaledes at Opfattelsen af dem
% --- p. 79 ---
\opage{79} \setcounter{footnote}{0}%
som enkelte holdes svævende (Eth. II. def. 7), og \emph{føres saa
tilbage} til de uendelige Modificationer\footnote{Ogsaa i
...
opfattes som \emph{delelige i det Uendelige} (Ep. 29). Netop ved denne
...
Uendelige, naaes der til at indsee, »\emph{at hele Naturen er eet
...
% =====================================================================
% --- p. 82 ---
\opage{82} psychologisk og dialektisk interessant, hos ham, som idethele hos hiin Periodes idealistiske Tænkere, at see, hvorledes
...
er \emph{intellectus absolute in\-%
% --- p. 83 ---
...
Væsen. Naar der tales om intellectus infinitus \emph{Dei}, bliver
dette Udtryk fuldkommen parallelt med \emph{idea Dei}.} staaer \emph{idea
...
Væsens Enhed er den \emph{een} (Eth. II, 4)\footnote{Cog. metaph. II,
...
uendelige \emph{idea Dei}\footnote{Da intellectus og idea ere
...
den er i \emph{Gud} (Eth. II, 3), saa er det i samme Betydning, i
...
endelige Ting \emph{siges} at existere, ikke blot forsaavidt de
...
Ved Bestemmelserne af Ideernes Forhold til Tingene, \emph{er
den menneskelige Aand} ikke sat som qualitativ forskjellig
fra de andre Tings mentes. Den er en bestemt Modification af
Tænkningens Attribut; det, der constituerer dens actuelle
Væren, er Ideen om en actuelt existerende \emph{enkelt} Ting (Eth.
...
corporis) er klar af det Foregaaende. Aanden er \emph{idea corporis};
...
% AS PRINTED (collation 2026-09-23, second reader agrees): «Eth. V. præf.)}»
...
Adskillelse mellem den \emph{adæquate} og \emph{uadæquate} Idee.
...
begriber en Ting deelviis eller \emph{uadæquat}« (Eth. II, 11
...
den Erkjendelsesart, Spinoza betegner som \emph{Imaginationen}. Det
...
% `Legemet': the scan shows `Legeinet'; sense-reading (JBIG2 rule; collation 2026-09-23).
...
bliver \emph{ikke}-erkjendende, viser Imaginationen og den
...
Erkjendelse af vort og de ydre Legemers \emph{Varighed}.\footnote{Begrebet
...
\emph{forgængelige}, hvilke Bestemmelser altsaa kun udtrykke en
...
Positivt:\footnote{%
% `princ.': the scan shows `priuc.'; sense-reading (JBIG2 rule; collation 2026-09-23).
jfr. de intell. emend. pag. 392; princ.
...
Ep. 64) og dette er en denominatio \emph{extrinseca}. --- Som
...
% `til et begribe': `et' as printed, where sense wants `at' (collation 2026-09-23, two readers).
...
schol. Ideen \emph{som} Idee indeholder Affirmation og Negation
...
men som \emph{nødvendige}, og idet Tingenes Nødvendighed er selve
Guds \emph{evige Naturs} Nødvendighed, opfatter den dem under en
...
udtrykke \emph{det, som er fælles for alle Ting}, og som altsaa, idet
...
\emph{efter sin Natur} (»loquor de \emph{ipsa natura} existentiæ«. Eth. II,
...
(\emph{ipsa} existentia, \emph{ipsa natura} existentiæ) satte i
...
erkjende Gud fuldstændig (\emph{omnino} cognoscere), ikke i at
...
Tingene.\footnote{jfr. tract. theol.-pol. pag. 185: intellectio
pura mente \emph{extra verba et imagines}.} --- 2) Den \emph{anden}
Erkjendelsesart, eller Fornuften (ratio); --- 3) Den \emph{tredie}
Erkjendelsesart, den \emph{intuitive Viden}. Som et Exempel (der dog
...
hvad Natur hiin Forenings Væsen er, det begribe vi ikke fuldkomment
...
Resultat er abstract. --- „\emph{Ved selve Tingens Væsen} begribes
...
men at Udtrykket maa betegne den \emph{gjennem de mange
...
det, der er i Tingene, kan stille det, der blot in \emph{intellectu}
...
en Mangel ved vor Erkjendelse, som: det \emph{Mulige}, det \emph{Tilfældige} (see
...
Negation gives end den, Ideen, \emph{som} Idee, indeslutter. Saaledes er f.
...
men da vil min Tvivl og min Benægtelse være \emph{nødvendig}. --- Den Grund
...
Læren om \emph{Mennesket i dets Trældom og i dets Frihed}, og af
...
% The quotation opened by »Der skeer ... closes at `Legemer«' on p. 110;
% it is balanced (collation 2026-09-23, two readers; an earlier note here said otherwise).
...
individuel Frihed, ikke stille \emph{Opgaver}, som Friheden
...
% `7.': a damaged, unidentifiable glyph stands in the slot of a digit
% directly before the 7 (two readers, collation 2026-09-23). Not supplied; Eth. III, 27
% has a Coroll. 3, but that is sense, not the page.
...
Antydning af, at den som enkelt opfattede Ting kun i \emph{sin
...
quantum in \emph{ea} est, hvilket Udtryk altid bruges i tract.
...
til en mindre Fuldkommenhed: \emph{Glæde} og \emph{Sorg}\footnote{\emph{Lætitia}
...
Aanden alene, \emph{hilaritas} og \emph{melancholia} derimod til hele
...
\emph{Hadet}, der i Virkeligheden ikke ere Andet end \emph{Glæde
og Sorg, ledsagede af Ideen om den ydre Aarsag}
% --- p. 115 ---
\opage{115} % No footnote on this page (confirmed on image; RECON.md's survey agreed).
...
% `til': the scan shows `tii'; sense-reading (JBIG2 rule; collation 2026-09-23).
...
% `passione' (for passiones): as printed, no s and no trace of one (collation 2026-09-23, two readers).
...
Menneskets Aand, en Mulighed for at tænke sig \emph{en Tilnærmelse}
...
Tilstand i \emph{Aanden} (animi pathema), kan nu gives saaledes:
den er
...
bestemmer han dem som \emph{gode} eller \emph{onde} efter deres
Overensstemmelse med den menneskelige Natur, og idet
% `gjennnem': three n's, as printed -- an added sort, not a substitution (collation 2026-09-23, two readers).
gjennnem disse Udviklinger Forholdet kommer frem mellem
den \emph{Frie} og \emph{Trællen}, er Veien banet til, afgjørende at
lade \emph{Befrielsen} vise sig som en \emph{værende} Frihed.
...
\emph{Godt} og \emph{Ondt}, \emph{Fuldkomment} og \emph{Ufuldkomment} finde
...
fang, at Naturen viser sig som en anden, Eth. IV, 39. schol.). --- Ogsaa maa alt det være nyttigt, der befordrer Menneskenes Forening,
...
Charakteer frem for afgjørende at hævdes ved Ethikens Afslutning. --- Spinoza udvikler ikke det Legemliges Forhold til den Sætning,
...
tiones, quæ ex ipsius naturæ necessitate consequuntur, intelligere. --- In hoc uno (ɔ: intellectum perficere) summa hominis felicitas
seu beatitudo consistit (ibid. c. 4).}.

% --- p. 126 ---
\opage{126} \setcounter{footnote}{0}%
Ligesaa nødvendigt som det, Fornuften foreskrev \emph{med
Hensyn til os selv}, fulgte af vort Væsens Begreb, følger
det, den foreskriver med Hensyn til \emph{Forholdet til Andre},
...
bart kommer den anden tilgode\footnote{Denne Begrundelse er af Betydning for Spinozas Opfattelse af \emph{det
...
Intet kan være \emph{eller begribes}, maa være fælles for Alle
...
Gudserkjendelsen er\footnote{En \emph{Gradation} i Gudserkjendelsen faaer kun sin Betydning for den
...
(see næste Capitel), og den sande Betydning af \emph{Reli\-%
...
Det viser sig ligeledes her, \emph{at der ogsaa gives Af\-%
fecter, der have deres Oprindelse fra Fornuften,}
% --- p. 128 ---
\opage{128} \setcounter{footnote}{0}%
% "og ere active." is spaced; "Disse maae henføres til de to af
% Grundaffecterne:" is NOT; then only "Attraa", "Glæde" and "Sorgen" are
% spaced, each alone (collation 2026-09-23, two readers; corrects the 120-131 audit note).
\emph{og ere active.} Disse maae henføres til de to af Grund\-%
affecterne: \emph{Attraa} og \emph{Glæde}; thi \emph{Sorgen} forudsætter en
...
der \emph{ikke besidde Gudsideen} (qui Dei ideam haud possident)
...
Anvendes Bestemmelserne \emph{Godt} og \emph{Ondt} paa de
...
Overgangen til en \emph{mindre Ufuldkommenhed.} Det \emph{positive}
Udtryk faaer kun Betydning saalænge Forestillingen om et \emph{værende}
Endeligt fastholdes; idet denne Forestilling opløser sig, maa først
det positive Udtryk ombyttes med det negative, og derpaa \emph{Overgangen} idethele vise sig som et Negativt, et Ikke-Værende (jfr.
Cap. 8.).}.
% Footnote 3 above is printed as one continuous note: it opens on
...
% Letterspacing in this note (collation 2026-09-23, 600 dpi; replaces an earlier note):
% spaced are "større Fuldkommenhed", "mindre Ufuldkommenhed.",
% "positive", "værende" and the whole of "Overgangen" (broken Over-/gangen
% at a line end); "idethele vise sig som et Negativt, et Ikke-Værende"
% is NOT spaced.

% --- p. 130 ---
\opage{130} \setcounter{footnote}{0}%
Gjennem disse forberedende Udviklinger viser For\-%
skjellen sig nu mellem den \emph{Frie} og \emph{Trællen} (den Vise og
...
traa søger \emph{directe} det Gode, da den opstaaer af Erkjen\-%
...
Spot, Hovmod o. desl., og, saavidt det staaer til ham, \emph{at
handle vel og at være glad} (Eth. IV, 73. schol.); thi
...
% =====================================================================
% --- p. 132 ---
\opage{132} % "causa libera" checked at 600 dpi (p. 132, l. 1): set in plain roman, NOT
...
% (in "Staten og Religionen" the "og" is solid; collation 2026-09-23).
eneste causa libera, og at lade dette Forhold kaste et Lys tilbage paa det under Hensynet til den almindelige Forestilling Udviklede. Men paa dette Punkt er det det passende Sted til at indskyde Spinozas Lære om \emph{Staten} og \emph{Religionen}, der, idet den tager denne Forestilling til Udgangspunkt, supplerer det nyligt Udviklede, og tillige indirecte baner Veien for den fuldstændigere Forstaaelse af Læren om \emph{den evigt værende menneskelige Frihed}, som Spinoza udvikler i Ethikens sidste Bog.
...
% JBIG2 rule.
% The quotation „Thi Aandens Frihed ...“ at the head of this page's note
% block is in NOTE type (line pitch and x-height of the notes, at the
% note-turnover indent): it is the run-on of p. 136's note 2 and is set
% there (collation 2026-09-23, two readers). An earlier transcription had it as body text.

% --- p. 138 ---
\opage{138} % Genuine new indented paragraph at the top of this page (confirmed at
% 600 dpi), hence the blank line before this marker.
\setcounter{footnote}{0}%
% Note 2 below: `jfr. tract theol.-polit.' -- no stop after `tract', as printed (collation 2026-09-23, two readers).
...
% --- p. 141 ---
\opage{141} bunden, vilde være en Selvmodsigelse. Mod Ufornuft og Fordomme, ogsaa religiøse, har Staten derimod en ubetinget Ret (tract. polit. III, 7--9. jfr. tract. theol.-polit. c. XVII).

Idet jus summæ potestatis ikke er Andet end selve Naturens Ret, saa følger det af sig selv, \emph{at to forskjellige Stater forholde sig til hinanden som to Individer i Naturtilstanden}. De ere altsaa af Naturen Fjender og have Ret til at paaføre hinanden Krig og undertvinge hinanden. Stille de sig i et Fredsforhold til hinanden, ere de Bundsforvandte (confoederatæ), og deres Forbund vedvarer saa længe som dets Aarsag, nemlig Frygt for Tab eller Haab om Fordeel. Er denne Aarsag hævet, har hver Stat Ret til at hæve Forbundet, og man kan ikke sige, at den Stat, der bryder det, handler troløst; thi Retten var lige for begge, og Forbundet forudsatte de bestemte Forhold. Staten sørger ved dette Brud kun for sin egen Sikkerhed ligeoverfor den anden, og det bliver endog en ligefrem Pligt for Statsmagten at bryde Løftet, naar den har lovet en anden Stat Noget, som siden enten Tiden eller Fornuften viser at være stridende mod Undersaatternes Vel (tract. polit. III, 11--14).
...
% vivere". The first time the compositor letterspaces "secure et commode"
% (not "vivere"; collation 2026-09-23, two readers) and, the second time, only the one
% word that differs ("optime");
...
% --- p. 146 ---
\opage{146} % Genuine new indented paragraph at the top of this page (confirmed at
% 600 dpi), hence the blank line before this marker.
\setcounter{footnote}{0}%
...
% was thought to be the note on pp. 75--76, withdrawn collation 2026-09-23); logged in RESUME-NOTES' quote-balance
% ledger. Transcribed exactly as printed, not normalised.
% `(pag: 149)' below: colon as printed (collation 2026-09-23).
Tract. theol.-polit. pag. 2, jfr. pag. 144: „Dei æternum verbum et pactum“ og „vera religio“ er „divinitus inscripta hominum cordibus ɔ: humanæ menti“, og den er »verum Dei syngraphum, quod ipse suo sigillo, \emph{nempe sua idea}, tanquam imagine suæ divinitatis consignavit.“}. Dens Aand er en spiritus Dei, spiritus sanctus, spiritus divinus; gjennem den udtrykker sig en lex divina universalis, og den bliver overensstemmende med lex catholica\footnote{Ved Udtrykket: catholica, betegner Spinoza nemlig det for alle Mennesker Fælles, der som saadant maa stemme overens med Fornuften og det naturlige Lys. Saaledes bestemmes religio catholica (der skarpt adskilles fra rel. \emph{Romana}) som lex divina, per prophetas et apostolos universo humano generi revelata (præfat.), den sættes som synonym med religio \emph{toti humano generi universalis} (pag. 148), og som \emph{maxime naturalis} (pag: 149); der tales om en rel. catholica \emph{lumine naturali et prophetia revelata} (Ep. 49), om catholica documenta rationis (tract. theol.-pol. pag. 217), om en \emph{lex} catholica, der bliver liig med lex divina universalis (pag. 151 f.), og om en \emph{fides} catholica (pag. 160), hvis Gjenstand bliver Religionens væsentlige, med
...
% --- p. 151 ---
\opage{151} % No footnote on this page.
\setcounter{footnote}{0}%
Mennesker; thi ogsaa i deres Aand er »Guds Aand og hans evige Udsagn (sententiæ) indskrevne.«

At Propheterne havde Guds Aand, betegner derfor, efter det G. Testamentes Sprogbrug, hvilken Spinoza udførlig udvikler, ikke, at de havde en høiere Indsigt, men alene, at de vare i Besiddelse af en \emph{særegen} og \emph{ualmindelig} Dyd (Evne), at de dyrkede Dyden med Standhaftighed, og at de fornam Guds Ord eller Tanke, samt endelig, at man ikke begreb Aarsagen til den prophetiske Erkjendelse. Efter hvilke Naturlove denne fandt Sted, lader Spinoza staae uafgjort.
...
% argumentari" is letterspaced in the print (on one line, p. 153;
% collation 2026-09-23) and is rendered \emph{} accordingly.
...
% genuine mixed pair, the opposite direction from note 1 p. 148 (which
% opens » and closes “; the pp. 75-76 pair was withdrawn collation 2026-09-23). Second mixed
...
% `det,- der': comma plus a short stroke, as printed (collation 2026-09-23, two readers). Also as printed,
% not corrected: `(f: Ex.' in p. 156 n. 1 and the unopened `)' in p. 158 n. 2.
...
% --- p. 162 ---
\opage{162} I 6te Capitel blev, gjennem Læren om Affecterne og
...
% `jfr,' in the note below: comma as printed (collation 2026-09-23, two readers).
...
Udvikling af \emph{Aandens sande, evige Væsen som det
under Evighedens Form opfattede Legemes Idee.}

Aandens høieste \emph{Dyd} ɔ: dens væsentlige \emph{Natur} (V,
25. dem.), og derfor dens høieste \emph{Stræben}, er at begribe
Tingene ved den »tredie Erkjendelsesart«, \emph{den intuitive}
% p.166: "Viden" at the head of the next line is letterspaced too (collation 2026-09-23,
% two readers; an earlier note here said plain roman).
\emph{Viden}, der skrider frem fra den adæquate Erkjendelse
...
med ham (Eth. V, 24, jfr. Eth. II, 45. schol., tract. theol.-polit. pag. 46 f.). Den udtrykker fuldkomnest det menneskelige
...
% `intellectualis': the scan shows `intelleotualis'; sense-reading (JBIG2 rule; collation 2026-09-23).
...
være \emph{den eneste} evige Kjærlighed. Den er selve Guds
...
36). \emph{Forsaavidt Gud altsaa elsker sig selv}, elsker
...
% --- p. 170 ---
\opage{170} Dyden, ikke en Belønning for denne; kun i selve den active
...
uendelige, evige Væren, der \emph{er}, og ikke \emph{vorder}, der, som
...
forsvinder, ikke som Noget, der \emph{har} været; men dets Væren
...
% =====================================================================
% --- p. 171 ---
\opage{171} rectivet givet for hiin Deling af Aanden; det, der kaldes
...
quadam necessitate} conscius,« er Overgangen og Gradforskjellen
atter hævet, den evige Væren hævdet, og Systemets Ende sluttet
sammen med dets Begyndelse.
% "æterna quadam necessitate" is spaced as one run; "sui et Dei et rerum"
% before it is plain (collation 2026-09-23, two readers; an earlier note here had
% "quadam" plain).
...
Uendelighed, at Tænkningens Opgave var at see den som \emph{al}
...
% --- p. 172 ---
\opage{172}begreb som Formen for Substantsens Væren. Vi saae, at
...
\emph{Aarsag} til nødvendige \emph{Virkninger}, ikke frit Ophav til
frit satte \emph{Øiemed}, og idet Virkningerne i Forestillingen
...
bliver den, i sin Enhed med Tanken umiddelbart selvvisse,
nødvendige Naturmagt}. »Gud eller Naturen« udtrykker
dette Begreb.
% The run is continuous from "Det uendeligt Værende" through "nødvendige
% Naturmagt", including the whole line "bliver den ... umiddel-"
% (collation 2026-09-23, two readers; an earlier note here had part of it plain).
% »Gud eller Naturen« after it is plain.
...
% --- p. 173 ---
\opage{173} Adskillelse hævet, enhver Skepsis udelukket med Hensyn
...
thi den har intet for sig bestaaende Selv: den \emph{er} kun i det
...
% --- p. 174 ---
\opage{174} Bestemmelserne \emph{Pantheisme} og \emph{Naturalisme} betegne
altsaa Systemet fra den objective Side, Bestemmelserne
\emph{Dogmatisme} og \emph{Rationalisme} fra den subjective, men
...
\emph{Først} altsaa skal Uendelighedens Tanke tænkes som
...
% Only `Tids' is spaced (collation 2026-09-23). As printed.
\emph{Tids}forhold, sættes i det kun ved \emph{Evighedens} Form
...
Evighedens og Nødvendighedens Form, det \emph{Uendeliges} Værens-Form,
...
Den \emph{anden} Opgave, Spinozismen havde stillet sig,
var, at opfatte Substantsens Attributer, Tanke og Udstræk\-%
% --- p. 178 ---
\opage{178}ning, som selvstændige, idet de, uden at indeslutte hinandens
...
ved ydre Aarsager som \emph{Grund} til de forvirrede Ideer
...
være \emph{den sig selv umiddelbar visse Tanke om Udstrækningen}.
...
Systemet. Den faaer Ret til, at fremhæve \emph{mod} Spinoza
...
% =====================================================================
% --- p. 181 ---
\opage{181} end det, han har kunnet fæstne i Tankens Former, og disses
...
% --- p. 182 ---
\opage{182} almindelige Retning modsat, til at det skulde kunne have faaet
...
% --- p. 183 ---
\opage{183} sige: Spinozam tota armenta in Belgio sequi ducem. Men
...
% --- p. 184 ---
\opage{184} hvilken han ellers anviser et bestemt Gebet, gjøre sig
...
% `Forkjel': as printed, no s and no room for one (collation 2026-09-23, two readers).
...
% Only `Selv-' is spaced (collation 2026-09-23). As printed.
...
% --- p. 185 ---
\opage{185} Kraften opfattes som et \emph{aandeligt} Princip, at Monade
bliver ensbetydende med \emph{Sjæl}, og at \emph{Aandens Form,
til afgjørende Adskillelse fra Spinoza, bliver Selvbevidsthedens
...
% --- p. 186 ---
\opage{186} Saaledes udtrykker Leibnitz selv, i Forholdet til Spinoza,
...
og \emph{Herder}, den Første endog uden den udvortes
...
samme Trang drev \emph{Novalis} og \emph{Schleiermacher} til en
